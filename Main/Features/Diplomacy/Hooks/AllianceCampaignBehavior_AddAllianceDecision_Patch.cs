...
///
/// When DOTS blocks <c>EndAlliance</c> for a Permanent lore pair, vanilla 1.4.5's
...
public static class AllianceCampaignBehavior_AddAllianceDecision_Patch
{
    ...
    public static bool Prefix(Kingdom kingdomToAddDecision, Kingdom kingdomToOffer)
    {
        ...
        // If they're already allies (DOTS blocked an EndAlliance call), don't queue
        ...
        if (kingdomToAddDecision.IsAllyWith(kingdomToOffer))
        {
            _logger?.LogDebug(
                ...
                $"{kingdomToOffer.StringId} are already allies (likely a DOTS-blocked EndAlliance)");
            return false;
        }
        return true;
    }
