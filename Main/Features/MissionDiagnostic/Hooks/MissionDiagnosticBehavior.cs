...
namespace DOTS.Features.MissionDiagnostic.Hooks;
...
public sealed class MissionDiagnosticBehavior : MissionLogic
{
    ...
    public MissionDiagnosticBehavior(IMissionDiagnosticService service, IModLogger logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    private void CaptureActionSetsFromAgents()
    {
        try
        {
            ...
            foreach (var agent in mission.Agents)
            {
                ...
                var raceName = raceId >= 0 ? $"id={raceId}" : "<none>";
