    ...
    public static bool Prefix(Scene mapScene, ref byte[] indexData, ref int width, ref int height)
    {
        ...
        for (int attempt = 1; attempt <= MaxRetries; attempt++)
        {
            try
            {
                ...
                var msg = $"DOTS: Battle scene index map loaded - {width}x{height} ({indexData?.Length ?? 0} bytes)";
                if (attempt > 1)
            ...
            {
                Debug.Print(
                    $"DOTS: GetBattleSceneIndexMap AccessViolation (attempt {attempt}/{MaxRetries}), retrying in {RetryDelayMs}ms...",
        ...
        Debug.Print("DOTS: GetBattleSceneIndexMap - final attempt, running unguarded", 0, Debug.DebugColor.Red, 17592186044416uL);
        return true;
    }
}
