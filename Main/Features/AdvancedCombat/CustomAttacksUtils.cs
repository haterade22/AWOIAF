...
public class CustomAttacksUtils
{
    ...
    static CustomAttacksUtils()
    {
        try
        {
            ...
            if (blowMethod == null)
            {
                ...
                TaleWorlds.Library.Debug.Print($"[DOTS] CustomAttacksUtils: {_initializationError}", 0, TaleWorlds.Library.Debug.DebugColor.Red);
                return;
            }

            _registerBlow = (RegisterBlowDelegate)Delegate.CreateDelegate(typeof(RegisterBlowDelegate), null, blowMethod);
            TaleWorlds.Library.Debug.Print("[DOTS] CustomAttacksUtils: RegisterBlow delegate created successfully", 0, TaleWorlds.Library.Debug.DebugColor.Green);
        }
        catch (Exception ex)
        {
            _initializationFailed = true;
    ...
    public static void RegisterBlow(Agent attacker, Agent victim, WeakGameEntity realHitEntity, Blow b, ref AttackCollisionData collisionData, in MissionWeapon attackerWeapon, ref CombatLogData combatLogData)
    {
        ...
        {
            TaleWorlds.Library.Debug.Print($"[DOTS] CustomAttacksUtils.RegisterBlow: Skipped - initialization failed: {_initializationError ?? "delegate is null"}", 0, TaleWorlds.Library.Debug.DebugColor.Yellow);
            return;
        }
