...
namespace DOTS.Features;

public class DotsSettings : AttributeGlobalSettings<DotsSettings>
{
    public override string Id => "DOTS";
    public override string DisplayName => "DOTS - Dawn of the Stag";
    ...
    [SettingPropertyBool("Fief Management Debug Mode", Order = 2,
        HintText = "Write diagnostic [FiefManagement] messages to the DOTS file log. Off = silent.")]
    ...
    public System.Action RebuildDistanceCacheAction { get; set; } = static () =>
    {
        ...
        try
        {
            var service = DOTS.IoC.Resolve<DOTS.Features.EditorCacheRebuild.IRuntimeCacheRebuildService>();
            service.Trigger();
        }
        catch (System.Exception ex)
        {
            InformationManager.DisplayMessage(new InformationMessage(
                $"[DOTS] Cache rebuild FAILED to start: {ex.GetType().Name}: {ex.Message}. See rgl_log_*.txt for details.",
                Colors.Red));
        }
    };
}
