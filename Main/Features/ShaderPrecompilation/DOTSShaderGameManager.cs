...
// Not IoC-managed — instantiated manually: MBGameManager.StartNewGame(new DotsShaderGameManager(...))
...
public class DotsShaderGameManager : CustomGameManager
{
    // Sized to fit all DOTS + vanilla characters with no silent drops.
    // ~1601 DOTS NPCCharacter entries + vanilla characters across all included cultures.
    ...
    // to enable stuck detection and auto-abort only during DOTS shader precompilation.
    ...
    public DotsShaderGameManager(IShaderPrecompilationService service, IModLogger logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    private CustomBattleData BuildBattleData()
    {
        ...
        var playerParty = new CustomBattleCombatant(
            new TextObject("{=!}DOTS Shader Player"),
            firstCulture,
            banner);
        ...
        var enemyParty = new CustomBattleCombatant(
            new TextObject("{=!}DOTS Shader Enemy"),
            firstCulture,
            banner);
