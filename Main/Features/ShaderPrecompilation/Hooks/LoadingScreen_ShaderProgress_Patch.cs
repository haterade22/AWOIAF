...
namespace DOTS.Features.ShaderPrecompilation.Hooks;
...
// We also log a progress line to dots_debug_*.log every 30 s for post-mortem visibility.
//
...
public static class LoadingScreen_ShaderProgress_Patch
{
    ...
    public static void Postfix(LoadingWindowViewModel __instance)
    {
        try
        {
            ...
            if (!DotsShaderGameManager.IsShaderBattleActive) return;
            ...
            if (remaining == 0 && _hasObservedWork)
            {
                ...
                DotsShaderGameManager.ResetShaderBattleActive();
