...
namespace DOTS.Features.TroopProgression.Models;

public class DotsPartyWageModel : DefaultPartyWageModel
{
    ...
    private static WageFeatInputs ResolveGarrisonInputs(MobileParty mobileParty)
    {
        ...
        return new WageFeatInputs(
            isApplicable: true,
            ereborGarrisonBonus: BonusIfHas(garrisonCulture, DotsCulturalFeats.EreborGarrisonWageFeat),
            lothlorienGarrisonBonus: BonusIfHas(garrisonCulture, DotsCulturalFeats.LothlorienGarrisonWageFeat),
            isengardGarrisonBonus: BonusIfHas(garrisonCulture, DotsCulturalFeats.IsengardGarrisonWageFeat),
            gondorGarrisonBonus: BonusIfHas(garrisonCulture, DotsCulturalFeats.GondorGarrisonWageFeat));
    }
    ...
    private static WageFeatInputs ResolvePartyInputs(CultureObject? partyCulture)
    {
        ...
        return new WageFeatInputs(
            isApplicable: true,
            gundabadWageBonus: BonusIfHas(partyCulture, DotsCulturalFeats.GundabadWageFeat),
            umbarWageBonus: BonusIfHas(partyCulture, DotsCulturalFeats.UmbarWageFeat),
            mordorWageBonus: BonusIfHas(partyCulture, DotsCulturalFeats.MordorWageFeat));
    }

    private static float ResolveRohanMountedWageBonus(CultureObject? partyCulture)
        => BonusIfHas(partyCulture, DotsCulturalFeats.RohanMountedWageFeat);
    ...
    private static MountedCostFeatInputs ResolveMountedCostFeats(CultureObject? buyerCulture, bool isMounted)
    {
        ...
        return new MountedCostFeatInputs(
            isengardMountedCostBonus: BonusIfHas(buyerCulture, DotsCulturalFeats.IsengardCheaperRecruitsFeat),
            rohanMountedCostBonus: BonusIfHas(buyerCulture, DotsCulturalFeats.RohanMountedCostFeat));
    }

    // Vanilla DefaultPartyWageModel.GetTroopRecruitmentCost applies the buyer hero's personal
    // skill-perk recruitment discounts (orthogonal to DOTS's extended cost table + cultural feats).
    // The full-replacement override dropped them; this restores them. KhuzaitRecruitUpgradeFeat is
    // intentionally NOT resolved here — DOTS replaces it with the Isengard/Rohan mounted-cost feats.
