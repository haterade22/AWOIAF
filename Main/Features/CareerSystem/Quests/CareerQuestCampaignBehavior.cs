...
public class CareerQuestCampaignBehavior : CampaignBehaviorBase
{
    ...
    {
        dataStore.SyncData("_dots_cq_declined", ref _declined);
    ...
    {
        _offerPending = true;
        var title = Text(def.TitleKey, "{=dots_cq_title}Career Quest");
        var body = Text(def.DescriptionKey, "{=dots_cq_body}Prove yourself.");
        var accept = new TextObject("{=dots_cq_accept}Accept").ToString();
        var decline = new TextObject("{=dots_cq_decline}Not now").ToString();
    ...
    private void OnAccept(CareerQuestDefinition def, Hero hero)
    {
        ...
        try
        {
            var quest = new CareerQuest("dots_cq_" + def.Id, hero, def);
            quest.StartQuest();
