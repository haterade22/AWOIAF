...
using DOTS.Features.EquipPresets.Models;

namespace DOTS.Features.EquipPresets.UI;
...
public sealed class PresetsOverlayVM : ViewModel
{
    ...
    private void OpenPresetMenu()
    {
        ...
        {
            DisplayInfo(new TextObject("{=dots_ep_no_hero}No active hero on the inventory screen.").ToString());
            return;
        }
        ...
        {
            new InquiryElement("save", new TextObject("{=dots_ep_save}Save New").ToString(), null, true,
                new TextObject("{=dots_ep_save_hint}Save current equipment as a new preset.").ToString()),
        };
        if (presets.Count > 0)
        {
            options.Add(new InquiryElement("load", new TextObject("{=dots_ep_load}Load").ToString(), null, true,
                new TextObject("{=dots_ep_load_hint}Equip a saved preset.").ToString()));
            options.Add(new InquiryElement("update", new TextObject("{=dots_ep_update}Update").ToString(), null, true,
                new TextObject("{=dots_ep_update_hint}Overwrite a saved preset with current equipment.").ToString()));
            options.Add(new InquiryElement("delete", new TextObject("{=dots_ep_delete}Delete").ToString(), null, true,
                new TextObject("{=dots_ep_delete_hint}Remove a preset.").ToString()));
        }

        MBInformationManager.ShowMultiSelectionInquiry(new MultiSelectionInquiryData(
            new TextObject("{=dots_ep_title}Equipment Presets").ToString(),
            FormatHeader(heroId!, presets.Count),
            options, true, 1, 1,
            new TextObject("{=dots_ep_ok}OK").ToString(),
            new TextObject("{=dots_ep_cancel}Cancel").ToString(),
            chosen =>
            {
    ...
    private string FormatHeader(string heroStringId, int count) =>
        new TextObject("{=dots_ep_header}Hero: {HERO}  Presets: {COUNT}/{MAX}")
    ...
    private void PromptSaveName(string heroStringId)
    {
        ...
        InformationManager.ShowTextInquiry(new TextInquiryData(
            new TextObject("{=dots_ep_save_title}Save Preset").ToString(),
            new TextObject("{=dots_ep_save_msg}Enter a name. The preset captures your current battle and civilian equipment, plus mount.").ToString(),
            true, true,
            new TextObject("{=dots_ep_ok}OK").ToString(),
            new TextObject("{=dots_ep_cancel}Cancel").ToString(),
            name =>
    ...
    private void PromptSelectPreset(string heroStringId, string action)
    {
        ...
        {
            "load"   => new TextObject("{=dots_ep_select_load}Load Preset").ToString(),
            "update" => new TextObject("{=dots_ep_select_update}Update Preset").ToString(),
            "delete" => new TextObject("{=dots_ep_select_delete}Delete Preset").ToString(),
            _ => new TextObject("{=dots_ep_select}Select Preset").ToString(),
        };
    ...
    private void LoadAndAnnounce(string heroStringId, string presetId)
    {
        ...
        {
            DisplayInfo(new TextObject("{=dots_ep_load_failed}Load failed.").ToString());
            return;
        }
        _screen.RefreshActiveVM();
        if (report.HasIssues)
        {
            var msg = new TextObject("{=dots_ep_load_partial}Loaded {EQUIPPED} item(s). Missing items: {MISS_ITEMS}. Missing modifiers: {MISS_MODS}.")
        ...
        {
            DisplayInfo(new TextObject("{=dots_ep_load_ok}Preset loaded ({COUNT} items).")
    ...
    private void UpdateAndAnnounce(string heroStringId, string presetId)
    {
        ...
        {
            UpdateOutcome.Updated     => new TextObject("{=dots_ep_updated}Preset updated.").ToString(),
            UpdateOutcome.NotFound    => new TextObject("{=dots_ep_notfound}Preset not found.").ToString(),
            UpdateOutcome.Disabled    => new TextObject("{=dots_ep_disabled}Equipment Presets are disabled in MCM.").ToString(),
            UpdateOutcome.NoActiveHero=> new TextObject("{=dots_ep_no_hero}No active hero on the inventory screen.").ToString(),
            _ => string.Empty,
        });
    }
    ...
    private void DeleteAndAnnounce(string heroStringId, string presetId)
    {
        InformationManager.ShowInquiry(new InquiryData(
            new TextObject("{=dots_ep_delete_title}Delete Preset").ToString(),
            new TextObject("{=dots_ep_delete_msg}Are you sure you want to delete this preset?").ToString(),
            true, true,
            new TextObject("{=dots_ep_yes}Yes").ToString(),
            new TextObject("{=dots_ep_no}No").ToString(),
            () =>
            {
                var outcome = _service.DeletePreset(heroStringId, presetId);
                DisplayInfo(outcome switch
                {
                    DeleteOutcome.Deleted  => new TextObject("{=dots_ep_deleted}Preset deleted.").ToString(),
                    DeleteOutcome.NotFound => new TextObject("{=dots_ep_notfound}Preset not found.").ToString(),
                    DeleteOutcome.Disabled => new TextObject("{=dots_ep_disabled}Equipment Presets are disabled in MCM.").ToString(),
                    _ => string.Empty,
                });
            },
            () => { }));
    ...
    private void AnnounceSaveOutcome(SaveOutcome outcome, string name)
    {
        switch (outcome)
        {
            case SaveOutcome.Saved:
                DisplayInfo(new TextObject("{=dots_ep_saved}Preset '{NAME}' saved.")
                    .SetTextVariable("NAME", name).ToString());
                break;
            case SaveOutcome.MaxReached:
                DisplayInfo(new TextObject("{=dots_ep_max}Max presets reached ({MAX}). Delete one to save a new preset.")
                    .SetTextVariable("MAX", _settings.MaxPresetsPerCharacter).ToString());
                break;
            case SaveOutcome.Disabled:
                DisplayInfo(new TextObject("{=dots_ep_disabled}Equipment Presets are disabled in MCM.").ToString());
                break;
            case SaveOutcome.NoActiveHero:
                DisplayInfo(new TextObject("{=dots_ep_no_hero}No active hero on the inventory screen.").ToString());
                break;
            case SaveOutcome.InvalidName:
                DisplayInfo(new TextObject("{=dots_ep_bad_name}Preset name cannot be empty.").ToString());
                break;
        }
    }

    private static string FormatPresetSummary(HoNEquipmentPreset p) =>
        new TextObject("{=dots_ep_psummary}Battle: {B}  Civilian: {C}  Mount: {M}")
