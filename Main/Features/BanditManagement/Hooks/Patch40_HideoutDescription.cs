...
namespace DOTS.Features.BanditManagement.Hooks;
...
/// <c>desert_bandits</c>, <c>steppe_bandits</c>). DOTS renamed those cultures, so the engine
...
public static class Patch40_HideoutDescription
{
    ...
    private static IHideoutDescriptionService GetService() =>
        _service ??= DOTS.IoC.Resolve<IHideoutDescriptionService>();
