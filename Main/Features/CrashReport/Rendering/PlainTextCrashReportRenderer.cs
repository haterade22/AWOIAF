...
public sealed class PlainTextCrashReportRenderer : ICrashReportRenderer
{
    ...
    public string Render(ExceptionContext c)
    {
        ...
        sb.AppendLine($"DOTS CrashReport — captured {c.CapturedAtUtc:yyyy-MM-dd HH:mm:ss.fff} UTC");
    ...
    private static void RenderIdentity(StringBuilder sb, IdentitySnapshot id)
    {
        ...
        sb.AppendLine($"DOTS:       {id.DotsVersion} (dll sha1 {id.DotsDllSha1})");
    ...
    private static void RenderDotsState(StringBuilder sb, DotsStateSnapshot t)
    {
        Section(sb, "DOTS State");
        if (t.Career != null)
    ...
    {
        Section(sb, "Logs");
        sb.AppendLine($"DOTS debug log: {l.DotsDebugLogPath ?? "?"}");
        if (l.DotsDebugLogTail.Count > 0)
        {
            sb.AppendLine($"DOTS tail (last {l.DotsDebugLogTail.Count}):");
            foreach (var line in l.DotsDebugLogTail) sb.AppendLine($"  {line}");
        }
