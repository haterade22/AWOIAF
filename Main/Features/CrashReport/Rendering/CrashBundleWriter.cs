...
//
// Bundle filename: dots_crash_{utc}_{sig8}.zip under Logs/
// Contents:
//   report.txt          — full PlainText rendering
//   report.json         — JSON serialisation of ExceptionContext
//   dots_debug.log      — current session's DOTS debug log (if path resolves)
...
public sealed class CrashBundleWriter
{
        ...
        string outputDirectory)
    {
        ...
        try
        {
            ...
            zipPath = Path.Combine(outputDirectory, $"dots_crash_{stamp}_{sig8}.zip");
        }
        catch { return null; }
        ...
        try
        {
            ...
            TryCopyFile(zip, "dots_debug.log", context.Logs?.DotsDebugLogPath);
    ...
    private static string BuildManifest(ExceptionContext c, string text, string json)
    {
        ...
        sb.AppendLine($"DOTS CrashReport bundle");
        sb.AppendLine($"Captured: {c.CapturedAtUtc:O}");
        sb.AppendLine($"Signature: {c.CrashSignature}");
        sb.AppendLine($"DOTS version: {c.Identity.DotsVersion}");
        ...
        if (!string.IsNullOrEmpty(c.Logs?.DotsDebugLogPath))
            sb.AppendLine($"dots_debug.log    source={c.Logs.DotsDebugLogPath}");
