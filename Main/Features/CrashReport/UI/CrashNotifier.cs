...
namespace DOTS.Features.CrashReport.UI;
    ...
    public static void Notify(string exceptionType, string? bundlePath, string? logPath)
    {
        try
        {
            ...
            var inquiry = new InquiryData(
                titleText: new TextObject("{=DOTS_CrashReport_Title}DOTS caught a crash").ToString(),
                text: msg,
                isAffirmativeOptionShown: true,
                isNegativeOptionShown: !string.IsNullOrEmpty(bundlePath),
                affirmativeText: new TextObject("{=DOTS_CrashReport_Continue}Continue (risky)").ToString(),
                negativeText: new TextObject("{=DOTS_CrashReport_OpenBundle}Open bundle folder").ToString(),
