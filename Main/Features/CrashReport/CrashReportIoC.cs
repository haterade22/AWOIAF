using DryIoc;
using DOTS.Features.CrashReport.Adapters;
using DOTS.Features.CrashReport.Collectors;
using DOTS.Features.CrashReport.Rendering;
...
public static class CrashReportIoC
{
    public static void RegisterCrashReportFeature(IContainer container)
    {
        ...
        // DotsStateCollector accepts feature-state lambdas; v1 stubs all of them.
        // Future PRs wire ICareerService etc. into here.
        container.RegisterDelegate<DotsStateCollector>(_ => new DotsStateCollector(),
            Reuse.Singleton);

        // LogTailCollector needs the IModLogger path.
        container.RegisterDelegate<LogTailCollector>(r =>
            new LogTailCollector(() => r.Resolve<DOTS.Core.Logging.IModLogger>().LogFilePath),
            Reuse.Singleton);
        ...
        container.Register<DOTS.Features.CrashReport.Hooks.AppDomainExceptionHook>(Reuse.Singleton);
        container.Register<DOTS.Features.CrashReport.Hooks.Native2ManagedPatcher>(Reuse.Singleton);
    }
}
