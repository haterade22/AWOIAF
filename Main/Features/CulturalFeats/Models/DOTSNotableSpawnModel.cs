...
/// specific DOTS cultures more notables for the AI's recruitment pool — most importantly
...
///
/// Maps the sealed TaleWorlds <see cref="Occupation"/> enum to DOTS-owned
/// <see cref="NotableOccupationKind"/> at the boundary so the service stays free of engine
/// types (ADR-007).
/// </summary>
public class DotsNotableSpawnModel : DefaultNotableSpawnModel
{
    private readonly ICulturalFeatsService _feats;

    public DotsNotableSpawnModel(ICulturalFeatsService feats)
    {
        _feats = feats;
    }
    ...
    /// the DOTS-owned <see cref="NotableOccupationKind"/> so the service is free of
