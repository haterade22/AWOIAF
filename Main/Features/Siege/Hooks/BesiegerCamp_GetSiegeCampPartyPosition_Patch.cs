...
namespace DOTS.Features.Siege.Hooks;
...
public class BesiegerCamp_GetSiegeCampPartyPosition_Patch
{
        ...
        ref CampaignVec2 __result)
    {
        try
        {
            ...
            Debug.Print(
                $"DOTS: WARNING — Settlement '{settlementName}' (id={settlementId}) has no siege_camp_1 scene entities (camp2={camp2Count} frames). Fix in map editor.",
                0, Debug.DebugColor.Red, 17592186044416uL);

            if (siegeCamp2GlobalFrames != null && siegeCamp2GlobalFrames.Length > 0)
            {
                Debug.Print(
                    $"DOTS: Using siege_camp_2 frames as fallback for '{settlementId}'",
            ...
            Debug.Print(
                $"DOTS: No siege camp frames at all for '{settlementId}', generating positions around gate",
                0, Debug.DebugColor.Red, 17592186044416uL);
        ...
        {
            Debug.Print($"DOTS: BesiegerCamp patch exception: {ex.Message}", 0, Debug.DebugColor.Red, 17592186044416uL);
            return true;
        }
    }
}
