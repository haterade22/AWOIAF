...
{
    public override string Id => "DOTS.BattleLoadDiagnostics";
    public override string DisplayName => "DOTS — Battle Load Diagnostics";
    ...
    [SettingPropertyBool("Enable Battle Load Diagnostics", Order = 0,
        HintText = "Logs the full attack->battle-playable lifecycle (encounter, scene selection, Mission.Initialize, every initial-spawn agent's equipment + collision-mesh names) to the DOTS debug log. Leave ON while diagnosing the intermittent battle-load hang — the LAST line in the log names the stuck phase / agent.")]
