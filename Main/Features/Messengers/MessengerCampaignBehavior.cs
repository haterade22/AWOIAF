...
// DOTS boundary class (ADR-002 / ADR-007). Touches sealed TaleWorlds types directly because that is what
...
public class MessengerCampaignBehavior : CampaignBehaviorBase, IMissionListener
{
    ...
    public bool CanSendMessenger(Hero targetHero, out TextObject reason)
    {
        ...
        {
            reason = new TextObject("{=dots_messenger_disabled}The messenger system is disabled in settings.");
            return false;
        }
    ...
    // Boundary conversion: TaleWorlds.Library.Vec2 → DOTS-owned MapCoord. Vec2 stays at the boundary;
    ...
    private void NotifyMessengerLost(PendingMessenger messenger, Hero deadTarget)
    {
        ...
        var text = new TextObject("{=dots_messenger_recipient_gone}Your messenger to {HERO_NAME} returned without delivering — the recipient is no longer reachable.");
        text.SetTextVariable("HERO_NAME", targetName);
        ShowInquiry(
            new TextObject("{=dots_messenger_lost_title}Messenger Lost").ToString(),
            text.ToString(),
