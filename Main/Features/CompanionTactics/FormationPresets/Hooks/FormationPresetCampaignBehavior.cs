...
namespace DOTS.Features.CompanionTactics.FormationPresets.Hooks;
...
public sealed class FormationPresetCampaignBehavior : CampaignBehaviorBase
{
    ...
    public override void SyncData(IDataStore dataStore)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Phase 9b #139 P1 — surface to player. Pre-fix only LogWarning to DOTS internal log;
            // player never saw the cause and lost presets repeatedly with no idea why.
            try
            {
                InformationManager.DisplayMessage(new InformationMessage(
                    "[DOTS] Formation presets could not be loaded — reset to empty (see log for details).",
