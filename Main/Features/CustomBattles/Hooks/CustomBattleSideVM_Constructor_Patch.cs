...
public static class CustomBattleSideVM_Constructor_Patch
{
    ...
    public static void Postfix(CustomBattleSideVM __instance)
    {
        try
        {
            ...
            __instance.FactionSelectionGroup = new DotsFactionSelectionVM(callback);
            _logger?.LogInfo($"[CustomBattles] Injected DotsFactionSelectionVM with {__instance.FactionSelectionGroup.Factions.Count} factions");
        ...
        {
            _logger?.LogError($"[CustomBattles] Failed to inject DotsFactionSelectionVM: {ex.Message}");
        }
    }
}
