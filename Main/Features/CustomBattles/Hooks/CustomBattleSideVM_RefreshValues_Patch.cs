...
public static class CustomBattleSideVM_RefreshValues_Patch
{
    ...
    public static void Postfix(CustomBattleSideVM __instance)
    {
        try
        {
            ...
            {
                _logger?.LogWarning($"[CustomBattles] No commanders matched culture '{selectedFaction.StringId}' on RefreshValues — dropdown left unfiltered. Verify lords.xml culture tags align with the BasicCultureObject.StringId surfaced by DotsFactionSelectionVM.");
                return;
            }
