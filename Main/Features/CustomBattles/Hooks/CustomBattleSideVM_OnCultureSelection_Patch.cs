...
public static class CustomBattleSideVM_OnCultureSelection_Patch
{
    ...
    public static void Postfix(CustomBattleSideVM __instance, BasicCultureObject selectedCulture)
    {
        try
        {
            ...
            {
                _logger?.LogWarning($"[CustomBattles] No commanders matched culture '{selectedCulture.StringId}' — dropdown left unfiltered. Verify lords.xml culture tags align with the BasicCultureObject.StringId surfaced by DotsFactionSelectionVM.");
                return;
            }
