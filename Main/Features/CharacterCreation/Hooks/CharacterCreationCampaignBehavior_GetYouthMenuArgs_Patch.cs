...
namespace DOTS.Features.CharacterCreation.Hooks;
...
public static class CharacterCreationNarrativeStageView_SpawnNonHuman_Patch
{
    ...
    static Exception Finalizer(Exception __exception)
    {
        ...
        {
            try { IoC.Resolve<DOTS.Core.Logging.IModLogger>()?.LogWarning(
...
/// body skeleton. For DOTS custom races (dwarf/elf/orc), this mismatch produced broken poses
...
public static class CharacterCreationNarrativeStageView_RefreshAgentVisuals_Patch
{
    ...
    static void Prefix(object __instance)
    {
        ...
        {
            try { IoC.Resolve<DOTS.Core.Logging.IModLogger>()?.LogWarning($"[Patch20-RaceSync] failed: {ex.Message}"); } catch { }
        }
    }
}
...
/// horizontally-stretched / lying-down mesh on every DOTS race (all use the human_skeleton
