...
public class FileLogger : IModLogger
{
    ...
    public FileLogger()
    {
        ...
        _logPath = Path.Combine(LogDirectory, $"dots_debug_{timestamp}.log");
        Directory.CreateDirectory(LogDirectory);
        _logFile = new StreamWriter(_logPath, true);

        _writerThread = new Thread(ProcessQueue) { IsBackground = true, Name = "DOTS.FileLogger" };
        _writerThread.Start();
    }
