...
namespace DOTS.Dependencies.Foundation;
...
///
/// Opt-out via <c>saveshield-swallow-disabled.flag</c> in the DOTS.Dependencies module
/// directory.
///
...
public static class SaveShield
{
    ...
    private const string HarmonyId = "DOTS.Dependencies.Foundation.SaveShield";
    ...
    //
    // Codex S2 MED fix 2026-05-27: previously this list contained "DOTS" as a broad
    // prefix, which incorrectly matched consumer assemblies named "DOTS_Online" /
    // "DOTS_Map" (those ARE third-party mods from DOTS's perspective during stack-walk
    // attribution). It also missed bundled BUTR infrastructure assemblies. The list
    // below is the bundled-runtime allowlist; DOTS-owned exact matches are handled
    ...
    private static bool IsEngineAssembly(string asmName)
    {
        ...
        // Exact-match DOTS-owned assemblies. Use exact equality + ".DOTS." prefix —
        // NOT a broad "DOTS" StartsWith which would incorrectly catch DOTS_Online
        // and DOTS_Map (which are independent consumer mods, valid culprits).
