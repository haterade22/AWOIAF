...
///
/// Refuses to shield DOTS-owned SubModules (let our own errors propagate during dev).
///
...
public static class SubModuleConstructionGuard
{
    ...
    private const string HarmonyId = "DOTS.Dependencies.Foundation.SubModuleConstructionGuard";
    ...
    /// the offending submodule's assembly, and swallows non-DOTS exceptions.
    ///
    ...
    private static Exception? SwallowFinalizer(object __instance, object[] __args, Exception __exception)
    {
        ...
        try
        {
            ...
            // Don't shield DOTS-owned ctor failures — we want to see those.
