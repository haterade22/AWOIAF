...
using DOTS.Dependencies.Foundation;
...
public class SubModule : MBSubModuleBase
{
    ...
    /// SubModule static cctor (when DOTS.Dependencies loads in normal order) AND by
    ...
    public static void InstallAssemblyResolveHandler()
    {
        ...
        try
        {
            ...
            EarlyLog.Info($"[DOTS.Dependencies] AssemblyResolve installed for {RedirectedSimpleNames.Length} simple names");
        }
        catch (Exception ex)
        {
            DiagLog.LogCaught("Dependencies", "InstallAssemblyResolveHandler", ex);
            EarlyLog.Error($"[DOTS.Dependencies] InstallAssemblyResolveHandler failed: {ex.Message}");
        }
    }
    ...
    private static Assembly? RedirectBundledDependencies(object sender, ResolveEventArgs args)
    {
        ...
        foreach (var simpleName in RedirectedSimpleNames)
        {
            ...
            foreach (var loaded in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    EarlyLog.Info($"[DOTS.Dependencies] AssemblyResolve: redirecting '{args.Name}' to loaded '{loaded.FullName}'");
                    return loaded;
                }
            }
        }
    ...
    protected override void OnSubModuleLoad()
    {
        ...
        try
        {
            ...
            EarlyLog.Info("[DOTS.Dependencies] UIExtenderEx static cctor executed (system patches applied)");
        }
        catch (Exception ex)
        {
        ...
        // have no `bin/<platform>/DOTS.Dependencies.dll` (the DLL lives in DOTS.Dependencies's
        // own bin/, not duplicated to each stub). OnSubModuleLoad fires for DOTS.Dependencies's
        ...
        // DOTS anyway since our code isn't loaded yet).
        ...
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "CollectAssemblyTypesShim.Install", ex); EarlyLog.Error($"[DOTS.Dependencies] CollectAssemblyTypesShim.Install failed: {ex.Message}"); }

        try { DiagLog.Log("Dependencies", "OnSubModuleLoad: → SubModuleConstructionGuard.Install"); SubModuleConstructionGuard.Install(); }
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "SubModuleConstructionGuard.Install", ex); EarlyLog.Error($"[DOTS.Dependencies] SubModuleConstructionGuard.Install failed: {ex.Message}"); }

        try { DiagLog.Log("Dependencies", "OnSubModuleLoad: → PatchShield.Install (pass 1)"); PatchShield.Install(); }
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "PatchShield.Install", ex); EarlyLog.Error($"[DOTS.Dependencies] PatchShield.Install failed: {ex.Message}"); }

        try { DiagLog.Log("Dependencies", "OnSubModuleLoad: → SaveShield.Install"); SaveShield.Install(); }
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "SaveShield.Install", ex); EarlyLog.Error($"[DOTS.Dependencies] SaveShield.Install failed: {ex.Message}"); }
        ...
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "ProcessExit hook", ex); EarlyLog.Error($"[DOTS.Dependencies] ProcessExit hook failed: {ex.Message}"); }
    ...
    public override void OnGameInitializationFinished(Game game)
    {
        ...
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "MarkSessionLaunchSuccessful", ex); EarlyLog.Error($"[DOTS.Dependencies] MarkSessionLaunchSuccessful failed: {ex.Message}"); }
        ...
        catch (Exception ex) { DiagLog.LogCaught("Dependencies", "PatchShield.Install pass2", ex); EarlyLog.Error($"[DOTS.Dependencies] PatchShield.Install (post-init) failed: {ex.Message}"); }
