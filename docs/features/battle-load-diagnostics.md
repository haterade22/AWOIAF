...
MCM page **"DOTS — Battle Load Diagnostics"** (`BattleLoadDiagnosticsSettings`, auto-registered by MCM). Defaults are the "diagnose now" posture — everything ON.
...
- `DOTS.Core.Logging.IModLogger` / `FileLogger` (the log sink; its per-line background flush is what makes the hang survivable).
- `DOTS.Features.CrashReport.ICrashReportService` (optional — the watchdog's bundle trigger).
- `DOTS.Core.Validation.FiniteFloatValidator` (watchdog threshold guard).
- MCM (`AttributeGlobalSettings`).

## Tests

`DOTS.Tests/Features/BattleLoadDiagnostics/` (26 tests, all green):
...
Open the user's `Modules/.../Logs/dots_debug_<timestamp>.log` and find the last `[BattleLoad]` line:
...
- a `WATCHDOG STILL LOADING after Ns — last phase=…` line → the watchdog fired; the `last phase` is the freeze point, and a `dots_crash_*.zip` bundle was written alongside.
