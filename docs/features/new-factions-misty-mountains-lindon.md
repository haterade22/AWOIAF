...
Adds **four new kingdoms** and two new cultures to DOTS: the **Misty Mountain Orcs** and
...
map author (drop them into `tools/dots_new_factions_layout.json` + re-run the settlements generator).
...
DOTS's diplomacy uses `AllianceTier`: **Permanent** (force-started at launch, war forbidden),
...
`AlignmentService` reads (free/evil/neutral side) — it drives `DotsExecutionRelationModel`
...
Both orc cultures have authored feats (`Main/Features/CulturalFeats/DotsCulturalFeats.cs` +
`CulturalFeatsService.cs` + `<cultural_feats>` in `dots_spcultures.xml`):
...
skill templates (`dots_elf_king/warrior/lady_skills`).

### Generators (re-runnable, idempotent)

| Script | Produces |
|---|---|
| `tools/dots_new_factions_layout.json` | Authoritative settlement/clan/kingdom layout (built from scene positions) |
| `tools/generate_new_factions.py` | `troops_{c}.xml`, `npcs_{c}.xml`, `dots_equipment_sets_{c}.xml` (orc cultures) |
...
| `tools/generate_new_faction_settlements.py` | `<Settlement>` blocks; `--apply` writes the live `DOTS_Map/settlements.xml` (+ timestamped backup) |
...
| `tools/make_new_factions_playable.py` | The 4 CC faction-map cards in `factions.json` (`--apply` dry-run guard); then `tools/harvest_factionmap_strings.py` propagates the card strings into `dots_module_strings.xml` |
...
(`<!-- DOTS-NEWFACTIONS:… -->` markers stripped + re-inserted).
...
|---|---|---|
| `dots_child_equipment_templates.xml` | `IsChildEquipmentTemplate` + `IsLordTemplate` (+`IsFemaleTemplate`) | 6 (noble/townsman/villager × m/f) |
| `dots_lord_template_equipment.xml` | `IsLordTemplate`, and `IsTeenagerEquipmentTemplate` on the teen rosters | 10 (lord/ruler battle/civ/teen × m/f) |
| `dots_education_equipment_templates.xml` | none (id-suffix `_<culture>`, childhood-education events) | 98 |
...
|---|---|---|
| Cultures | `dots_spcultures.xml` | +2 `<Culture>` blocks (no `<cultural_feats>`) |
...
| Equipment | `equipmentsets/dots_equipment_sets_{goblin,mistymountainorcs}.xml` | new (10 rosters each) |
| Wanderers | `dots_wanderers.xml`, `equipmentsets/dots_wanderer_equipment.xml` | +10 wanderers + roster per culture |
| Party templates | `dots_partyTemplates.xml` | +12 kingdom templates per orc culture |
| Kingdoms | `dots_spkingdoms.xml` | +3 `<Kingdom>` blocks + sibling relationships |
...
| Settlements | `DOTS_Map/ModuleData/settlements.xml` (LIVE, external) | +45 `<Settlement>` (968 total) |
...
| Localization | `dots_module_strings.xml` | `str_faction_*` / `str_culture_*` per orc culture |
...
6. **Careers** — the orc cultures are CC-selectable but have no career trees yet (parked in
   `CareerCultureCoverageTests` documentedExceptions). Add to `dots_careers.xml` when desired.
