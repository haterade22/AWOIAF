...
Ported from LOTRAOM (Bannerlord 1.2.12) with DOTS conventions (adapter discipline, primitive-dict save, MCM-via-`DotsSettings`, JSON-validated advanced tunables, no `SaveableTypeDefiner`).
...
- **DOTS requirement:** DOTS disables fast-travel and the LOTR-flavored campaign relies heavily on cross-map diplomacy (alliances, named-companion recruitment, lord conversations). Hunting heroes across the map is friction without payoff.
...
Layered DOTS architecture. The behavior is the TaleWorlds-bound boundary; the service is pure logic; the store owns mutable state; the config provider validates JSON tunables; the settings provider wraps MCM.
...
- **`MessengerSettingsProvider`** — wraps `DotsSettings.Instance.*` so the service can be mocked in tests.
...
```
DotsSettings.cs (MCM)            messenger_config.json (advanced)
        |                                  |
        v                                  v
...
### MCM (player-facing) — [DotsSettings.cs](../../Main/Features/DotsSettings.cs)
...
| [Main/Features/Messengers/MessengerSettingsProvider.cs](../../Main/Features/Messengers/MessengerSettingsProvider.cs) | Reads `DotsSettings.Instance.*` |
...
| [Main/_Module/ModuleData/dots_messenger_strings.xml](../../Main/_Module/ModuleData/dots_messenger_strings.xml) | EN base — 29 keys |
| `Main/_Module/ModuleData/Languages/<LANG>/std_dots_messenger_strings_*.xml` | 12 language variants (BR/CNs/CNt/DE/FR/IT/JP/KO/PL/RU/SP/TR) — English placeholder text where untranslated |

## Dependencies

- `IMessengerSettingsProvider` — wraps `DotsSettings.Instance` for the 4 MCM toggles
...
- [DOTS.Tests/Features/Messengers/MessengerServiceTests.cs](../../DOTS.Tests/Features/Messengers/MessengerServiceTests.cs) — 26 tests covering all 11 `MessengerValidationResult` rejection paths, accident roll boundaries (p=0/1/disabled), position math edge cases (distance < / = / > speed; invalid target), speed calculation (zero/negative days clamping, multiplier).
- [DOTS.Tests/Features/Messengers/MessengerStateStoreTests.cs](../../DOTS.Tests/Features/Messengers/MessengerStateStoreTests.cs) — 15 tests covering Add/Remove/Get/Contains/Clear, duplicate-id replacement, null/empty input, save roundtrip preserves all fields, malformed-on-load drops + warns.
- [DOTS.Tests/Features/Messengers/MessengerConfigProviderTests.cs](../../DOTS.Tests/Features/Messengers/MessengerConfigProviderTests.cs) — 14 tests covering valid JSON, missing file, malformed JSON, every validation rule (negative chance, >1.0 chance, zero/negative speed mult, absurd speed mult), boundary values, partial JSON merge, caching, info-vs-warning logging.

The boundary classes (`MessengerCampaignBehavior`, UI mixin/prefab, `MessengerSettingsProvider`, `MessengerRandomSource`, `MessengerIoC`) follow DOTS's "boundaries are not unit-tested" convention and are validated through manual game-testing (see GitHub issue #109 manual test plan).
...
4. Add the localization key (`dots_messenger_<reason>`) to all 13 string files (1 EN base + 12 language variants).
...
- **Issue:** [#109 — feat(messengers): port LOTRAOM messenger system to DOTS (1.3.15)](https://github.com/haterade22/DOTS/issues/109)
