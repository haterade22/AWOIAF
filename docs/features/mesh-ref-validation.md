...
**Key design decisions (mirrors `dots_schema.py` patterns):**
...
- **No MCP tool / no pre-commit hook (yet).** This is a diagnostic run on demand, not a commit gate. An MCP tool (`mesh_ref_check`) over the same engine could be added later alongside the `dots-moduledata` server if interactive querying proves useful; it was deliberately left out of v1 to keep scope tight.
