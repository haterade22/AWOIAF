...
physics that never registers. Hooks are installed at boot from `DotsSubModule.OnBeforeInitialModuleScreenSetAsRoot`
...
- **Vanilla behavior (covers_head):** When a character equips a helmet with
  ...
  during ragdoll / animation in DOTS cultures with covers_head helmets (Gondor
...
- **DOTS requirement:** Many DOTS cultures use cloth-flagged hair / beard
...
mod ships a v1.3.15-only DLL. DOTS is on v1.4.5 and wants the hooks to keep
...
1. **Vendor the C++ source in-repo** under `Dependencies/NativeSkinFixes.NativeHooks/`
   so the source-of-truth ships with DOTS and any developer with MSVC can
...
3. **Inline the C# wrapper into `DOTS.dll`** under `Main/Features/NativeSkinFixes/`.
   ...
   `DotsSubModule.OnBeforeInitialModuleScreenSetAsRoot` (after all modules
...
```
DotsSubModule.OnBeforeInitialModuleScreenSetAsRoot
        |
        v
...
NativeHookLoader.EnsureLoaded()                    -- Win32 LoadLibrary
        |                                              "Main/_Module/bin/.../DOTS.NativeSkinFixes.dll"
        v
...
| `Main/Features/NativeSkinFixes/Interop/NativeHookLoader.cs` | LoadLibrary + GetProcAddress wrappers for `DOTS.NativeSkinFixes.dll` |
...
| `Dependencies/NativeSkinFixes.NativeHooks/Logging.{h,cpp}` | Unified log to `%USERPROFILE%\Documents\Mount and Blade II Bannerlord\Logs\DOTS\NativeSkinFixes.log` |
| `Dependencies/NativeSkinFixes.NativeHooks/NativeSkinFixes.NativeHooks.vcxproj` | VS C++ project (x64, MSVC v143, C++17) outputting `DOTS.NativeSkinFixes.dll` directly into `Main/_Module/bin/Win64_Shipping_Client/` |
...
| `Main/_Module/bin/Win64_Shipping_Client/DOTS.NativeSkinFixes.dll` | Compiled native DLL (vendored binary, committed via `.gitignore` allowlist) |
| `Main/_Module/bin/Win64_Shipping_Client/MinHook.x64.dll` | Runtime dep of the native DLL (vendored binary) |
| `Main/_Module/ModuleData/dots_module_strings.xml` | Localization entry `dots_nativeskinfixes_loaded` for the boot banner |
...
- `DOTS.Tests/Features/NativeSkinFixes/NativeSkinFixesInstallerTests.cs` — 8
...
1. **Reproduce the failure.** Launch Bannerlord with DOTS, then read
   `%USERPROFILE%\Documents\Mount and Blade II Bannerlord\Logs\DOTS\NativeSkinFixes.log`.
...
3. **Find each target function.** Each entry in `Signatures.h` carries the
   ...
   - `dots-src` dumps of related managed callers that pinvoke into native.
...
8. **Commit the binary** alongside the pattern change: the `.gitignore`
   allowlist permits `Main/_Module/bin/Win64_Shipping_Client/DOTS.NativeSkinFixes.dll`,
...
- **Issue:** TODO — create on next session via `/issue feature "Adopt NativeSkinFixes into DOTS (v1.4.5 port, in-repo)"`
