...
- **DOTS requirement:** DOTS is a Sandbox-only total conversion. "New Campaign" starts vanilla story mode — a completely different experience that bypasses all DOTS content. "Sandbox" is a generic label that doesn't reflect the mod's identity.
- **Without this feature:** Players see "New Campaign" alongside DOTS's entry point, risk clicking into the wrong game mode, and see a generic "Sandbox" label with no mod identity.
...
Pre-compile Shaders     ← DotsPrecompileShaders (guarded, single instance)
Custom Battle
Host Co-op
Join Co-op
Options
...
Bannerlord's initial menu options are managed by `Module.CurrentModule` as a list of `InitialStateOption` objects. They're registered by other modules in `OnSubModuleLoad` (SandBox.View.dll, StoryMode.View.dll), so DOTS must override them after registration completes. Two additional constraints:
...
SubModule.OnBeforeInitialModuleScreenSetAsRoot()  [fires every menu visit]
    ...
    │
    └── if GetInitialStateOptionWithId("DotsPrecompileShaders") == null  [guard]
            → AddInitialStateOption("DotsPrecompileShaders", ...)
```
...
| `"DotsPrecompileShaders"` | Pre-compile Shaders | DOTS (SubModule.cs) | Guarded add |
...
- `DOTS.Tests/Features/MainMenuCustomizer/MainMenuCustomizerServiceTests.cs` — 5 tests:
