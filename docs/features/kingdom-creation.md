...
```
dots_module_strings.xml       (string IDs resolved first)
         |
dots_spcultures.xml           (Culture object registered)
         |
DOTS_spkingdoms.xml           (Kingdom object registered, references Culture)
         |
...
characters/npcs_{id}.xml      (Notable NPCCharacters for settlements)
         |
dots_education_character_templates.xml  (Character creation branch templates)
         |
dots_wanderers.xml            (Wanderer NPCs)
dots_wanderer_skill_sets.xml  (Wanderer SkillSet definitions)
         |
equipmentsets/dots_equipment_sets_{id}.xml  (if new armor; optional)
         |
...
3. `dots_spcultures.xml` — culture definition
4. `DOTS_spkingdoms.xml` — kingdom definition
...
9. `dots_education_character_templates.xml` — 6 entries
10. `dots_wanderers.xml` and `dots_wanderer_skill_sets.xml` — 10 entries each
11. `equipmentsets/dots_equipment_sets_{id}.xml` — only if new culture-specific armor (register in `SubModule.xml`)
...
| Kingdom strings | `dots_{id}_*` prefix | `dots_shaghana_name` |
| Culture/lord strings | `aom_{id}_*` prefix | `aom_lord_SH1_1_name` |

## File Reference

### File 1 — `Main/_Module/ModuleData/dots_module_strings.xml`

Add all localizable strings for the kingdom before touching any other file. Required groups:

**Kingdom strings** (referenced by `DOTS_spkingdoms.xml`):
```xml
<string id="dots_{id}_name"        text="{=dots_{id}_name}Full Kingdom Name" />
<string id="dots_{id}_short_name"  text="{=dots_{id}_short_name}Short Name" />
<string id="dots_{id}_title"       text="{=dots_{id}_title}Kingdom of ..." />
<string id="dots_{id}_ruler_title" text="{=dots_{id}_ruler_title}King" />
<string id="dots_{id}_desc"        text="{=dots_{id}_desc}Encyclopedia description." />
```

**Culture strings** (referenced by `dots_spcultures.xml`):
```xml
...
### File 2 — `Main/_Module/ModuleData/dots_spcultures.xml`
...
### File 3 — `Main/_Module/ModuleData/DOTS_spkingdoms.xml`
...
| `name` | `{=dots_{id}_name}Display Name` |
| `short_name` | `{=dots_{id}_short_name}Short Name` |
| `title` | `{=dots_{id}_title}Kingdom of ...` |
| `ruler_title` | `{=dots_{id}_ruler_title}King` |
| `text` | `{=dots_{id}_desc}Description text` |

The `<relationships>` block must include **every other kingdom** — both custom kingdoms in `DOTS_spkingdoms.xml` and the vanilla kingdoms wrapped by XSLT (aserai, empire, empire_w, empire_s, vlandia, battania, sturgia, khuzait). Relationship stances:
...
### File 8 — `Main/_Module/ModuleData/dots_education_character_templates.xml`
...
### File 9 — `Main/_Module/ModuleData/dots_wanderers.xml`
...
### File 10 — `Main/_Module/ModuleData/dots_wanderer_skill_sets.xml`
...
### File 11 — `equipmentsets/dots_equipment_sets_{culture}.xml` (conditional)
...
```xml
<XmlName id="EquipmentRosters" path="equipmentsets/dots_equipment_sets_{id}"/>
```
...
### File 12 — `DOTS_Map/ModuleData/settlements.xml`

> **Important:** The authoritative settlements file is in the **installed Bannerlord module**, not in the repo. Path: `E:\Steam\steamapps\common\Mount & Blade II Bannerlord\Modules\DOTS_Map\ModuleData\settlements.xml`
...
**Fix:** Find the source clan in `clans.xml` (the clan the new one was derived from) and copy its `banner_key` exactly. For the Harad split, `clan_shaghana_1` (Ezarkia) copies from `clan_aserai_10` (the original Ezarkia definition). The kingdom's own `banner_key` in `DOTS_spkingdoms.xml` should match its ruling clan (clan tier 6).
...
**Fix:** Verify all 10 wanderer entries in `dots_wanderers.xml` have `culture="Culture.{id}"` where `{id}` is the new culture ID.
...
**Fix:** Search `DOTS_Map/ModuleData/settlements.xml` (installed module) for each settlement ID assigned to the new kingdom and verify both `owner` and `culture` attributes are correct.
...
- **String key format:** `{=string_id}Display Text` — the string ID before the closing `}` must exactly match a `<string id="...">` entry in `dots_module_strings.xml`
- **Banner keys:** Long serialized strings (typically 200–2000+ characters). When splitting an existing kingdom, copy the key exactly from the original source clan — do not generate a placeholder. The ruling clan's key should also be used as the kingdom's `banner_key` in `DOTS_spkingdoms.xml`. A short key (under ~100 chars) is always a placeholder and will show a plain generic banner in-game. The Bannerlord banner editor in-game can generate a new key if designing from scratch.
...
| File | Purpose |
|------|---------|
| `Main/_Module/ModuleData/DOTS_spkingdoms.xml` | Kingdom definitions (custom kingdoms only) |
| `Main/_Module/ModuleData/dots_spcultures.xml` | Culture definitions for all custom kingdoms |
...
| `Main/_Module/ModuleData/dots_education_character_templates.xml` | Character creation branch templates |
| `Main/_Module/ModuleData/dots_wanderers.xml` | Wanderer NPCCharacter entries |
| `Main/_Module/ModuleData/dots_wanderer_skill_sets.xml` | Wanderer skill set definitions |
| `Main/_Module/ModuleData/equipmentsets/dots_equipment_sets_{id}.xml` | Custom equipment sets (conditional) |
| `DOTS_Map/ModuleData/settlements.xml` (installed module) | Settlement ownership and culture assignments |
| `Main/_Module/ModuleData/dots_module_strings.xml` | All localizable strings |
...
- `dots_module_strings.xml` — must be loaded before all other kingdom/culture/lord files
