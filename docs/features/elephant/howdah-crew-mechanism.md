...
> the installed **v1.4.5** DLLs (`dots-src`). Answers the question: *"how does ADOD get multiple AI troops on the
> back of the elephant?"* ADOD_Beasts is built for ~1.2.12, so this also records the **API drift** a DOTS port
...
matching signatures. But a verbatim port would silently fail on these (verified against `dots-src` v1.4.5):

| # | Sev | API | ADOD (1.2.12) | v1.4.5 | Fix for a DOTS port |
|---|-----|-----|---------------|--------|---------------------|
| 1 | HIGH | `AgentComponent.OnTickAsAI(float)` | overridden for trample/crew AI | **does not exist** (only `OnTick`/`OnTickParallel`) | move logic to `OnTick`/`OnTickParallel`. **DOTS's elephant already did this** — `ElephantMissionBehavior : MissionLogic.OnMissionTick`, not an AgentComponent. |
...
## What DOTS would need to build a howdah (gaps)
...
4. **Armor→crew→count config** — a DOTS-native equivalent of `additional_elephant_characters.xml` (which LOTR crew
   troop, how many seats per tier). Route the decision through a service + adapter per ADR-007/002.
5. **A stand-and-shoot action** — `act_howdah_stand_bow` + its clip are ADOD assets; DOTS needs its own action-set
   entry / anim (or to reuse a vanilla "ranged from standing point" action).
6. **Apply the 4 drift fixes above** from the start.

## Relationship to DOTS's current elephant

DOTS's elephant today is **mount + trample + mount-lock only** — no crew platform. The trample (`ElephantMissionBehavior`)
