...
Vanilla Bannerlord has no mechanism to alert the player when their allies' settlements are attacked. The player only learns about sieges by stumbling across them on the map. In DOTS's LOTR setting — where Mordor besieging Minas Tirith or Isengard attacking Helm's Deep are defining story moments — having a passive discovery model breaks narrative immersion.

- **Vanilla behavior:** No notification; player discovers sieges organically by map scanning
- **DOTS requirement:** Active alert with a timed response window, matching the "call to arms" mechanic seen in other LOTR total conversion mods
...
### MCM Settings (`DotsSettings.cs`, group "Siege Defense")
...
| `Main/Features/Siege/SiegeDefenseSettingsProvider.cs` | Reads `DotsSettings.Instance` |
...
| `DOTS.Tests/Features/Siege/SiegeDefenseServiceTests.cs` | 17 unit tests |
...
- `DOTS.Tests/Features/Siege/SiegeDefenseServiceTests.cs` — 17 tests covering:
...
- **Issue:** haterade22/DOTS#67 — feat: Siege Defense — timed settlement defense events for player kingdom
- **Status:** Open
