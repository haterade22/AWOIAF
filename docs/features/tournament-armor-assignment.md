...
- **DOTS requirement:** DOTS has 13+ cultures with race-specific character skeletons (dwarves, elves, orcs). Armor is modeled to fit specific skeletons. Applying dwarf chainmail to a human skeleton, or human armor to a dwarf, produces visible clipping and scaling glitches.
...
Tournament participant (any culture/race)
        |
DotsTournamentModel.GetParticipantArmor(participant)
        |
...
Every DOTS culture has a `gear_practice_dummy_<culture>` entry. A grep of `id="gear_practice_dummy_` across `npcs_*.xml` (2026-06-09) finds **17** (coverage filled in session 2026-03-31). Representative sample below — `rohan`, `mistymountainorcs`, and `goblin` also have entries but are not tabulated here:
...
| File | Purpose |
|------|---------|
| `Main/Features/Arena/Models/DotsTournamentModel.cs` | 5 overrides (thin; delegates to `ITournamentService`): participant armor, regular/elite prizes, start/end chance |
...
| `DOTS.Tests/Features/Arena/TournamentServiceTests.cs` | 21 unit tests (`ResolveDummyId` fallback chain, start/end chance, `ShouldDismountInTournament`) |
| `DOTS.Tests/Features/Arena/DotsTournamentModelTests.cs` | 7 unit tests (tier-constant invariants on the model) |
| `Main/SubModule.cs:385` | Registration: `campaignStarter.AddModel(new DotsTournamentModel(IoC.Resolve<ITournamentService>()))` |
| `Main/_Module/ModuleData/characters/npcs_{culture}.xml` | `gear_practice_dummy_*` entries per culture |

## Dependencies

`DotsTournamentModel` takes `ITournamentService` via constructor injection (registered `Reuse.Singleton` in [ArenaIoC.cs](../../Main/Features/Arena/ArenaIoC.cs)). `TournamentService` in turn injects [`IRaceManager`](../../Main/Core/Domain/IRaceManager.cs) (for the dwarf-dismount check). It is no longer instantiated with a no-arg `new`.

## Tests

- `DOTS.Tests/Features/Arena/TournamentServiceTests.cs` — **21 tests**. `ResolveDummyId` fallback chain (participant culture → settlement culture → empire), start/end-chance functions, and `ShouldDismountInTournament` (dwarf/case/non-dwarf/invalid). These moved here from the model test when the logic was extracted to the service (#137).
- `DOTS.Tests/Features/Arena/DotsTournamentModelTests.cs` — **7 tests** (tier-constant invariants on the model).
...
2. No code changes needed — `DotsTournamentModel` picks it up automatically via the culture StringId lookup

## GitHub Issue

- **Issue:** [#52 — feat: DotsTournamentModel — per-participant culture armor assignment](https://github.com/haterade22/DOTS/issues/52)
