...
- **DOTS requirement:** DOTS supports multiple playable races (e.g., Elf, Dwarf, Orc) each with distinct skeletons and facegen animations. The body generator must use the race-appropriate `_facegen` action set so the preview renders correctly.
...
## Tests
No unit tests exist for `CharacterSelection` in `DOTS.Tests/Features/`. The patch modifies generated IL and its correctness depends on the Bannerlord runtime; unit testing a Transpiler requires loading the game assemblies and is not covered.
