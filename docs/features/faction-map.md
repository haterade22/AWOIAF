...
- **DOTS requirement:** DOTS has 10+ custom cultures mapped to specific geographic regions of Middle-earth. Players need to understand what each faction is, where it is, and what its playstyle implies before committing to a culture.
- **Without this feature:** Players see a plain list of DOTS culture names that mean nothing to players unfamiliar with the lore. Culture selection has no geographic grounding.
...
`SetCultureOnCharacterCreation` assigns `Hero.MainHero.Culture` **before** invoking vanilla `SetSelectedCulture`, so the auto-generated family name is drawn from the selected culture's `<clan_names>` rather than the stale default culture. (Vanilla's culture stage sets the culture on click, *before* name generation; the faction map calls `SetSelectedCulture` first, so it must assign the culture explicitly.) For the `vlandia` id — DOTS's Rohan — it then regenerates the clan name to bypass vanilla `FactionHelper.GenerateClanNameforPlayer`'s hardcoded `"dey Corvand"`, guarded on a non-empty `ClanNameList`. See issue [#264](https://github.com/haterade22/DOTS/issues/264).

A separate `TrySwitchToNextMenu_Patch` guards the vanilla next-menu transition to prevent double-advance when DOTS has already advanced the stage.
...
- `ICultureResolverService` (`CultureResolverService`) resolves a DOTS faction id to a live `CultureObject` for passing to `SetCultureOnCharacterCreation`.
...
| `game_faction` | string | DOTS culture id to set on confirmation (empty = no culture). Non-text — leave raw. |
...
| `difficulty` | int | 1–7 scale; 0 omits difficulty line. `1=Very Easy`, `2=Easy`, `3=Medium`, `4=Medium-Hard`, `5=Hard`, `6=Very Hard`, `7=Extreme`. The label text comes from `FactionSelectionService.FormatDifficultyText` which returns `{=dots_faction_difficulty_N}default`. |

### Localization

Every player-facing string in `factions.json` is wrapped in `{=KEY}default` format using the convention `dots_faction_<faction_json_key>_<section>_<index>` (e.g. `dots_faction_stewardship_of_gondor_perk_0_name`). At runtime, `FactionDisplayHelper.Localize(string)` wraps each VM-bound string in `new TextObject(s).ToString()`, which resolves `{=KEY}` against the GameTextManager. Plain English strings pass through unchanged.

Workflow when editing content:
1. Edit `factions.json` content (preserve `{=KEY}` prefix or add new keys per the convention).
2. Run `python tools/harvest_factionmap_strings.py` to update the auto-harvested block in `dots_module_strings.xml`. Idempotent — re-runs replace the marked block without duplication.
3. For difficulty strings or any C# code that returns keyed strings (`FactionSelectionService.FormatDifficultyText`), hand-author the matching `<string>` entry in `dots_module_strings.xml` above the auto-harvested block.
4. Run `python tools/translate_with_claude.py --lang <LANG> --module DOTS --apply` per language to propagate to 11 AI languages (Polish is hand-translated). Cache deduplicates already-translated strings.
5. Validate: `dotnet test DOTS.Tests --filter FactionMap` covers JSON parse, key coverage, per-playable-faction shape; `LanguageDataXmlTests` covers per-language file parse.
...
| `DOTS.Tests/Features/FactionMap/FactionRegistryServiceTests.cs` | Registry lookup |
| `DOTS.Tests/Features/FactionMap/FactionSelectionServiceTests.cs` | Selection result building, color derivation, difficulty text |
| `DOTS.Tests/Features/FactionMap/FactionHoverServiceTests.cs` | Hover state change detection |
| `DOTS.Tests/Features/FactionMap/FactionConfigProviderTests.cs` | JSON parsing |
| `DOTS.Tests/Features/FactionMap/CultureResolverServiceTests.cs` | Culture resolution |
| `DOTS.Tests/Features/FactionMap/LandmarkServiceTests.cs` | Landmark data |
...
3. If the faction is playable, set `"playable": true` and set `"game_faction"` to the matching DOTS culture id.
...
## GitHub Issue
- **Issue:** [#260](https://github.com/haterade22/DOTS/issues/260) — `feat(faction-map): rewrite CC pages + full localization sweep` (2026-06-01)
...
- **Follow-up:** [#264](https://github.com/haterade22/DOTS/issues/264) — `fix(character-creation): family/clan name uses default culture instead of selected; review-stage name field empty` (2026-06-01). `CultureSettingService` culture-before-name ordering + `vlandia`/Rohan clan-name override.
