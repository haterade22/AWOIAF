...
Three independently-toggleable battle-tactics features bundled in one DOTS module:
...
Ported from `Downloads/Features_fixed/CompanionTactics/` (Bannerlord 1.3 mod template) for DOTS v1.3.15. Patch35 reserves the Harmony category. SaveableTypeDefiner BaseId 726900601 (matches the original mod for save-import compat).
...
- **DOTS requirement:** A single feature module that adds (a) glanceable role badges, (b) per-campaign preset persistence with a refuse-on-overflow cap, (c) a contextual action bar in field battles for common stance commands.
...
```
DOTS.Features.CompanionTactics/
...
```
DotsSettings (MCM, GroupOrder 27/28/29)
        |
...
### MCM Settings (DotsSettings.cs, GroupOrder 27/28/29)
...
| `Main/Features/CompanionTactics/CompanionTacticsSettingsProvider.cs` | Reads `DotsSettings.Instance` directly (no reflection) |
...
- `ICompanionTacticsSettingsProvider` — typed read from `DotsSettings.Instance` (testable seam).
- `IModLogger` (DOTS core) — file logger; gated HUD output via Debug toggles.
...
74 tests across 6 files in `DOTS.Tests/Features/CompanionTactics/`:
...
- **Stances are display-only.** Pressing 1–9 in the action bar updates the stance dict and highlights the button, but the formation behavior is unchanged. The original developer's mod was the same — DOTS Phase 1 ports verbatim. Real stance enforcement (firing-order changes, tightened spacing, brace-pose triggers) requires APIs not exposed by the engine and is deferred to a follow-up feature.
...
- SyncData key: `"DOTS_FormationPresets"`.
