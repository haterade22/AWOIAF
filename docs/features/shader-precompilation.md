...
Adds a "Pre-compile Shaders" button to the main menu that runs a hidden custom battle containing all DOTS characters. The Bannerlord engine compiles shaders for every unique model/material it renders, eliminating mid-game stutter caused by first-encounter shader compilation. The loading screen shows live progress during compilation.
...
- **DOTS requirement:** With 13 custom cultures and hundreds of unique armor sets from `LOTRLOME_Armory`, first-encounter stutter is frequent. Players fighting Gondor troops for the first time, entering a new tournament, or encountering a new faction all trigger shader compilation mid-combat.
- **Without this feature:** Players experience frame drops ranging from 100–2000ms whenever the renderer first encounters a DOTS-specific material. This is especially severe on first install when the shader cache is cold.
...
2. Override `OnLoadFinished()` to call `CustomBattleHelper.StartGame()` with a `CustomBattleData` that has all DOTS characters split across both sides.
...
SubModule.OnBeforeInitialModuleScreenSetAsRoot()
    └── Module.CurrentModule.AddInitialStateOption("Pre-compile Shaders", orderIndex=100)
            └── Action: MBGameManager.StartNewGame(new DotsShaderGameManager(service, logger))

DotsShaderGameManager : CustomGameManager
    └── OnLoadFinished()
...
No configuration file. The feature has tunable constants in `DotsShaderGameManager.cs` and `Hooks/LoadingScreen_ShaderProgress_Patch.cs`:

| Constant | Value | Description |
|----------|-------|-------------|
| `MaxTroopsPerSide` | `3000` | Cap on troop slots per side. 6000 total slots, sized to fit ~1600 DOTS characters + vanilla characters with no silent drops. |
...
| `Main/Features/ShaderPrecompilation/DotsShaderGameManager.cs` | Extends `CustomGameManager`, builds `CustomBattleData` from service output |
...
- `DOTS.Tests/Features/ShaderPrecompilation/ShaderPrecompilationServiceTests.cs` — 7 tests covering:
...
`DotsShaderGameManager` and `LoadingScreen_ShaderProgress_Patch` are not unit-tested — they are entry points that directly call TaleWorlds APIs (no logic to test).

## How to Add Coverage for a New Culture

When a new DOTS culture is added, its characters are automatically included — no changes needed here. The service queries `IObjectManagerAdapter.GetAllCultureInfos()` and `GetAllCharacterInfos()` at runtime, picking up every loaded culture (vanilla, DOTS custom, and bandit) and all of its characters.
...
3. The slot budget hasn't filled — the manager logs `[ShaderPrecompilation] N characters skipped` to `rgl_log` if the cap is hit. If you see that line with a non-zero count, raise `MaxTroopsPerSide` or lower `SoldierCopies` in `DotsShaderGameManager.cs`.
...
- [#57 — feat: Shader Pre-compilation at Main Menu](https://github.com/haterade22/DOTS/issues/57) — original feature, OPEN
- [#106 — fix: silent character drop + premature 120s abort + stale latch on retry/abort](https://github.com/haterade22/DOTS/issues/106) — 2026-05-04 stability fix, OPEN until in-game verification
