...
- **Vanilla behavior:** N/A — Gondor in vanilla Bannerlord is `empire_w` with vanilla Battanian gear. DOTS's prior Gondor pass equipped Anórien, Minas Tirith, Osgiliath, Cair Andros, and Minas Ithil with custom `sk_gd_*` items. The 8 southern fiefs reused Anórien generics.
- **DOTS requirement:** Each Gondor region (Lossarnach, Pinnath Gelin, Harondor, Anfalas, Serelond, Lebennin, Belfalas, Lamedon) wears region-specific armor consistent with KEYforce's intent.
...
Two coupled artifacts — one in DOTS, one in the artist's separate LOTRLOME_Armory module — must move in lockstep:
...
- `DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs` — 84 tests, all pass after the `_noble` → `_axebearer` rewiring. No new tests added (the change is data-only ID swap).
...
- **Issue:** [#99 — feat(gondor): KEYforce armor revamp — add 99 items + restructure 13 regional troop trees](https://github.com/haterade22/DOTS/issues/99)
