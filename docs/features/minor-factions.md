...
- **DOTS requirement:** Every faction in the game world should feel like it belongs in Middle-earth
...
Minor factions are defined in SandBox/ModuleData/spclans.xml. DOTS cannot replace this file entirely (other modules depend on it), so changes must be applied via XSLT transformation at load time.
...
SandBox/spclans.xml (vanilla)
        |
  spclans.xslt (DOTS XSLT transform)
        |
  Engine loads transformed XML
        |
  dots_module_strings.xml (localization keys)
```
...
| `Main/_Module/ModuleData/dots_module_strings.xml` | 42 localization strings (name + short_name + text per faction) |
...
No runtime configuration. All data is in the XSLT and string table. To change a faction's name or description, edit the corresponding XSLT template in `spclans.xslt` and the matching string entry in `dots_module_strings.xml`.
...
6. Add corresponding `<string id="DOTS_..." text="..."/>` entries in `dots_module_strings.xml`
...
- String key format matches DOTS conventions
