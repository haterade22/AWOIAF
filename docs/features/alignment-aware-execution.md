...
ExecutionActionHook (IOnExecutionAction)
    ...
    /   \
   /     \
DotsExecution         TraitLevelingHelper
...
[Vanilla] HeroExecutionSceneNotificationData — shows relation preview
    |  (calls DotsExecutionRelationModel.GetRelationChangeForExecutingHero
...
SubModule.OnGameStart:
  campaignStarter.AddModel(new DotsExecutionRelationModel(executionAction))
...
| `Main/Features/Execution/Models/DotsExecutionRelationModel.cs` | GameModel override — delegates to hook | ~33 |
...
### `DOTS.Tests/Features/Execution/AlignmentServiceTests.cs` — 15 tests
...
### `DOTS.Tests/Features/Execution/ExecutionActionHookTests.cs` — 13 tests
...
- [Architecture](../ai-includes/architecture.md) — DOTS architecture patterns
