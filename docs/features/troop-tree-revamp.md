...
Follow-up to #211 (Armory item authoring): #211 shipped 1000+ new armor items per KEYforce's spec but deferred the corresponding troop tree work. This issue closes that gap for 5 cultures — Mordor, Isengard, Dol Guldur, Gundabad, Erebor — by adding 48 new troops, deleting 30 DOTS-fabricated extras not in the spec, and refitting 113 existing troops with spec-compliant equipment. Rhun is handled in a separate session.

## Why This Exists

- **Vanilla behavior:** Bannerlord has no Middle-earth troop trees.
- **Pre-#211 DOTS:** Each culture had a hand-fabricated troop tree shipping in `troops/troops_<culture>.xml`. Many troops were never wired into a Tolkien-canonical role and used legacy equipment IDs predating the LOTRLOME_Armory authoring pass.
...
- New troops must be added to relevant `dots_partyTemplates.xml` blocks or lord armies never spawn them.
...
- **`tools/cleanup_deleted_troops_212.py`** — Sweeps deleted IDs from `dots_partyTemplates.xml`, `troop_weights.xml`, `troop_resource_costs.xml`.
...
tools/apply_<culture>_troop_revamp.py    (per-culture mechanical script)
                  ...
                  --apply ->  | mutates troops_<culture>.xml
                       ...
                       troops_<culture>.xml
                              |
                              v
tools/cleanup_deleted_troops_212.py   ->  dots_partyTemplates.xml
                                          troop_weights.xml
                                          troop_resource_costs.xml
tools/expand_party_templates_212.py   ->  dots_partyTemplates.xml (new stacks)
...
| `Main/_Module/ModuleData/dots_partyTemplates.xml` | Party stack composition per culture |
...
- **"Delete if not in spec" applied strictly** to DOTS-fabricated extras (`mordor_uruk_feller/ravager/executioner/...`, `dg_initiate/disciple/infantry/...`, berserker line, Gundabad duplicates).
...
- **No LOTRLOME Gondor variant exists for crossbow / bolt / javelin** — used vanilla `Item.crossbow_b–g`, `Item.bolt_b/c/d`, `Item.imperial_throwing_spear_1_t4`. Empire-themed vanilla items are intentional Gondor stand-ins (Bannerlord ships Empire as Gondor's aesthetic match; DOTS XSLT remaps Empire→Gondor culture).
