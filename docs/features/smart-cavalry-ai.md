...
- **DOTS requirement:** cavalry should hit-and-run as a clean line, pass through, reform; in mixed-army Middle-earth battles where Rohirrim or Easterlings cavalry support friendly Gondorian/Dol Guldur infantry, riders should route around their allies, not over them.
...
| `Main/Features/SmartCavalryAI/SmartCavalryAISettingsProvider.cs` | `DotsSettings.Instance` wrapper with `??` defaults + clamps. |
...
| `Main/Adapters/Models/MovementOrderType.cs` | DOTS-owned enum (Charge/ChargeToTarget/Other) — wraps `MovementOrder.MovementOrderEnum` so service stays free of TaleWorlds types. |
...
- `IModLogger` (existing)
- `DotsSettings` (existing)

## Tests

`DOTS.Tests/Features/SmartCavalryAI/` — 44 tests across 2 files, MSTest + NSubstitute:
