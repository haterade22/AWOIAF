...
Every DOTS clan that the mod authors or renames gets a distinct `color`/`color2` (heraldry tint)
...
DOTS's armor items use **grayscale (desaturated) textures** so the engine tints them per-faction.
...
tools/generate_clan_heraldry.py   ── consume specs, idempotently edit 3 files:
                                       ...
                                       C. dots_partyTemplates.xml   (upsert <MBPartyTemplate> rosters)
```
...
there); Lothlorien fields **Rivendell**; **Khand (battania)** has no DOTS troop pool → **colours only**
...
| `dots_partyTemplates.xml` | 176 new `kingdom_hero_party_<faction>_<clan>_template` blocks |
...
- `dotnet build Main/DOTS.csproj` → 0 errors (data-only; C# unaffected).
