...
- **DOTS requirement:** Warg bite attacks and any future ability-driven attacks need to check whether a specific animated bone on the attacker intersects bones on nearby targets, then inject a `Blow` into the engine with correct damage and knockdown flags.
...
## Tests
`DOTS.Tests/Features/AdvancedCombat/BoneCollisionServiceTests.cs` — 11 tests covering `IBoneCollisionService.CreateAnimationBoneCheck` / `CreateTimedBoneCheck` and the bone-tracking lifecycle via `IAgentAdapter` + `IAgentVisualsAdapter` substitutes.
