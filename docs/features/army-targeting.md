...
- **DOTS requirement:** LOTR factions need coherent strategic campaigns — Mordor should methodically push west through Osgiliath toward Minas Tirith, not bounce between targets.
...
`DotsTargetScoreModel` overrides `GetTargetScoreForFaction`. For `Besieger` armies only, it multiplies the vanilla base score by a factor from `IArmyTargetingService`:
...
MCM (DotsSettings)
  ...
  - LongRangePriorityBoostScale
        |
ArmyTargetingSettingsProvider (reads DotsSettings.Instance with defaults)
```
...
| File | Purpose |
|------|---------|
| `Main/Features/ArmyTargeting/Models/DotsTargetScoreModel.cs` | GameModel override — intercepts score, extracts primitives, calls service |
...
| `Main/Features/ArmyTargeting/ArmyTargetingSettingsProvider.cs` | Reads MCM `DotsSettings.Instance` with fallback defaults |
...
| `Main/Features/DotsSettings.cs` | MCM property declarations |
...
- `DOTS.Tests/Features/ArmyTargeting/ArmyTargetingServiceTests.cs` — 20 tests:
...
`GetTargetScoreForFaction` is called O(armies × settlements) per 3h AI tick (~500–2000 calls per cycle at DOTS scale). All three service methods are allocation-free:
...
- **Issue:** [#64 — feat: AI Strategic Intelligence — army commitment stickiness + faction priority target lists](https://github.com/haterade22/DOTS/issues/64)
- **Status:** Open
