...
- **DOTS requirement:** Middle-earth fiefs have distinct identities. Minas Tirith should produce Citadel Guard trainees; Dol Amroth should produce Swan Knight squires; Khûndol (deep Rhûn) should produce Dragon-Wrath acolytes. The recruitable pool is also one of the strongest "this place feels different" signals available to the player.
...
`DotsVolunteerModel` extends `DefaultVolunteerModel` and delegates `GetBasicVolunteer` to `IVolunteerRecruitmentService`. The service holds three static dictionaries (settlement / clan / culture) plus a fourth conditional-settlement map for predicated pools. The model also overrides `MaxVolunteerTier => 6` (vanilla 4).
...
| [`Main/Features/TroopProgression/Models/DotsVolunteerModel.cs`](../../Main/Features/TroopProgression/Models/DotsVolunteerModel.cs) | GameModel override. Thin — delegates to service. |
...
- [`DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs`](../../DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs) — ~150 test methods covering:
...
6. **Run `dotnet test DOTS.Tests --filter FullyQualifiedName~VolunteerRecruitmentService`** — must be green.
