...
- `ISiegeDismountSettingsProvider` (this feature) — wraps `DotsSettings`; testable
...
- [DOTS.Tests/Features/SiegeDismount/SiegeDismountServiceTests.cs](../../DOTS.Tests/Features/SiegeDismount/SiegeDismountServiceTests.cs) — 24 tests covering:
...
1. Edit [Main/Features/DotsSettings.cs](../../Main/Features/DotsSettings.cs) `SiegeMountBehavior` property — change `= 3;` to the desired index (0=Vanilla, 1=KeepOnMap, 2=ToInventory, 3=AutoRemount).
...
3. Update the `SiegeMountBehavior` MCM bound: in `DotsSettings.cs`, raise the upper int range (currently `0, 3` → bump to `0, 4`) and update the hint text.
...
2. MCM → DOTS → "Battle Tactics / Siege Dismount" → confirm `Enable = true`, `Mode = 3 (AutoRemount)`.
