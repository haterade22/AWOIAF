...
Custom `ScriptComponentBehavior` subclasses that DOTS's map authors can attach to scene entities in Bannerlord's built-in scene editor. The first script is `CS_Road` — a procedural road/river mesh generator that walks a named scene Path entity, samples it at adaptive spacing, and builds a quad-strip mesh between path edges.

## Why This Exists

A DOTS map author wanted Alliance mod's `CS_Road.cs` so they could paint roads/rivers along scene paths instead of hand-placing individual mesh tiles. The Alliance code is GPL v3 (copyleft), so a verbatim copy would force DOTS into GPL v3 too. Instead, we did a clean-room rewrite: read the Alliance source once to extract a behavioural spec, then implemented from the spec without re-reading the source.
...
- **DOTS requirement:** A reusable scene-script library so map authors can express "road follows this path with this width and material," and the engine builds the geometry automatically. Live preview in the editor.
- **Without this feature:** Map authors hand-place dozens of road tile prefabs per scene; iteration on path shape is tedious; ad-hoc copies of Alliance's GPL code would contaminate DOTS's license.

## Architecture

### Design Challenge

1. **License hygiene.** Alliance is GPL v3. Verbatim copy means DOTS-as-a-whole must distribute under GPL v3. Solution: clean-room rewrite from a behavioural spec, documented in `docs/scene-scripts/ATTRIBUTION.md`.
...
3. **Reflection-only registration.** Scene scripts are discovered when their containing DLL loads — no IoC, no SubModule.xml entry. DOTS.dll is already loaded by the active DOTS module, so simply adding `CS_Road : ScriptComponentBehavior` to it makes the script available.
...
- `DOTS.Core.Validation.FiniteFloatValidator` — NaN/Infinity guards on editor floats

## Tests

- `DOTS.Tests/SceneScripts/Roads/HexColorParserTests.cs` — 14 tests (7/9-char parsing, invalid input, ARGB packing)
- `DOTS.Tests/SceneScripts/Roads/StepCurveParserTests.cs` — 16 tests (braces, whitespace, malformed, NaN/Infinity, sort, fallback)
- `DOTS.Tests/SceneScripts/Roads/StepCurveEvaluatorTests.cs` — 14 tests (lerp, clamp, single-key, null/empty)
- `DOTS.Tests/SceneScripts/Roads/RoadGeometryBuilderTests.cs` — 14 tests (vertex count, perpendicular math, UV bounds, FlipFaces, InvertU, RotateUV)
- `DOTS.Tests/SceneScripts/Roads/RoadPathSamplerTests.cs` — 9 tests (distance walk, NaN/Infinity guards, dense-curve density check)
...
4. **File header on every DOTS file** porting external inspiration: cite source + spec doc + ATTRIBUTION.md.
5. **Append to `docs/scene-scripts/ATTRIBUTION.md`** with the new entry.

## Editor compatibility (launching the modding kit)

DOTS.dll loads in the standalone Bannerlord modding kit (`Win64_Shipping_wEditor`) when the launcher's editor profile has DOTS **and** its community-mod runtime dependencies all active:
...
These mods' SubModule.xml files carry `IsNoRenderModeElement=false` + `DedicatedServerType=none` (added during this project's early editor-mode work — see `Bannerlord.*/SubModule.xml`), so the engine activates them in the editor's no-render context. Their DLLs enter the AppDomain, satisfying DOTS.dll's compile-time references to UIExtenderEx and Harmony types.

**If you launch with only DOTS active** (community mods unchecked), the engine still loads `Modules/DOTS/bin/Win64_Shipping_wEditor/DOTS.dll` but `Assembly.GetTypes()` throws `ReflectionTypeLoadException: Could not load file or assembly 'Bannerlord.UIExtenderEx'` → `TaleWorlds.ModuleManager.Debug.FailedAssert` → user-cancelable crash dialog. Fix: enable the four community mods in the launcher's editor profile.

`Modules/DOTS/bin/Win64_Shipping_wEditor/` is automatically mirrored from `Win64_Shipping_Client/` on every build by the `MirrorWin64ShippingClientToEditor` target in `Main/DOTS.csproj` (runs `AfterTargets="PostBuildCopyToModules"`). This covers DOTS.dll + .pdb plus all companion DLLs (DryIoc, MCMv5, Newtonsoft.Json, BehaviorTrees, BehaviorTreeWrapper, MinHook.x64, DOTS.NativeSkinFixes).
...
1. Open your launcher, switch to the editor profile. Verify enabled: Bannerlord.Harmony, Bannerlord.UIExtenderEx, Bannerlord.MBOptionScreen, Bannerlord.ButterLib, DOTS (plus DOTS_Map if you want the DOTS main map open by default).
...
| CS_StateObject | **Defer — needs SP redesign** | ~310 lines on `SynchedMissionObject`. State-machine concept (states-as-child-entities) is sound but MP plumbing dominates. If DOTS needs a destructible/state system, design SP-native from scratch using this as conceptual reference. |
| CS_UsableObject | **Defer — blocked** | ~245 lines on `UsableMachine`. Depends on Alliance's custom `AnimationPlayer.AnimationSystem.Instance` — that's its own multi-hundred-line port. Use TaleWorlds' native `UsableMachine` + vanilla `act_*` action lookup instead if DOTS ever needs this. |
...
| CS_HealingObject | **Skip** | Subclass of CS_UsableObject → blocked. SP-clean conceptually but no value w/o the base. DOTS's healing model lives in `DotsPartyHealingModel` anyway. |
...
| CS_EditorFixer | **Skip** | Editor utility for fixing prefab `VectorArgument2` import bugs in Alliance's own pipeline. Doesn't affect gameplay. Not applicable to DOTS. |
...
- **Issue:** [#119](https://github.com/haterade22/DOTS/issues/119) — Scene scripts library: CS_Road procedural mesh generator (clean-room port)
