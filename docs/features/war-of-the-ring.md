...
### Layer 1: DotsDiplomacyModel (GameModel)
...
### Layer 2: DotsKingdomDecisionPermissionModel (GameModel)
...
```
war_of_the_ring.json     DotsSettings (MCM)
         \                  /
    WarOfTheRingConfig   IDotsSettingsProvider
        Provider        (injected, testable)
              \          /
          WarOfTheRingService ←── IDiplomacyService (hostile tier lookup)
             /       |       \
            /        |        \
    DotsDiplomacy  DotsKingdom   MakePeace
...
### MCM Integration via IDotsSettingsProvider

MCM settings are accessed through an injected `IDotsSettingsProvider` interface rather than direct static access to `DotsSettings.Instance`. This:
...
| `Main/Features/Diplomacy/Models/DotsDiplomacyModel.cs` | GameModel — IsAtConstantWar override |
...
| `Main/Features/Diplomacy/IDotsSettingsProvider.cs` | MCM settings interface (testable) |
| `Main/Features/Diplomacy/DotsSettingsProvider.cs` | MCM settings implementation |
...
| `Main/Features/DotsSettings.cs` | MCM settings class |
...
- `IDotsSettingsProvider` — MCM settings access (injected, testable)
...
- `DotsKingdomDecisionPermissionModel` now serves both alliance blocking AND peace blocking
- `DiplomacyIoC` registers all WotR services alongside diplomacy services

## Tests

- `DOTS.Tests/Features/Diplomacy/WarOfTheRingServiceTests.cs` — 15 tests covering phase transitions, war declarations, peace blocking, test mode, idempotent re-checks
- `DOTS.Tests/Features/Diplomacy/PeaceActionHookTests.cs` — 3 tests for hook behavior
