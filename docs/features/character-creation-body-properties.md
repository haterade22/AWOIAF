...
- **DOTS requirement:** Lock in a specific cultural silhouette per culture so the starting body matches lore expectations (e.g., Rohirrim taller/leaner, Dunlendings stockier, dwarves shorter). The modder needs to retune frequently without rebuilding C#.
...
1. **`CharacterCreationContent.SetSelectedCulture`** — public method that sets the `SelectedCulture` property; called by DOTS's `FactionMap.CultureSettingService` reflectively when the player confirms a culture on the map. Postfix here applies our body to `Hero.MainHero` and `CharacterObject.PlayerCharacter`.
...
Standard DOTS 4-layer stack: Harmony patches → service → adapter → engine. Three Harmony patches (all in the `Patch29_CCBodyProperties` category) cover the three intercept points; all delegate to the same `ICCBodyPropertiesService.ApplyForCulture(stringId)` so the application logic is single-sourced.
...
|-------|------|-------------|
| `Culture/@id` | string | Culture string id (case-insensitive). Vanilla: `vlandia`, `empire`, `aserai`, `battania`, `sturgia`, `khuzait`. DOTS custom: `mordor`, `gondor`, `erebor`, `mirkwood`, `lothlorien`, `rivendell`, `dolguldur`, `gundabad`, `isengard`, `umbar`, `dale` |
...
### DOTS Culture-ID Reference

In DOTS, several vanilla culture ids are XSLT-rebound to LOTR factions:
...
DOTS custom cultures (`mordor`, `gondor`, `erebor`, `mirkwood`, `lothlorien`, `rivendell`, `dolguldur`, `gundabad`, `isengard`, `umbar`, `dale`) keep their natural ids.
...
No code changes required. Validation warnings will appear in `dots_debug_*.log` (under `<game install>/bin/Win64_Shipping_Client/Logs/`) if the entry is malformed.
