...
Standard DOTS feature module pattern (ADR-002 thin entry, ADR-007 adapter, single-responsibility services). The runtime side has three pieces:

```
DotsBanditDensityModel : DefaultBanditDensityModel       ← GameModel override
...
| Surface | Vanilla | DOTS scaling target |
|---|---|---|
...
Settings live under **DOTS → World / Bandit Scaling** (`GroupOrder = 35`):
...
Each LOTR bandit culture has `is_bandit="true"` and `can_have_settlement="true"` (allows hideout ownership). Troops are pulled from each culture's existing DOTS troop XML — no new troop definitions needed.
...
The 99 existing hideouts in `DOTS_Map/ModuleData/settlements.xml` (external module) had their `culture=` attribute swapped to the new LOTR cultures and their display names rewritten from "Hideout" to "Dunlending Raider's Camp" / "Gundabad Orc Raider's Camp" / "Haradrim Raider's Camp" / "Rhûn Raider's Camp" / "Corsair's Cove". Settlement IDs (`hideout_forest_N`, etc.) were intentionally **left unchanged** to preserve save compatibility — a save started before this migration will load fine and just see the renamed hideouts.
...
When the player visits a hideout, the encounter menu prose led with the literal placeholder **"(Undefined hideout type)"** for every DOTS bandit hideout. Root cause (decompile-verified, v1.4.5): vanilla `HideoutCampaignBehavior.game_menu_hideout_place_on_init` sets the `HIDEOUT_DESCRIPTION` GameText variable to `{=DOmb81Mu}(Undefined hideout type)` and then overrides it **only** for the five hardcoded vanilla bandit culture StringIds. DOTS renamed those cultures, so none match and the placeholder leaks through. The hideout *name* renders correctly because it comes from the settlement `name=` attribute (a different code path); only the description prose is keyed on culture StringId in C#.
...
`IHideoutDescriptionService.GetDescription(cultureStringId)` returns the `{=key}default` template for the five DOTS bandit cultures and `null` for any other culture (vanilla / other-mod hideouts keep their own engine description untouched). The service takes a `string` and returns a `string` — no TaleWorlds types cross the boundary (ADR-007).

| Bandit culture | Description key | Biome flavor |
|---|---|---|
| `dunland_raiders` | `dots_hideout_desc_dunland` | Wooded hills of Dunland |
| `gundabad_raiders` | `dots_hideout_desc_gundabad` | Crags of the Misty Mountains |
| `harad_raiders` | `dots_hideout_desc_harad` | Dunes of Harad |
| `rhun_raiders` | `dots_hideout_desc_rhun` | Grasslands of Rhûn |
| `umbar_corsairs` | `dots_hideout_desc_umbar` | Sheltered southern cove |

The five strings live in [`dots_module_strings.xml`](../../Main/_Module/ModuleData/dots_module_strings.xml). The C# default text in `HideoutDescriptionService` matches the XML default verbatim (so the compiled fallback equals the loaded string).
...
| [`Main/Features/BanditManagement/HideoutDescriptionService.cs`](../../Main/Features/BanditManagement/HideoutDescriptionService.cs) | Culture StringId → `{=key}default` template (null for non-DOTS cultures) |
...
| [`Main/Features/BanditManagement/Models/DotsBanditDensityModel.cs`](../../Main/Features/BanditManagement/Models/DotsBanditDensityModel.cs) | GameModel override (hideout count, parties/hideout, fight troops) |
...
| [`Main/_Module/ModuleData/dots_spcultures.xml`](../../Main/_Module/ModuleData/dots_spcultures.xml) | 5 LOTR bandit culture entries (appended) |
| [`Main/_Module/ModuleData/dots_partyTemplates.xml`](../../Main/_Module/ModuleData/dots_partyTemplates.xml) | 10 raider + boss party templates (appended) |
| [`Main/_Module/ModuleData/dots_module_strings.xml`](../../Main/_Module/ModuleData/dots_module_strings.xml) | Culture display names + male/female names (~80 keys) |
| [`tools/migrate_hideouts_to_lotr.py`](../../tools/migrate_hideouts_to_lotr.py) | DOTS_Map hideout culture + name swap |
| [`DOTS.Tests/Features/BanditManagement/`](../../DOTS.Tests/Features/BanditManagement/) | 50 unit tests (service, config provider, density-model helpers, hideout descriptions) |

## Dependencies

- `DefaultBanditDensityModel` — overridden via `campaignStarter.AddModel` in [SubModule.cs](../../Main/SubModule.cs).
- `DefaultPartySizeLimitModel.FindAppropriateInitialRosterForMobileParty` — Harmony Postfix. Coexists peacefully with [DotsPartySizeModel](../../Main/Features/CulturalFeats/Models/DotsPartySizeModel.cs) (which overrides `GetPartyMemberSizeLimit` only).
- `DotsSettings` (MCM) — 7 properties in the `World/Bandit Scaling` group (added `BanditInitialHideoutsPerFaction`).
- Existing DOTS culture troop XMLs — pulls raider-tier T1–T4 troops by ID; no new troop authoring.
- `dots_partyTemplates.xml` — 10 new templates (5 raider + 5 boss).

## Tests

[`DOTS.Tests/Features/BanditManagement/BanditScalingServiceTests.cs`](../../DOTS.Tests/Features/BanditManagement/BanditScalingServiceTests.cs) — 16 tests covering:
...
[`DOTS.Tests/Features/BanditManagement/BanditScalingConfigProviderTests.cs`](../../DOTS.Tests/Features/BanditManagement/BanditScalingConfigProviderTests.cs) covering:
...
[`DOTS.Tests/Features/BanditManagement/DotsBanditDensityModelTests.cs`](../../DOTS.Tests/Features/BanditManagement/DotsBanditDensityModelTests.cs) — 7 tests on the `internal static` `Cap`/`Scale` helpers (the model's only computation), including the regression for the "vanilla is the floor" invariant: `Cap(base, mult, hardCap)` with `hardCap < base` returns `base`, never `hardCap`.

[`DOTS.Tests/Features/BanditManagement/HideoutDescriptionServiceTests.cs`](../../DOTS.Tests/Features/BanditManagement/HideoutDescriptionServiceTests.cs) — 9 tests: each of the 5 cultures returns its expected `{=key}`, and unknown / vanilla-bandit / empty / null culture IDs return `null`.
...
1. Open MCM in-game → DOTS → World / Bandit Scaling.
...
1. Edit `E:\Steam\...\Modules\DOTS_Map\ModuleData\settlements.xml`.
...
5. Add a `<string>` entry in `dots_module_strings.xml` for the new name key.
...
1. Add `<Culture>` entry in [`dots_spcultures.xml`](../../Main/_Module/ModuleData/dots_spcultures.xml). Required attributes: `id`, `name`, `bandit_chief/raider/bandit/boss`, `elite_basic_troop`, `basic_troop`, `is_bandit="true"`, `can_have_settlement="true"`, `encounter_background_mesh`, `bandit_boss_party_template`.
2. Add two party templates in [`dots_partyTemplates.xml`](../../Main/_Module/ModuleData/dots_partyTemplates.xml): `{culture}_raider_party_template` and `{culture}_boss_party_template`. Each references troop NPCCharacter IDs.
3. Add localization keys for culture name + male/female names in [`dots_module_strings.xml`](../../Main/_Module/ModuleData/dots_module_strings.xml).
4. Author hideouts referencing the new culture in `DOTS_Map/settlements.xml`.
...
Culture display names, male/female names, and the 5 hideout encounter descriptions (`dots_hideout_desc_*`) live in [`dots_module_strings.xml`](../../Main/_Module/ModuleData/dots_module_strings.xml). English defaults are baked into the `text="{=KEY}default"` attribute pattern, so non-English players see English text until translations are produced. To localize, run `python tools/translate_with_claude.py` after authoring; the new keys are picked up automatically.

The 99 hideout name strings in `DOTS_Map/Languages/<LANG>/loc_settlements.xml` were set to the English LOTR camp names by [`tools/migrate_hideouts_to_lotr.py`](../../tools/migrate_hideouts_to_lotr.py). Future hand-translation per language is straightforward (each language file has 99 entries with consistent text patterns).
