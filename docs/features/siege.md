...
## Overview
The Siege feature guards against a crash in `BesiegerCamp.GetSiegeCampPartyPosition` that occurs when a DOTS settlement has no `siege_camp_1` scene entities configured in the map. The Harmony Prefix intercepts the call before the engine throws, logs a diagnostic message identifying the problematic settlement, falls back to `siege_camp_2` frames if available, and as a last resort uses the settlement gate position.
...
- **DOTS requirement:** The DOTS world map contains settlements whose scenes were authored without `siege_camp_1` placement. These must not crash the game during a siege.
...
## Tests
No unit tests exist for the Siege feature in `DOTS.Tests/Features/`. The patch delegates no logic to a service — the entire guard is implemented inline in the Prefix. Testing would require constructing a `BesiegerCamp` instance with controlled frame arrays, which is not feasible without the game runtime.

## How to Fix a Settlement with Missing Siege Camp Entities
The patch logs a message in the format:
```
DOTS: WARNING — Settlement 'Name' (id=id_string) has no siege_camp_1 scene entities (camp2=N frames). Fix in map editor.
```
