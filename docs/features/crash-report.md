...
Comprehensive crash diagnostic capture for DOTS. When an exception escapes any of 10 TaleWorlds lifecycle methods (or the `*CallbacksGenerated` native↔managed shims), DOTS captures a full snapshot of process, game, and Harmony state, writes it to the debug log + a per-crash ZIP bundle, optionally suspends BUTR's exception handler, and surfaces a 2-button native dialog to the player.

This feature is **inspired by but not a port of** [BetterExceptionWindow](https://www.nexusmods.com/mountandblade2bannerlord/mods/3535) (BEW) v8.0.0 — BEW is GNU AGPL v3, so we authored DOTS-native equivalents from scratch using BEW only as a design reference for *what to patch* and *what to display*. The TaleWorlds API surface BEW patches is itself uncopyrightable.

## Why This Exists

Players running DOTS hit exceptions all the time — third-party mod conflicts, vanilla edge cases, save-game corruption, our own bugs. Before CrashReport, they got:
...
After CrashReport, every crash produces a structured `Logs/dots_debug_*.log` entry plus a `Logs/dots_crash_<timestamp>_<sig>.zip` containing:

- `report.txt` — full diagnostic dump (sectioned plain text)
- `report.json` — machine-parseable equivalent
- `dots_debug.log` — current session's DOTS log
...
Native2ManagedPatcher (CallbacksGenerated *) ────────► │
                       ...
                       PlainTextRenderer        JsonRenderer            CrashBundleWriter
                              │                        │                          │
                              ▼                        ▼                          ▼
                       IModLogger              (bundle JSON)            Logs/dots_crash_*.zip
                       (dots_debug.log)
                              │
                              ▼
...
DOTS ships ButterLib 2.10.4 via `DOTS.Dependencies`, which contains its own `ExceptionHandlerSubSystem`. By default we suspend it on the first capture via reflection — `ButterLibExceptionHandlerAdapter.TrySuspend()` calls `ExceptionHandlerSubSystem.Instance.Disable()`. This avoids both BUTR's window and ours firing for the same crash. The user can disable our suspension via MCM (`DOTS — Crash Report → Master → Suspend BUTR Exception Handler`).

## Configuration (MCM)

Settings live on a dedicated MCM page **DOTS — Crash Report** (separate from the main `DotsSettings` page).
...
| Bundle | Write Crash Bundle ZIP | true | Produces `Logs/dots_crash_*.zip`. Set false if you only want the log line. |
| QA — Dev Triggers | Throw On Next Mission Tick | false | Throws `DotsDevTriggerException` on the next `MissionLogic.OnMissionTick`. Auto-resets. |
| QA — Dev Triggers | Throw On Next Application Tick | false | Throws `DotsDevTriggerException` on the next `Module.OnApplicationTick`. Auto-resets. Works on the main menu (no mission needed). |
...
| Section | Source |
|---|---|
| Identity | `Native` + `DOTS` module versions, Bannerlord.exe FileVersion, DOTS.dll SHA1, language code |
...
| DOTS State | Career, special resources, cultural feats, revolt tuning, active messengers — populated when feature adapters are wired (v1 ships with stubs; subsystems can register state providers later) |
| MCM Settings | Reflected snapshot of every `AttributeGlobalSettings` / `AttributePerCampaignSettings` provider (DOTS + third-party) |
...
| AppDomain & Environment | Friendly name, base dir, trust flag, env vars matching `BANNERLORD_* / DOTS_* / DOTNET_* / STEAM_*` |
...
| Logs | DOTS debug log path + tail, RGL log path + tail (best-effort from `%USERPROFILE%\Documents\Mount and Blade II Bannerlord\logs\rgl_log_*.txt`) |
...
| [Main/Features/CrashReport/DevTriggers/](../../Main/Features/CrashReport/DevTriggers) | QA-only throw triggers + tagged `DotsDevTriggerException` |
...
- `Newtonsoft.Json` 13.0.3 (already a DOTS PackageReference) — JSON renderer
...
[DOTS.Tests/Features/CrashReport/](../../DOTS.Tests/Features/CrashReport) — 21 tests:
...
1. Launch the game with DOTS.
2. Open MCM → **DOTS — Crash Report** → **QA — Dev Triggers**.
3. Toggle either:
...
4. Within ~1 tick: the Inquiry dialog appears with "Exception: DOTS.Features.CrashReport.DevTriggers.DotsDevTriggerException" plus paths to the log + bundle.
...
ls -t Logs/dots_crash_*.zip | head -1

# Inspect contents:
unzip -l Logs/dots_crash_<...>.zip

# Read the plain-text report:
unzip -p Logs/dots_crash_<...>.zip report.txt | less
```

Grep the live log for the report (each line is prefixed `[CrashReport]`):

```bash
grep '\[CrashReport\]' Logs/dots_debug_*.log
```
...
Default: DOTS's handler wins (priority 800 + first registration + suspends BUTR). If you want another mod's UI:
...
- **`MBSubModuleBase.OnSubModuleLoad` chicken-and-egg.** DOTS's own `OnSubModuleLoad` is what registers Patch37. Throws in `OnSubModuleLoad` of mods that load BEFORE DOTS are not catchable by us — those land in vanilla / BUTR. Patch37 is registered first in DOTS's load to maximise coverage of our own init.
- **`MissionBehavior.OnMissionTick` is abstract.** Harmony patches its overrides at JIT time. Mods whose `MissionBehavior` subclasses were already JIT'd before DOTS loaded won't have the Finalizer attached. Acceptable — same caveat applies to BEW.
...
- **DOTS-state collectors are stubs.** `CareerStateSnapshot`, `SpecialResourceEntry`, etc. are part of the DTO but the collector returns null/empty until each feature wires a state-provider lambda. Comprehensive captures of these will land per-feature as PRs ship.
...
- Per-feature state providers (career, resources, feats, messengers, revolt) wired into `DotsStateCollector`
