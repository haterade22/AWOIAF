...
- **DOTS requirement:** Players need faster map traversal across the larger Middle-earth map; configurable multipliers let players tune speed to their hardware
- **Without this feature:** Players must install BetterTime as a separate dependency, which may conflict with other DOTS systems and lacks MCM integration
...
| `Main/Features/TimeAcceleration/TimeAccelerationSettingsProvider.cs` | Reads `DotsSettings.Instance` |
...
- `DotsSettings` (MCM) — Provides the three multiplier values
- `MapScreen` (SandBox.View.dll) — Per-frame input polling
- `Campaign.Current` — Speed multiplier and time control mode

## Tests

- `DOTS.Tests/Features/TimeAcceleration/TimeAccelerationServiceTests.cs` — 14 tests covering:
...
4. Add a new MCM setting in `DotsSettings.cs` if configurable
