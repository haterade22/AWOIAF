...
- **DOTS requirement:** DOTS has 50 careers across 16 cultures, but the CC flow auto-assigned the first eligible career with no player choice. Most cultures have 2-4 careers, so players always got the same one.
...
```
dots_careers.xml (career defs + EligibleCultures)
        |
...
career_menu.json -----> CareerMenuDataProvider (CC bonus data)
                        ...
                        CareerMenuService (builds NarrativeMenu + options)
                              |
                    DotsCharacterCreationContentHandler (registers menu)
                              |
...
Each entry maps a career to its CC skill/attribute bonuses. Career names, descriptions, and culture eligibility come from `dots_careers.xml` via the career registry.

| Field | Type | Description |
|-------|------|-------------|
| `career_string_id` | string | Must match a `Career id` in `dots_careers.xml` |
...
| `Main/Features/CharacterCreation/DotsCharacterCreationContentHandler.cs` | Entry point — calls `RegisterCareerMenu()` |
...
- `DOTS.Tests/Features/CharacterCreation/CareerMenuServiceTests.cs` — 12 tests covering option building, culture filtering, selection storage, fallback option, registry edge cases
- `DOTS.Tests/Features/CharacterCreation/CareerMenuDataProviderTests.cs` — 9 tests covering JSON loading, parsing, caching, error handling, career lookup

## How to Add a Career to CC

1. Add the `<Career>` element to `Main/_Module/ModuleData/career_system/dots_careers.xml` with `<EligibleCultures>`
...
2. Add career(s) to `dots_careers.xml` with the culture in `<EligibleCultures>` (use correct culture ID — vanilla IDs for XSLT cultures)
...
Career sprites use a **dedicated atlas** `ui_dots_career_system` (registered in `GUI/SpriteParts/Config.xml` with `<AlwaysLoad />`). This prevents large career images from overflowing the main `ui_taom` atlas and corrupting other UI.

### Sprite Dimensions

| Sprite Type | Widget Size | Generate At | Format | Location |
|------------|-------------|-------------|--------|----------|
| Career portrait | 400x200 | 800x400 | Landscape 16:9 | `GUI/SpriteParts/ui_dots_career_system/CareerSystem/Portraits/` |
| Ability icon | 120x120 | 256x256 | Square | `GUI/SpriteParts/ui_dots_career_system/CareerSystem/Abilities/` |
| Career button | varies | 1024x256 | Wide strip | `GUI/SpriteParts/ui_dots_career_system/CareerSystem/` |
...
| Type | Filename | Registered Name (DOTSSpriteData.xml) |
|------|----------|--------------------------------------|
...
3. Save as `GUI/SpriteParts/ui_dots_career_system/CareerSystem/Portraits/career_{career_id}_portrait.png`
4. Register in `Main/_Module/GUI/DOTSSpriteData.xml` — add a `<GenericSprite>` entry:
   ```xml
   <GenericSprite>
   ...
   ```
5. Copy to game install: `E:\Steam\...\Modules\DOTS\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Portraits\`
6. Run sprite generator, then rebuild
7. The VM auto-prefixes `CareerSystem\Portraits\` to the `portrait_sprite` value from `dots_careers.xml`

### How to Add an Ability Icon

1. Generate a square image (Midjourney: `--ar 1:1`)
2. Resize to **256x256**
3. Save as `GUI/SpriteParts/ui_dots_career_system/CareerSystem/Abilities/{ability_template_id}.png`
4. Register in `Main/_Module/GUI/DOTSSpriteData.xml` — add a `<GenericSprite>` entry:
   ```xml
   <GenericSprite>
...
3. Update `CategoryName` in DOTSSpriteData.xml `<SpritePart>` entries (or remove — generator recreates them)
...
- **Issue:** #84 — [feat: Career selection in character creation](https://github.com/haterade22/DOTS/issues/84)
- **Status:** Closed
