...
- **DOTS requirement:** Total conversion mod needs DOTS cultures, DOTS lords as commanders, and DOTS troops in formations. Without this, Custom Battle is unusable for testing mod content.
- **Without this feature:** Players see vanilla faction names, commanders resolve to null (no `commander_1` etc. in DOTS data), and troops fall through to null causing crashes or empty armies.
...
1. **Prefix** on `CustomBattleData.Characters` getter — replaces vanilla commanders with DOTS lords
2. **Prefix** on `CustomBattleData.Factions` getter — replaces vanilla cultures with DOTS cultures
3. **Postfix** on `CustomBattleHelper.GetDefaultTroopOfFormationForFaction` — fills DOTS troops when vanilla returns null
...
6. **Postfix** on `CustomBattleSideVM` constructor — replaces `FactionSelectionGroup` with `DotsFactionSelectionVM` and explicitly fires the `OnCultureSelection` callback so the initial commander dropdown aligns with the visible faction (vanilla `SelectFaction(0)` doesn't fire the callback)
...
No UI patches needed — DOTS's `CustomBattleScreen.xml` GUI prefab automatically overrides vanilla via Gauntlet module load order (DOTS loads after the CustomBattle module).

### Commander filter+cap (per faction)

Vanilla `CustomBattleSideVM.RefreshValues()` adds every entry from `CustomBattleData.Characters` to `CharacterSelectionGroup.ItemList`. With DOTS's expanded lord pool, this means picking "Dunland" still showed every culture's commanders. Vanilla `OnCultureSelection` only updates banner colors — it does NOT re-filter the dropdown.
...
| `Main/Features/CustomBattles/Hooks/CustomBattleHelper_Troop_Patch.cs` | Harmony postfix — fills DOTS troops |
...
| `Main/Features/CustomBattles/DotsFactionSelectionVM.cs` | Custom faction-selection VM with prev/next navigation |
...
|-----------|---------|----------|
| `DOTS.Tests/Features/CustomBattles/CustomBattleServiceTests.cs` | 22 | Faction filtering, commander filtering, formation mapping, takeMax cap (4 dedicated tests: cap, deterministic order, fewer-than-cap, zero-cap), null/empty edge cases |
| `DOTS.Tests/Features/CustomBattles/CustomBattleCommandersHookTests.cs` | 3 | Resolution, null filtering, empty case |
| `DOTS.Tests/Features/CustomBattles/CustomBattleFactionsHookTests.cs` | 3 | Resolution, null filtering, empty case |
| `DOTS.Tests/Features/CustomBattles/CustomBattleTroopHookTests.cs` | 5 | Vanilla passthrough, DOTS resolution, null service/adapter results |
| `DOTS.Tests/Features/CustomBattles/SideCommanderFilterTests.cs` | 6 | Null/empty culture, cap=3 propagation, ID resolution, null-resolution filtering, empty result |

Patches and `CustomBattleTeamFixBehavior` are thin entry points — tested indirectly via in-game smoke tests per ADR-002.

## How-To

### How to add a new DOTS culture to Custom Battle
