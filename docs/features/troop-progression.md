...
## Overview
TroopProgression extends Bannerlord's troop tier and wage systems to support DOTS's 10-tier troop trees, raises the volunteer recruitment cap from tier 4 to tier 6, and provides per-settlement and per-clan weighted recruitment pools so that each faction's locations offer culturally appropriate troops.
...
- **DOTS requirement:** DOTS troop trees go up to tier 10 (T7–T10 elite and named units). Without extending the cap, any troop above tier 6 is treated as tier 6 by the engine, and wages at tier 7–10 collapse to the vanilla `_ => 57` fallback, making high-tier armies wildly expensive or cheap.
...
## Architecture
### Design Challenge
`DefaultCharacterStatsModel`, `DefaultVolunteerModel`, and `DefaultPartyWageModel` are all GameModel classes that Bannerlord resolves from its internal model registry. They cannot be subclassed while also injecting services, because Bannerlord instantiates models directly, not through DI. The solution is to subclass them and pull dependencies through the DOTS IoC container at construction time (models are registered as singletons in `SubModule.cs`).

The recruitment pool problem is a separate concern: which basic volunteer a settlement notary offers is determined by `GetBasicVolunteer(Hero sellerHero)`. Vanilla uses culture to pick `basic_troop`; DOTS needs settlement- and clan-specific pools with weighted random selection.

### Solution Approach
Three GameModel overrides handle the extension points:

- `DotsCharacterStatsModel` — single-line override: `MaxCharacterTier => 10`.
- `DotsVolunteerModel` — overrides `MaxVolunteerTier` (delegated to `IVolunteerTierService`) and `GetBasicVolunteer` (uses `IVolunteerRecruitmentService` with context from `IVolunteerContextAdapter`).
- `DotsPartyWageModel` — overrides `GetCharacterWage` (tier-to-wage table via `ITroopCostService`), `GetTroopRecruitmentCost` (level-based cost table + horse cost), and `GetTotalWage` (applies cultural feat bonuses for Gondor, Erebor, Lothlorien, Isengard, Gundabad, Umbar, Mordor, Rohan).
...
### Component Diagram
```
DotsVolunteerModel.GetBasicVolunteer(Hero)
...
DotsPartyWageModel.GetCharacterWage(CharacterObject)
    |-> ITroopCostService.GetCharacterWage(tier, isMounted, isMercenary)

DotsPartyWageModel.GetTotalWage(MobileParty, TroopRoster)
    |-> base.GetTotalWage()
    |-> DotsCulturalFeats.* checks (garrison + party culture feats)
```
...
| File | Purpose |
|------|---------|
| `Main/Features/TroopProgression/Models/DotsCharacterStatsModel.cs` | Sets `MaxCharacterTier = 10` |
| `Main/Features/TroopProgression/Models/DotsVolunteerModel.cs` | Overrides `MaxVolunteerTier` and `GetBasicVolunteer` |
| `Main/Features/TroopProgression/Models/DotsPartyWageModel.cs` | Overrides `GetCharacterWage`, `GetTroopRecruitmentCost`, `GetTotalWage` |
...
| `DOTS.Tests/Features/TroopProgression/TroopCostServiceTests.cs` | Wage and recruitment cost table coverage |
| `DOTS.Tests/Features/TroopProgression/VolunteerTierServiceTests.cs` | MaxVolunteerTier value |
| `DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs` | Settlement, clan, and culture pool resolution + weighted pick |
...
- `DotsCulturalFeats` — feat constants used in wage calculation
