...
- [IBattleBalanceSettingsProvider](../../Main/Features/BattleBalance/IBattleBalanceSettingsProvider.cs) — exposes MCM switches and float properties, falling back to compile-time defaults if `DotsSettings.Instance` is null.
...
BattleBalanceConfigProvider (Singleton, IPathService + IModLogger)
        ...
        |  IBattleBalanceSettingsProvider  ← reads DotsSettings.Instance
        |  |
   +----+--+----+----------+
   |       |    |          |
   v       v    v          v
DotsMilitaryPowerModel       DotsCombatSimulationModel       DotsPartyHealingModel
...
Both providers register `Reuse.Singleton` — the JSON file is cached for the entire Bannerlord process. **JSON edits require a full Bannerlord restart**, not a save-load and not a new campaign. MCM switches **do** apply live (each model reads `IBattleBalanceSettingsProvider` properties on every call, and the provider proxies to `DotsSettings.Instance` per access).

## Key Files

| File | Purpose |
|---|---|
| [Main/Features/BattleBalance/Models/DotsMilitaryPowerModel.cs](../../Main/Features/BattleBalance/Models/DotsMilitaryPowerModel.cs) | Override — tier power table + hero/mounted multipliers |
| [Main/Features/BattleBalance/Models/DotsCombatSimulationModel.cs](../../Main/Features/BattleBalance/Models/DotsCombatSimulationModel.cs) | Override — player vs AI blunt-damage chance |
| [Main/Features/BattleBalance/Models/DotsPartyHealingModel.cs](../../Main/Features/BattleBalance/Models/DotsPartyHealingModel.cs) | Override — cultural survival bonus + career passive integration |
...
- `ICareerPassiveService` (`CareerSystem`) — resolved lazily inside `DotsPartyHealingModel.GetSurvivalChance` via `IoC.Resolve` to avoid hard coupling at construction. Provides `TroopRegeneration` passive magnitude per hero.

## Tests

- [DOTS.Tests/Features/BattleBalance/DotsMilitaryPowerModelTests.cs](../../DOTS.Tests/Features/BattleBalance/DotsMilitaryPowerModelTests.cs) — **10 tests**: T7-T10 configured values, T6 vanilla/override fallback, T11+ formula extension, low-tier override behavior.
- [DOTS.Tests/Features/BattleBalance/DotsCombatSimulationModelTests.cs](../../DOTS.Tests/Features/BattleBalance/DotsCombatSimulationModelTests.cs) — **5 tests**: player vs AI blunt-chance routing, fallback to vanilla when `EnableCustomCasualtyRatios=false`.
- [DOTS.Tests/Features/BattleBalance/DotsPartyHealingModelTests.cs](../../DOTS.Tests/Features/BattleBalance/DotsPartyHealingModelTests.cs) — **13 tests**: cultural bonus zero / positive / negative, boundary clamping (0..1), career passive multiplier integration, null-safety paths.
...
Use the MCM panel: **DOTS → Troop Power → Tier7Power / Tier8Power / Tier9Power / Tier10Power**. Changes apply on the next `GetDefaultTroopPower` call, no restart needed. To make ≤T6 also use JSON values, flip MCM **OverrideVanillaTierPower** to `true`.
