...
- **DOTS requirement:** When a formation contains both melee and ranged, position them per a deliberate layout. Players can pick a default and cycle layouts on the fly via hotkey.
...
```
DotsSettings.cs (4 MCM settings: Enable, DefaultLayout, CycleHotkey, Debug)
       │
...
| [Main/Features/MixedFormations/MixedFormationsSettingsProvider.cs](../../Main/Features/MixedFormations/MixedFormationsSettingsProvider.cs) | Wraps `DotsSettings.Instance` for testability |
...
- `IMixedFormationsSettingsProvider` (this feature) — wraps `DotsSettings`; testable
- `IFormationAdapter` (Adapters) — wraps `Formation`
- `IModLogger` (Core/Logging) — DOTS's file logger
- One Harmony patch: `Formation.GetOrderPositionOfUnit` (Prefix)

## Tests

- [DOTS.Tests/Features/MixedFormations/LayoutPositionerTests.cs](../../DOTS.Tests/Features/MixedFormations/LayoutPositionerTests.cs) — 11 tests covering all 4 layouts (block placement, wings, checkerboard parity, no slot overlap), narrow-formation fallback, mid-mission newcomer assignment paths
- [DOTS.Tests/Features/MixedFormations/FormationLayoutServiceTests.cs](../../DOTS.Tests/Features/MixedFormations/FormationLayoutServiceTests.cs) — 25 tests covering: gating paths (disabled, not-holding, invalid order, no-layout, vanilla-layout), layout get/set/cycle (full 4-step cycle wraparound, empty formation skip), mixed-detection thresholds (5 negative cases), default-applier paths, mission end cleanup
...
3. MCM → DOTS → "Battle Tactics / Mixed Formations" → confirm `Enable=true`, `DefaultLayout=0 (InfFront)`, `CycleHotkey=L`.
