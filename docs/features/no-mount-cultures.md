...
Certain DOTS cultures (currently Erebor/dwarves) intentionally omit horses from character creation equipment. This requires four Harmony patches to prevent vanilla's CC narrative stage from crashing when no horse is present in a culture's battle equipment roster.
...
- **DOTS requirement:** Dwarves don't ride horses (lore). Their CC equipment rosters have no horse slots.
...
In `Main/_Module/ModuleData/equipmentsets/dots_char_creation_equipment.xml`, ensure all non-civilian `<EquipmentSet>` blocks for `player_char_creation_{culture}_*` do **not** contain:
...
| `Main/_Module/ModuleData/equipmentsets/dots_char_creation_equipment.xml` | CC equipment rosters — Erebor has no Horse slots |
...
1. Remove `<Equipment slot="Horse" ...>` and `<Equipment slot="HorseHarness" ...>` from all non-civilian `<EquipmentSet>` blocks in `dots_char_creation_equipment.xml` for the culture.
2. No C# changes needed — the patches detect horse absence from `DefaultEquipment[Horse].Item` at runtime.
3. Verify: `grep -c 'slot="Horse"' dots_char_creation_equipment.xml` — confirm only horse-enabled cultures have the slot.

## GitHub Issues

- **Issue #49** — [Arena practice crash for all 13 DOTS cultures](https://github.com/haterade22/DOTS/issues/49) — Closed
- **Issue #50** — [Dwarf character creation crashes and horse removal](https://github.com/haterade22/DOTS/issues/50) — Closed
