...
> during self-contained clip consolidation). Remaining steps: author DOTS-owned `as_war_elephant` action-set with
> the DOTS-authored clip names, revert the TEMP `troops_harad.xml` test entry, author the actual Harad rider troop.
>
...
> - `<Item id="dots_war_elephant">` added to live `LOTRAOM_horses.xml`.
...
> engine. DOTS's port pre-fixed this: `ElephantMissionBehavior : MissionLogic`, trample runs in `OnMissionTick`
...
> authoring the full DOTS-owned action-set with DOTS clip names.
>
> **CREW / HOWDAH — slide root cause DIAGNOSED (2026-06-10); fix DEFERRED by project-owner decision.**
> `DotsHowdahMachine` + `DotsHowdahStandingPoint` ship 4 seats in a 2×2 grid. Crew is force-spawned at battle
...
> - **Bone tracking (Spine1_05) implemented — but is a SUSPECTED slide source (under test):** `DotsHowdahMachine`
...
>
> 1. **`GetTickRequirement()` missing** — `DotsHowdahStandingPoint` didn't override `GetTickRequirement()` so the
...
>    cannot walk to exit positions → sequencer stalls. Additionally `DotsHowdahMachine.OnEndMission` nulled
...
> `adod_howdah_4_agents.xml`) 2026-06-09. Key findings vs our `dots_howdah_agent.xml`:
...
> (one Harad crewman rides it). DOTS already represents the mumakil separately (the existing `mumak_rider` troop
...
| Bone-tracking | `DotsHowdahMachine.RepositionToElephant` — `TryRepositionToBone()` branch commented (fixed-offset only) | The floor-physics fix lands (drop the `bo_` floor's collision, or raise the bone frame so the floor clears the capsule). `TryRepositionToBone`/`ResolveBoneIndex` retained. |
...
| [`…/CareerSystem/Models/DotsAgentStatCalculateModel.cs`](../../Main/Features/CareerSystem/Models/DotsAgentStatCalculateModel.cs) | EDITED — the shared `AgentStatCalculateModel` slot now also carries the **mount-lock**: `CanAgentRideMount`→false for the elephant + `MountDifficulty=999` (both via the injected `IElephantAttackService`, applied with ternaries per gamemodels.md rule 4). |
...
| [`DOTS.Tests/Features/Elephant/ElephantAttackServiceTests.cs`](../../DOTS.Tests/Features/Elephant/ElephantAttackServiceTests.cs) | 16 tests (IsElephantMonster ×3, ShouldEngage ×5 incl. the no-enemy −1 sentinel, IsOffCooldown ×6 incl. exact-boundary + future-stamp clock skew, ComputeInflictedDamage ×2). The BT calls these same pure methods, so they remain the attack decision's regression guard. |
...
(DOTS's clean damage primitive, with a `knockDown` flag that maps to ADOD's knockdown BlowFlag) instead of a raw
...
1. **Monster + Item data** — **ready-to-drop files authored**: [`elephant/lotr_monster_elephant.xml`](elephant/lotr_monster_elephant.xml)
   ...
   [`elephant/horse_item_dots_war_elephant.xml`](elephant/horse_item_dots_war_elephant.xml) (paste the `<Item>` into
   `LOTRAOM_horses.xml`). Both validated well-formed; the Monster id stays `dots_war_elephant` to match `ElephantConfig`.
...
2. **1.4.5 action-set + clips** — author `as_war_elephant` against the **vanilla 1.4.5** quadruped-mount schema
   (NOT ADOD's 1.2.12 `as_elephant`), binding DOTS-compiled clips. This is the gating dependency for animation.
...
(tramples / gores with its tusks) on its own, modelled on DOTS's working **warg** mount. The asset we import is **our
own FBX** (`E:\LOTRAOMAssets\Elephant\Meshes BL\elephant_harad_armor_01.fbx`); its **rig + mesh + textures** match
ADOD's elephant (the asset was purchased from Artem, the ADOD author, for use in DOTS), while the **animations are
DOTS-authored on that FBX** — *not* reused from ADOD (project-owner decision, 2026-06-05). The data is re-authored
under DOTS ids and themed for Harad, held to DOTS standards (adapter pattern, tests, ADR compliance).
...
| | DOTS's elephant FBX (`E:\LOTRAOMAssets\Elephant\...\elephant_harad_armor_01.fbx`) | ADOD's shipped elephant (`ADOD_Beasts/Assets/elephants/adod_elephant_geo.tpac`) |
|---|---|---|
...
ADOD's **rig + mesh + textures** (which are our FBX) and the proven `Mountable=true` mount recipe, but **DOTS authors
...
(pelvis 13.4, spine ~32.7). **This is the roster DOTS's imported `elephant_skeleton` must match bone-for-bone**
...
| | **ADOD elephant** (works) | **DOTS spider** (paused, crashed) |
|---|---|---|
...
is **pure managed C#**, the same mechanic as DOTS's `WargAttackService` — `OnTickAsAI` (when a rider's target
...
data under DOTS/Harad ids, port no native code.** Concretely — `Main/Features/Elephant/` as a sibling of
...
fold the `CanAgentRideMount` + `MountDifficulty 999` mount-lock into the existing DOTS `AgentStatCalculateModel`;
...
(e.g. `dots_war_elephant`), `culture=Culture.harad` — **not** a mumakil/Oliphaunt id (those are a separate, giant
creature DOTS already frames via `mumak_rider`).
...
2. **Assets** — copy ADOD's elephant rig/mesh/textures into DOTS (or `LOTRLOME_Armory`); keep the mount
   `Usage='horse'` on the skeleton; run `tools/tpac_skeleton_transplant.py <tpac> elephant_skeleton --usage horse`
   if physics needs re-applying after a re-import. (Animations: DOTS-authored on our FBX — step 4b, NOT ADOD's clips.)
3. **Data — Monster + Item + usage + action_set** (re-authored under DOTS/Harad ids — a plain war-elephant name,
   e.g. `dots_war_elephant`, **not** a mumakil id; final id is your call):
   - **Monster** `dots_war_elephant`: copy ADOD's `<Monster>` bone block verbatim, keep `Mountable/CanRear/CanCharge=true`,
     `action_set=as_war_elephant`, `monster_usage=…`, `sound_and_collision="bovine"`.
   - **Item** (Horse) `dots_war_elephant`: `Type="Horse"`, `culture=Culture.harad` (not empire), tuned `charge_damage`,
     body mesh + `<AdditionalMeshes>` for armor/howdah + `<Materials>` → the elephant textures,
     `<Horse monster="Monster.dots_war_elephant" is_mountable="true">`; + a `HorseHarness` (`family_type="10"`).
...
4b. **Animations (DOTS-authored, on our FBX)** — author the elephant clips on our FBX (walk / run / idle /
   ...
   motion into each clip — no separate bones — so DOTS must key them into our clips). Reference ADOD's action
   *coverage* only (it reuses `elephant_walk` for trot + strafe — DOTS can author real trot/strafe for polish).
...
6. **Mount-lock** — fold `CanAgentRideMount=false` + `MountDifficulty=999f` (non-rider agents) into the existing
   DOTS `AgentStatCalculateModel` so the AI can't steal the mount.
...
author DOTS's own clips on our FBX instead (2026-06-05), so ADOD's clip set is a declined fallback / coverage
...
| Monster XML | `…/LOTRLOME_Armory/ModuleData/Monsters/LOTR/lotr_monster_elephant.xml` (id `dots_war_elephant`) | **deployed** (2026-06-06) |
| Item XML | `…/LOTRLOME_Armory/ModuleData/LOTRLOME_items/LOTRAOM_horses.xml` (Horse item `dots_war_elephant`, mesh `elephant_mesh`) | **deployed** (2026-06-06) |
...
| Animation clips | `LOTRLOME_Armory/Assets/creature/elephant/animations/` (35 ADOD tpacs — working; DOTS-authored clips pending) | **copied** (2026-06-06); animates in-game |
...
`DependedModule` declaration. **DOTS ports no native code and ships neither DLL.** Structure is cloned from the
warg; data is re-authored from ADOD; meshes/textures are our own FBX (purchased — see License below).

## License / provenance

The elephant asset was **purchased from Artem (the ADOD author) for use in DOTS** — no clean-room re-derivation or
...
- [x] **Mount-lock** (`CanAgentRideMount=false` + `MountDifficulty=999`) folded into `DotsAgentStatCalculateModel` — DONE (2026-06-05).
- [x] **Mesh + skeleton imported** to `LOTRLOME_Armory/Assets/creature/elephant/` — DONE (2026-06-05, by the project owner).
- [x] **Monster + Horse Item** (`dots_war_elephant`) deployed into LOTRLOME_Armory — DONE (2026-06-06). `lotr_monster_elephant.xml` + `dots_war_elephant` Item in `LOTRAOM_horses.xml` + SubModule.xml registrations.
...
- [ ] Author a **DOTS-owned action-set** `as_war_elephant` (rename from `as_elephant`, bind DOTS-authored clip names) to make the action-set fully DOTS-authored (currently uses ADOD ids / clip names verbatim).
- [ ] **Build the elephant animations on our FBX** (Blender → Modding-Kit compile) — DOTS-owned, NOT ADOD's 1.2.12 clips. Gating seam for the rename to `as_war_elephant`.
...
- [ ] **DEFERRED — re-enable spine bone-tracking (slide source #2).** Disabled (`TryRepositionToBone` branch
      commented in `DotsHowdahMachine.RepositionToElephant`; fixed-offset only). Re-enable with a floor-physics fix —
...
- [ ] **Add physical barrier entities to `dots_howdah_agent.xml`** — all ADOD howdah variants include 4 `_barrier_04x04m`
...
- [ ] **Evaluate `TranslateUser = true`** — all ADOD howdah seats use `TranslateUser="true"` (physics-level frame
      ...
      on seat entities in `dots_howdah_agent.xml`, (2) calling `base.OnTick(dt)` at the end of `DotsHowdahStandingPoint.OnTick`.
