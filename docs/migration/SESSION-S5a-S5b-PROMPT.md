...
## You are continuing the DOTS v1.3.15 → v1.4.5 migration
...
- **S5b** — author ~96 new equipment rosters across DOTS's 12 custom cultures to satisfy v1.4.3's mandatory roster contract (`IsLordTemplate`, `IsKingdomRulerTemplate`, `IsFemaleTemplate`, `IsChildEquipmentTemplate`, `IsTeenagerEquipmentTemplate` combinations).
...
4. `docs/migration/templates/equipment-rosters.md` — canonical reference for 1.4.5 equipment roster shape (vanilla examples + DOTS excerpts + diff)
...
```powershell
cd c:\Users\mikew\source\repos\DOTS
git worktree add ../dots-s5ab bannerlord-1.4.5
cd ../dots-s5ab
```

Do all S5a/S5b work in the worktree. Commit + push from there. The main session can independently land its commit.

If `git worktree add` reports the branch is checked out elsewhere, that's the main session — use `git worktree add ../dots-s5ab -b bannerlord-1.4.5-data-migration bannerlord-1.4.5` to make a side branch, then PR/merge back when both sessions are done.
...
1. **Pre-migration baseline:**
   ...
   ```
   Capture the output. Expect ~3,372 `<EquipmentSet>` migrations across ~51 files, 160 deprecated flag hits in `dots_child_equipment_templates.xml`, all 12 cultures failing the mandatory matrix.
...
3. **Spot-check the diff** in 3 representative files:
   ...
   - `Main/_Module/ModuleData/equipmentsets/dots_equipment_sets_gondor.xml` (per-culture equipment file)
...
4. **Manual rewrite of `dots_child_equipment_templates.xml`:**
...
5. **Update Python tooling that GENERATES DOTS XML** to emit the new format (so future regen doesn't bring back deprecated patterns). Files to update:
...
7. **Build + test gate** (verify XML changes didn't break C# tests via embedded resource references):
   ```powershell
   dotnet build Main/DOTS.csproj -p:DisableModuleCopy=true
   # Must be 0 errors.
   
   dotnet test DOTS.Tests/DOTS.Tests.csproj -p:DisableModuleCopy=true
...
8. **Commit S5a:**
   ...
   dots_child_equipment_templates.xml (IsNobleTemplate→IsLordTemplate rename,
...
Per v1.4.3 spec — each of DOTS's 12 custom cultures needs at minimum these 8 roster combinations (12 if the culture has kingdom-tier rulers):
...
12 cultures × 8 mandatory = 96 minimum rosters. Kingdom-tier cultures (those that own kingdoms in `dots_spkingdoms.xml`: gondor, mordor, erebor, rivendell, mirkwood, isengard, gundabad, dolguldur, lothlorien, umbar, possibly shaghana/abanissa) may also need the kingdom-ruler set (+4 more = 12 × 4 = up to 48 additional).

### DOTS custom cultures (from `Main/_Module/ModuleData/dots_spcultures.xml`)
...
The 6 XSLT-renamed vanilla cultures (vlandia=Rohan, empire=Dunland, etc.) inherit vanilla rosters and don't need DOTS-authored rosters.
...
3. **Author each missing roster** using vanilla 1.4.5 as the structural reference. Read `templates/equipment-rosters.md` for the canonical shape. Each roster:
   - Goes in `Main/_Module/ModuleData/equipmentsets/dots_equipment_sets_<culture>.xml`
...
9. **Commit S5b:**
   ...
   Authored 96 (or N, replace) rosters across 12 DOTS custom cultures:
...
| Build | `dotnet build Main/DOTS.csproj` 0 errors |
| Tests | `dotnet test DOTS.Tests/DOTS.Tests.csproj` 2,323/2,325 pass |
...
- Authoring vanilla-aligned rosters for the 6 XSLT-renamed cultures (Rohan, Dunland, etc. — they inherit vanilla rosters, no DOTS-authored rosters needed)
...
| `Main/_Module/ModuleData/equipmentsets/` | Where DOTS's per-culture rosters live |
