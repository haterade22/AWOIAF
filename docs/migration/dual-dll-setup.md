...
Procedure for keeping DOTS buildable against both Bannerlord 1.3.15 (for verification of the source branch) and 1.4.5 (the migration target) during the v1.4.5 migration window.
...
- Run DOTS tests against either version
...
| `~/.dots-src/v1.3.15\` | Existing on-demand decompile cache (dots-src skill) | varies |
...
2. **Verify backup is buildable** (one-time pre-flight check) — before letting Steam update:
   ...
   dotnet build Main\DOTS.csproj -p:DisableModuleCopy=true --verbosity quiet
...
5. **Set environment variables** for dual-DLL workflow:
   ...
   - `BANNERLORD_1_3_15_DLLS` → 1.3.15 backup path for tooling (decompile scripts, dots-src skill). Not consumed by `Directory.Build.props`.

6. **`Directory.Build.props` is wired** (done 2026-05-22) — it honors `BANNERLORD_OVERRIDE_DIR` with the existence gate above. No further csproj edits needed; existing `$(GameFolder)\bin\$(GameBinariesFolder)\...` HintPaths in `Main/DOTS.csproj` and `DOTS.Tests/DOTS.Tests.csproj` continue to work unchanged.
...
- **Save games** created on 1.3.15 should load on 1.4.5 (TaleWorlds maintains save compat across minor versions), but DOTS's SyncData persistence is a separate question.
...
## dots-src skill cache

The `tools/dots-src.ps1` skill caches decompiled types under `~/.dots-src/<version>/`. Current state:
- `~/.dots-src/v1.3.15/` — populated incrementally over months of development
- `~/.dots-src/v1.4.5/` — empty, will populate as S0 + later sessions decompile
...
- [v1.4.x-dots-impact.md](v1.4.x-dots-impact.md)
...
- [docs/migration/v1.4.x-dots-impact.md](./v1.4.x-dots-impact.md)
