...
> What "right" looks like in 1.4.5 for character-typed XML files. Companion to the [equipment-rosters.md](equipment-rosters.md) doc — together they define the contract a DOTS character XML must satisfy.
...
|---|---|---|
| `<NPCCharacters>` | `lords.xml`, `spspecialcharacters.xml`, `obsolete_characters.xml` (both modules), `spnpccharacters.xml`, `caravans.xml`, troop XMLs | Defines a character object — face, skills, traits, equipment, occupation. **This is what DOTS means by "a character XML."** |
...
This doc covers the four `<NPCCharacter>` sub-types DOTS cares about and one auxiliary `<Hero>` block:

| Sub-type | Vanilla file | DOTS file(s) | Identifying attributes |
|---|---|---|---|
...
| **Wanderer** (recruitable tavern companion) | `SandBox/spspecialcharacters.xml` (67 wanderers) | `Main/_Module/ModuleData/dots_wanderers.xml` (170) | `occupation="Wanderer"`, `is_template="true"`, `is_hero="false"` |
...
> **Caveat — wanderers location.** The task brief pointed to `SandBox/obsolete_characters.xml` for wanderers; that file actually contains obsolete lord templates + tournament characters. The real wanderer source is `SandBox/spspecialcharacters.xml`. DOTS follows the correct convention (`dots_wanderers.xml` uses `occupation="Wanderer"` matching vanilla).
...
| **`culture`** | `Culture.<id>` | yes | Maps to `<Culture id=>` from spcultures.xml. **DOTS custom cultures use LOTR ids (`Culture.gondor`); XSLT cultures use vanilla engine ids (`Culture.empire`, etc.) — see `xml-data.md` rule.** |
...
| ~~`civilianTemplate`~~ | id | **DEPRECATED in 1.4.3** | Logs warning at load. Rely on equipment roster flags + `culture` instead. **DOTS never used these — verified 0 occurrences.** |
...
|---|---|---|
| `<EquipmentRoster>` (with `<equipment slot=... id=.../>` children) | Inline equipment set written directly on the character. | **Still uses the old `civilian="true"` attribute in vanilla 1.4.5 lords.xml on 3 stragglers** (e.g. `main_hero` line 156). For new DOTS rosters, prefer `<EquipmentSet equipmentType="Civilian"/>` referencing an external roster. |
...
So vanilla itself ships with 3 unmigrated `civilian="true"` survivors — the engine still accepts that attribute form, but at warning level. **DOTS has 3,372 occurrences** to migrate (per `v1.4.x-equipment-overhaul.md`).
...
- Inline `<skills>` is OMITTED — skills come from `skill_template`. Compare to DOTS's lords.xml which inlines explicit skill values.
...
### DOTS current state — direct lords.xml
...
### DOTS current state — lords.xslt overlay

`Main/_Module/ModuleData/lords.xslt` rewrites vanilla `lord_1_1` (Empire kingdom ruler) into DOTS's Dunland warlord:
...
| # | DOTS | Vanilla 1.4.5 | Severity |
|---|---|---|---|
| 1 | `<EquipmentSet ... civilian="true" />` | `<EquipmentSet ... equipmentType="Civilian" />` | 🔥 Migration required (the headline 1.4.3 change) |
| 2 | DOTS inlines full `<skills>` block in addition to `skill_template=` | Vanilla uses `skill_template=` alone, no inline `<skills>` for most lords | 🟢 Both forms work; inline takes precedence |
| 3 | DOTS lords use kingdom-ruler-tier rosters with `_bat_template_medium_a` (not `n_<culture>_king_template_bat_m`) | Vanilla rulers use `n_<culture>_king_template_*` prefixed rosters | 🟡 DOTS doesn't yet distinguish ruler-tier from regular-lord-tier rosters per the 1.4.3 `IsKingdomRulerTemplate` flag pattern |
| 4 | No `<EquipmentRoster />` empty placeholder | Vanilla often opens with `<EquipmentRoster />` empty placeholder | 🟢 Optional cosmetic |
| 5 | DOTS 566 NPCCharacters vs vanilla 400 — additional LOTR lords | — | 🟢 Intentional |

### Migration checklist for DOTS lords
...
- [ ] For each DOTS kingdom (Mordor, Gondor, Erebor, Rivendell, Lothlorien, Mirkwood, Isengard, Gundabad, Dol Guldur, Umbar — plus the 6 XSLT kingdoms), ensure the rulers (`clan_<kingdom>_1` heads) reference dedicated ruler-tier equipment rosters tagged `IsKingdomRulerTemplate`. Currently DOTS rulers share `_bat_template_*` with regular lords.
- [ ] Verify no `civilianTemplate=` / `battleTemplate=` regressions slip in via copy-paste (these never existed in DOTS but are easy to add wrongly).
...
- [ ] Confirm `is_hero="true"` and `occupation="Lord"` are set on every NPCCharacter in DOTS lords.xml (566 entries).
...
| 2 | DOTS heroes.xml has no `<NPCCharacter>` blocks — relationships only | Same | 🟢 Correct pattern |

### Migration checklist for DOTS heroes.xml
...
> **1.4.3 wanderer-specific note.** Per the 1.4.3 dev notes, a Nord wanderer regression was fixed (correct equipment on culture). Confirm Nord-equivalent (any DOTS custom-culture wanderers) loads correctly — the fix is in `EquipmentSelectionModel`, not in XML.
...
`Main/_Module/ModuleData/dots_wanderers.xml`, `spc_wanderer_gondor_0`:
...
DOTS ships **170 wanderers** in `dots_wanderers.xml` (vs vanilla's 67).
...
| 2 | Face template = `BodyProperty.fighter_<culture>` | Face template = `BodyProperty.townsman_<culture>` | 🟡 Cosmetic — DOTS uses fighter physique for companions; vanilla uses lighter townsman. Both work; choose intentionally. |
| 3 | DOTS has 170 wanderers vs vanilla 67 | — | 🟢 Intentional |
...
### Migration checklist for DOTS wanderers

- [ ] Mechanical: `civilian="true"` → `equipmentType="Civilian"` on all `<EquipmentSet>` in `dots_wanderers.xml` and `named_companions.xml` (170 + 18 = 188 entries × ~2 rosters each).
...
- [ ] If any DOTS wanderer is intended as a noble (Aragorn, etc.), audit whether they need `IsLordTemplate` flagged rosters available for promotion (per the 1.4.3 "companions promoted to lord" behavior).

---

## NPCs (notables: merchants, gang leaders, headmen)

> **DOTS convention.** Each of DOTS's 10 custom + 6 XSLT cultures has 26 notable templates in `characters/npcs_<culture>.xml`, per the naming convention in `xml-data.md`:
...
| # | DOTS | Vanilla 1.4.5 | Severity |
|---|---|---|---|
| 1 | DOTS uses inline `<EquipmentRoster civilian="true">` with explicit slot/id | Vanilla refs external `<EquipmentSet id="..." equipmentType="Civilian"/>` | 🔥 Both issues at once: deprecated attribute AND inline-vs-referenced. The deprecated attribute is the urgent fix; the inline-vs-referenced is a design pattern choice. |
| 2 | DOTS uses `skill_template="SkillSet.infantry_heavyinfantry_level1_template_skills"` (generic troop skill set) on a Merchant notable | Vanilla uses `skill_template="SkillSet.spc_notable_empire_0"` (dedicated notable skill set) | 🟡 DOTS notables share a generic skill set — works, but ignores the trait-driven skill derivation pattern vanilla uses |
| 3 | DOTS uses `BodyProperty.fighter_<culture>` for notables | Vanilla uses `BodyProperty.villager_<culture>` / `townsman_<culture>` / `townswoman_<culture>` | 🟡 Cosmetic — notables look like fighters in DOTS (intentional?), softer civilians in vanilla |
| 4 | Only ONE EquipmentRoster — only civilian | Vanilla has battle + civilian + flair | 🟡 DOTS notables can't appear armed/in battle context |
| 5 | DOTS has 26 notables per culture × ~16 cultures = ~416 notables | Vanilla has ~10 per main culture × 7 = ~70 | 🟢 Intentional LOTR breadth |

### Migration checklist for DOTS NPCs
...
- [ ] Consider hoisting inline rosters into named `<EquipmentRoster id="...">` definitions in `sandboxcore_equipment_sets.xml` (or a DOTS equivalent), then referencing them via `<EquipmentSet id="..." equipmentType="Civilian"/>`. This matches vanilla's pattern and survives 1.4.3+ flag-based selection cleanly.
...
Cross-reference: [v1.4.x-equipment-overhaul.md](../v1.4.x-equipment-overhaul.md) **§ DOTS mandatory roster matrix** (10 cultures × 8 rosters = 80 required) is the authoritative target for the per-culture equipment side. This doc covers the **character-XML** side.
...
- [ ] Grep verify zero `civilianTemplate=` / `battleTemplate=` on any `<NPCCharacter>` (already verified 0 in DOTS but re-run on every commit touching characters).

### Schema-level (`tools/audit_equipment_roster_coverage.py`)

- [ ] For each of DOTS's 10 custom cultures + 6 XSLT cultures, every lord-tier character has access to:
...
- [ ] Boot the game with DOTS loaded; check the log for `civilian="true"` / `civilianTemplate=` / `battleTemplate=` deprecation warnings — there should be zero by end of S5.
- [ ] Visual smoke test: in-game encyclopedia open at least one lord, one wanderer, one merchant per DOTS culture; verify they're clothed correctly (no underwear, no wrong-culture).

### Out of scope (per v1.4.x-equipment-overhaul.md)

- [ ] **Do NOT** adopt `equipmentType="Stealth"` for DOTS disguise system — separate enhancement.
...
- [v1.4.x-dots-impact.md](../v1.4.x-dots-impact.md) — DOTS-specific impact summary
- [xml-data.md](../../../.claude/rules/xml-data.md) — culture/kingdom/settlement id cross-reference + DOTS notable naming convention
