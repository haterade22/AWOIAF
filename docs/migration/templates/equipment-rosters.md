...
>
> See also: [v1.4.x-equipment-overhaul.md](../v1.4.x-equipment-overhaul.md) (migration spec), [v1.4.x-dots-impact.md](../v1.4.x-dots-impact.md) (surface impact), [equipment-roster-coverage.csv](../equipment-roster-coverage.csv) (per-culture audit output).
...
The matching API change: `EquipmentSelectionModel.GetEquipmentRostersForInitialChildrenGeneration(Hero)` and similar methods now return a single resolved `Equipment` instead of `MBList<MBEquipmentRoster>`. See `Main/Adapters/ChildCreatorAdapter.cs:40` for the only direct DOTS consumer.
...
| Pattern | Element | Container | Attribute for civilian | DOTS equivalent |
|---|---|---|---|---|
| **A. Standalone roster** in `<EquipmentRosters>` root | `<EquipmentSet>` (no `id`) | Top-level `<EquipmentRosters>` XML file | `equipmentType="Civilian"` | `dots_equipment_sets_*.xml`, `dots_wanderer_equipment.xml`, `dots_child_equipment_templates.xml`, `dots_char_creation_equipment.xml`, `dots_career_starting_equipment.xml`, `dots_education_equipment_templates.xml` |
| **B. Inline roster** in NPC `<Equipments>` block | `<EquipmentRoster>` (no `id`) | Nested inside `<NPCCharacter>/<Equipments>` | `civilian="true"` (still valid in 1.4.5) | DOTS character files — `characters/npcs_*.xml`, `characters/lords.xml`, `troops/troops_*.xml`, `dots_wanderers.xml`, `named_companions/named_companions.xml` |
...
Slots used by vanilla and DOTS rosters: `Item0`, `Item1`, `Item2`, `Item3` (weapon slots), `Head`, `Body`, `Cape`, `Gloves`, `Leg`, `Horse`, `HorseHarness`.
...
- DOTS does NOT yet use `Stealth` — explicitly out of scope per the migration spec.
...
These rosters are **selected by code** (the education campaign behavior) via roster-id lookup, not by template resolution. They don't need flags. DOTS's `dots_education_equipment_templates.xml` (10,154 lines) follows the same pattern, which is correct — leave it alone except for the missing `equipmentType=` default (vanilla also omits — the engine treats omitted as Battle, which is fine for these "stage" rosters which are equipment progression sets, not template selectors).

---

## Mapping: deprecated flags → 1.4.3 replacement

**THIS IS THE LOAD-BEARING TABLE.** Every deprecated flag in DOTS XMLs must be translated using this map. The rows are derived from:
...
### Special DOTS combinations encountered

DOTS's `dots_child_equipment_templates.xml` uses these multi-flag combinations (from the `/validate` tool output) — concrete replacements:

| DOTS current (1.3.15) | DOTS target (1.4.5) |
|---|---|
...
### `dots_equipment_sets_<culture>.xml` (15 files — all 12 custom cultures + dale/rohan/dunland)
...
1. No `<Flags>` element → engine cannot resolve via `EquipmentSelectionModel` for any of:
   ...
   - Male/female lord child/teen (same — and missing entirely; covered only in `dots_child_equipment_templates.xml`)
...
**Excerpt — current state (`dots_equipment_sets_gondor.xml` line 75–87, `gondor_civ_template_default_a`):**
...
### ⚠️ HIGHEST PRIORITY: `dots_child_equipment_templates.xml`
...
### `dots_char_creation_equipment.xml`
...
### `dots_career_starting_equipment.xml`
...
### `dots_education_equipment_templates.xml`
...
### `dots_equipment_sets_named_companions.xml`
...
`tools/audit_equipment_roster_coverage.py` audits DOTS's 12 custom cultures against this required matrix. Per spec ([v1.4.x-equipment-overhaul.md](../v1.4.x-equipment-overhaul.md)):
...
### Current DOTS coverage (audit output 2026-05-22)
...
**100% of mandatory rosters missing** — because DOTS rosters lack the `<Flags>` element entirely. Once Phase 1 mechanical migration adds `<Flags>` to the existing rosters, this audit will pass for the configurations that already exist. Net new authoring required: probably ~40 rosters (child/teen/female-lord/ruler) per culture — see S5b authoring queue.
...
## Migration recipes per DOTS file
...
# 4. Manually fix the deprecated EquipmentFlags values in dots_child_equipment_templates.xml
...
For each `dots_equipment_sets_<culture>.xml`:
...
| [v1.4.x-dots-impact.md](../v1.4.x-dots-impact.md) | Surface impact matrix. |
...
| `Main/Adapters/ChildCreatorAdapter.cs:40` | Only direct DOTS consumer of `GetEquipmentRostersForInitialChildrenGeneration` (return-type change). |
