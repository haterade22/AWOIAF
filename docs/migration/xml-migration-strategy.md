...
## DOTS XSLT Transformation Approach

DOTS uses XSLT transformations instead of maintaining full XML copies. This allows us to:
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="Kingdom[@id='empire']">
        <xsl:copy>
            ...
            <xsl:attribute name="name">{=DOTS_dunland}Dunland</xsl:attribute>
            <xsl:apply-templates select="node()"/>
        </xsl:copy>
    </xsl:template>
</xsl:stylesheet>
