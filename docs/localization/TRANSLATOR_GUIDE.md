# DOTS Translation Guide

This guide is for anyone translating DOTS (Dawn of the Stag) into another language. It covers the files to edit, the format rules, how to use the AI-assisted translation pipeline, and how to test in-game.

## Overview

DOTS ships with translatable strings across **three modules** and **seven file types**. Total: ~12,200 strings per language.

### DOTS module (7 files, ~6,238 strings)

Located at `Main/_Module/ModuleData/Languages/<LANG>/`:

| File | What it contains | Entries |
|------|------------------|---------|
| `std_dots_module_strings_{locale}.xml` | Faction names, titles, culture terms, UI labels (career screen, main menu, etc.) | ~653 |
| `std_dots_wanderer_strings_{locale}.xml` | Wanderer backstories for all cultures | ~1,177 |
| `std_dots_named_companion_strings_{locale}.xml` | Named companion dialog (Aragorn, Legolas, Gimli, etc.) | ~126 |
| `std_dots_cc_strings_{locale}.xml` | Character creation narratives (parents, childhood, youth, education, adulthood) | ~772 |
| `std_dots_career_strings_{locale}.xml` | Career system names, descriptions, ability tooltips, choices | ~2,050 |
| `std_dots_messenger_strings_{locale}.xml` | Messenger feature UI | ~29 |
| `std_dots_xslt_strings_{locale}.xml` | Kingdom/culture/clan/lord/hero descriptions injected via XSLT (Encyclopedia content) | ~1,431 |

### DOTS_Map module (1 file, ~1,102 strings)

Located at `DOTS_Map/ModuleData/Languages/<LANG>/`:
...
<base xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <strings>
    <string id="dots_str_faction_ruler.empire" text="Wodz" />
...
# Run full translation across all 7 DOTS files + DOTS_Map + Armory:
python tools/translate_with_claude.py --lang PL --apply

# Translate just one module:
python tools/translate_with_claude.py --lang PL --module DOTS --apply
python tools/translate_with_claude.py --lang PL --module DOTS_Map --apply
python tools/translate_with_claude.py --lang PL --module Armory --apply

# Pilot small batch first to validate quality:
python tools/translate_with_claude.py --lang PL --module DOTS --max-entries 50 --apply
```
...
This populates the DOTS-module files with English placeholders. For DOTS_Map and LOTRLOME_Armory, English templates are already populated in their per-language directories.
...
```json
{
  ...
  "DOTS_eleftheroi_name": "Беорнинги",
  "dots_main_menu_new_game": "Войти в Эпоху Людей",
  "DOTS_gondor": "Гондор"
}
```
...
2. Launch Bannerlord with DOTS, DOTS_Map, and LOTRLOME_Armory all enabled in the launcher's load order
...
DOTS mod code doesn't touch this — it's pure engine convention.

---

## String Categories Reference

### Module Strings (dots_module_strings, ~653)
...
| `DOTS_*_name`, `DOTS_*_text` | Kingdom encyclopedia entries | Lore descriptions |
| `dots_career_*`, `dots_main_menu_*` | UI labels | Buttons/menus from C# code |
...
Each CC narrative entry has `_text` (short prompt) + `_desc` (long flavor) pair, keyed by `dots_cc_<menu>_<culture>_<n>_text/desc`.
...
### Settlement Names (DOTS_Map, ~1,102)
...
| Lang | DOTS | DOTS_Map | Armory | XSLT (Encyclopedia) |
|------|------|----------|--------|---------------------|
| RU   | 99%  | 99%      | 92%    | 99.9%               |
| SP   | 97%  | 65%      | 87%    | 97%                 |
| DE   | 97%  | 65%      | 82%    | 97%                 |
