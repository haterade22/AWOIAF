...
How to use Claude Code Agent Teams for parallel work on the DOTS project.
...
## DOTS Team Compositions

DOTS is a single Bannerlord mod, so team compositions are based on **directory ownership** within the mod, not separate components.

| Role | Owns | Build/Verify Command |
|------|------|---------------------|
| Feature Dev | `Main/Features/{Name}/` | `dotnet build Main` |
| Test Dev | `DOTS.Tests/` | `dotnet test DOTS.Tests` |
| XML/XSLT Dev | `Main/_Module/ModuleData/` | N/A (validated at runtime) |
| TaleWorlds Researcher | Read-only (decompile DLLs) | `pwsh tools/dots-src.ps1 path <Type>` (ilspycmd fallback) |
...
- `DOTS.Tests/Features/{FeatureName}/` mirrors the feature owner (or is owned by a dedicated Test Dev)
...
- Use `dotnet build Main` or `dotnet test DOTS.Tests` for isolated compilation during development
...
| Test Dev | Writes tests in `DOTS.Tests/Features/NewFeature/` |
| Lead | Coordinates, integrates `IoC.cs` registrations, runs final build |

**Spawn prompt (Feature Dev):**
```
You are a Feature Developer for the DOTS Bannerlord mod. Read docs/ai-includes/architecture.md
...
```
You are a Test Developer for the DOTS Bannerlord mod. Read docs/ai-includes/tdd-enforcement.md
and docs/ai-includes/testing-guide.md first. Your scope is DOTS.Tests/Features/PartySpeed/ only.
Use MSTest + NSubstitute. Mock all adapters. 100% service/engine coverage required (ADR-008).
Run tests with: dotnet test DOTS.Tests
```
...
```
You are a TaleWorlds Researcher for the DOTS Bannerlord mod. Read
docs/ai-includes/agent-operating-manual.md + taleworlds-research-guide.md first; you cannot
invoke skills (recommend them instead). Use `pwsh tools/dots-src.ps1 path <Type>` (primary;
...
```
You are an XML/XSLT Developer for the DOTS Bannerlord mod. Your scope is
...
| Agent | Task |
|-------|------|
| Feature Dev A | `Main/Features/PartySpeed/` + `DOTS.Tests/Features/PartySpeed/` |
| Feature Dev B | `Main/Features/CombatBonus/` + `DOTS.Tests/Features/CombatBonus/` |
...
2. **Give each subagent complete context in the spawn prompt — never make it re-read your session.** This is already DOTS's "Briefing subagents" convention (CLAUDE.md): the orchestrator puts task text, scope, and the docs-to-read in the prompt. A reviewer subagent gets the description + the plan/requirement + the diff (or `BASE_SHA..HEAD_SHA`), not "go figure out what changed."
...
4. **Severity triage:** fix Critical/HIGH in-session (DOTS's `/deep-review` already mandates this), fix Important before proceeding, note Minor for later.
...
Test scope: DOTS.Tests/Features/{FEATURE_NAME}/ only.
...
Test: dotnet test DOTS.Tests
```

### TaleWorlds Researcher
```
You are a TaleWorlds Researcher for the DOTS Bannerlord mod.
...
Primary tool: pwsh tools/dots-src.ps1 path <FullTypeName>   (cache-aware; prints a .cs path to grep)
...
### Code Reviewer
```
You are a Code Reviewer for the DOTS Bannerlord mod.
...
| Problem | Solution |
|---------|----------|
| **Build contention** | Only one agent runs `./build.ps1` at a time. Use `dotnet build Main` or `dotnet test DOTS.Tests` for isolated compilation. |
