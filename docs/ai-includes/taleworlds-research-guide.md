...
- **When you don't have complete information about**:
  - How TaleWorlds systems call into DOTS
...
> **Step 0 — check the engine study docs first.** 19 engine processes are pre-analyzed, DOTS-relevant, and file:line-cited at [`docs/reference/engine/`](../reference/engine/). If your question is "how does X work" (agent spawn, formation/team AI, mount/rider, campaign→mission seam, campaign heartbeat, MBSubModuleBase lifecycle, GauntletUI, GameModel system, save/object system, usable machines, campaign object graph) — read the doc first. It's faster than cold decompilation and pre-filters for DOTS gotchas. Only proceed to decompilation below when the engine study docs don't cover your specific type or method.
...
**When to Use**: Understanding how TaleWorlds systems call into DOTS, mapping base classes to overrides
...
**Purpose**: Document how TaleWorlds' base [system] works and where DOTS hooks into it
...
## DOTS Integration Points

**DOTS Override**: `Main/Path/To/YourClass.cs`

**Class**: `YourNamespace.YourClassName : BaseClassName`

**Overridden Methods**:
1. `MethodName` - [What DOTS customizes]
2. [Additional methods]
...
[Calls virtual method on registered model/component]
    |
DOTS.YourClass.MethodName()  <- DOTS OVERRIDE
    |
[Your custom logic]
    |
...
**Implication**: [How this affects DOTS implementation]
...
- DOTS Override: `Main/Path/YourClass.cs:Line`
...
**Purpose**: Map TaleWorlds base systems to DOTS integration points
...
3. Document call flow from TaleWorlds framework -> DOTS override
...
4. Identify integration points where DOTS can hook in
...
MapEvent.ApplySimulatedBattleResults()
    |
Campaign.Current.Models.CombatSimulationModel.SimulateBattle()  <- DOTS CAN OVERRIDE
    |
CombatSimulationModel.SimulateHit()  <- DOTS CAN OVERRIDE
```
