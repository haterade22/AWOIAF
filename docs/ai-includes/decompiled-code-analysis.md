...
Guide for analyzing decompiled Bannerlord mod code and translating it into DOTS-compatible implementations.
...
### Phase 3: DOTS Adaptation

**Namespace placement:**
```csharp
namespace DOTS.Features.[FeatureName]
{
    // Implementation here
}
```

**Map to DOTS architecture** (see [architecture.md](architecture.md)):
```
Decompiled code → DOTS layers:
...
```csharp
// JSON-based configuration (DOTS pattern)
...
**Potential issues:**
- Compatibility with existing DOTS features
...
**4. DOTS Implementation** - Complete, copy-paste ready code following DOTS conventions
...
## DOTS-Specific Considerations

### Faction Structure

DOTS has 13 kingdoms total:
...
DOTS adds custom races (Human, Dwarf, Elf, Orc, Uruk-hai, Hobbit) via:
...
7. Does it fit DOTS's architecture (patches -> hooks -> services -> adapters)?

---

## Related Guides

- [architecture.md](architecture.md) - DOTS architecture overview
