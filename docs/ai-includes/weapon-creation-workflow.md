...
## The four files (all live in the **game install**, NOT the DOTS repo)
...
LOTR factions map onto vanilla or DOTS-custom culture StringIds (see CLAUDE.md / memory
...
loc files live outside the DOTS repo); see [`localize`](../localization/TRANSLATOR_GUIDE.md) for the pipeline.
...
(`WeaponDesign.CalculatePivotDistances`). It's self-contained (no DOTS/Bannerlord build dependency) and
