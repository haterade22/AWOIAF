...
End-to-end repeatable workflow for giving DOTS lords lore-driven skill values and personality traits, modeled on the May 2026 16-culture sweep (~880 NPCs across 18 GitHub issues #228-#245, commits `feabca2` → `8665ca6` → `c5dc168`). Use this when:

- A canonical Tolkien lord (Boromir, Galadriel, Théoden, etc.) has wrong in-game stats.
- A new DOTS-invented lord needs skills/traits assigned.
...
2. **Generator**: `python tools/apply_culture_skills_traits.py --all-cultures --apply` emits 3 outputs — `dots_lord_skill_sets.xml` (SkillSets), updated `lords.xml` + `lords.xslt` (skill_template attr swaps + populated `<skills>`/`<Traits>` blocks).
3. **Engine consumption**: every adult NPCCharacter has `skill_template="SkillSet.dots_..."` that points to a DOTS-owned SkillSet — that's what the engine actually uses. Explicit `<skills>` blocks on hero NPCs are IGNORED (kept as documentation only).
...
DOTS adds three files (all registered in [`Main/_Module/SubModule.xml`](../../Main/_Module/SubModule.xml)):
...
| [`Main/_Module/ModuleData/characters/lords.xml`](../../Main/_Module/ModuleData/characters/lords.xml) | Adds DOTS-NET-NEW NPCs (Ciriel, Dorwen, the entire EW_/WE8_/WE9_/E*/M*/G*/D* ranges) | `<XmlName id="NPCCharacters" path="characters/lords"/>` |
| [`Main/_Module/ModuleData/dots_lord_skill_sets.xml`](../../Main/_Module/ModuleData/dots_lord_skill_sets.xml) | 120 DOTS-owned SkillSets — 35 archetype + 85 canonical-character | `<XmlName id="SkillSets" path="dots_lord_skill_sets"/>` |
...
- `characters/lords.xml` → DOTS-NET-NEW NPC (wins at runtime)
...
If DOTS-invented, look at the heroes.xslt bio text for archetype signals (`"captain"`, `"ranger"`, `"wife of"`, etc.) — the script's `keyword_archetypes` does this automatically.
...
The script:
- Regenerates `dots_lord_skill_sets.xml` from scratch (35 archetypes + every canonical with `skills=`)
- Walks every NPCCharacter in lords.xml + lords.xslt with `culture="Culture.<id>"`
- For each adult (age ≥14 unless canonical override): swaps the `skill_template` attribute to the matching DOTS SkillSet, populates the `<skills>` and `<Traits>` blocks (documentation only — engine ignores `<skills>`)
...
python -c "import xml.etree.ElementTree as ET; ET.parse('Main/_Module/ModuleData/dots_lord_skill_sets.xml'); print('dots_lord_skill_sets.xml OK')"
```
...
| Commit | Files | Type |
|---|---|---|
| 1 | `dots_lord_skill_sets.xml` + `lords.xml` + `lords.xslt` + `SubModule.xml` + `CHANGELOG.md` | `feat(lords-skills): ...` or `fix(lords-skills): ...` |
...
|---|---|---|
| `python tools/apply_culture_skills_traits.py --skillsets-only --apply` | Regenerates `dots_lord_skill_sets.xml` only | After tweaking `BASE_ARCHETYPES` values; no NPCCharacter edits needed |
...
3. Run:
   ...
   The script auto-generates a new SkillSet `dots_canonical_lord_R3_2_skills` and points Cirdan's `skill_template` at it.
...
3. Re-run the script. A new SkillSet `dots_corsair_admiral_skills` is auto-generated.
...
- [ ] `git status` shows expected files only: `lords.xml`, `lords.xslt`, `dots_lord_skill_sets.xml`, `CHANGELOG.md`, possibly `SubModule.xml` if registering new files, possibly the script itself.
...
| **Explicit `<skills>` block on hero NPCs is ignored by the engine** — only `skill_template` matters; this is the bug that led to the SkillSet rewrite | NEW: `feedback_skill_template_overrides_explicit_skills.md` | Always swap `skill_template` to a DOTS SkillSet, never hand-edit `<skills>` blocks for heroes |
| Same-ID NPCs in both `lords.xslt` (vanilla transform) and `characters/lords.xml` (DOTS additions) → last-loaded wins, which is `lords.xml` per SubModule.xml load order | `harness-facts.md` | If a fix isn't taking effect, check whether lords.xml has the same ID and edit there instead of (or in addition to) lords.xslt |
...
| [`Main/_Module/ModuleData/dots_lord_skill_sets.xml`](../../Main/_Module/ModuleData/dots_lord_skill_sets.xml) | Generated — 120 SkillSets (DO NOT hand-edit) |
| [`Main/_Module/ModuleData/characters/lords.xml`](../../Main/_Module/ModuleData/characters/lords.xml) | DOTS-NET-NEW NPCCharacters (wins over XSLT-transformed vanilla at runtime) |
...
| `c5dc168` | 2026-05-27 | DOTS-owned SkillSets — created `dots_lord_skill_sets.xml`, swapped `skill_template` on every adult lord, in-game verified on Boromir |
