...
3. **Wire** the culture in `spcultures.xslt` (every CultureObject template/troop attribute), `dots_partyTemplates.xml` (9 standard templates), `VolunteerRecruitmentService.cs` (culture + optional settlement/clan pools), and add tests.
...
- [ ] **Culture ID decision**. Check `~/.claude/projects/.../memory/kingdom-culture-mapping.md`:
  - Custom culture (`gondor`, `erebor`, etc.) — adds rows in `dots_spcultures.xml`.
...
| Attribute | Always bind to DOTS value | Optional / passthrough |
|---|---|---|
| `basic_troop` | yes | |
...
Skipping the **bind-required** rows means the new DOTS templates you wrote for that culture are dead code — vanilla's `militia_sturgia_template` etc. flow through the passthrough.

### 3b. Party templates

Add 9 standard templates to `dots_partyTemplates.xml` (mirror an existing well-developed culture like Rohan or Dale):
...
**Add tests** in `DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs`. Cover at minimum:
...
dotnet test DOTS.Tests --no-build --filter "<Culture>Culture|<Culture>Settlement|<Culture>Clan"
```
...
python tools/generate_<culture>_troops.py --apply
python tools/validate_all_troop_refs.py
dotnet test DOTS.Tests --no-build --filter "<Culture>Culture"
```
...
| **Party templates** | `Main/_Module/ModuleData/dots_partyTemplates.xml` | modified (+9 templates) |
| **Recruitment** | `Main/Features/TroopProgression/VolunteerRecruitmentService.cs` | modified (+ `InitializeXxxCulture` minimum) |
| **Tests** | `DOTS.Tests/Features/TroopProgression/VolunteerRecruitmentServiceTests.cs` | modified (+ low/high/fallthrough tests) |
...
- [`docs/cultures.md`](../cultures.md) — net-new culture in `dots_spcultures.xml` (Phase 0 prerequisite if applicable)
