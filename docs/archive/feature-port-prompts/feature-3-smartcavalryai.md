...
You are porting feature #3 of 7 from the external-developer drop at `Downloads/Features_fixed/SmartCavalryAI/` into DOTS's `Main/Features/SmartCavalryAI/`. The other 6 features are tracked separately. Don't touch them.
...
├── SmartCavalryAISettingsProvider.cs      ← wraps DotsSettings.Instance for testability
├── Models/
...
DOTS.Tests/Features/SmartCavalryAI/
...
Add `using DOTS.Features.SmartCavalryAI;` to `Main/IoC.cs`, then in `Configure()`:
```csharp
...
- If RCA produced a generalizable lesson: codify it as a new feedback memory in `C:/Users/mikew/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/` and index in `MEMORY.md`
...
2. MCM → DOTS → "Battle Tactics / Smart Cavalry" → confirm `Enable=true`.
...
- Files modified: DotsSettings.cs, IoC.cs, SubModule.cs (Patch31 + behavior registration)
