...
You are porting feature #4 of 7 from the external-developer drop at `Downloads/Features_fixed/FiefManagement/` into DOTS's `Main/Features/FiefManagement/`. The other 6 features are tracked separately. Don't touch them.
...
├── FiefManagementSettingsProvider.cs     ← wraps DotsSettings.Instance
├── Models/
...
DOTS.Tests/Features/FiefManagement/
...
Add `using DOTS.Features.FiefManagement;` to `Main/IoC.cs`, then in `Configure()`:
```csharp
...
3. **F6 conflict check.** Verify DOTS doesn't bind F6 elsewhere:
   ```bash
...
2. MCM → DOTS → "Fief Management" → confirm `Enable=true`.
...
- Files modified: DotsSettings.cs, IoC.cs, SubModule.cs (Patch36 + behavior registration)
