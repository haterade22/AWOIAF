...
You are porting feature #5 of 7 from the external-developer drop at `Downloads/Features_fixed/TransferbuttonMenu/` into DOTS's `Main/Features/QuickActions/` (note: feature renamed from "TransferbuttonMenu" to "QuickActions" for clarity per the integration plan). The other 6 features are tracked separately. Don't touch them.

## Prerequisites — read before writing any code

1. **The integration plan**: `C:/Users/mikew/.claude/plans/one-of-our-coders-steady-raccoon.md` — section "5. TransferbuttonMenu (rename to 'QuickActions' in DOTS — clearer)" has the planned file layout.
...
├── QuickActionsSettingsProvider.cs        ← wraps DotsSettings.Instance for 14 settings
├── Models/
...
├── QuickActionsIoC.cs
└── Hooks/
    ├── InventorySearchCampaignBehavior.cs ← port InventorySearchBehavior; SyncData("DOTS_IsInventorySearchAvailable") — DIFFERENT key from original to avoid save-compat collision
...
DOTS.Tests/Features/QuickActions/
...
### MCM settings — append to `Main/Features/DotsSettings.cs`
...
The original `InventorySearchBehavior` uses a per-frame `OnTick` to bi-directionally sync `SPInventoryVM.IsSearchAvailable`. **DO NOT** copy this into `SubModule.OnApplicationTick` (DOTS's main tick). Instead, register `CampaignEvents.TickEvent` handler on the behavior and run the sync there. Per-frame is overkill for a settings sync; once-per-campaign-tick is enough and avoids burning CPU for an idle inventory screen.
...
4. **`SyncData` key collision.** The original uses `SyncData("HoN_IsSearchAvailable", ref _flag)`. **Rename to `"DOTS_IsInventorySearchAvailable"`** when porting (this is a fresh feature for DOTS users; no save-compat constraint).

5. **`ExecuteSellAllItems` patch order.** DOTS has no other patches on `SPInventoryVM.ExecuteSellAllItems` (verified in CLAUDE.md patch table), so no explicit ordering needed. But DOTS Patch23 patches `SPInventoryVM.UpdateCurrentCharacterIfPossible` — different method, no collision.
...
2. MCM → DOTS → "Inventory / Quick Actions" → confirm `Enable Quick Actions = true`.
...
- Files created: [count]
- Files modified: DotsSettings.cs (14 settings), IoC.cs, SubModule.cs (Patch34 + behavior registration)
