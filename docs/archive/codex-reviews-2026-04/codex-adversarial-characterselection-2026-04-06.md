...
**Not available from decompiled source** — `BodyGeneratorView.cs` was not found in `E:\Decompiled_Bannerlord\`. Analysis relies on API documentation and DOTS source.
...
`ActionSet` is a normal fluent setter, not terminal. If vanilla `RefreshCharacterEntityAux` already calls `.ActionSet(...)` later in the fluent chain, DOTS's injected value is overwritten. **Cannot confirm or deny without decompiled source** — this is the core release risk.
...
**DOTS code:** Scans IL and stops at first `newobj` matching `AgentVisualsData::.ctor()` (lines 36-42), then injects `ldarg.0 -> call GetActionSet -> callvirt ActionSet` after it (lines 48-53).
...
**DOTS code:** Injects fluent `ActionSet(...)` immediately after `new AgentVisualsData()`.

**Evidence:** `AgentVisualsData.ActionSet` is a normal fluent setter among many. If vanilla calls `.ActionSet(...)` later in the chain, DOTS's value is overwritten. The patch becomes behaviorally dead while still applying cleanly. Cannot verify without decompiled source.
...
### [MEDIUM] Race-aware action lookup bypasses DOTS's unknown-race fallback pattern

**File:** `RefreshCharacterEntityAuxPatch.cs:16-19`

**DOTS code:** `GetActionSet` calls `FaceGen.GetBaseMonsterFromRace(bodyGeneratorView.BodyGen.Race)` with no validation/fallback.
...
**Remediation:** Add explicit guard/fallback before `MBGlobals.GetActionSetWithSuffix`, reusing DOTS's existing race-validation path. If unknown race IDs are impossible here, prove with decompiled code and document.
