...
**What's wrong:** `_dots_careerData` stores `Dictionary<string, HeroCareerData>` directly. No `SaveableTypeDefiner`, `ConstructContainerDefinition`, or `[SaveableField]` registration for `HeroCareerData` exists anywhere in DOTS. Bannerlord's save system requires custom classes to be registered explicitly.
...
**Fix:** Load ability data from `dots_ability_templates.xml`, add activation entry point, implement battle-side targeting/cast execution.
...
5. **Race-awareness** — DOTS's `CareerDefinition` has `AllowedRaces`, preventing cross-race career assignment. TOR has no race system.

## Architecture Comparison

| Aspect | TOR | DOTS | Verdict |
|--------|-----|------|---------|
| Career definitions | 22 C# static classes | XML config | **DOTS** — extensible |
...
| Test coverage | Zero | 6 test files | **DOTS** — testable |
...
| Extensibility | New C# class per career | XML entry | **DOTS** — data-driven |
