...
1. **Inline-hook activation claims in new skills**
   ...
   - **`/skill-stocktake`: DISPUTED.** Frontmatter has no `hooks:` block at [skill-stocktake](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/skills/skill-stocktake/SKILL.md:1), and the skill is read-only audit text.
...
3. **Subagent boundary integrity**
   - **`debugger`: mostly clean, with one routing ambiguity edge.** The agent body draws a clear boundary against DOTS C# bugs and agent-loop problems at [debugger](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/agents/debugger.md:15), and CLAUDE.md routes non-DOTS script/build/hook problems there at [CLAUDE.md](/abs/path/c:/Users/mikew/source/repos/DOTS/CLAUDE.md:115). For “my Python script is failing because of a Bannerlord DLL path,” the routing still resolves to `debugger` because the failure surface is tooling/script, not DOTS C#. I do **not** consider that boundary broken.
   - **`refactoring-specialist`: clean.** Its boundary against `/deslop`, `code-architect`, and `feature-builder` is explicit at [refactoring-specialist](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/agents/refactoring-specialist.md:17), and it matches the existing `/deslop` deletion-first contract at [deslop](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/skills/deslop/SKILL.md:45).
   - **`error-detective`: clean.** It is explicitly for multi-symptom / cross-system correlation at [error-detective](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/agents/error-detective.md:13), and tells the caller to fall back into one or more `/investigate` runs after correlation at [error-detective](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/agents/error-detective.md:63). For “save-load corruption AND mission-init crash AND companion-spawn freeze,” this agent should go first.

4. **`suggest-compact.sh` boundary detection edge cases**
   - **`git commit`: CONFIRMED for bare form only.** It matches `*"git commit"*` at [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:76).
   - **`git -C path commit`: CONFIRMED MISS.** No `*"git -"*" commit"*` pattern exists in this hook, unlike the reference pattern codified in [harness-facts.md](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/rules/harness-facts.md:74).
   - **`git -c key=val commit`: CONFIRMED MISS.** Same reason and same file lines.
   - **`git commit-tree` / `git commit-graph`: CONFIRMED REJECT.** The inner `*"git commit-"*` rejection at [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:79) correctly avoids those plumbing commands.
   - **Build/test/push substring false positives: CONFIRMED RISK.** `*"./build.ps1"* | *"dotnet build"* | *"dotnet test"* | *"git push"*` at [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:84) and [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:88) are raw substring matches on the whole command string, so a command that merely mentions those literals in an argument can trip the boundary hint.
   - **Throttle reset: DISPUTED (works as written).** The hook persists the last boundary count at [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:95) and only re-fires when `COUNT - LAST_BOUNDARY > 10` at [suggest-compact.sh](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/hooks/suggest-compact.sh:68). That permits repeated hints in long sessions; it is not stuck.

5. **`/context-save` state file location safety**
   - **Gitignore status: CONFIRMED SAFE.** `git check-ignore -v .claude/state/context/hypothetical.md` resolves to `.claude/state/.gitignore:1:*`. The ignore file itself is [state/.gitignore](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/state/.gitignore:1).
...
6. **`/skill-stocktake` checklist completeness**
   - **Amend-exemption recursion-risk check: CONFIRMED MISSING.** The checklist at [skill-stocktake](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/skills/skill-stocktake/SKILL.md:35) does not include the “amend exemptions in pre-commit hooks” rule codified in [harness-facts.md](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/rules/harness-facts.md:86).
   - **`effort:` validity: CONFIRMED PRESENT.** It is explicitly checked at [skill-stocktake](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/skills/skill-stocktake/SKILL.md:41).
   - **`triggers:` drift: CONFIRMED PRESENT.** It is explicitly checked at [skill-stocktake](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/skills/skill-stocktake/SKILL.md:39).
   - **DOC-BACKED vs EMPIRICAL labeling: CONFIRMED MISSING.** The checklist never asks whether facts are labeled per the convention added to [harness-facts.md](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/rules/harness-facts.md:114).
...
- **CHANGELOG updated for `79350f2`: YES.** Verified by `git diff 2c4d414..79350f2 -- CHANGELOG.md`; the new entry is at [CHANGELOG.md](/abs/path/c:/Users/mikew/source/repos/DOTS/CHANGELOG.md:3).
...
- **AGENTS.md counter consistent with REVIEW-LOG.md: YES, by manual comparison.** Git Bash was not usable in this environment (`bash.exe` failed before `tools/audit-review-counter.sh` could run), so I compared [AGENTS.md](/abs/path/c:/Users/mikew/source/repos/DOTS/AGENTS.md:40) against [REVIEW-LOG.md](/abs/path/c:/Users/mikew/source/repos/DOTS/docs/reviews/REVIEW-LOG.md:54). Both say `28` reviews / `77` bugs.
- **Was a deep-review run before shipping? No local evidence found.** The hard rule is C#-specific at [CLAUDE.md](/abs/path/c:/Users/mikew/source/repos/DOTS/CLAUDE.md:25), so this `.claude/`-only commit did not violate the literal wording. I found no pre-ship artifact showing a `/deep-review` run for `79350f2`; the files I found under `.claude/tmp/` are post-hoc review artifacts from this review pass.
- **Scope note:** the shipped diff also modifies [.claude/settings.local.json](/abs/path/c:/Users/mikew/source/repos/DOTS/.claude/settings.local.json:32), which was not listed in the user’s changed-file summary.
