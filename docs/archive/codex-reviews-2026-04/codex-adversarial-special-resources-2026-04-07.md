...
No-ship. DOTS is cleaner and more data-driven than TOR, but the current port still miskeys Mordor against Bannerlord kingdom IDs, grants raid income from unrelated world events, and only enforces upgrade affordability in the UI after vanilla has already mutated the roster.

**Note:** This is v2 of the review. Codex could not clone TOR_Core (network restricted) but grounded analysis in local DOTS sources, `docs/research/tor-resource-system.md`, and decompiled Bannerlord v1.3.15 from `E:\Decompiled_Bannerlord\`.

## Known Suspects Verdict (Section 9)

1. **PENDING TRANSACTION GAP:** CONFIRMED — `PartyScreenLogic_UpgradeTroop_Patch.cs:15-29` deducts in postfix immediately. TOR uses `_resourceChanges[]` pending transaction with Done/Cancel handling. DOTS loses resources on Cancel.

2. **UPGRADE COST TARGET MISMATCH:** CONFIRMED CONSISTENT — Both DOTS patches use upgrade TARGET troop ID, and `troop_resource_costs.xml` keys by TARGET. However, the `kingdom_id="mordor"` mismatch (Finding 1) means the whole system no-ops for Mordor regardless.

3. **GAMEMODEL DEAD CODE:** CONFIRMED — `DotsSpecialResourceModel` exposes `GetCurrentResource()` and `CanAffordUpgrade()` but grep shows no callers outside SubModule GameModel registration. These methods are dead code.
...
**DOTS code:** Runtime lookup uses `hero.Clan?.Kingdom?.StringId` which returns `"empire_s"` for Mordor.
...
**DOTS code:** Handler checks `side == BattleSideEnum.Attacker` and player's kingdom. Never checks `component.IsPlayerMapEvent` or player party involvement.
...
**DOTS code:** Deducts in postfix after vanilla `UpgradeTroop` executes. Never blocks or clamps the command before execution.
...
**DOTS code:** `RestoreData` accepts any values from save data. `Get` returns unchanged. Only `AddCapped` enforces limits.
...
**DOTS code:** Map-bar mixin creates `MapInfoItemVM("special_resource", ...)` then overrides `VisualId` with sprite name. Widget exits unless `IconID == "special_resource"`. After override, custom lookup branch is skipped. `IconBrushWidget` falls back to brush-layer lookup.
...
**DOTS code:** `TroopResourceCostEntry.ResourceId` is loaded from XML but never consulted. Storage is `Dictionary<string,float>` keyed by hero ID only. No resource ID dimension.
...
## What DOTS Does Better (Section 11)

1. **IoC vs static singletons** — DOTS uses DryIoc with interface contracts (`ISpecialResourceService`, `ISpecialResourceStorageService`). TOR uses `CustomResourceManager.Instance`. DOTS is testable and mockable; TOR requires static initialization order management.

2. **XML-driven config** — DOTS's `special_resources_config.xml` + `troop_resource_costs.xml` define everything declaratively. TOR hardcodes 10 resources in `CustomResourceManager.Initialize()` plus per-resource C# helper classes. Adding a DOTS resource = XML edit; adding a TOR resource = new C# class + registration.

3. **Adapter boundary** — DOTS wraps TaleWorlds types behind adapter interfaces. TOR accesses `Hero`, `CharacterObject`, `Settlement` directly throughout `CustomResourceManager`, `TeefBehavior`, etc. DOTS's boundary is cleaner for version migration.

4. **Test foundation** — DOTS has 2 test files (~24 test methods) covering service logic with NSubstitute mocks. TOR has zero unit tests for custom resources. DOTS's coverage is incomplete but the testable architecture exists.

5. **Thin entry points** — DOTS's Harmony patches delegate to `IOnPartyUpgradeResourceCheck` hook → service. TOR's `CustomResourcePatches.cs` contains inline business logic with direct `Hero.MainHero` access. DOTS's layering is more maintainable.

6. **Separation of concerns** — DOTS splits into Domain, Services, Hooks, UI, Config. TOR combines persistence, earning, spending, and UI concerns in `CustomResourceManager` (~800 lines). DOTS's decomposition follows Single Responsibility.
...
| Extensibility | New C# class per resource | XML entry | **DOTS** — data-driven, no recompile |
| Test coverage | Zero | 24+ test methods | **DOTS** — testable architecture |
| Save compat | Engine handles field versioning | Raw dict, no migration or cap | **TOR** — graceful forward compat |
| Config pattern | Hardcoded constants | Centralized XML + IConfigProvider | **DOTS** — single source of truth |
| v1.3.15 compat | Built for older BL versions | Built for v1.3.15 | **DOTS** — verified against decompiled 1.3.15 |
