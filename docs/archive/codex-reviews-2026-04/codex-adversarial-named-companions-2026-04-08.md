...
**DOTS code:** `NamedCompanionBehavior.EnsureCompanionsPlaced()` runs on every game load. Gate is only `IsPlacedInSettlement()` which checks `StayingInSettlement != null || CurrentSettlement != null`. `PlaceInSettlement()` unconditionally calls `hero.ChangeState(Active)` + `EnterSettlementAction.ApplyForCharacterOnly(hero, settlement)`.
