...
**File:** `DotsAgeModel.cs:15-17`

**DOTS code:** Config advertises early reproduction for short-lived races (`orc comesOfAge=12`, `uruk=10`, `uruk_hai=8`, `berserker=6`). But `DotsAgeModel` only overrides `MaxAge` and `BecomeOldAge` (lines 15-17).
...
**DOTS code:** Race config carries per-race `becomeOld` values and `IRaceAgeService.GetBecomeOldAge()` exists, but no production code uses it. `DotsAgeModel` hardcodes `BecomeOldAge => 5000` and `MaxAge => 10000`.
