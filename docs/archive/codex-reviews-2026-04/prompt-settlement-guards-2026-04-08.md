...
a) DOTS.Tests/Features/SettlementGuards/SettlementGuardConfigProviderTests.cs (13 tests)
b) DOTS.Tests/Features/SettlementGuards/SettlementGuardServiceTests.cs (14 tests)
...
c) What happens when the garrison is empty -- does TakeGuardAgentDataFromGarrisonTroopList fall back to culture.Guard? Will DOTS's prefix intercept this before the fallback?
...
f) Cross-reference each against dots_spcultures.xml and spcultures.xslt
...
d) Each finding MUST cite both DOTS file:line AND vanilla code. Findings without both = rejected.

Prior review lessons (from 18 reviews at 81% accuracy):
a) Do NOT assume empire=Rohan. empire=Dunland. Rohan=vlandia.
b) Do NOT flag DOTS code that matches vanilla behavior as a bug.
