# Codex Adversarial Review: Arena (DotsTournamentModel)

**Date:** 2026-04-06
**Target:** branch diff against master
**Verdict:** needs-attention

No-ship. The Arena override changes tournament cadence, prize selection, and participant armor resolution in ways that materially diverge from Bannerlord vanilla, and the culture-dummy path is provably incomplete for real DOTS cultures.
...
**DOTS:** Hard switch `0/0.45/0.75/0.90/1.0` based on active adult hero count. No skill filter, no weekly gate.
...
**DOTS:** 20-day grace period + 0.033/day linear ramp.
**Divergence:** More predictable tournament lifetimes.

### BuildPrizePool

**DOTS:** Iterates ALL items filtering by culture and tier. O(n) over entire item registry.
**Vanilla:** Uses value-bounded filtering with pre-categorized items.
**Divergence:** DOTS scans wider but unbounded — no value caps on regular rewards.
...
**File:** `DotsTournamentModel.cs:23-36`

**DOTS code:** Hard switch `0/0.45/0.75/0.90/1.0` counting any active adult hero.
...
**File:** `DotsTournamentModel.cs:45-77`

**DOTS code:** Ignores `regularRewardMinValue`/`regularRewardMaxValue`. Regular: any same-culture weapon/armor in tier band. Elite: every same-culture item with `Tierf >= 4`.
...
**File:** `DotsTournamentModel.cs:80-99`

**DOTS code:** Directly requests `gear_practice_dummy_{participant.Culture.StringId}`. Falls back to `base.GetParticipantArmor`. Tested `ResolveDummyId` helper is never called.
...
- BuildPrizePool O(n) scan is acceptable for DOTS's item count but less efficient than vanilla's pre-filtered approach
