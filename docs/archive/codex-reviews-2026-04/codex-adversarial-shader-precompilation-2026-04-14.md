...
**File:** DOTS.Tests/Features/ShaderPrecompilation/ShaderPrecompilationServiceTests.cs:132
**Rule:** ADR-008
...
|---|---------|---------|---------|
| 1 | TROOP CAP OVERFLOW | **CONFIRMED** | Silent loss happens in DotsShaderGameManager.cs:103. It only drops characters when neither side can fit the next copy count, but that still matters because later characters never contribute shader coverage. |
...
3. Confirm calls `MBGameManager.StartNewGame(new DotsShaderGameManager(...))` in Main/SubModule.cs:191; vanilla `MBGameManager.StartNewGame` creates `GameLoadingState` and pushes it (MBGameManager.cs:46-52).
4. After loading, DotsShaderGameManager.cs:42 sets `IsShaderBattleActive = true`, builds the custom battle, and calls `CustomBattleHelper.StartGame(data)`.
