...
**DOTS code:** The prefix short-circuits by checking `!__instance.SelectedOptions.ContainsKey(__instance.CurrentMenu)` and assigns `CurrentMenu` directly (lines 18-29).
...
**DOTS code:** Changing `BannerImage` resets `_textureLoaded`, `_loadFailed`, `_loadedSprite` but never clears the inherited `Sprite`. If the next lookup misses (`LoadTextureFromPath` returns null), the widget logs and bails without assigning a new sprite, then still calls `base.OnRender(...)`.
