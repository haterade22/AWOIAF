...
### Anchor data
- Main/_Module/ModuleData/characters/spider_creature.xml (NPCCharacter id="dots_spider_creature")

### Tests
- DOTS.Tests/Features/Spider/SpiderAttackServiceTests.cs (14 tests)
- DOTS.Tests/Features/Spider/SpiderSpawnerServiceTests.cs (6 tests)
...
E. **LOTRLOME_Armory not loaded.** `<DependedModule Id="LOTRLOME_Armory" optional="true" />`. If LOTRLOME_Armory is missing/disabled, MBObjectManager.GetObject<Monster>("spider") returns null. SpiderSpawnerService logs an error and returns empty list. Verify: does the engine block DOTS from loading, or does DOTS load with the spider feature gracefully no-oping? Does the optional flag actually allow DOTS to load without LOTRLOME_Armory?
...
- `"dots_spider_creature"` (anchor id) — appears in SpiderSpawnerService.SpiderCharacterId and spider_creature.xml
...
For the anchor character `dots_spider_creature`:
...
- BodyProperty.fighter_dolguldur — verify this template exists in DOTS_bodyproperties.xml
...
1. Quote the exact line(s) of DOTS source you're flagging
