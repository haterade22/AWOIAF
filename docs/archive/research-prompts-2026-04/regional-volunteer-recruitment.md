...
## Goal
Port and adapt the LOTRAOM regional volunteer recruitment system to DOTS. This system overrides `GetBasicVolunteer` so that troop recruitment is determined by a **settlement → clan → culture** fallback hierarchy, rather than vanilla's culture-only approach.
...
- **`Main/Features/TroopProgression/Models/DotsVolunteerModel.cs`** — Currently only overrides `MaxVolunteerTier => 6`. Needs `GetBasicVolunteer` override.
...
## DOTS Design Requirements

### Follow DOTS architecture patterns
...
5. **Update `DotsVolunteerModel`** — Override `GetBasicVolunteer`, delegate to service
...
1. Add `GetBasicVolunteer` override to `DotsVolunteerModel`
...
1. **DOTS uses adapter pattern** — LOTRAOM accessed `Hero.CurrentSettlement` directly. DOTS must use `IHeroAdapter`, `ISettlementAdapter`, `IClanAdapter`
2. **DOTS uses proper IoC** — LOTRAOM used its own DI. DOTS uses the standard IoC.cs registration
3. **New troop IDs** — LOTRAOM used old Gondor troop IDs. DOTS uses the new regional IDs (gondor_loss_*, gondor_da_*, etc.)
4. **TDD** — LOTRAOM had no tests for this. DOTS requires full test coverage.
5. **DOTS extends `DefaultVolunteerModel`** — LOTRAOM extended `VolunteerModel`. Both approaches work in 1.3.

## Files to Read Before Starting

| File | Why |
|------|-----|
| `Main/Features/TroopProgression/Models/DotsVolunteerModel.cs` | Current model to extend |
...
| `DOTS.Tests/` | Test patterns and conventions |
