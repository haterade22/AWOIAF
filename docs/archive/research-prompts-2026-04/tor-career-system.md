...
> TOR targets **Bannerlord v1.3.2** — close to DOTS's **v1.3.15**.
...
## 8. Version Compatibility Notes (TOR v1.3.2 → DOTS v1.3.15)

| Area | Status | Notes |
|------|--------|-------|
| ObjectManager.RegisterType | **Check** | Type IDs (103U, 104U, 105U) must not conflict with DOTS's existing registrations |
...
| System | What Career Needs | Lore-Specific? | DOTS Equivalent |
|--------|------------------|----------------|-----------------|
...
| System | What Career Uses | Lore-Specific? | DOTS Approach |
|--------|-----------------|----------------|---------------|
...
Every component below is required for the complete DOTS career system. No subsystems deferred.
...
| Component | Complexity | What It Does |
|-----------|-----------|--------------|
| **PassiveEffect system** | Large | 44-type enum + ExplainedNumber integration. Every passive type must wire into DOTS's existing 16+ GameModels via `CareerHelper.ApplyBasicCareerPassives()`. |
...
| **Career definitions** (per-career choice classes) | Large (volume) | One `DotsCareerChoicesBase` subclass per career. Each defines root node + all choice groups with keystones, passives, and mutations. Data-driven XML preferred over TOR's code-driven approach. |
...
| **Ability templates (XML)** | Medium | `dots_abilitytemplates.xml` defining all career abilities: effect type, cast type, cooldown, radius, particles, sound, targeting. |
...
Career passives must wire into DOTS's existing GameModels. For each model, add `CareerHelper.ApplyBasicCareerPassives()` calls:

| Existing DOTS Model | Career PassiveEffectTypes to Support |
|---------------------|--------------------------------------|
| DotsCharacterStatsModel | Health |
| DotsPartyWageModel | TroopWages |
| DotsPartySizeModel | PartySize |
| DotsPartySpeedModel | PartyMovementSpeed, MovementSpeed |
| DotsPartyMoraleModel | TroopMorale |
| DotsBattleRewardModel | BattleRenownGain |
| DotsPartyTroopUpgradeModel | TroopUpgradeCost |
| DotsPartyHealingModel | HealthRegeneration, TroopRegeneration |
| DotsFoodConsumptionModel | (food reduction passives) |
| DotsSmithingModel | EnchantmentCostReduction |
| DotsClanFinanceModel | CustomResourceUpkeepModifier |
| DotsSettlementProsperityModel | (settlement passives if applicable) |
| DotsRaidModel | TroopDamage |
| DotsCombatSimulationModel | Damage, Resistance |
| DotsMilitaryPowerModel | (troop power scaling) |
| DotsTournamentModel | (tournament bonuses if applicable) |
| **New: DotsAgentStatModel** | Ammo, SwingSpeed, ArmorPenetration, StealthBonus, EquipmentWeightReduction |
| **New: DotsAbilityModel** | SpellRadius, SpellEffectiveness, BuffDuration, DebuffDuration |
| **New: DotsMapVisibilityModel** | PartySpottingRange |
| **New: DotsInventoryModel** | InventoryCapacity |
...
Layer 6: GameModels  ◄──────┘ (can start in parallel with Layer 3+)
  Wire PassiveEffectTypes into existing DOTS models
