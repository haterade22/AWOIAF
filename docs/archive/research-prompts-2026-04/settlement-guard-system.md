...
| **Approach** | `DotsSettlementPatrolModel` that checks settlement ID for custom patrol template |
...
| **Approach** | Map each DOTS culture to an appropriate spear item ID |
...
This mirrors the existing `VolunteerRecruitmentService` pattern already proven in DOTS.
...
Only two spear IDs for ALL cultures. Battania (Khand in DOTS) gets `northern_spear_2_t3`, everyone else gets `western_spear_3_t3`.

### DOTS Impact

These are **vanilla item IDs**. If DOTS's armory module doesn't include these items, spear guards will have **null weapons** (invisible spears or crashes). Options:

1. **Ensure items exist**: Verify `northern_spear_2_t3` and `western_spear_3_t3` exist in LOTRLOME_Armory or vanilla
2. **Patch GetSuitableSpear**: Harmony Prefix that maps each DOTS culture to a lore-appropriate spear
...
The patrol system (`PatrolPartiesCampaignBehavior`) DOES save: `_partyGenerationQueue`, `_lastHomeSettlementVisitTimes`, `_interactedPatrolParties`. A custom `DotsSettlementPatrolModel` is also save-safe since it's a stateless GameModel.
...
### Interaction with Existing DOTS Patches
...
| **Patch5_FaceGen** | CHECK | Face generation for custom races. Guards use `FaceGen.GetMonsterWithSuffix(race, "_settlement")` — ensure `_settlement` monster variants exist for all DOTS races |
...
If DOTS custom races (orc, elf, dwarf, hobbit) don't have `{monster}_guard` and `{monster}_unarmed_guard` action sets, guards will T-pose or crash. Verify in `action_sets.xml`.
...
## G. Existing DOTS Guard Infrastructure

### Current Guard Characters per Culture

All 16 DOTS cultures define these guard NPCs:
...
3 levels per culture in `dots_partyTemplates.xml`:
...
├── Models/
│   └── DotsSettlementPatrolModel.cs    — Per-settlement patrol templates
└── Adapters/
    ├── ISettlementAdapter.cs           — Settlement abstraction
    └── ICultureAdapter.cs              — Already exists in DOTS
```
