...
You are researching how to implement a **per-kingdom special resource system** for DOTS (Dawn of the Stag), a Lord of the Rings total conversion mod for Bannerlord v1.3.15.
...
**Task 2: Analyze DOTS's Architecture for Integration**

Read DOTS's codebase to understand where this system would plug in:

1. **Existing Economy Models** — Read `Main/Features/TroopProgression/Models/DotsPartyWageModel.cs` and `Main/Features/CulturalFeats/Models/DotsPartyTroopUpgradeModel.cs`. How do they work? What services do they depend on?
...
3. **Save/Load Pattern** — Read `Main/Features/HeroRace/RacePersistenceService.cs` or any `CampaignBehaviorBase` with `SyncData`. How does DOTS persist custom data?
4. **Adapter Pattern** — Read `Main/Adapters/` directory. How do services avoid touching sealed TaleWorlds types?
5. **Config Loading** — Read `Main/Features/StartupResources/StartupResourcesConfigProvider.cs`. How does DOTS load XML config?
6. **UIExtenderEx Usage** — Search for `BaseViewModelMixin` or `ViewModelMixin` in the codebase. How does DOTS inject into vanilla UI?
7. **Cultural Feats** — Read `Main/Features/CulturalFeats/DotsCulturalFeats.cs`. How are per-culture bonuses implemented?
...
1. **Feature module file structure** following DOTS conventions (IoC, service, adapter, behavior, model, hooks, tests)
...
6. **What GameModels need changes** — which existing DOTS models need modification vs new models needed
...
### Rules
- Follow DOTS's architecture: `[Entry Point] → Service → IAdapter` (ADR-007)
