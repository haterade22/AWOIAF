...
## Objective
Reverse-engineer the custom resource system from TOR_Core (Warhammer Bannerlord mod) and assess how to port it to DOTS (LOTR Bannerlord mod). You are providing an independent second opinion — another agent is doing the same research separately.
...
## DOTS Context (for portability assessment)

DOTS is a LOTR total conversion mod targeting Bannerlord v1.3.15. Key constraints:
...
- **Existing economic models**: `DotsPartyTroopUpgradeModel`, `DotsPartyWageModel`, `DotsClanFinanceModel`, `DotsBattleRewardModel`
...
20. What would need to change to use DOTS's adapter pattern instead of TOR's direct `Hero.MainHero` access?
...
6. **Portability Assessment** — what ports clean to DOTS, what needs adaptation, what to skip, with specific file-by-file analysis
7. **Recommended DOTS Design** — your independent recommendation for class hierarchy, file structure, and implementation order
