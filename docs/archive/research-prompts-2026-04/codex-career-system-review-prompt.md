# Codex Adversarial Review: DOTS Career System vs TOR_Core Career System

> **Purpose:** After implementing the DOTS career system, use this prompt to tear it apart by comparing against TOR_Core's battle-tested career system. Find every gap, missed edge case, silent failure, and architectural weakness.
...
You are an adversarial code reviewer. Your job is to find problems, not praise good work. Compare DOTS's career system against TOR's mature implementation and flag every place where DOTS cut corners, missed mechanics, or introduced fragility.
...
- GameModel overrides (PassiveEffectType integration into existing DOTS models)
...
**Tests:**
- All test files in `DOTS.Tests/Features/CareerSystem/`
...
DOTS's career system was designed to differ from TOR in these ways. Do NOT flag these as bugs:

1. **Data-driven vs code-driven:** DOTS defines careers in XML config; TOR hardcodes in C# static classes. This is intentional -- DOTS wants to add careers without recompilation.
2. **Adapter pattern:** DOTS wraps TaleWorlds types behind `IHeroAdapter` etc.; TOR accesses `Hero` directly. DOTS's boundary is stricter by design.
3. **DryIoc vs static singletons:** DOTS uses constructor injection; TOR uses `TORCareers.Instance`. Both work, DOTS is more testable.
4. **UIExtenderEx vs custom VM patches:** If DOTS uses UIExtenderEx for career UI injection instead of TOR's 6-patch ViewModel extension system, that's a valid design choice.
5. **Level-based career points:** Both use `hero.Level + 1` for max choices. This is confirmed identical.

DO flag if DOTS's data-driven approach silently drops capabilities that TOR's code-driven approach provides (e.g., mutation lambdas that can't be expressed in XML).
...
Compare against DOTS's equivalent:
- Does DOTS enforce tier gating correctly? (Tier 2/3 locked until attribute/quest unlocks them)
- Does DOTS enforce mutual exclusion within a tier? (Only one keystone per tier)
- Does DOTS enforce the max career points formula: `Math.Min(hero.Level + 1, MaxPerkPoints + 1)`?
- Can a player select the same choice twice? TOR prevents this via `CareerChoices.Contains(StringId)` check.
- Does DOTS auto-select the root node on career assignment?
- Does DOTS refresh the CareerHelper cache after every choice change?
...
Compare against DOTS's persistence:
...
- Does DOTS use Bannerlord's `[SaveableField]` or `SyncData` pattern? What are the trade-offs?
...
If DOTS implements this:
- Can XML express everything TOR's C# lambdas can? (e.g., `AddSkillEffectToValue` which reads the hero's skill level)
- Does DOTS clone ability templates per-hero before mutating? (TOR does -- mutations must not affect other heroes)
- Does DOTS apply mutations in the correct order? (Root node first, then selected choices)
- Does DOTS support all 3 mutation target types? (AbilityTemplate, TriggeredEffectTemplate, StatusEffectTemplate)

If DOTS does NOT implement mutations:
...
Compare:
- Does DOTS implement all charge types? (CooldownOnly, DamageDone, Kills, DamageTaken, Healed, Custom)
- Does DOTS handle the slow-time targeting mode? (0.3x time scale, weapon sheath/restore)
- Does DOTS handle the crosshair system? (Self, Missile, SingleTarget, Wind, Pointer, TargetedAOE)
- Does DOTS reset charge to 0 after ability activation?
- Does DOTS support double-use keystones? (e.g., SecretsOfTheGrailKeystone allows second consecutive cast)
...
Compare:
- How many PassiveEffectTypes does DOTS implement? List all.
- Which DOTS GameModels call career passive application? List all.
- For each TOR model that calls `ApplyBasicCareerPassives`, does the corresponding DOTS model also call it?
- Are there DOTS-specific GameModels (e.g., DotsMilitaryPowerModel) that should have career integration but don't?
- Does DOTS use the same `ExplainedNumber` integration pattern? (Add vs AddFactor for percentage vs flat bonuses)
...
Compare:
- Does DOTS's career screen show all 3 tiers with correct lock/unlock state?
- Does DOTS's career screen update FreeCareerPoints correctly after each selection?
...
Compare:
- Does DOTS implement per-career battle effects? (Necrarch spell charge, Ironbreaker damage reflect, etc.)
- Does DOTS's mission behavior tick at the correct interval?
- Does DOTS track mission-scoped variables? (kill counts, block counts, etc.)
- Does DOTS handle career ability casting during missions?
...
Compare:
- Can DOTS players switch careers? If so, through what mechanism?
...
List which DOTS hooks and which are missing:
...
## DOTS-Specific Checks (things TOR doesn't have)

These are DOTS-specific requirements. Verify even if TOR doesn't do them:

1. **Race-awareness:** Do career definitions respect DOTS's race system? Can an elf career be assigned to a dwarf? Is there validation?
2. **Cultural feat interaction:** Do career passives stack correctly with existing DotsCulturalFeats bonuses? (e.g., both adding to PartySpeed -- do they double-dip or compose correctly via ExplainedNumber?)
...
DOTS uses vanilla Bannerlord IDs remapped to LOTR:
...
[Areas where DOTS's approach is cleaner -- be honest]

## ARCHITECTURE COMPARISON
| Aspect | TOR | DOTS | Verdict |
|--------|-----|------|---------|
| Career definitions | Code-driven (22 C# classes) | XML-driven | [trade-offs] |
| Mutation system | C# lambdas | [DOTS approach] | [can XML match?] |
| Passive application | CareerHelper cache + 21 models | [DOTS approach] | [coverage gap] |
| Charge system | 12 per-career suppliers | [DOTS approach] | [completeness] |
| Save/load | [SaveableField] on HeroExtendedInfo | [DOTS approach] | [robustness] |
| UI depth | CareerScreen + CharDev + Party + HUD | [DOTS approach] | [gap] |
| Test coverage | Zero | [DOTS count] | [assessment] |
| Ability casting | Full targeting + time slow + crosshair | [DOTS approach] | [completeness] |
```
...
- When you find a gap, cite the specific TOR file/line and the specific DOTS file/line.
- Be specific about fixes -- file paths, line numbers, what to change.
- If DOTS's data-driven approach handles something better than TOR's hardcoded approach, say so.
- For Bannerlord API verification, check decompiled source at `E:\Decompiled_Bannerlord\`.
- DOTS targets Bannerlord v1.3.15. TOR targets v1.3.2. Flag any API differences.
