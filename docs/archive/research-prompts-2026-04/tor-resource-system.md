...
## 8. DOTS Portability Assessment

### Ports Directly (Low Effort)
| Component | Notes |
|-----------|-------|
| `CustomResource` data model | Simple POCO, no TOR dependencies |
| `CustomResourceManager` singleton pattern | Replace TOR culture constants with DOTS culture IDs |
| `CustomResourcePatches` Harmony patches | Target same vanilla classes — should work on 1.3.15 |
| `tor_extendedunitproperties.xml` schema | Rename, populate with DOTS troop IDs |
| Tooltip/icon injection pattern | Same Bannerlord UI APIs |

### Needs Adaptation (Medium Effort)
| Component | What Changes |
|-----------|-------------|
| **Save storage** | TOR uses `HeroExtendedInfo` (their custom extension system). DOTS needs its own — either a similar `CampaignBehaviorBase` with `SyncData` dictionary, or a simpler JSON sidecar save |
| **Culture mapping** | TOR hardcodes `TORConstants.Cultures.GREENSKIN` etc. DOTS replaces with `"mordor"`, `"isengard"`, etc. |
| **Battle gain formulas** | TOR has Warhammer-specific bonuses (vs Beastmen, etc.). DOTS needs LOTR equivalents |
| **Behavior classes** | TeefBehavior's Quartermaster NPC dialog is Warhammer-flavored. DOTS needs LOTR equivalent NPCs/dialogs |
| **Career system coupling** | TOR's `HasCareerChoice()` multipliers won't exist in DOTS (unless career system is implemented) |
...
| Career perk multipliers | DOTS has no career system yet |
| `CustomResourceContainerScript` | Item-based resource granting — nice-to-have, not MVP |

### Critical Differences from DOTS Architecture
| TOR Pattern | DOTS Equivalent Needed |
|-------------|----------------------|
...
| `ExplainedNumber` in Manager methods | Same pattern works — DOTS already uses it |
...
## 9. Minimum Viable Port for DOTS

### Phase 1: One Kingdom (Mordor — "War Spoils")

**Must build:**
1. `CustomResource` data model (port directly)
2. `ISpecialResourceService` + `SpecialResourceService` (DOTS adapter pattern)
...
7. XML config: `dots_resource_definitions.xml` (resource ID, culture, icon) + `dots_troop_resource_costs.xml` (per-troop upgrade cost)
