...
(2026-05-27), Isengard (Saruman) adult NPCs in DOTS had empty `<skills/>` blocks or vanilla-default
...
- All adult Isengard (Saruman) NPCs receive one of the shared DOTS archetype templates (10 base
...
**Notes:** Uruk-Hai are stronger than regular orcs but use the same archetype values (DOTS bin lookup uses `uruk_hai` race). Berserkers + Warg-Riders covered by `orc_berserker` / `orc_warg` variants.
