...
(2026-05-27), Harad (Haradrim Southrons) adult NPCs in DOTS had empty `<skills/>` blocks or vanilla-default
...
- All adult Harad (Haradrim Southrons) NPCs receive one of the shared DOTS archetype templates (10 base
...
DOTS Harad is mostly Mumakil-lord wives + serpent-banner heirs + trade-caravan managers. Auto-assigned to `haradrim_lord` / `haradrim_cav` / `mumak_rider` / `desert_lady` archetypes via bio scan.
...
**Notes:** High Riding + Trade for desert / mumakil culture. Scarlet-and-gold flavor. No canonical override (Khadurak is named in DOTS as Taskral; his explicit override is in a separate enemy-lord roster TBD).
