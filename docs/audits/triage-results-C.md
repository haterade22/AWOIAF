...
| 173 | VALID | 1 P1 + 3 P2 | CareerPassiveHelper:27/32 static IoC.Resolve cache; CareerPassiveService:11-12 plain Dictionary + :21 Clear; DotsPartyWageModel:65-79 inline foreach in GameModel body; DotsSmithingModel:46 truncates to int before ApplyFactor at :51 |
...
| 196 | VALID | P1 | `docs/features/execution.md` does NOT exist (glob finds only `alignment-aware-execution.md`); `Main/Features/Execution/` has 12 C# files including DotsExecutionRelationModel, AlignmentService, Patch14_Execution hooks |
...
`Main/_Module/GUI/SpriteParts/ui_dots_career_system/CareerSystem/Portraits/` contains 21 PNGs (counted via Glob). 50 careers defined in `dots_careers.xml` ⇒ 29 missing. Audit list checked against actual files — every named missing portrait (e.g., `mulkerhili_cultist`, `snaga_rider`) is absent.

`Main/_Module/GUI/SpriteParts/ui_dots_career_system/CareerSystem/Abilities/` contains 9 PNGs:
```
...
**Fix (short):** add 29 portrait PNGs + 41 ability PNGs + N career_choice_* PNGs + `<SpritePart>` entries; add `DisplayName` field to `CareerChoiceDefinition` (P3 #18); add `{=dots_career_button}Career` binding to Career button (P3 #19).
...
**P1 #6 — VALID.** `Main/_Module/GUI/SpriteParts/ui_taom/SpecialResources/` contains exactly 3 PNGs: `dots_caster_icon.png`, `dots_gems_icon.png`, `dots_marks_icon.png`. 8 missing per audit confirmed.
...
**Fix:** decide intent (Option A use `TimeControlMode == StoppableFastForward`; Option B add service call that raises `SpeedUpMultiplier`); wrap tooltip in `TextObject("{=dots_extra_fast_forward_hint}...")`.
...
**Note:** #175 below notes Gauntlet threading is single-threaded for DOTS widgets (confirmed via v1.4 decomp). That downgrades #15 and #16 to P3. The other findings in #169 remain at audit severity.
...
Both have inline comments referencing the 2026-05-06 Codex finding. `DOTS.Tests/Features/MixedFormations/` has 2 test files; grep for `RepresentativeIsCavalry`/`cavalry`/`Cavalry` returns zero matches. Test gap confirmed.
...
**F3 (P1)** `DotsPartyWageModel.cs:65-79`:
```csharp
...
**F4 (P2)** `DotsSmithingModel.cs:46-52`:
```csharp
...
`Glob docs/features/execution*.md` returns no match. `Glob docs/features/*execution*.md` returns only `alignment-aware-execution.md` (an Execution subsystem doc). `Main/Features/Execution/` exists with 12 C# files (AlignmentService, DotsExecutionRelationModel, KillCharacterAction_ApplyInternal_Patch, TraitLevelingHelper_OnLordExecuted_Patch, etc.). The `detect-docs-gaps.sh` SessionStart hook still fires this gap.
...
- `Main/DOTS.csproj:67`: `<!-- Sibling parallel-ports restored 2026-05-07: SmartCavalryAI, CompanionTactics. -->` — no `<Compile Remove>` line for CompanionTactics
...
> No unit tests exist for AdvancedCombat services in `DOTS.Tests/Features/`.

`DOTS.Tests/Features/AdvancedCombat/BoneCollisionServiceTests.cs` exists at 252 lines with 11 `[TestMethod]` attributes. Glob confirms the file is the only test file in that folder — `SpatialGridDebugService` remains uncovered, but `BoneCollisionService` has 11 tests. The doc is stale. P2 confirmed.
...
`DOTS.Tests/Features/Warg/WargAttackServiceTests.cs` exists with 7 `[TestMethod]`. Doc is stale. P2 confirmed.
