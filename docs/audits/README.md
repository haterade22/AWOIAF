...
A multi-phase audit verifying that every feature under `Main/Features/` is wired correctly and implemented per DOTS conventions.
...
- **Source of truth** for "what features exist": top-level directories under `Main/Features/` (43 features as of 2026-05-13). `DotsSettings.cs` at that level is a file, not a feature.
