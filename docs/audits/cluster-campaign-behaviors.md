...
The 16 reviews surfaced **5 recurring DOTS bug classes** in CampaignBehavior code. Naming them upfront because they explain the bulk of the findings and the fixes will be highly repetitive:
...
| **R5** | Load-path mutation lacks Entity State Matrix coverage (NamedCompanions Review #23 class) | NamedCompanions (Prisoner + Fugitive states), SpecialResources (desertion-on-load), DotsPregnancyModel (state pre-checks) | `NamedCompanionAdapter.cs:27-31` |
...
- **SyncData key:** `_dots_messengers` → `Dictionary<string, string>`.
...
- **Wiring:** 3 behaviors registered at `SubModule.cs:319, 321, 326` in correct order (Persistence first → Campaign → Dialogue). All deps Singleton. SyncData keys (`_dots_careerIds`, `_dots_careerChoices`, `_dots_careerTiers`) namespaced and non-colliding.
...
- **Phase 2 cross-reference:** [`cluster-gamemodels.md`](cluster-gamemodels.md) Diplomacy section adds rule-4 findings on `DotsAllianceModel`, `DotsDiplomacyModel`, `DotsKingdomDecisionPermissionModel`. Phase 2 explicitly chose NOT to open a duplicate issue — these notes are tracked under #129 for Phase 9 fix work.
...
- **SyncData key:** `_dots_heroRaceMap` → `Dictionary<string, int>`.
...
- **P1** — `DotsPregnancyModel.GetDailyChanceOfPregnancyForHero` contains 32 lines of inline business logic (sealed `Hero` access, `Math.Min`, `ExplainedNumber`, `GetPerkValue`, full vanilla pregnancy reimplementation) at `DotsPregnancyModel.cs:18-58`. Out of Phase 3 scope (Phase 2 GameModel territory) but flagged P1 because it's an ADR-007 + GameModel rule double-violation. Fix: extract to `IRaceAgeService.GetDailyPregnancyChance(IHeroAgeInfo)`.
...
- **P3 (out of scope)** — `DotsAgeModel.MaxAge` / `BecomeOldAge` hardcoded to `10000`/`5000` constants — Phase 4 GameModel territory.
- **Phase 2 cross-reference:** [`cluster-gamemodels.md`](cluster-gamemodels.md) RaceAge section adds new null-safety findings (`hero.Spouse` / `hero.Clan` chains at `DotsPregnancyModel.cs:40-41`). Phase 2 explicitly chose NOT to open a duplicate issue — these notes are tracked under #131 for Phase 9 fix work.
...
- **P3 (out of scope)** — `DotsSiegeEventModel`, `Patch8_SiegeCampGuard` — Phase 2/4 territory.
...
- **SyncData key:** `_dots_specialResources` → `Dictionary<string, float>`. Composite key `heroId:resourceId` (unparsed — write-only).
...
- **SyncData key:** `DOTS_FormationPresets` → `List<HoNFormationPreset>` (via `SaveableTypeDefiner` BaseId 726900601).
...
- **SyncData key:** `DOTS_IsInventorySearchAvailable` → `bool`.
...
| # | Feature |
|---|---|
| [#123](https://github.com/haterade22/DOTS/issues/123) | Messengers |
| [#124](https://github.com/haterade22/DOTS/issues/124) | BannerInjection |
| [#125](https://github.com/haterade22/DOTS/issues/125) | CharacterCreation |
| [#126](https://github.com/haterade22/DOTS/issues/126) | InitialChildGeneration |
| [#127](https://github.com/haterade22/DOTS/issues/127) | NamedCompanions (special target) |
| [#128](https://github.com/haterade22/DOTS/issues/128) | CareerSystem |
| [#129](https://github.com/haterade22/DOTS/issues/129) | Diplomacy |
| [#130](https://github.com/haterade22/DOTS/issues/130) | HeroRace |
| [#131](https://github.com/haterade22/DOTS/issues/131) | RaceAge |
| [#132](https://github.com/haterade22/DOTS/issues/132) | Siege |
| [#133](https://github.com/haterade22/DOTS/issues/133) | SpecialResources |
| [#136](https://github.com/haterade22/DOTS/issues/136) | StartupResources |
| [#139](https://github.com/haterade22/DOTS/issues/139) | CompanionTactics |
| [#141](https://github.com/haterade22/DOTS/issues/141) | EquipPresets |
| [#143](https://github.com/haterade22/DOTS/issues/143) | FiefManagement |
| [#146](https://github.com/haterade22/DOTS/issues/146) | QuickActions |
...
- **Phase 3 #131 (RaceAge)** already covers `DotsPregnancyModel` rule-4 violation, NaN/Infinity config gap, and validate-before-lookup pattern. Phase 2 added new null-safety findings on `hero.Spouse`/`hero.Clan` chains and noted them in `cluster-gamemodels.md` RaceAge section without opening a duplicate issue.
- **Phase 3 #129 (Diplomacy)** already covers `WarOfTheRingConfigProvider` ordering invariants and `DiplomacyConfigProvider` `?? new T()` fallback. Phase 2 added rule-4 findings on `DotsAllianceModel`, `DotsDiplomacyModel`, `DotsKingdomDecisionPermissionModel` and noted them in `cluster-gamemodels.md` without duplicating.
...
- **Phase 4+ targets surfaced:**
  - GameModel reviews (Phase 2 / future): `DotsPregnancyModel` ADR-007 violation; `DotsAgeModel` hardcoded constants; `DotsSiegeEventModel`; `DotsTargetScoreModel` (untouched).
  ...
  - Cross-feature handshake (Phase 6): RevoltTuning vs CulturalFeats `DotsSettlementLoyaltyModel`; HeroRace race ID stability across CharacterCreation/RaceAge/NamedCompanions/SpecialResources.
