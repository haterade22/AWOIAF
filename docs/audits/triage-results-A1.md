...
| 131 | RaceAge DotsPregnancyModel ADR-007 violation + R3 + R4 | VALID | P1+P1+P2+P2+P3 | Extract GetDailyPregnancyChance(IHeroAgeInfo) to IRaceAgeService; reset _raceIdCache on new-campaign; add FiniteFloatValidator + ordering invariants; IsValidRaceId gate before GetRaceNameFromId | Adapter for IHeroAgeInfo (new) | DotsPregnancyModel.cs:18-58 is 40+ lines of inline biz logic against sealed Hero — clear gamemodels.md rule-4 violation |
...
public void SyncData(IDataStore dataStore)
{
    ...
    dataStore.SyncData("_dots_playerModifiedBanners", ref list);
    if (list != null)
    {
...
**Current code at `Main/Features/InitialChildGeneration/DotsInitialChildGenerationBehavior.cs:22-23`:** `if (index == 0)` triggers `GenerateInitialChildren()`.
...
### #131 — audit(impl): RaceAge — DotsPregnancyModel ADR-007 violation + singleton race cache stale + R3+R4

**Current code at `Main/Features/RaceAge/Models/DotsPregnancyModel.cs:18-59`:** 40+ lines of inline business logic:
```csharp
...
**Audit finding:** Inline business logic in DotsPregnancyModel (ADR-007 + GameModel rule 4); _raceIdCache singleton not reset; missing validate-before-lookup; missing semantic config validation.
