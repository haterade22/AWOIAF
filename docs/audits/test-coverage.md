...
**Phase-7 verdict:** DOTS is in better test-coverage shape than the Phase 3 P1 count (24) suggested. Of 44 features, 36% (16) meet ADR-008 outright; 18% (8) have shallow but functional coverage; 39% (17) have a concrete cross-feature or callback gap; 7% (3) have a fundamental P1 problem.
...
**No `*Engine.cs` files exist in DOTS** — the "100% engines" rule from ADR-008's spirit is vacuous in this repo. Future engine-like additions (`*Builder.cs` in EditorCacheRebuild, planning helpers) should inherit the 100%-service rule; documented for future rule scope.

## Master findings table

| # | Severity | Feature | Test gap | File:Line (production) | Test file | Issue |
|---|---|---|---|---|---|---|
| 1 | **P1** | CulturalFeats | 16 GameModels, only 1 structural test file (8 reflection tests). Zero behavior-hook tests of override calculation logic. 8 models inline `CareerPassiveHelper.ApplyFactor` from override body (cross-feature static coupling). | `Main/Features/CulturalFeats/Models/*.cs` (16 files) | `DotsCulturalFeatsDefinitionTests.cs` (structural only) | #176 |
...
| 4 | **P2** | RaceAge | `DotsPregnancyModel.GetDailyChanceOfPregnancyForHero` (22 lines, 5 branches) has no test. Cross-tick consistency with `RaceAgeBehavior.OnDailyTick` (Phase 6 #3, #13) unverified. | `Main/Features/RaceAge/Models/DotsPregnancyModel.cs` | `RaceAgeServiceTests.cs` + 2 others | #179 (refs #131) |
| 5 | **P2** | TroopProgression | `DotsPartyWageModel.GetTotalWage` (~50 lines, multi-branch) has no test. Inlines `CareerPassiveHelper.ApplyFactor` from override body (Phase 6 #32, #34, #148). | `Main/Features/TroopProgression/Models/DotsPartyWageModel.cs` | `*ServiceTests.cs` (services tested) | #180 (refs #148) |
...
| 21 | P3 | Encyclopedia | 2 tests cover 2 code paths; no boundary cases | — | `DotsInformationRestrictionModelTests.cs` | doc only |
...
| 25 | P3 | CustomBattles | `DotsFactionSelectionVM` nav methods untested (low complexity) | `Main/Features/CustomBattles/DotsFactionSelectionVM.cs:14` | 5 test files | doc only |
...
| 80% behavior-hook coverage | **8 features fall below** (CulturalFeats, FiefManagement, TroopWeight, Messengers regression-class, SettlementGuards, SiegeDismount, CharacterCreation cross-feature, HeroRace cross-feature). The lowest-quality coverage layer in DOTS. |
...
| #3 / #13 / #131 — RaceAge same-tick consistency | Confirmed (DotsPregnancyModel + RaceAgeBehavior.OnDailyTick both read `hero.Race`; no consistency test) |
| #8 — `DotsSettlementLoyaltyModel` (RevoltTuning consumer) | Confirmed (CulturalFeats P1 — model untested; producer-side RevoltTuning is fine) |
...
Verified across all 44 features: DOTS has zero `*Engine.cs` files. The closest analogues are EditorCacheRebuild's `*Builder.cs` files (Phase1/2 parallel + serial builders), all of which are tested. Future "engine"-like classes should inherit the 100%-service rule. Documenting for future rule scope.

### Manifest 43 vs disk 44 off-by-one

The Phase 0 manifest text reads "43 features" but the manifest table and `ls Main/Features/` show 44 (excluding `DotsSettings.cs` file). Minor; flagged for manifest correction in Phase 8.
...
| #179 | P2 | audit-tests: RaceAge — DotsPregnancyModel.GetDailyChanceOfPregnancyForHero untested + cross-tick consistency | refs #131, Phase 6 #3, #13 |
| #180 | P2 | audit-tests: TroopProgression — DotsPartyWageModel.GetTotalWage untested + CareerPassiveHelper coupling | refs #148 |
...
- Phase 0 manifest text says "43 features" but the table and `Main/Features/` show 44 (excluding `DotsSettings.cs`). Trivial; correct in Phase 8 manifest sweep.
