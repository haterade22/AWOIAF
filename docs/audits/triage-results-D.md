...
| 179 | RaceAge — DotsPregnancyModel.GetDailyChanceOfPregnancyForHero untested | VALID | P2 | Add `DotsPregnancyModelTests.cs` (5-branch coverage + cross-tick consistency vs `RaceAgeBehavior.OnDailyTick`) | Phase 6 #3, #13 (consistency contract) | `DotsPregnancyModel.cs` exists; only `DotsAgeModelTests.cs` is in the test dir |
| 180 | TroopProgression — DotsPartyWageModel.GetTotalWage untested | VALID | P2 | Add `DotsPartyWageModelTests.cs` covering tier 0-10 + culture wage feats + Rohan mounted feat + career TroopWages passive (with no-passive baseline) | #148 (CareerPassiveHelper) | Test dir has 3 service-test files; no model test |
...
| 182 | CompanionTactics+SmartCavalryAI — shared SetMovementOrder postfix ordering | VALID | P2 | Integration test driving one `Formation` through both postfixes in deterministic order; assert each subscriber's invariant survives | #170 (SmartCavalry handshake) | `DOTS.Tests/Features/CompanionTactics/` (BattleActionBar/FormationPresets/Roles subdirs) — no `SetMovementOrder` references |
...
**Test dir contents at HEAD:** `DOTS.Tests/Features/CulturalFeats/DotsCulturalFeatsDefinitionTests.cs` (single file).

**Existing test surface** (`DotsCulturalFeatsDefinitionTests.cs:18-28`):
```csharp
[TestMethod]
public void AllFeatProperties_ReturnFeatObject_CountIs59()
{
    var properties = typeof(DotsCulturalFeats)
...
**Production state:** 16 `DotsXxxModel.cs` files still present in `Main/Features/CulturalFeats/Models/` (DotsArmyManagementModel through DotsVillageProductionModel). Verdict: **VALID**.

### #177 — FiefManagement: 5 behavior callbacks untested

**Test dir contents:** `DOTS.Tests/Features/FiefManagement/FiefHubServiceTests.cs` (single file). Grep for `RegisterEvents|OnSessionLaunched|OnNewGameCreated|OnGameLoaded|SyncData|FiefHubCampaignBehavior` returns no files. Behavior class still present at `Main/Features/FiefManagement/Hooks/FiefHubCampaignBehavior.cs`. Verdict: **VALID**.
...
### #179 — RaceAge: DotsPregnancyModel untested

`Main/Features/RaceAge/Models/DotsPregnancyModel.cs` exists. Test dir: `RaceAgeConfigProviderTests.cs`, `RaceAgeServiceTests.cs`, `DotsAgeModelTests.cs` — no Pregnancy test. Grep for `DotsPregnancyModel|GetDailyChanceOfPregnancyForHero` returns no files. Verdict: **VALID**.

### #180 — TroopProgression: DotsPartyWageModel.GetTotalWage untested

Test dir: `TroopCostServiceTests.cs`, `VolunteerRecruitmentServiceTests.cs`, `VolunteerTierServiceTests.cs` — services only. Grep for `DotsPartyWageModel|GetTotalWage` returns no files. Verdict: **VALID**.
