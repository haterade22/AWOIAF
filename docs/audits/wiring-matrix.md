...
| CulturalFeats | N/A | ✅ 17 models — 16 parameterless + 1 (DotsSettlementLoyaltyModel) correctly resolves `IRevoltTuningConfigProvider` | N/A | N/A | ✅ | — |
| CustomBattles | N/A | N/A | ✅ 4 hook interfaces — 1 impl each | ✅ | ✅ | — |
| Diplomacy | N/A | ✅ DotsAllianceModel/DotsKingdomDecisionPermissionModel/DotsDiplomacyModel correctly resolve `IDiplomacyService` + `IWarOfTheRingService` | ✅ IOnAllianceAction / IOnPeaceAction | N/A | ✅ | — |
| EditorCacheRebuild | N/A | N/A | N/A | ✅ (consumed via MCM lambda in `DotsSettings.cs`) | N/A | — |
| Encyclopedia | N/A | ✅ DotsInformationRestrictionModel (default ctor with `Func<bool>` settings fallback) | N/A | N/A | N/A | — |
...
| RevoltTuning | N/A | (consumed by CulturalFeats `DotsSettlementLoyaltyModel`) | N/A | ✅ (consumed via cross-feature) | N/A | — |
...
- **CulturalFeats** — 16 of 17 models are parameterless; only `DotsSettlementLoyaltyModel` takes an `IRevoltTuningConfigProvider`, correctly resolved at `SubModule.cs:288` via `IoC.Resolve<IRevoltTuningConfigProvider>()`. This is exactly the cross-feature path the manifest flagged (RevoltTuning feeds CulturalFeats) — it works as designed.
- **Arena** — `DotsTournamentModel` (parameterless, constructed inline at `SubModule.cs:284`).
- **Encyclopedia** — `DotsInformationRestrictionModel` takes a `Func<bool>` settings predicate, default-constructed with a `DotsSettings.Instance` lambda at `SubModule.cs:301`.
- **Diplomacy (inline)** — `DotsAllianceModel`, `DotsKingdomDecisionPermissionModel`, `DotsDiplomacyModel` all resolve their `IDiplomacyService` / `IWarOfTheRingService` dependencies correctly at SubModule.cs:260-262.
