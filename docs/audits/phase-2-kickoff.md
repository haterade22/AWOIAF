...
- [.claude/rules/gamemodels.md](../../.claude/rules/gamemodels.md) — DOTS GameModel rules (override pattern, base.X() fallback, no inline logic, registration)
...
- `Main/Features/<X>/Models/Dots*Model.cs` — the review targets
- `DOTS.Tests/Features/<X>/` — corresponding test files for coverage assessment

## Scope — 12 features, ~39 model classes

| Feature | Model count | Models |
|---|---|---|
| **CulturalFeats** | **17** | DotsArmyManagementModel, DotsPartySpeedModel, DotsSettlementProsperityModel, DotsSettlementMilitiaModel, DotsBuildingConstructionModel, DotsVillageProductionModel, DotsCaravanModel, DotsBattleRewardModel, DotsTournamentModel (note: see Arena), DotsPartyTroopUpgradeModel, DotsPartySizeModel, DotsFoodConsumptionModel, DotsSettlementLoyaltyModel, DotsPartyMoraleModel, DotsSmithingModel, DotsClanFinanceModel, DotsRaidModel |
| CareerSystem | 5 | DotsMapVisibilityModel, DotsInventoryCapacityModel, DotsAgentStatCalculateModel, DotsAgentApplyDamageModel, DotsClanTierModel |
| BattleBalance | 3 | DotsMilitaryPowerModel, DotsCombatSimulationModel, DotsPartyHealingModel |
| Diplomacy | 3 | DotsAllianceModel, DotsKingdomDecisionPermissionModel, DotsDiplomacyModel |
| RaceAge | 3 | DotsAgeModel, DotsPregnancyModel, DotsHeroCreationModel |
| TroopProgression | 2 | DotsPartyWageModel, DotsVolunteerModel |
| Arena | 1 | DotsTournamentModel (verify whether this is the same class as CulturalFeats' or a different one — manifest is ambiguous) |
| ArmyTargeting | 1 | DotsTargetScoreModel |
| CharacterCreation | 1 | DotsCharacterStatsModel |
| Encyclopedia | 1 | DotsInformationRestrictionModel |
| Execution | 1 | DotsExecutionRelationModel |
| Siege | 1 | DotsSiegeEventModel |

**Highest-value targets:** CulturalFeats (17 models, batched registration), CareerSystem (5 models, includes per-hot-path `DotsAgentStatCalculateModel`), Diplomacy (cross-feature interactions).
...
```
You are reviewing the GameModel cluster for feature <X> as part of Phase 2 of the DOTS
...
For each model in Main/Features/<X>/Models/Dots*Model.cs:
...
6. **Test coverage.** Open DOTS.Tests/Features/<X>/ and find tests for this model.
   - Service-layer tests covering the business logic: count + assertion strength.
   - Model-layer tests covering the override path itself (vanilla call → DOTS call): present or missing?
