...
Audit every feature with a `[ViewModelMixin]` / `[PrefabExtension]` / Custom Widget surface against DOTS's UI rules. Find ViewModel mutation bugs, prefab extension target drift in v1.3.15, sprite verification gaps, localization mistakes, and binding case-sensitivity issues.
...
|---|---|---|
| **CareerSystem** | `CareerScreenView`, `CareerScreenVM`, sprite atlas `ui_dots_career_system`, prefab extensions | Heaviest — career portraits 800×400, ability icons 256×256, dedicated atlas. 50 careers × 3 archetypes. Many bindings. |
