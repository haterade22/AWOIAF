...
Phases 2–5 reviewed features in isolation. Phase 6 reviews the **gaps between features** — places where two DOTS features both touch the same TaleWorlds API, GameModel parameter, mission state, or static helper, and the result depends on which fires last (or first).
...
| **TimeAcceleration × MapBar** | TimeAcceleration injects into vanilla map bar's CenterPanel; other features also extend MapBar | Verify DOTS extra fast-forward button doesn't conflict with future MapBar mods or SiegeDefense map notifications | cluster-ui |
...
- `Main/Features/**/Models/Dots*Model.cs` — GameModel parameter dependencies
...
3. Any group with > 1 DOTS patch class = a collision pair to inspect.
