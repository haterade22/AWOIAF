...
|---|---------|---------|---------|---------|---------|
| 134 | DotsSiegeEventModel.cs:23-24 | 1 × P1 NRE | 1/1 | P1 | VALID |
| 135 | DotsPartySpeedModel.cs:30 + 47-61 | 1 × P1 NRE + roster perf | 2/2 | P1 + P2 | VALID |
| 136 | StartupResourcesConfigProvider.cs + StartupResourcesBehavior.cs | 1 × P1 + 1 × P2 + 1 × P3 | 3/3 | mixed | VALID |
| 137 | DotsTournamentModel.cs | 5 × P2 | 5/5 | P2 | VALID |
| 138 | DotsTargetScoreModel.cs:26-34 | 2 × P2 | 2/2 | P2 | VALID |
| 139 | FormationPresetSaveableTypeDefiner.cs + FormationPresetCampaignBehavior.cs | 2 × P1 + 1 × P2 | 3/3 | mixed | VALID |
| 140 | DotsPartyHealingModel + DotsMilitaryPowerModel + BattleBalanceConfigProvider | 4 × P2 | 4/4 | P2 | VALID |
...
| 145 | DotsInformationRestrictionModel.cs | 1 × P2 | 1/1 | P2 | VALID |
| 146 | InventorySearchCampaignBehavior + Patch34 capture/finalize | 2 × P2 + 1 × P3 | 3/3 | P2 + P3 | VALID |
| 147 | DotsExecutionRelationModel.cs | 3 × P2 | 3/3 | P2 | VALID |
| 148 | DotsPartyWageModel.cs + IoC.cs:110 | 4 × P2 | 4/4 | P2 | VALID |
...
File: `Main/Features/Siege/Models/DotsSiegeEventModel.cs`
...
Verdict: **VALID.** `party.MobileParty.HasPerk(...)` is called twice on an unguarded receiver. Garrison-defender `PartyBase` has `MobileParty == null` (settlement parties with no leader army). The audit's text cites perks `SiegeWorks`/`Counterweights`; current code uses `Stonecutters`/`SiegeEngineer`. Perk names differ but the NRE risk is identical — both unguarded calls on the same null-reachable receiver. Per `adapters.md` "`?.` for computed TaleWorlds properties." `git log -- Main/Features/Siege/Models/DotsSiegeEventModel.cs` shows only the original feature commit (`1c89d6f feat: defender trebuchets in siege management UI`).
...
### #135 — DotsPartySpeedModel Campaign.Current.MapSceneWrapper NRE (P1, VALID)

File: `Main/Features/CulturalFeats/Models/DotsPartySpeedModel.cs`
...
### #137 — DotsTournamentModel inline branching + unguarded chain (5 × P2, VALID)

File: `Main/Features/Arena/Models/DotsTournamentModel.cs`
...
### #138 — DotsTargetScoreModel inline ternary + early-return (2 × P2, VALID)

File: `Main/Features/ArmyTargeting/Models/DotsTargetScoreModel.cs`
...
Verdict: **VALID.**
- **P1 — Generic container registration:** `Dictionary<string,int>`, `Dictionary<int,int>`, `List<string>` are extremely common vanilla container shapes; cross-mod collision risk is high. Class doc-comment even acknowledges "First DOTS use of SaveableTypeDefiner; CareerSystem deliberately avoided the pattern via primitive SyncData." Audit's recommendation to switch to primitive-dict SyncData is consistent with the CareerSystem precedent.
...
Files: `Main/Features/BattleBalance/Models/DotsPartyHealingModel.cs` + `Main/Features/BattleBalance/Models/DotsMilitaryPowerModel.cs` + `Main/Features/BattleBalance/BattleBalanceConfigProvider.cs`

DotsPartyHealingModel line 53:
...
DotsMilitaryPowerModel lines 19-36:
...
- DotsMilitaryPowerModel inline tier derivation + branching multiplier — rule 4. `CalculateTierPower` static is partial extraction (still on the model class, not a service).
...
- (IoC.Resolve) Add a 3rd ctor param `ICareerPassiveService` on `DotsPartyHealingModel`; pass at SubModule.cs:299.
...
Files: `Main/Features/CareerSystem/Models/DotsAgentStatCalculateModel.cs`, `DotsAgentApplyDamageModel.cs`, `DotsInventoryCapacityModel.cs`, `DotsMapVisibilityModel.cs`

`DotsAgentStatCalculateModel.UpdateAgentStats` (lines 31-89): 55-line override with hero/passive/buff-tracker dispatch inline — rule 4.
`DotsAgentApplyDamageModel.ApplyDamageReductions` (lines 34-65): nested `if (heroId != null) { if (_passiveService != null) ... }` versus the other overrides' early-return — inconsistent guard style.
`DotsInventoryCapacityModel` (lines 10-24) + `DotsMapVisibilityModel` (lines 10-19): both call static `CareerPassiveHelper.ApplyFactor(hero, ref result, ...)`. Per audit: `CareerPassiveHelper` is a static service-locator wrapper. Defensive `if (_passiveService == null) return baseValue` guards present in `DotsAgentStatCalculateModel` (line 25) — unreachable because service is resolved unconditionally at SubModule.cs:317.

Verdict: **VALID** on all 5 findings.

Smallest fix: extract `ICareerAgentStatService.UpdateAgentStats(heroId, ref AgentDrivenProperties)` for AgentStatCalculate; unify ApplyDamage guard style; inject `ICareerPassiveService` constructor-arg into `DotsInventoryCapacityModel` + `DotsMapVisibilityModel`; remove `CareerPassiveHelper` static after migration; remove unreachable null-guards.
...
`DotsCaravanModel.cs:13`:

```csharp
if (CharacterObject.PlayerCharacter?.Culture?.HasFeat(DotsCulturalFeats.UmbarCheaperCaravansFeat) == true)
    return MathF.Round(baseCost * (1f + DotsCulturalFeats.UmbarCheaperCaravansFeat.EffectBonus));
```

`DotsBattleRewardModel.cs:21,25`:
...
`DotsClanFinanceModel.cs:19`:

```csharp
var culture = clan?.Culture;
```

`DotsSettlementProsperityModel.cs:22-29`:
...
`DotsSmithingModel.cs:29-56`: 28-line `ApplySmithingFeatReduction` private static on the model class — rule-4 extraction must go to a Service, not to a model-private static.

Verdict: **VALID** on all 7 findings. Specifics:
1. Systemic rule 4 across 16 models — confirmed.
2. `DotsCaravanModel` uses `CharacterObject.PlayerCharacter` static — feat is player-only by accident, semantically inconsistent with feat description.
3. `DotsBattleRewardModel` asymmetric coalesce — feat path `party.Owner?.Culture ?? party.Culture` vs career path `party.Owner ?? party.LeaderHero`.
4. `DotsClanFinanceModel` defensive `?.` on non-nullable `Clan` parameter.
5. `DotsSettlementProsperityModel` 3× compound `if (HasFeat && ResultNumber >= 0f)` — business rule inline.
6. `DotsSmithingModel` extraction to model-private static — wrong target.
7. `DotsPartySpeedModel` roster `foreach` on per-tick hot path — overlaps with #135's P2.

Smallest fix: introduce `ICulturalFeatsService` (per-domain or unified); reduce each model body to a 1-line delegate. Per specific findings: route caravan cost through a party-owner adapter; align coalesce semantics in `DotsBattleRewardModel`; remove defensive `?.` (or assert non-null); move compound conditions into service business rule; move `ApplySmithingFeatReduction` to `ISmithingFeatsService`.

Dependencies: large refactor — gate on tests-first. `DotsSettlementLoyaltyModel` consumes `IRevoltTuningConfigProvider` (1-arg ctor at SubModule.cs:288) which already lives separately; other 15 models are no-arg. The Pattern-D coupling to `CareerPassiveHelper` is umbrella'd in #142; coordinate the deletion.

### #145 — DotsInformationRestrictionModel static-coupling (1 × P2, VALID)

File: `Main/Features/Encyclopedia/Models/DotsInformationRestrictionModel.cs`

Current code (full):

```csharp
public DotsInformationRestrictionModel()
    : this(() => DotsSettings.Instance?.ShowAllEncyclopediaCharacters ?? true) { }

internal DotsInformationRestrictionModel(Func<bool> showAll) => _showAll = showAll;
```

Verdict: **VALID.** The public ctor (line 11-12) hard-couples to `DotsSettings.Instance` static. The internal `Func<bool>` ctor is a test seam but production still reaches the static. Per `csharp-architecture.md` "Constructor injection only — no service locator in services."
...
### #147 — DotsExecutionRelationModel architectural smell + ADR-007 + rule-4 (3 × P2, VALID)

File: `Main/Features/Execution/Models/DotsExecutionRelationModel.cs`
...
public class DotsExecutionRelationModel : DefaultExecutionRelationModel
{
    ...
    public DotsExecutionRelationModel(IOnExecutionAction executionHook)
    {
...
Verdict: **VALID.**
- **Hook injected into model:** `IOnExecutionAction` is a hook interface (resolved via `IoC.ResolveAll<IOnX>()` in patches per `csharp-patterns.md`). Injecting it into a GameModel is structurally unusual — no other DOTS GameModel does this. Should be wrapped in an `IExecutionRelationService`.
...
### #148 — DotsPartyWageModel inline + cross-feature helper + IoC cohesion gap (4 × P2, VALID)

File: `Main/Features/TroopProgression/Models/DotsPartyWageModel.cs` + `Main/IoC.cs:110`

DotsPartyWageModel lines 37-86 (GetTotalWage body):
...
public override ExplainedNumber GetTotalWage(MobileParty mobileParty, TroopRoster troopRoster, bool includeDescriptions = false)
{
    ...
    {
        ApplyGarrisonWageFeat(ref result, garrisonCulture, DotsCulturalFeats.EreborGarrisonWageFeat);
        ...
    }
    ...
    {
        if (partyCulture.HasFeat(DotsCulturalFeats.GundabadWageFeat))
            result.AddFactor(...);
        ...
        if (partyCulture.HasFeat(DotsCulturalFeats.RohanMountedWageFeat) && troopRoster != null)
        {
...
DotsPartyWageModel lines 88-112 (GetTroopRecruitmentCost body):
...
if (troop.IsMounted && buyerHero?.Culture?.HasFeat(DotsCulturalFeats.IsengardCheaperRecruitsFeat) == true)
    result.AddFactor(...);
```
...
- **Doc cross-reference:** the CLAUDE.md GameModel table at "Patch_MissionTime_SetMovementOrder" / "Patch31_SmartCavalryAI" is unrelated. The CulturalFeats umbrella in #144 cites "16 models" but the gamemodels rule file's existing-overrides list also assigns `DotsPartyWageModel` to CulturalFeats; current actual file location is `Main/Features/TroopProgression/Models/DotsPartyWageModel.cs`. CLAUDE.md → `Main/Features/CulturalFeats/Models/` table count and the rule file's `DotsPartyWageModel` row may benefit from alignment, but this is doc drift not a bug.
