...
| `IExecutionRelationService` | #147 | Replaced injected hook in `DotsExecutionRelationModel`; `ExecutionRelationResult` struct; `IPlayerContextAdapter` replaces `Hero.MainHero.MapFaction.StringId` access. |
| `ITournamentService` | #137 | 4 pure decision functions extracted from `DotsTournamentModel`; rule-4 compliance achieved. |
...
| #180 | DotsPartyWageModel tests | Pair with #148 `IWageModifierService` extraction track. |
...
| Issue | Commit | Result |
|---|---|---|
| **#144 + #176** CulturalFeats systemic | `4431cff` | `ICulturalFeatsService` (19 methods) + `ICultureFeatAdapter` (ADR-007) + concrete service + IoC + all 16 `Dots*Model.cs` refactored to thin boundaries. **+49 tests** in `CulturalFeatsServiceTests`. 26 files, +1437/-367. |
...
| **#142** CareerSystem 3 remaining P2s | `df0c7c9` | `ICareerAgentStatService` extracted (4 methods); `DotsAgentStatCalculateModel.UpdateAgentStats` body 55→4 lines; `DotsAgentApplyDamageModel` 3 overrides each 2-3 lines; unreachable null guards removed across 3 models. **+22 tests.** Net -58 model lines. |
