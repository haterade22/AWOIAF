...
Single source of truth for "what features exist in DOTS and what wiring touchpoints do they have." Feeds Phase 1 (wiring matrix) by sketching expected vs. observed registration. Built from a parallel sweep of `Main/Features/`, `Main/IoC.cs`, `Main/SubModule.cs`, `Main/Features/DotsSettings.cs`, `Main/_Module/SubModule.xml`, and `Main/_Module/GUI/SpriteParts/Config.xml`.

## Audit dimensions (column legend)

| Column | Meaning |
|---|---|
| **Archetype** | Primary wiring shape — `Behavior` (has `*CampaignBehavior.cs`), `Model` (has `Dots*Model.cs`), `Patch` (only `[HarmonyPatch]`), `Service` (IoC + services only, no patches/models), `UI` (has `[ViewModelMixin]` or `[PrefabExtension]`), `Mixed`. Multiple labels = combined archetype. |
...
| **MCM** | `DotsSettings.cs` section name covering this feature (verbatim). Blank if no MCM exposure. |
| **Doc** | `docs/features/<X>.md` exists. ✅ / ❌. |
| **Test** | `DOTS.Tests/Features/<X>/` directory exists. ✅ / ❌. |
...
| Arena | Model | ❌ | ❌ | — | 0 | 1 (`DotsTournamentModel`) | — | — | ✅ | ✅ | No IoC class; `DotsTournamentModel` constructed inline. Phase 1: confirm it has no deps that need IoC. |
| ArmyTargeting | Patch+Model | ✅ | ✅ | `Patch22_ArmyTargeting` | 0 | 1 (`DotsTargetScoreModel`) | `configs/army_targeting.json` | AI Strategic Intelligence | ✅ | ✅ | OK |
...
| BattleBalance | Model | ✅ | ✅ | — | 0 | 3 (`DotsMilitaryPowerModel`, `DotsCombatSimulationModel`, `DotsPartyHealingModel`) | `configs/battle_balance_config.json` | Battle Balance/Troop Power, Battle Balance/Casualty Ratios | ✅ | ✅ | OK |
...
| CareerSystem | Mixed (Behavior+Model+Patch+UI) | ✅ | ✅ | `Patch27_CareerSystem` | 3 (`CareerPersistenceBehavior`, `CareerCampaignBehavior`, `CareerSwitchDialogueBehavior`) | 5 (`DotsMapVisibilityModel`, `DotsInventoryCapacityModel`, `DotsAgentStatCalculateModel`, `DotsAgentApplyDamageModel`, `DotsClanTierModel`) | `career_system/`, `charactercreation/career_menu.json` | — | ✅ | ✅ | Heaviest feature. Sprite atlas: `ui_dots_career_system`. `CareerPerkMissionBehavior` mission-level wiring at SubModule line 502+. |
| CharacterCreation | Mixed (Behavior+Model+Patch) | ✅ | ✅ | `Patch9_RaceFilter`, `Patch29_CCBodyProperties`, `Patch20_NarrativeHorseGuard` | 1 (`CharacterCreationRegistrationBehavior`) | 1 (`DotsCharacterStatsModel`) | `charactercreation/` | — | ✅ | ✅ | OK |
...
| Diplomacy | Behavior+Model+Patch | ✅ | ✅ | `Patch11_Diplomacy`, `Patch12_WarOfTheRing` | 2 (`DiplomacyBehavior`, `WarOfTheRingBehavior`) | 3 (`DotsAllianceModel`, `DotsKingdomDecisionPermissionModel`, `DotsDiplomacyModel`) | `diplomacy/` | War of the Ring | ✅ | ✅ | OK |
...
| Encyclopedia | Model | ❌ | ❌ | — | 0 | 1 (`DotsInformationRestrictionModel`) | — | Encyclopedia | ✅ | ✅ | No IoC; model constructed inline at SubModule line 301. |
...
| Execution | Patch+Model+Service | ✅ | ✅ | `Patch14_Execution` | 0 | 1 (`DotsExecutionRelationModel`) | `execution/` | — | ❌ | ✅ | **Doc gap** — `detect-docs-gaps.sh` already flags this. Need `docs/features/execution.md`. |
...
| InitialChildGeneration | Behavior | ✅ | ✅ | — | 1 (`DotsInitialChildGenerationBehavior`) | 0 | `configs/initial_child_generation.json` | — | ✅ | ✅ | OK |
...
| RaceAge | Behavior+Model | ✅ | ✅ | — | 1 (`RaceAgeBehavior`) | 3 (`DotsAgeModel`, `DotsPregnancyModel`, `DotsHeroCreationModel`) | `raceage/` | — | ✅ | ✅ | OK |
| RevoltTuning | Service (config feeding `DotsSettlementLoyaltyModel`) | ✅ | ✅ | — | 0 | 0 (consumed by `DotsSettlementLoyaltyModel` in CulturalFeats) | `configs/revolt_tuning_config.json` | — | ✅ | ✅ | Cross-feature: feeds CulturalFeats' `DotsSettlementLoyaltyModel`. Phase 2 cluster review. |
...
| Siege | Behavior+Model+Patch | ✅ | ✅ | `Patch8_SiegeCampGuard` | 1 (`SiegeDefenseBehavior`) | 1 (`DotsSiegeEventModel`) | `siege/` | Siege Defense | ✅ (multiple: siege.md, siege-defense.md, siege-trebuchets.md) | ✅ | OK; IoC class name is `SiegeDefenseIoC` (mismatch with dir name `Siege` — cosmetic). |
...
| TroopProgression | Model | ✅ | ✅ | — | 0 | 2 (`DotsPartyWageModel`, `DotsVolunteerModel`) | `troops/` | — | ✅ | ✅ | Also feeds `DotsCharacterStatsModel` (registered under CharacterCreation in matrix; data span overlaps). |
...
2. **CulturalFeats 17-model wave (SubModule.cs lines 276-292)** — confirm none of the 17 `Dots*Model` classes have constructor dependencies that should be IoC-resolved. The Messengers-class lesson: silent construction with `new Xxx()` masks missing service wiring.
