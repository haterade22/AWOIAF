...
| Researching a TaleWorlds API before editing | [ai-includes/taleworlds-research-guide.md](ai-includes/taleworlds-research-guide.md) + `pwsh tools/dots-src.ps1 path <Type>` |
...
- [race-age-system](features/race-age-system.md) — race-appropriate lifespans (elven immortality, dwarf/hobbit aging) via DotsAgeModel + DotsPregnancyModel
...
- [native-skin-fixes](features/native-skin-fixes.md) — managed wrapper for `DOTS.NativeSkinFixes.dll` (covers_head morph + hair/beard cloth sim)
...
- [adod-beasts-architecture-and-dots-port](reference/adod-beasts-architecture-and-dots-port.md) — **the whole ADOD_Beasts mod end-to-end** (lifecycle + the WHY) + line-by-line DOTS port comparison across all 4 subsystems; the 1.2.12→1.4.5 drift catalogue. Read this before re-decompiling ADOD.
- [troop-weight-system](features/troop-weight-system.md) — TroopWeightSettings, PartyBase / TroopRoster patches (Patch17)
- [battle-balance](features/battle-balance.md) — DotsMilitaryPowerModel, DotsCombatSimulationModel
...
- [custom-battles](features/custom-battles.md) — DOTS factions/commanders/troops in custom battles (Patch19)
...
- [volunteer-recruitment](features/volunteer-recruitment.md) — per-settlement / clan / culture recruitment pools (DotsVolunteerModel)
- [equip-presets](features/equip-presets.md) — save/load equipment preset overlay on inventory
- [lord-skills](features/lord-skills.md) — lore-driven SkillSets for every DOTS lord (~880 NPCs, 17 cultures, 35 archetypes); authoring guide [lord-skills-authoring.md](ai-includes/lord-skills-authoring.md)
...
- [tournament-armor-assignment](features/tournament-armor-assignment.md) — per-participant culture armor in DotsTournamentModel
...
- [siege-trebuchets](features/siege-trebuchets.md) — DotsSiegeEventModel: defender Trebuchet option
...
- [revolt-tuning](features/revolt-tuning.md) — JSON-tunable revolt soft-nerf, DotsSettlementLoyaltyModel
...
- [arena](features/arena.md) — DotsTournamentModel with culture armor + prize pools
...
- **`reference/engine/` — phased engine study** (19 phases, COMPLETE; one process traced end-to-end from the decompile, what/how/why). The arc: campaign heartbeat → object → encounter → mission → agent → render, plus every cross-cutting system + the integration meta-layer:
  ...
  - [monster-model](reference/engine/monster-model.md) (Phase 3 — the `monsters.xml` schema: bone-index map by name, IK/capsules/flags/usage, base-monster inheritance; what DOTS authors per creature)
  ...
  - [save-system](reference/engine/save-system.md) (Phase 6 — `SyncData` vs `SaveableTypeDefiner`+`[SaveableField]`; the base+localId collision gotcha; DOTS composite-string idiom)
  - [gamemodel-system](reference/engine/gamemodel-system.md) (Phase 7 — `GetModel<T>` last-added-wins; `AddModel` inheritance vs decorator; register-after-defaults + one-override-per-type; DOTS's ~40 overrides)
  ...
  - [mount-and-rider-runtime](reference/engine/mount-and-rider-runtime.md) (Phase 14 — mount = own `Agent`, rider = another, linked via `Agent.Mount` two-phase → seated at `RiderSitBoneIndex`; `IsMount`=`AgentFlag.Mountable`; the 3 DOTS seating modes: cavalry `Mount` / elephant `Mount`+howdah-StandingPoints / riderless spider)
  - [agent-stats-and-driven-properties](reference/engine/agent-stats-and-driven-properties.md) (Phase 15 — `AgentDrivenProperties` (~99 `DrivenProperty` float channels read by native sim per tick) filled by `AgentStatCalculateModel.Initialize/UpdateAgentStats`; DOTS's `DotsAgentStatCalculateModel : SandboxAgentStatCalculateModel` = career passives + elephant mount-lock consolidated in one slot; engine-scale + NaN gotchas)
  - [campaign-object-graph](reference/engine/campaign-object-graph.md) (Phase 16 — `Hero`∈`Clan`∈`Kingdom` (IFaction), `MobileParty`→`PartyBase`, `Settlement`(town/castle/village, `OwnerClan`); the graph every DOTS campaign behavior mutates; ADR-007 adapters, `?.`-chains, non-saved `Settlement.Culture`, castle `.Village==null`; **entirely managed, no native boundary**)
...
- [ai-includes/research-workflow.md](ai-includes/research-workflow.md) — `dots-src` + decompilation workflow
...
- **DOTS_Map live-vs-shadow trap** — CLAUDE.md "DOTS_Map settlements" + memory: `feedback_dots_map_live_vs_stale_shadow`, [reference/dots-map-settlement-naming.md](reference/dots-map-settlement-naming.md)
...
- [karpathy-autoresearch](research/karpathy-autoresearch.md) — full review of Karpathy's autoresearch repo (10 files, 52 patterns extracted) + Tier-1/2/3 adoption map for DOTS. Source: `docs/raw/ai-research/karpathy-autoresearch/`.
- [reference/external-resources.md](reference/external-resources.md) — verified external resources to improve DOTS (official-vs-community Bannerlord docs hierarchy, BUTR deps, Harmony, LOTR/Tolkien lore + naming generators, comparable total-conversion mods, save-compat versioning). Cite this instead of re-searching the web.
