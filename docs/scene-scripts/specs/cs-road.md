...
## Dependencies replaced by DOTS-owned helpers

- Alliance `EntityUtils.ColorFromHex` → DOTS `HexColorParser` (clean-room).
- Alliance `Logger` → DOTS logging (whatever pattern the rest of the codebase uses).
