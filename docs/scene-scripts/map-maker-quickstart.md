...
Open your launcher and switch to the editor profile. **All five of these mods must be enabled**, or the editor will crash on DOTS load:

- Bannerlord.Harmony
- Bannerlord.UIExtenderEx
- Bannerlord.MBOptionScreen
- Bannerlord.ButterLib
- DOTS (and DOTS_Map if you want the main map auto-opened)
...
## Troubleshooting — `DOTS:` lines in the editor log

If `Generate` does nothing, check the editor's log window for `DOTS:` lines (yellow = warning, white = info).

**Step 1 — confirm the click is reaching the script.** Look for a white line like:

```
DOTS: CS_Road on '<entity>': Generate button clicked.
DOTS: CS_Road on '<entity>': GenerateMesh start.
```
...
```
DOTS: CS_Road on '<entity>': generated mesh from path '<X>' (totalDistance=42.50m, 86 samples, 170 triangles, material='dirt_road_a').
```
...
When `CS_Road` runs `Generate`, it attaches a generated mesh named `dots_cs_road_generated` to the host entity. **If you delete the `CS_Road` script after saving, that mesh stays attached** as an orphan — the script is no longer there to clean it up.
...
- **After-the-fact cleanup:** find the entity, look at its component list, find the mesh named `dots_cs_road_generated`, and remove it manually.
