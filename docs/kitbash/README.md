...
Persistent reference documentation for the kitbash asset families in DOTS.
...
| Mirkwood (elven) | — | Not catalogued yet (referenced in `dots_mirkwood_kitbash.xml`) |
| Gondor | — | Not catalogued yet (referenced in `dots_gondor_kitbash.xml`) |
| Mordor | — | Not catalogued yet (referenced in `dots_mordor_kitbash.xml`) |

## Naming conventions (shared across all DOTS kits)
...
Kit prefixes used in DOTS: `dw` (dwarven — Erebor), `gd` (Gondor), `mordor`
