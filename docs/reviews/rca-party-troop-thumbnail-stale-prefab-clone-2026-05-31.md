...
| Stale binding | DOTS clones bound `ImageTypeCode="@ImageTypeCode"`; vanilla 1.4.5 binds `TextureProviderName="@TextureProviderName"` |
...
- **Re-synced to vanilla 1.4.5** (clones of still-shipped vanilla prefabs, replaced verbatim — no DOTS-custom content existed in them; the re-sync also picked up other 1.4.5 redesigns the stale clones had missed): `Party/PartyTroopTuple.xml`, `Party/PartyTroopTupleLeft.xml`, `Party/PartyTroopManagerPopUp/PartyTroopRecruitItem.xml`, `Party/PartyTroopManagerPopUp/PartyTroopUpgradeItem.xml`, `CustomBattle/CompositionSlider.xml`, `CustomBattle/TroopTypeSelectionPopUp.xml`.
- **`Party/PartyScreen.xml`** re-synced too (it carried other stale bindings — missing `ScrollToCharacter`/`IsScrollTargetPrisoner`/`ScrollCharacterId`, a stale `ScrollYOffset`, `TextWidget`→`RichTextWidget` on the formation dropdown), with DOTS's **5** deliberate cosmetic tweaks re-applied on top: `PartyNameLabel.Height=55`, both party-name labels `MarginTop=2`, wage-widget width 80, cost-icon width 40, wage-label `MarginLeft=40` (for T10 wages + longer LOTR party names). **Correction (post-review):** the original `098ede9` re-sync was a blunt whole-file vanilla copy that *dropped* these tweaks while the commit message wrongly claimed they were preserved; the adapted adversarial review (below) caught the code-vs-doc falsehood and the tweaks were restored. Now verified: PartyScreen diffs vanilla 1.4.5 by exactly those 5 tweaks.
- **Surgical binding rename** (DOTS-original prefabs, no vanilla counterpart; datasource is the vanilla `ImageIdentifierVM`, so XML-only): `MomentumView/MomentumView.xml` (×2), `MomentumView/KingdomIcon.xml`, `MomentumView/Relationship.xml`.
...
- **"Missing `fighter_sturgia` body property"** (dead). `fighter_sturgia` is defined in vanilla `SandBoxCore/ModuleData/sandboxcore_bodyproperties.xml`; DOTS depends on SandBoxCore (`LoadBeforeThis`) and no XSLT strips it. Decisively, DOTS's own CHANGELOG documents that troops on *wrong* body templates "rendered as generic Empire men in-game" — a body-template mismatch renders a **wrong-looking** troop, never a hang.
...
The decisive evidence was a `diff` of DOTS's `PartyTroopTuple.xml` against vanilla 1.4.5 at the exact stuck widget: vanilla `TextureProviderName`, DOTS `ImageTypeCode`.

## Blast-radius audit (workflow, adversarially verified)

A follow-up audit inventoried **every** DOTS GUI prefab against vanilla 1.4.5 (an independent verify agent corrected the first pass — occurrence count 12 not 24; one missed clone; a legend stat):

- **48** DOTS GUI prefab XML files; **32** are clones of vanilla 1.4.5 (by filename); **16** are DOTS-original. Only **4** clones are byte/whitespace-identical to vanilla.
...
- **Two audit "follow-up" items were FALSE POSITIVES — verified against vanilla and NOT changed:**
  - `EaseIn="true"` was left untouched (correct action), but BOTH the audit's framing AND a first "correction" got the *evidence* wrong via substring grep. Verified truth (attribute-name regex + decompile of `VisualDefinitionTemplate`): bare `EaseIn=` appears in **0** vanilla files (the "18×" was a substring miscount of `EaseType="EaseInOut"`/`IsEaseInOutEnabled`); the parser has no `EaseIn` property and silently ignores it — so `EaseIn="true"` on the 3 DOTS-original screens (`CareerScreen`, `CustomBattleScreen`, `PreBuildCharacterSelection`) is **inert dead markup**, harmless to leave, not "still used by vanilla."
  - The `AutoScroll*Offset` ↔ `ScrollYOffset` rename was stated **backwards**: `ScrollYOffset` is the stale form (0 in vanilla), `AutoScrollTopOffset/BottomOffset` is current (137× in vanilla). The original stale DOTS clones had `ScrollYOffset`; the re-sync in `098ede9` already adopted vanilla's `AutoScroll*Offset`, so there is no remaining drift.
- **Intentional DOTS redesigns — LEFT ALONE (not rename casualties):** the Encyclopedia banner-widget swaps (`EncyclopediaClanListElement` replacing vanilla's `EncyclopediaClanSubPageElement` across Clan/Faction/Settlement/Hero pages), the `SettlementNameplateItem{Large,Medium,Small}` diamond-layout redesign, and the `CharacterCreationCultureStage`/`CharacterCreationNarrativeStage` theming. These diverge from vanilla by design; re-syncing them would erase DOTS customization. The CustomBattle clones were fixed **surgically** (not re-synced) for the same reason — their other divergences are vanilla *additions* DOTS lacks, not breaks.
...
- **Doc-graph + methodology gaps:** the new artifacts didn't link the prior audit-fallibility memories, and the rule's vanilla counts didn't warn that the install's `Modules/*` includes the deployed DOTS module (count pollution). Both fixed in the rule caution + cross-links.
