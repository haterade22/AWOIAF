...
**ID-typo-without-canonical-grep** is the same shape as multiple prior bugs in DOTS history:
...
- **Agent 2 (Compatibility):** Verifies TaleWorlds API surface (`Hero.CurrentSettlement`, `Settlement.OwnerClan`, etc.). Item IDs and NPCCharacter IDs are DOTS/Armory content, not TaleWorlds API — the agent's prompt scoped item-ID checks to NEW items I authored (`lrd_horse_armour_4` etc.), not to EXISTING troop IDs I referenced. Could be extended: "Also verify any NPCCharacter ID referenced from C# code exists in `troops/*.xml`." This is a real prompt gap.
