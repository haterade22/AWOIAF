...
Issue #110 added these two cultures to `startup_resources_config.xml` based on the assumption that they were full peer kingdoms (which they are — see `dots_spkingdoms.xml`, 9 + 8 NPC lords). The gold/influence config for the NPC lords + clans is correct and useful at new-game start. The `playerGold` rows are correct in shape but unreachable in practice until narrative coverage lands.
...
For youth options specifically, also need matching equipment rosters in `Main/_Module/ModuleData/equipmentsets/dots_char_creation_equipment.xml` for every `(culture, title_type, gender)` combination — see #110 for the convention.
