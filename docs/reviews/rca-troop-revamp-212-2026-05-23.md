...
| # | Agent | Claim | Reality |
|---|-------|-------|---------|
| FP3 | Agent 5 | `gundabad_sword_warrior`, `gundabad_spear_warrior`, `gundabad_dread_rider_of_the_tower` deleted from `troops_gundabad.xml` but referenced in `dots_partyTemplates.xml` — load-time crash risk | NOT deleted. Exist at `troops_gundabad.xml:1265/2514/2730`. Agent inferred the deletion set wrong. Verified via `grep -c 'id="..."'`. |
