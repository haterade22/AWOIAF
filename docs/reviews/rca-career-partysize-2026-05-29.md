...
Both were fixed surgically (Issue A: `DotsPartySizeModel` → `ApplyFlat`; Issue B: parser reads the
...
parser/consumer's assumptions — it never round-tripped the actual shipped `dots_career_choices.xml`.

**Preventive enforcement implemented:** [`CareerChoicesIntegrationTests`](../../DOTS.Tests/Features/CareerSystem/CareerChoicesIntegrationTests.cs)
loads the REAL `dots_career_choices.xml` and asserts (a) it loads >100 choices, (b) every
...
inline in `DotsPartySizeModel` and in the feature doc; the deferred fix (data-driven routing via
