...
- `DOTS.Tests/Features/QuickActions/` -- 50 tests, all passing
...
1. **MultiSelectionInquiryData callback signatures** -- the constructor signature has both `affirmativeAction: Action<List<InquiryElement>>` and `negativeAction: Action<List<InquiryElement>>`. The DOTS service passes a callback for both. Verify callback shapes against `TaleWorlds.Core.MultiSelectionInquiryData` v1.3.15 ctor.
...
- `IQuickActionsSettingsProvider.cs` / `QuickActionsSettingsProvider.cs` -- 15 settings wrapped from DotsSettings
...
- `Hooks/InventorySearchCampaignBehavior.cs` -- SyncData("DOTS_IsInventorySearchAvailable") + RegisterEvents
...
- `DOTS.Tests/Features/QuickActions/QuickActionsServiceTests.cs` -- 34 tests (skip-guard exhaustion, threshold matrix, modifier preservation, audio, refresh, no-inventory degrade)
- `DOTS.Tests/Features/QuickActions/InventorySearchCampaignBehaviorTests.cs` -- 7 tests (default state, OnTick reconcile both directions, OnNewGameCreated seed both values, OnGameLoaded reconcile)
- `DOTS.Tests/Features/QuickActions/DamagedQualityPresetTests.cs` -- 9 tests (preset → threshold, dropdown index → preset, out-of-range fallback)

### Modified

- `Main/Features/DotsSettings.cs` -- 15 new MCM settings (GroupOrder 30/31/32) under "Inventory/Quick Actions"
...
**Scenario D: User loads a campaign saved BEFORE QuickActions was installed. SyncData has no "DOTS_IsInventorySearchAvailable" key. The bool field defaults to `true`. MCM EnableInventorySearch is `false`.**
