...
- Config/XML: `Main/_Module/ModuleData/career_system/*.xml`, `Main/_Module/GUI/DOTSSpriteData.xml`
- Tests: `DOTS.Tests/Features/CareerSystem/**`
...
Conclusion: managed code exposes `IsKeyPressed` as a distinct engine call from `IsKeyDown`; DOTS is using the edge-trigger API. Exact native lifetime is engine-side, not visible in decompiled C#.
...
- Careers use ability template IDs with `_ability`, e.g. `ranger_of_ithilien_ability` at `Main/_Module/ModuleData/career_system/dots_careers.xml:12`.
- Registered sprite names match that path shape, e.g. `CareerSystem\Abilities\ranger_of_ithilien_ability` at `Main/_Module/GUI/DOTSSpriteData.xml:7278` and `:10690`.
...
Evidence:
- `dots_career_choices.xml` contains 98 `property="MaxCharge"` mutations, e.g. `Main/_Module/ModuleData/career_system/dots_career_choices.xml:82`.
...
Vanilla `DefaultClanTierModel.GetCompanionLimit` does not accept null clans, so `DotsClanTierModel.GetCompanionLimit` also cannot safely support null because it calls `base.GetCompanionLimit(clan)` first at `Main/Features/CareerSystem/Models/DotsClanTierModel.cs:18`. I found no evidence this rework introduced a null-clan call path.
...
- `Main/_Module/ModuleData/career_system/dots_career_choices.xml:82` and 97 other `MaxCharge` mutations are now designer-facing dead config. `MutationService` still applies them, but readiness ignores them because every ability is constructed as `CooldownOnly` with `maxCharge: 0f`.
- `Main/Features/CareerSystem/Abilities/CareerAbility.cs:32` and `:54` still retain production-dead charge APIs (`AddCharge`, `SetMaxCharge`) and `DOTS.Tests/Features/CareerSystem/CareerAbilityTests.cs:13` still codifies charge-based behavior. This is not a runtime defect, but it keeps removed design semantics alive.
