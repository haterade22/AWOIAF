...
You are reviewing the DOTS.Dependencies runtime defensive infrastructure -- 11 classes in `Dependencies/Foundation/` plus `Dependencies/AliasStubSubModule.cs` and `Dependencies/SubModule.cs` (orchestrator). This is NOT a typical DOTS feature in `Main/Features/`. It's low-level Harmony-patching + reflection + exception-handling glue inspired by BetaDeps v0.7.5.1 (Nexus 11274), ported under MIT clean-room rewrite for Bannerlord v1.4.5.
...
- `Dependencies/_Module/SubModule.xml` (DOTS.Dependencies module manifest -- shows load order + which SubModules construct from this assembly)
- `Stubs/Bannerlord.Harmony/_Module/SubModule.xml` (canonical alias-stub example with the `<SubModuleClassType value="DOTS.Dependencies.AliasStubSubModule"/>` entry)
...
2. Stub modules deploy via MSBuild target at build time, not at runtime. The 4 alias stub folders (Modules/Bannerlord.Harmony/, etc.) have NO bin/ folder -- they reference DOTS.Dependencies.dll which lives only in DOTS.Dependencies/bin/. As confirmed in diag.log session 4, the launcher silently skips constructing AliasStubSubModule for this reason. Practical impact: IncompatibleModDetector.RunEarlyPhase was moved from the stub ctor to Dependencies/SubModule.OnSubModuleLoad (commit bc8f5c3). The AliasStubSubModule code remains as fallback if DLL deployment changes later. Do NOT flag the stub ctor as "dead code" -- it's deliberately kept.
...
The method refuses to unpatch owners whose Harmony ID starts with "DOTS" (case-insensitive). Question: is this the right filter? Third-party BUTR mods, MCM, ButterLib, etc. have Harmony IDs like "Bannerlord.ButterLib.Implementation", "MCM.Implementation.MCMv5", etc. -- those would be unpatched on the first MissingMethodException, potentially breaking the entire BUTR stack.
...
The `_enginePrefixes` array filters out engine/infrastructure assemblies from the stack walk to find the culprit. Read it. Then ask: is "DOTS" the right prefix to exclude? What about consumer mods that ARE DOTS-namespaced (e.g., `DOTS_Online`, `DOTS_Map`)? If a DOTS_Online assembly is the culprit, it'd get filtered out as "engine" and the attribution would fall through to "(unknown)".
...
Also verify: does `Harmony.GetAllPatchedMethods()` return DOTS's own Patch-Shield-Finalizer patches as patched methods? If so, the second pass would try to shield our own shield-finalizers. The DOTS-namespace filter should catch this -- verify it does.
...
|---|---------|-----|----------|---------------|
| S1 | PatchShield owner-filter "DOTS" prefix only | | | |
| S2 | SaveShield engine-prefix completeness vs DOTS_Online | | | |
...
1. EVERY suspect verdict must include a code-block citation from either DOTS source (with file:line) or a Bannerlord v1.4.5 decompile (paste the relevant method body).
...
5. NO speculation about "this might happen on Linux/Mac" -- DOTS ships Windows-only via Steam.
