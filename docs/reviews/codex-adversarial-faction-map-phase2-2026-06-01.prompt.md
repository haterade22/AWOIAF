...
The CharacterCreation "faction map" stage shows a per-faction sidebar page with Perks / Bonuses / Special Units / Strengths / Weaknesses. The pre-existing content was authored before DOTS's cultural-feats system existed and lied to the player about what the in-game effects were. This rewrite (issue #260) ships in three phases:
...
- **Phase 2 main** (commit `cbbcc41`): full content rewrite of all 16 playable factions in `factions.json`, every player-facing string wrapped in `{=KEY}default` with the convention `dots_faction_<faction_json_key>_<section>_<index>`. Non-playable factions (29 stubs) get name + description + traits keyed in place. 599 new keys harvested into `dots_module_strings.xml` via a new `tools/harvest_factionmap_strings.py` tool. 20 new tests in `FactionMapDataTests`.

- **Phase 2 fix** (commit `7f0de78`, post-deep-review): `FactionSelectionService.FormatDifficultyText` was a Phase 1 holdover with 7 hard-coded English strings — now wrapped in `{=dots_faction_difficulty_N}default`. 7 hand-authored `<string>` entries added to `dots_module_strings.xml` above the auto-harvested block.
...
- `Main/_Module/ModuleData/dots_module_strings.xml` — 599 new auto-harvested keys (in a marked block from `<!-- ===== FactionMap (CC) — auto-harvested ===== -->` to `<!-- ===== END FactionMap (CC) ===== -->`) + 7 hand-authored difficulty keys above the harvested block.
- `Main/Features/CulturalFeats/DotsCulturalFeats.cs` — the 97 FeatObject declarations. Source-of-truth for what each culture's feats actually do.
...
- `DOTS.Tests/Features/FactionMap/FactionMapDataTests.cs` — 20 new data tests (per-faction DataRow, key coverage gate).
...
1. **Content accuracy vs. shipped cultural feats.** For each of 16 playable factions, the Perks / Bonuses / Special Units / Strengths / Weaknesses sections should accurately reflect the `DotsCulturalFeats.cs` feats attached to that culture. Specifically:
...
2. **XSLT-wrapped culture feat inheritance.** Dale (sturgia), Khand (battania), Harad (aserai), Rhûn (khuzait), Dunland (empire), Rohan (vlandia) inherit vanilla DefaultCulturalFeats via the XSLT passthrough in `spcultures.xslt`. The new content should mention BOTH the inherited vanilla feats (player sees them in encyclopedia) AND the DOTS additions. Verify each:
   - Dunland: inherited Battanian +15% forest speed, +20% militia production, −15% construction speed; DOTS addition Hill Marchers (+10% plain), Hill-Tribe Levy (+5% party), Hill-Tribe Recruitment (+10% volunteer respawn).
   - Khand: inherited Battanian feats + DOTS Steppe Charioteers (+10% steppe).
   - Harad: inherited Aserai (caravan + desert hardiness) + DOTS Sons of the Sun (+10% desert), Haradrim Warbands (+5% party).
   - Rhûn: inherited Khuzait + DOTS Easterling Outriders (+10% steppe), Easterling Host (+5% party).
   - Dale: inherited Sturgian + DOTS Vale Traders (+10% plain).
   - Rohan: DOTS-only (vlandia inherited mounted feats overridden by DOTS rewrites).
   - Decompile `pwsh tools/dots-src.ps1 path TaleWorlds.CampaignSystem.GameComponents.DefaultCulturalFeats` to verify what the vanilla cultures actually ship.
...
3. **Special-units accuracy.** Each `special_units[]` entry's `name` should reference an actual elite troop in `Main/_Module/ModuleData/troops/troops_<culture>.xml`. Names used:
   ...
   These are lore-appropriate generic names. Grep the matching `troops_<culture>.xml` for each. If a troop name is fabricated and not in any DOTS troop tree, flag as MEDIUM (cosmetic — the UI shows the name as a hint, not an in-game ref — but the player expects to encounter the unit).

4. **JSON ↔ XML key alignment.** For each `{=key}default` token in `factions.json`, verify a matching `<string id="key" text="{=key}default"/>` entry exists in `dots_module_strings.xml`. The test `FactionMapDataTests.EveryFactionMapKey_HasMatchingStringInDotsModuleStrings` enforces this, but verify a sample of 10 keys end-to-end:
   - `dots_faction_stewardship_of_gondor_name`
   - `dots_faction_stewardship_of_gondor_perk_2_desc`
   - `dots_faction_dominion_of_mordor_bonus_4` (the night speed bonus)
   - `dots_faction_dominion_of_isengard_bonus_7` (Isengard 20-notable hub)
   - `dots_faction_overlordship_of_dol_guldur_perk_2_name` (Shadow Captains)
   - `dots_faction_kingdom_of_rohan_bonus_6` (Cavalry Dependent negative)
   - `dots_faction_difficulty_5` (hand-authored difficulty)
   - `dots_faction_havens_of_umbar_special_unit_0_name`
   - `dots_faction_kingdom_of_imladris_weakness_2`
   - `dots_faction_clans_of_dunland_strength_3`

5. **Key naming convention consistency.** The harvester uses keys of the form `dots_faction_<faction_json_key>_<section>_<index>` (or with sub-fields like `_perk_<n>_name` / `_perk_<n>_desc`). Verify no off-by-one underscores, no plural-vs-singular slippage (e.g. `_strengths_0` vs `_strength_0`), no inconsistencies between sibling factions. Compare 3-4 factions side-by-side.
...
8. **Old hard-coded content fully removed.** The old Gondor page used "Dunedain Blood / Lords gain experience 10% faster", "Defense Boost", "Varies / Elite Units in Specific Regions". Confirm zero occurrences of these legacy strings in factions.json, dots_module_strings.xml, or any C# file (they should all be gone).
...
- `Main/_Module/ModuleData/dots_module_strings.xml` — +599 auto-harvested + 7 hand-authored difficulty keys
- `Main/Features/CulturalFeats/DotsCulturalFeats.cs` — source-of-truth for the 97 feats the content should reflect

### Tests

- `DOTS.Tests/Features/FactionMap/FactionMapDataTests.cs` (new, 20 tests)
- `DOTS.Tests/Features/FactionMap/FactionDisplayHelperTests.cs` (Phase 1, 6 tests)
- `DOTS.Tests/Features/FactionMap/FactionSelectionServiceTests.cs` (FormatDifficultyText test updated)
...
Use `pwsh tools/dots-src.ps1 path <Type>` against the installed v1.4.5 DLLs at `E:\Steam\steamapps\common\Mount & Blade II Bannerlord\bin\Win64_Shipping_Client\`. Paste these:
