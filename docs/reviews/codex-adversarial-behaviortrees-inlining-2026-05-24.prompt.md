...
You are reviewing a DOTS commit that does THREE things in one session:

1. **Decompiled two vendored DLLs** (`Main/_Module/bin/Win64_Shipping_Client/BehaviorTreeWrapper.dll` ~1300 LOC and `BehaviorTrees.dll` ~980 LOC) and **inlined the source into DOTS.dll**. Both vendored binaries are deleted. The libraries had no upstream source repo; this rebuild gives DOTS full ownership for the first time.
...
- `~/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/feedback_missionbehaviortype_logic_requires_missionlogic_inheritance.md` -- the rule that should have caught this earlier.
...
**Library — new DOTS-owned source (decompiled from former vendored DLLs):**
...
```
DOTS.Tests/BehaviorTreeWrapper/BehaviorTreeMissionLogicInheritanceTests.cs
```
...
```
Main/DOTS.csproj  -- dropped <Reference Include="BehaviorTrees"> and <Reference Include="BehaviorTreeWrapper">
```
