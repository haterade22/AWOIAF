...
This is the "the math is right, but the data is wrong" failure mode. Adjacent patterns in DOTS history:
- Codex review (DotsSmithingModel, 2026-05) caught int truncation before career passive — `(int)(baseCost * (1f + factor))` rounded prematurely. Same root: a type narrowing inside math.
...
| **Agent 2 (Compatibility)** | Compatibility checks v1.3.15 API signatures — confirmed `Hero.Age` is float. But it didn't ask "does the patch CALLER preserve the float type when passing it onward." Compatibility was scoped to vanilla API matching, not to DOTS-internal type flow. |
...
| **Agent 4 (Completeness)** | Completeness verified tests exist + CHANGELOG entries + RCA presence. It didn't analyze whether tests would actually CATCH the bug — only that test files existed. The pre-existing `DotsPregnancyModelTests` had no fractional-age coverage because all test inputs used int literals. |
