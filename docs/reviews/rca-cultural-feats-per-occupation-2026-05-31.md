...
dotnet test  DOTS.Tests --filter CulturalFeats     -> 192 / 0 / 0  (was 191 before; +1 confirms new test ran)
dotnet test  DOTS.Tests (full)                     -> 2772 / 0 / 2  (was 2771 before)
python tools/validate_moduledata.py                -> CLEAN
```

## Files changed in this RCA fix

- `DOTS.Tests/Features/CulturalFeats/CulturalFeatsServiceTests.cs` — added `ApplyNotableCountFeat_DolGuldurArtisan_AddsOne` between `_DolGuldurGangLeader_AddsThirteen` and `_MordorGangLeader_AddsTwo`.
