...
Two related UX changes to the DOTS CareerSystem:
...
- DOTS rules pertinent to this changeset: `.claude/rules/csharp-architecture.md`, `.claude/rules/gui-ui.md`, `.claude/rules/csharp-patterns.md`.
...
- `Main/_Module/ModuleData/dots_module_strings.xml` -- 8 new keys: `dots_career_choice_while_active`, `_passive_tooltip`, `_keystone_tooltip`, `dots_career_switch_title`, `_choose`, `_subtitle`, `dots_career_switch`, `dots_career_switched_open_screen`.

## Files in scope (tests)

- `DOTS.Tests/Features/CareerSystem/CareerRegistryTests.cs` -- +5 `GetEligibleSwitchTargets_*` tests.
- `DOTS.Tests/Features/CareerSystem/CareerSwitchServiceTests.cs` -- +2 same-career tests.
- `DOTS.Tests/Features/CareerSystem/CareerScreenVMTests.cs` -- +5 switch-mode tests via new `CreateSwitchModeVM` helper.
...
3. **`hero_main_options` dialogue token** -- grep `E:\Decompiled_Bannerlord` for vanilla `AddPlayerLine(..., "hero_main_options", ...)`. List the canonical vanilla NPC-options branches under this token. Verify DOTS's `career_switch_start` doesn't collide with a vanilla id.
...
For each scenario, read DOTS source and answer with file:line:
...
4. **The dialogue text "Very well -- let us discuss what paths lie before you" is referenced as `{=dots_career_switched_open_screen}...`.** Verify the loc key exists in `dots_module_strings.xml` and that the file is registered as a `GameText` source in `Main/SubModule.xml`.
...
1. **All `{=dots_...}` references in the changed files cross-checked against `dots_module_strings.xml`.** Build the list of every `{=` in changed `.cs` + the prefab, then verify each has a `<string id=...>` entry. Flag orphans.

2. **`hero_main_options` outputToken** -- verify DOTS's `career_switch_response` chain (`career_switch_start -> career_switch_response`) doesn't collide with another DOTS behavior that registers `career_switch_response` separately.
