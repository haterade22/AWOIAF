...
Port the external developer's CompanionTactics module (#7 of 7 dropped at `Downloads/Features_fixed/`) into DOTS's adapter / service / IoC pattern. Three independently-toggleable sub-features under one DOTS module:
...
- SaveableTypeDefiner BaseId `726900601` (matches the original mod for save-import compat — first SaveableTypeDefiner in DOTS; CareerSystem deliberately avoided this pattern via primitive SyncData).
...
- `DOTS.Tests/Features/CompanionTactics/` — 6 test files / 74 tests
...
1. **`Main/DOTS.csproj`** — remove `<Compile Remove="Features\CompanionTactics\**\*.cs" />`
2. **`DOTS.Tests/DOTS.Tests.csproj`** — remove the same line
3. **`Main/IoC.cs`** — uncomment the `using DOTS.Features.CompanionTactics;` directive and the `CompanionTacticsIoC.RegisterCompanionTacticsFeature(container);` call
...
5. **`Main/Features/DotsSettings.cs`** — append the 10 MCM settings at GroupOrder 27/28/29 (snippet in `docs/features/companion-tactics.md`)
...
10. Run `dotnet build Main/DOTS.csproj -p:DisableModuleCopy=true` and `dotnet test DOTS.Tests --filter "FullyQualifiedName~CompanionTactics"` to confirm the 74 tests pass.
