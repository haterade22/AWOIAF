# Codex Adversarial Review: DOTS.Dependencies/Foundation (DR3 Phase 4)
...
|---|---|---:|---:|---|
| S1 | PatchShield owner-filter `DOTS` prefix only | CONFIRMED | HIGH | Protect vendored infrastructure Harmony owners before unpatching. |
| S2 | SaveShield engine-prefix completeness vs `DOTS_Online` | CONFIRMED | MEDIUM | Replace broad `DOTS` prefix with exact DOTS-owned checks and add missing infrastructure assembly prefixes. |
...
[HIGH] `Dependencies/Foundation/PatchShield.cs:248` — Owner Protection — `TryUnpatchOffendingPatches` protects only Harmony owners beginning with `DOTS`, so the first shielded `MissingMethodException` on a method patched by vendored BUTR/MCM infrastructure can unpatch that infrastructure owner — Add an explicit protected-owner allowlist for DOTS plus vendored infrastructure owner prefixes.
...
if (owner.StartsWith("DOTS", StringComparison.OrdinalIgnoreCase))
{
    DiagLog.Log(Tag, $"refusing to unpatch DOTS-owned owner '{owner}' on {targetKey}");
    continue;
}
...
Vendored infrastructure Harmony owners do not start with `DOTS`:
...
[HIGH] `Dependencies/Foundation/SubModuleConstructionGuard.cs:177` — Exception Attribution — The `Module.AddSubModule` path attributes constructor failures via `ex.TargetSite`, which points at the method that threw, not necessarily the offending submodule constructor. A DOTS constructor that throws inside TaleWorlds code can be swallowed because the guard sees the TaleWorlds throwing method instead of the DOTS submodule — Resolve the intended submodule type from `subModuleInfo` and `subModuleAssembly` before falling back to stack walking.
...
if (asmName.StartsWith("DOTS", StringComparison.OrdinalIgnoreCase))
{
    return __exception;
}
```
...
In the scenario `FooMod.SubModule.ctor -> MBObjectManager.GetObject<T>("missing") -> throw`, `ex.TargetSite` is the TaleWorlds method that throws, while the real culprit is still `FooMod.SubModule`. The current DOTS guard therefore protects the wrong assembly.
...
[MEDIUM] `Dependencies/Foundation/SaveShield.cs:98` — Attribution Filter — `_enginePrefixes` uses the broad prefix `DOTS`, so a third-party consumer assembly named `DOTS_Online` or `DOTS_Map` is treated as engine/infrastructure and skipped during culprit attribution. The same list also misses bundled infrastructure assemblies such as `Bannerlord.MBOptionScreen`, `Bannerlord.ModuleLoader.Bannerlord.MBOptionScreen`, `MCM.UI.Adapter.MCMv5`, and `BUTR.CrashReport` — Replace the broad DOTS prefix with exact DOTS-owned checks and add missing infrastructure prefixes.
...
private static readonly string[] _enginePrefixes = new[]
{
    ...
    "DOTS", "Bannerlord.Harmony", "Bannerlord.UIExtenderEx",
...
{
    if (string.Equals(asmName, "DOTS", StringComparison.OrdinalIgnoreCase) ||
        string.Equals(asmName, "DOTS.Dependencies", StringComparison.OrdinalIgnoreCase) ||
        asmName.StartsWith("DOTS.", StringComparison.OrdinalIgnoreCase))
        return true;
...
DOTS source protects only `DOTS` owners:

```csharp
// Dependencies/Foundation/PatchShield.cs:245-258
if (owner == HarmonyId) continue;
if (owner.StartsWith("DOTS", StringComparison.OrdinalIgnoreCase)) continue;
PatchSafe(originalMethod, owner);
```
...
The broad `DOTS` prefix filters unrelated `DOTS_*` consumer assemblies:
...
Conclusion: too aggressive for `DOTS_Online`/`DOTS_Map`, while still incomplete for bundled BUTR/MCM assemblies. See MEDIUM finding above.

### S3: VersionProbe Null-Arg FromParametersFile

Verdict: DISPUTED, INFO.

DOTS invokes the one-arg overload with `null`:
...
DOTS strips XML comments before matching `<Id>`:
...
<SubModuleClassType value="DOTS.Dependencies.AliasStubSubModule"/>
```
