...
- DOTS's v1 `as_elf_facegen`: 14 actions, works only for parent menu. ✗ Counter-example.
- DOTS's v2 `as_elf_facegen`: ~106 actions matching dwarf verbatim. ✓ Works.
...
- **Phase-1 Explore agent (2026-05-22, finding #1):** ✅ caught the elf hole. Audited every DOTS `race=` attribute against LOTRLOME's facegen list and reported `elf` as the only missing entry. Specifically called out: "no `as_elf_facegen` ... the engine falls back ... contorted-mesh bug visible on Mirkwood / Rivendell."
...
- **No DOTS C# changes** — the fix is pure XML, no Harmony, no GameModel, no SubModule edits. `Patch20_NarrativeHorseGuard`'s race-sync prefix was correct all along; it just needed the engine's lookup target (`as_elf_facegen`) to actually exist with a complete action surface.
...
- **No GitHub issue** — XML-only fix lives entirely in another mod's `ModuleData`; no DOTS-feature change to ticket. Documented via CHANGELOG + this RCA + memory.
...
| # | Sev | Bug | Category | Why missed | Preventive action |
|---|-----|-----|----------|------------|-------------------|
| 1 | HIGH | Vanilla age-30 CC option plays a broken animation on the human_skeleton chain (all DOTS races affected) | TaleWorlds engine bug | The bug shipped in vanilla v1.3.15 itself; not introduced by DOTS or LOTRLOME. Caught only at the user's first traversal of the Starting Age menu. | **Memory addendum v3** in `feedback_lotrlome_action_set_aliases.md`: when all races break at the same CC stage despite identical action_set data, the bug is at the engine/anim binding layer and a DOTS-side Harmony override is the only fix. Pre-flight audit checklist: (a) is the action_set declaration present + correct? (b) is the action type registered in `Native/action_types.xml`? (c) does the anim file render correctly on the relevant skeleton (test in-game)? |
...
- **User in-game test of all races:** ✅ confirmed global scope (not orc-specific), which de-scoped data fixes and confirmed a DOTS-side patch was needed.
