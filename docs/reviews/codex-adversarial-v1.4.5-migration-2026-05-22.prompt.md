...
You are reviewing the DOTS (Dawn of the Stag) v1.3.15 → v1.4.5 Bannerlord migration changeset on branch `bannerlord-1.4.5`. Claude has already run a 5-agent `/deep-review` and caught 1 CRITICAL (silent reflection no-op) + 2 doc inconsistencies. Your job is to find what those 5 agents missed.

The migration scope deliberately did NOT redesign anything — it just brought DOTS up to v1.4.5 API compat. **Anything you flag should be a real bug or a real risk, not a style/polish concern.** Be adversarial.

## DOTS ID CHEATSHEET (for any config-reference checks)
...
- `Main/Features/CulturalFeats/Models/DotsBattleRewardModel.cs` — `CalculateRenownGain` signature: 3-param → 5-param (added `renownMultiplierForWinnerSide`, `includeDescriptions`).
- `Main/Features/Diplomacy/Models/DotsAllianceModel.cs` — `GetScoreOfStartingAlliance` signature: dropped `IFaction evaluatingFaction` param (was added in v1.4.0 fix, removed in v1.4.5).
...
- `tools/dots-src.ps1` — auto-detect version from `Version.xml`
...
### DOTS.Dependencies restoration (1,444 files)
...
Also confirm: in v1.3.15 the old code picked one Equipment from a list of MBEquipmentRoster via `GetRandomElementInefficiently().GetRandomCivilianEquipment()`. Engine-level filtering by gender + culture was NOT done before — DOTS picked any roster. In v1.4.5 the engine filters internally. Is there any scenario where v1.3.15 produced a child with non-gender-appropriate equipment (because the random roster wasn't gender-filtered) but v1.4.5 won't? Acceptable behavioral change OR latent bug?

### Suspect 2: DotsBattleRewardModel multiplier scaling

The fix added `renownMultiplierForWinnerSide` as the 4th param, passed to `base.CalculateRenownGain`. Vanilla `DefaultBattleRewardModel.CalculateRenownGain` constructs `new ExplainedNumber(contributionShare * renownValue * renownMultiplier)` — the multiplier is baked into the base value. DOTS's `ApplyRenownFeats` and `ApplyFactor` then call `.AddFactor(...)` on that base.

Hypothesis: DOTS feats and career passives now scale proportionally with `renownMultiplierForWinnerSide`. This is consistent with vanilla perk scaling.

**Verify:** Decompile `DefaultBattleRewardModel.CalculateRenownGain` in v1.4.5. Confirm the multiplier is in the initial ExplainedNumber value, not applied at the end via AddFactor. If it's at the end, then DOTS feats would NOT scale with it — that's a behavioral change.

Also: was there an `*ImplicitMultiplier` in v1.3.15's `CalculateRenownGain(party, value, share)` that DOTS was relying on? Decompile v1.3.15's signature from `E:\BannerlordBackup\1.3.15\bin\Win64_Shipping_Client\TaleWorlds.CampaignSystem.dll`. Compare.

### Suspect 3: DotsAllianceModel — does CanMakeAlliance bypass MaxNumberOfAlliances?

DOTS's `MaxNumberOfAlliances => int.MaxValue` was the old escape hatch to allow unlimited LOTR-faction alliances. In v1.4.5, `DefaultAllianceModel` added 5 new methods including `CanMakeAlliance` (per the Claude API-diff agent), which has its own score-threshold + player-support gates that may independently veto an alliance even when DOTS's MaxNumberOfAlliances is unbounded.

Hypothesis: DOTS's `GetScoreOfStartingAlliance` modifier of "lore alignment" (`+modifier`) flows into `CanMakeAlliance`'s score check, so LOTR-aligned pairs with high scores still pass. But LOTR-misaligned pairs that DOTS previously allowed via the unlimited cap may now fail the 50f threshold.
...
- Faction pairs where DOTS's `IDiplomacyService.GetAllianceScoreModifier` returns < 50f cumulative will be VETOED in v1.4.5.
- This may BREAK lore-friendly alliances DOTS previously allowed (e.g., Rohan-Gondor at low base score plus DOTS's modifier might not reach 50f).

Also: the `using TaleWorlds.CampaignSystem;` import on line 1 of DotsAllianceModel.cs — is it still needed after dropping the IFaction param? (The Claude perf agent said yes because `CampaignTime` lives there. Verify.)

### Suspect 4: SpecialResourcesBehavior — `HideoutBattleEndState.Retreated` permissiveness

The new 3rd param `HideoutBattleEndState` has 5 values: `None, Retreated, Defeated, Victory, SendTroops`. DOTS ignores the param and earns the resource on any `winnerSide == Attacker && IsPlayerMapEvent`.

Hypothesis (from Claude data-flow agent): `Retreated` fires when `winnerSide=Attacker && !HasWinner` — an abandoned-field case where the player won't actually claim hideout loot. DOTS still awards the special resource in that case, which may be unintended.
...
Bonus: in v1.3.15 the 2-param event always fired with `winnerSide=Attacker` ONLY on clean victory. If v1.4.5 expands the conditions under which Attacker fires, DOTS's permissive code may be earning resources in cases v1.3.15 never did. Decompile v1.3.15's `HideoutEventComponent` from the backup to compare.

### Suspect 5: Other reflection sites with the same bug class as IsFemale

The deep-review caught `ChildCreatorAdapter.cs:30` reflecting on `BasicCharacterObject.<IsFemale>k__BackingField` — silent no-op because `CharacterObject.IsFemale` override unconditionally reads `HeroObject.IsFemale`. Grep returned 2 other files using `*FieldValue.*BackingField` patterns in DOTS:
...
DOTS ships `civilian="true"` on 3,372 `<EquipmentSet>` elements across 51 XML files (S5a will migrate). Until then, vanilla v1.4.5 will:
...
DOTS's `Main/_Module/SubModule.xml` declares Native dep as `e1.4.5.*` (BUTR schema). The `.*` is a patch-version wildcard.
...
If too restrictive, DOTS may refuse to load on Bannerlord v1.4.6 or v1.5.x. If too loose, DOTS may load on v1.4.0–v1.4.4 and crash. Check the BUTR.Bannerlord.ButterLib loader logic if accessible.

## REQUIRED SECTIONS

### VANILLA CODE

For each of Suspects 1-4 above, paste the decompiled v1.4.5 source as a fenced code block alongside DOTS's calling code. If you only paste DOTS source without vanilla, the finding is unverified.
...
- **Suspect 2:** A battle where vanilla `renownMultiplierForWinnerSide = 0.5` (e.g., a Pyrrhic victory). DOTS's Umbar renown feat = +20%. What's the final number? Confirm against vanilla's perk-bonus math.
- **Suspect 3:** Gondor-Mordor alliance attempt. DOTS's diplomacy service returns -10000f (hostile pair). What does `CanMakeAlliance` return? Confirmed VETOED?
- **Suspect 4:** Player attacks bandit hideout. Mid-fight, half of bandits retreat. `winnerSide` resolves to... what? Does DOTS earn the resource for this messy outcome?
...
- [ ] All findings cite specific file:line in DOTS code
...
**FAILURES TO AVOID:**
- Codex once assumed `empire=Rohan` (it's Dunland in DOTS)
...
| 2 — DotsBattleRewardModel multiplier | TBD | TBD |
| 3 — DotsAllianceModel CanMakeAlliance veto | TBD | TBD |
