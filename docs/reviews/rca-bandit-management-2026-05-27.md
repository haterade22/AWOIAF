...
**Issue:** [#247](https://github.com/haterade22/DOTS/issues/247)
...
|---|---|---|
| Standards | PASS | 0 |
| Bannerlord API Compatibility | PASS | 0 (all v1.4.5 APIs verified via `dots-src`) |
| Efficiency | PASS | 0 |
...
|---|---|---|---|---|---|
| 1 | MEDIUM | `MinPartiesToInfest` declared in `BanditScalingConfig.cs`, loaded + validated in `BanditScalingConfigProvider.Validate`, then never consumed by any downstream class (interface, service, GameModel, patch). Pure dead config field. | Data flow gap — DTO populated but no consumer | Authored the POCO + provider + validation first, then forgot to extend `IBanditScalingSettingsProvider` + `IBanditScalingService` + `DotsBanditDensityModel` to actually read the field. Trust-the-test-suite blind spot: 14 tests covered the provider's validation behavior but no test exercised the "is this field ever READ by a consumer?" question — because it's a cross-class observation, not a within-class assertion. | **Fixed in same session.** Extended `IBanditScalingSettingsProvider` + `IBanditScalingService` + `DotsBanditDensityModel` to override `NumberOfMinimumBanditPartiesInAHideoutToInfestIt`. Added 1 unit test for the delegation chain. |
...
- `dotnet build Main/DOTS.csproj` — 0 errors.
...
`MinPartiesToInfest` now flows: JSON → `BanditScalingConfig` → `BanditScalingConfigProvider` (validate) → `BanditScalingSettingsProvider` (live-clamp to `MaxPartiesPerHideoutCap` to preserve `min ≤ max` invariant at runtime) → `BanditScalingService.MinPartiesToInfest` → `DotsBanditDensityModel.NumberOfMinimumBanditPartiesInAHideoutToInfestIt`.
...
New memory: **`feedback_cross_module_data_dependency_declaration.md`** — when one DOTS-managed module's XML references entities defined in another DOTS-managed module, the consuming module's `SubModule.xml` MUST declare a `<DependedModule>` on the producer. The launcher does not infer this from XML cross-references. (Will write after closeout.)
