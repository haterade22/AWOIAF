...
- Introduces a DOTS-owned enum `NotableOccupationKind { Other, Merchant, GangLeader, Artisan, RuralNotable, Headman }`.
...
- Registers each new NPC in BOTH layers: `<NPCCharacter id="...">` in `npcs_<culture>.xml` AND `<template name="NPCCharacter...." />` in the culture's `<notable_templates>` block in `dots_spcultures.xml`. (Two-layer registration was the systemic lesson from the 582275f RCA.)
...
3. **Mapping of `Occupation` values to `NotableOccupationKind.Other`.** `MapOccupation` in `DotsNotableSpawnModel.cs` switches on `Occupation` and maps `Merchant`/`Artisan`/`GangLeader`/`RuralNotable`/`Headman` to the matching DOTS enum, and everything else to `Other`. Does v1.4.5 vanilla `DefaultNotableSpawnModel.GetTargetNotableCountForSettlement` return a non-zero target for any other `Occupation` value (e.g., `Preacher`, `Mercenary`, `Lord`)? If yes, mapping to `Other` (which returns `baseCount` unchanged) is still correct (no override, vanilla wins). If we accidentally mapped one of those to Merchant/Artisan/etc., that would be a bug — verify the switch handles only the 5 spawn-pool occupations.
...
5. **Feat string-ID consistency.** Confirm that the 9 string IDs registered in `DotsCulturalFeats.RegisterAll` (e.g., `dots_isengard_notable_count_town_merchant`) exactly match the 9 `<feat id="...">` lines in `dots_spcultures.xml` under the four cultures' `<cultural_feats>` blocks. Off-by-one underscores or pluralization typos here = the feat never fires.

6. **Old uniform feats completely removed.** Grep the entire repo for the four deleted feat IDs (`dots_isengard_notable_count_town`, `dots_dolguldur_notable_count_town`, `dots_mordor_notable_count_town`, `dots_gundabad_notable_count_town` — without per-occupation suffix). Any leftover reference is dead config (slow burn — looks fine on day 1, fails when someone tries to reuse the ID).
...
7. **Two-layer registration for the 17 new NPCs.** For each of `spc_notable_isengard_gl5..gl12` (8) and `spc_notable_dolguldur_gl5..gl13` (9):
   ...
   - The `<template name="NPCCharacter.<id>" />` entry is in the matching culture's `<notable_templates>` block in `Main/_Module/ModuleData/dots_spcultures.xml`.
...
- `Main/Features/CulturalFeats/Models/DotsNotableSpawnModel.cs` (GameModel override; adds `MapOccupation` boundary helper)
- `Main/Features/CulturalFeats/DotsCulturalFeats.cs` (−4 fields/accessors/Register/Initialize/yields, +9 fields/accessors/Register/Initialize/yields)

### XML (in scope)

- `Main/_Module/ModuleData/dots_spcultures.xml` — 4 cultures' `<cultural_feats>` blocks rewritten (−4 lines / +9 lines), `<notable_templates>` blocks for Isengard (+8 GL templates) and Dol Guldur (+9 GL templates).
...
- `DOTS.Tests/Features/CulturalFeats/CulturalFeatsServiceTests.cs` — rewrites the 8 old uniform-multiplier tests as 13 per-occupation dispatch tests.
- `DOTS.Tests/Features/CulturalFeats/DotsCulturalFeatsDefinitionTests.cs` — bumps total feat count 92 → 97, per-culture expected counts updated.
...
Walk each of the 9 new `FeatObject.Initialize(...)` calls in `DotsCulturalFeats.cs` and:
...
For the 9 new feats:
- C# `Register("dots_isengard_notable_count_town_merchant")` ↔ XML `<feat id="dots_isengard_notable_count_town_merchant" />`
- C# `Register("dots_isengard_notable_count_town_artisan")` ↔ XML `<feat id="dots_isengard_notable_count_town_artisan" />`
...
- `grep -r 'dots_isengard_notable_count_town"' .` (note the trailing quote — to exclude per-occupation IDs)
