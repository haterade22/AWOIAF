...
The MCM settings provider references `DotsSettings` properties that are absent from the current `DotsSettings.cs`.
...
| 6 | Overlay z-order 1000 vs vanilla and DOTS collisions | CONFIRM, with caveat | `Patch33_GauntletInventoryScreen.cs:31,61` uses z-order 1000. Vanilla `SandBox.GauntletUI.GauntletInventoryScreen.OnInitialize` creates `new GauntletLayer("InventoryScreen", 15, true)`. DOTS grep found other z-orders: 1, 50, 100, 200, 206; no 1000 collision. Caveat: `SubModule.cs:381-384` does not patch `Patch33_EquipPresets`, so the valid layer code never runs in current source. | Keep z-order 1000. Restore Patch33 wiring in `SubModule.cs`. |
...
File(s)+line(s): `Main/Features/EquipPresets/EquipPresetsSettingsProvider.cs:9,22,28`; `Main/Features/DotsSettings.cs:237-314`; `Main/DOTS.csproj:63-80`

Vanilla API evidence: not a TaleWorlds behavior claim. Source evidence is sufficient: `DOTS.csproj` does not exclude `Features\EquipPresets\**`, and no `DotsSettings` declarations exist for `EnableEquipmentPresets`, `MaxPresetsPerCharacter`, or `EquipPresetsDebug`.

Why it matters: `EquipPresetsSettingsProvider` dereferences three properties that are absent from the only `DotsSettings` class in the source tree. If these files are part of the build, Roslyn must fail name resolution. If build/test is green elsewhere, this workspace is not the green source tree described in the prompt.

Recommended fix: add the three MCM settings exactly as planned under `Inventory/Equipment Presets` with `GroupOrder = 33`, then add a test that reflects `DotsSettings` and asserts those properties exist.
...
### Z-order 1000 does not collide with current DOTS layers
