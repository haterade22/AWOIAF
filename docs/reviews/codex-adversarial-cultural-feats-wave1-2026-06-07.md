...
|---|---|---|
| `dots_mordor_smithing` | `ApplySmithingFeats` | `DotsSmithingModel` |
| `dots_erebor_tariff_income` | `ApplyTariffIncomeFeats` | `DotsClanFinanceModel` |
| `dots_umbar_raid_damage` | `ApplyRaidDamageFeats` | `DotsRaidModel` |
| `dots_umbar_food_consumption` | `ApplyFoodConsumptionFeats` | `DotsFoodConsumptionModel` |
| `dots_lothlorien_volunteer_rate` | `ApplyVolunteerRespawnFeats` | `DotsVolunteerModel` |
| `dots_mirkwood_army_influence_cost` | `ApplyArmyInfluenceCost` | `DotsArmyManagementModel` |
| `dots_goblin_smithing` | `ApplySmithingFeats` | `DotsSmithingModel` |
| `dots_goblin_raid_damage` | `ApplyRaidDamageFeats` | `DotsRaidModel` |
| `dots_mistymountainorcs_smithing` | `ApplySmithingFeats` | `DotsSmithingModel` |
| `dots_mistymountainorcs_raid_damage` | `ApplyRaidDamageFeats` | `DotsRaidModel` |
| `dots_mistymountainorcs_construction_speed` | `ApplyConstructionSpeedFeats` | `DotsBuildingConstructionModel` |
| `dots_dale_tariff_income` | `ApplyTariffIncomeFeats` | `DotsClanFinanceModel` |
| `dots_dale_renown` | `ApplyRenownFeats` | `DotsBattleRewardModel` |
| `dots_dale_loyalty` | `ApplyLoyaltyFeats` | `DotsSettlementLoyaltyModel` |
| `dots_khand_renown` | `ApplyRenownFeats` | `DotsBattleRewardModel` |
| `dots_khand_tariff_income` | `ApplyTariffIncomeFeats` | `DotsClanFinanceModel` |
| `dots_khand_food_consumption` | `ApplyFoodConsumptionFeats` | `DotsFoodConsumptionModel` |
| `dots_khand_party_size` | `ApplyPartySizeFeats` | `DotsPartySizeModel` |
| `dots_harad_morale` | `ApplyMoraleFeats` | `DotsPartyMoraleModel` |
| `dots_harad_food_consumption` | `ApplyFoodConsumptionFeats` | `DotsFoodConsumptionModel` |
| `dots_harad_raid_damage` | `ApplyRaidDamageFeats` | `DotsRaidModel` |
| `dots_harad_army_influence_cost` | `ApplyArmyInfluenceCost` | `DotsArmyManagementModel` |
| `dots_rhun_loyalty` | `ApplyLoyaltyFeats` | `DotsSettlementLoyaltyModel` |
| `dots_rhun_raid_damage` | `ApplyRaidDamageFeats` | `DotsRaidModel` |
...
- All 24 `Register("dots_*")` ids appear exactly once in `DotsCulturalFeats.cs`.
- All 24 ids appear exactly once in a culture attachment surface.
- Custom-culture XML context:
  - `mordor`, `erebor`, `umbar`, `lothlorien`, `mirkwood`, `goblin`, and `mistymountainorcs` attachments are under the expected `dots_spcultures.xml` culture ids.
...
- `factions.json` localized keys:
  - 739 unique `dots_faction_*` keys in `factions.json`.
  - 0 missing ids in `dots_module_strings.xml`.
  - 0 U+2212 minus signs in `factions.json`.

## 5. Known Suspects

1. Sign/flag correctness for cost-reduction-as-positive feats - CONFIRMED CLEAN. Existing Erebor smithing uses `-0.3f, isPositiveEffect: true, AddFactor` at `DotsCulturalFeats.cs:534-537`. New smithing and food reductions follow that convention: negative `EffectBonus`, `isPositiveEffect: true`, `AddFactor`.
...
[LOW] Main/_Module/ModuleData/spcultures.xslt:1345 - Comment accuracy - The XSLT append section is still labeled "terrain movement-speed feats" even though it now appends tariff, renown, loyalty, food, raid, morale, party-size, and army-cost feats. The implementation is correct, but the comment now understates the purpose of the block. Fix: rename the comment to describe all DOTS cultural-feat appends for vanilla-wrapped cultures.
...
```powershell
dotnet test DOTS.Tests --filter CulturalFeats --no-restore
```
