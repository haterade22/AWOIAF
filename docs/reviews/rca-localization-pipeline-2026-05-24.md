...
No new feedback memory needed. Both findings are one-off bugs in newly-written tooling that the existing agent rules already catch correctly. The "compile-once, apply-once" pattern is a generic Python perf idiom not specific enough to DOTS to warrant a new memory.
