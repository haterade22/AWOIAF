...
## DOTS ID CHEATSHEET (use ONLY these)
...
- `~/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/feedback_no_aspirational_enum_values.md` -- the rule the Phase 1 RCA cites as the repeat-offender pattern (3 occurrences in 19 days).
- `~/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/feedback_user_facing_promise_must_match_code.md` -- related rule.
- `~/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/feedback_movementorder_cctor_mission_current.md` -- DOTS-specific Harmony-attach timing rule.
- `~/.claude/projects/c--Users-mikew-source-repos-DOTS/memory/feedback_missionbehaviortype_logic_requires_missionlogic_inheritance.md` -- relevant for the dev-trigger MissionBehavior.

## Known Suspects -- CONFIRM or DISPUTE each

1. **Harmony Finalizer on `MBSubModuleBase.OnSubModuleLoad` chicken-and-egg.** DOTS registers `Patch37_CrashReport` inside its own `OnSubModuleLoad` -- the very method it's trying to Finalize. The Finalizer is in place BEFORE the patch list is registered, so DOTS's own `OnSubModuleLoad` should be covered. But what about the gap between the Patch37 registration and the Finalizer actually being JIT-attached to the method? Walk the Harmony `PatchCategory(...)` call and confirm the attach happens synchronously before `PatchCategory` returns. If async or deferred, DOTS's own `OnSubModuleLoad` (lines 96-212 after the CrashReport block) is uncatchable.
...
5. **`CrashBundleWriter.TryCopyFile` cannot read the live DOTS debug log.** `FileLogger` opens `_logPath` via `new StreamWriter(_logPath, true)` -- default `FileShare` for `StreamWriter` write mode is `FileShare.Read`. So another reader can open it for reading, but our writer holds the file open. Our `TryCopyFile` opens with `FileShare.ReadWrite` which is the READER side's declaration of what it tolerates from OTHER openers -- it does not relax the writer's restriction. Verify by reading FileLogger.cs constructor and tracing the StreamWriter default. If the writer's lock blocks copies, the bundle ZIP never contains `dots_debug.log` and the diagnostic value is halved.
...
Main/DOTS.csproj                                  (System.Management + System.IO.Compression refs)
...
```
DOTS.Tests/Features/CrashReport/ExceptionFrameBuilderTests.cs
DOTS.Tests/Features/CrashReport/StackFrameSnapshotBuilderTests.cs
DOTS.Tests/Features/CrashReport/CrashSignatureCalculatorTests.cs
DOTS.Tests/Features/CrashReport/RingBufferTests.cs
DOTS.Tests/Features/CrashReport/PlainTextCrashReportRendererTests.cs
```
...
Decompile `Bannerlord.ButterLib.ExceptionHandler.ExceptionHandlerSubSystem.Disable()` from `C:/Users/mikew/source/repos/DOTS/Dependencies/_Module/bin/Win64_Shipping_Client/Bannerlord.ButterLib.dll` -- confirm it can be called more than once safely.

### 2. Feature-specific deep analysis

Walk EACH of the 12 Known Suspects above. For each, paste evidence from the DOTS source AND the vanilla decompile. Mark each CONFIRMED, DISPUTED, or NEEDS-DESIGN-INPUT.
...
- Why it's a bug (evidence from vanilla + DOTS source)
...
2. Have you read DOTS's actual source code (not just trusted the file list)?
...
- Codex once flagged vanilla-matching code as a DOTS bug
