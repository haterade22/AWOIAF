...
Session shipped a 4-fix Gondor noble-roster cleanup (Amrothos clan, 3 noblewomen rename + culture flip, Ciriel body = Dorwen body, Fix 4 round-robin body keys + 8 Tier 2 culture flips). `/deep-review` surfaced 2 confirmed bugs introduced/missed by the session work, plus 1 pre-existing DOTS inconsistency unrelated to this work. Net: minor — both confirmed bugs are cosmetic and either fixed in-session (settlements.xml) or pending user authorization (GitHub issue).
...
| Memory | Status |
|---|---|
| `feedback_rename_grep_all_moduledata.md` — when renaming any DOTS entity, grep `Main/_Module/ModuleData/**/*.xml` not just the directly-defining files | TO ADD |
...
| `feedback_submodule_xml_load_order_decides_winning_source.md` — for any XML source overlap question in DOTS, the answer is in `Main/_Module/SubModule.xml` `XmlNode` ordering, NOT in XSLT semantics. Last-loaded wins among additive sources with the same `XmlName id=`. | CONSIDER ADDING (would help future Data Flow agents) |

## Pre-existing DOTS inconsistency (out of session scope but worth follow-up)

`Main/_Module/ModuleData/lords.xslt` lines 4593-4827 have 7 NPC name overrides (Caldamir, Rúmil, Calathiel, Imloth, Anariel, Barandor, Belwen) that NEVER appear in-game because DOTS's `lords.xml` (loaded second per SubModule.xml line 123) wins. These were authored to give Tolkien-flavor names to Forlong/Hirluin/Lossarnach family members but are shadowed.
