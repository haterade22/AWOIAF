...
**Date:** 2026-05-06
**Feature:** Issue #110 — port LOTRAOM 1.2.12 `StartingEquipmentGold` to DOTS 1.3.15
...
## Bug B — `dots_youth_sturgia_1` `title_type="retainer"` with no roster XML [HIGH]
...
**What I did wrong:** The `title_type` strings are user-authored in `youth_menu.json`. I assumed every (culture, title_type) tuple from the JSON had a corresponding `<EquipmentRoster id="player_char_creation_..."/>` somewhere — vanilla SandBox XML or DOTS custom XML. I never grepped to verify.
...
1. They are full **independent kingdoms** in the Harad region (registered in `dots_spkingdoms.xml` with their own ruler titles `Taskral` / `Châjaphân`, banner keys, settlements), NOT Aserai sub-cultures.
...
1. Grep `dots_spkingdoms.xml` to see if the IDs are kingdoms.
...
- Filed follow-up issue [#111](https://github.com/haterade22/DOTS/issues/111) with three remediation options (A: author full menu coverage, B: hide from CC, C: safe fallback)
...
1. **Memory entries created/reinforced:**
   - [`feedback_codex_caught_api_misread.md`](C:\Users\mikew\.claude\projects\c--Users-mikew-source-repos-DOTS\memory\feedback_codex_caught_api_misread.md) (created this session)
