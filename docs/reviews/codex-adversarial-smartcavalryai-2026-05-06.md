...
- Settings live-reload mid-battle: provider reads `DotsSettings.Instance` live; toggle takes effect on next tick.
...
1. **Settings-NaN gap:** the existing rules library (`feedback_validate_before_lookup_with_fallback.md`, "Config Providers MUST Validate") covered loader-side validation. But the MCM provider is a CONSUMER of `DotsSettings.Instance`, not a loader. There was no rule for "providers that wrap MCM must NaN-guard the values they expose." Now there is.
