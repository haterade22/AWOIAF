...
This is broader than the current `csharp-architecture.md` "Config Providers MUST Validate" rule (which is loader-side only). The new rule covers the consumer side wherever the consumer is the boundary between DOTS and the engine.
