...
--------
user
# Adversarial review -- DOTS CareerSystem screen revamp (2026-05-30)

You are an adversarial reviewer. DOTS is a Lord of the Rings total-conversion mod of Mount & Blade II: Bannerlord v1.4.5. This session revamped the Career screen UI and adopted per-tier rank titles from TOR_Core (permission granted; GPLv3). Find real bugs. Confirm or DISPUTE each Known Suspect with evidence from the actual files. Do not flag vanilla-matching code as bugs. Use `--` not em-dash.
...
C#:
- Main/Features/CareerSystem/UI/CareerScreenVM.cs -- removed Tier2GateBottomHalf/Tier3GateTopHalf/Tier3GateFull/Tier1Locked props; added Tier2RequirementText/Tier3RequirementText; tier labels Tier1/2/3Label now set from career.Rank1/2/3Name with a "{=dots_career_tierN}Tier N" fallback.
...
Data:
- Main/_Module/ModuleData/career_system/dots_career_choices.xml -- 294 `display_name="{=dots_career_grp_<groupId>}<name>"` on <ChoiceGroup> (288 active groups + 6 inside a pre-existing DISABLED comment block for cave_troll_master).
- Main/_Module/ModuleData/career_system/dots_careers.xml -- 147 `rank1/2/3_name="{=dots_career_rank{N}_<careerId>}<title>"` on all 49 <Career>.
- Main/_Module/ModuleData/dots_module_strings.xml -- +441 `<string id="dots_career_grp_*"/>` and `dots_career_rank{1,2,3}_*` keys (and dots_career_tier_requirement).
- Main/_Module/GUI/DOTSSpriteData.xml -- ui_dots_career_system category SpriteSheetCount 1 -> 2, added `<SpriteSheetSize ID="2" Width="256" Height="256"/>`, added a <SpritePart> (Name=CareerSystem\career_point_pip, SheetID=2, SheetX=0, SheetY=0, Width=256, Height=256) and a <GenericSprite> for it. New PNG at Main/_Module/GUI/SpriteParts/ui_dots_career_system/CareerSystem/career_point_pip.png (256x256, alpha).
...
3. LOCALIZATION KEY CONSISTENCY. The `{=key}` prefixes in the display_name (dots_career_choices.xml) and rank_name (dots_careers.xml) attributes must EXACTLY match the `<string id=...>`/`{=...}` entries harvested into dots_module_strings.xml. Spot-check several. Also: are there any DUPLICATE `<string id>` entries introduced in dots_module_strings.xml? Is the file still well-formed? Are all 441 new keys unique?
...
7. COMMENTED-BLOCK INJECTION. The 6 cave_troll_master `<ChoiceGroup>` display_name attrs were injected INSIDE a pre-existing `<!-- DISABLED ... -->` comment in dots_career_choices.xml. Confirm the file is still well-formed XML (the injected attrs are inside the comment, inert) and that this does not break parsing.
...
- Read the actual DOTS files before claiming a binding is missing -- grep, don't assume.
...
codex
I’ll review this as read-only and start with the requested source files plus the TaleWorlds sprite loading path, since Suspect 1 depends on engine behavior rather than DOTS assumptions.
  → Read requested docs and DOTS UI/data files
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/Features/CareerSystem/UI/CareerScreenVM.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/Features/CareerSystem/UI/CareerChoiceGroupObjectVM.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'docs/features/career-system.md'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/Features/CareerSystem/UI/CareerChoiceObjectVM.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'docs/reviews/tor-career-ui-comparison-2026-05-30.md'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/_Module/GUI/Prefabs/CareerSystem/CareerScreen.xml'" in C:\Users\mikew\source\repos\DOTS
 succeeded in 804ms:
using System;
...
namespace DOTS.Features.CareerSystem.UI;
...
- **DOTS requirement:** Middle-earth factions need distinct playstyles beyond cultural feats — a Mordor Warboss should feel different from a Gondor Knight
...
TOR_Core's career system uses hardcoded C# classes, static singletons, and 6 Harmony patches on ViewModel. DOTS needs XML-driven careers (add without recompilation), DryIoc injection, adapter pattern compliance, and UIExtenderEx integration.

### Solution Approach

- **Data model:** Plain C# classes (not PropertyObject) loaded from XML via `ICareerConfigProvider`
- **Persistence:** `CareerPersistenceBehavior` with `SyncData("_dots_careerData")` storing `Dictionary<string, HeroCareerData>`
...
- **Ability effects:** `CareerAbilityEffectRegistry` dispatches to per-career `ICareerAbilityEffectExecutor` implementations. 3 role-based archetypes (Infantry/Ranged/Cavalry) serve all 50 careers with XML-tunable values via `dots_ability_tuning.xml`. All three archetypes apply AoE friendly-troop buffs within a 50-unit radius (standardized in templates): Infantry (damage + damage reduction), Ranged (speed + ranged damage + draw speed), Cavalry (mount speed + charge damage + damage). Buffs applied via `CareerAbilityBuffTracker` with separate hero and ally buff dictionaries (read by `DotsAgentStatCalculateModel` — survives stat recalc).

### Component Diagram

```
dots_careers.xml / dots_career_choices.xml
        |
...
### Career Definitions (`Main/_Module/ModuleData/career_system/dots_careers.xml`)
...
### Choice Trees (`Main/_Module/ModuleData/career_system/dots_career_choices.xml`)
...
### Ability Templates (`Main/_Module/ModuleData/career_system/dots_ability_templates.xml`)
...
`Main/_Module/ModuleData/career_system/dots_ability_tuning.xml` declares a single `<Global cooldown_seconds="30" />` element shared by all 50 careers. Edit to retune.
...
**Roster ID convention:** `player_career_{cultureId}_{infantry|ranged|cavalry}_{f|m}`. Built by [`CareerEquipmentRosterIds.Build`](../../Main/Features/CharacterCreation/CareerEquipmentRosterIds.cs), looked up via `MBObjectManager.GetObject<MBEquipmentRoster>`. Rosters are authored in [`Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml`](../../Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml).
...
4. Append 6 rosters to [`dots_career_starting_equipment.xml`](../../Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml) — one per (archetype, gender). Reference existing low-tier culture weapons + the new starter armor. **Don't forget the explicit Horse/HorseHarness clears for ranged + infantry** — `Equipment.FillFrom` is a slot-by-slot merge and will leave culture-default horses in place if you don't override.
...
| `Main/Features/CareerSystem/Models/` (3 files) | DotsAgentApplyDamageModel, DotsAgentStatCalculateModel, DotsClanTierModel |
...
| `Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml` | Per-(culture, archetype, gender) rosters; Gondor only as of 2026-05-19 |
...
### Add a new career
1. Add `<Career>` element to `dots_careers.xml` with unique id, eligible cultures, choice groups
2. Add `<ChoiceGroup>` elements to `dots_career_choices.xml` (6 groups: 2 per tier, each with 1 keystone + 4 passives)
3. Add `<Choice id="xxx_root">` as the root choice
4. Add ability template to `dots_ability_templates.xml`
...
1. Edit `Main/_Module/ModuleData/career_system/dots_ability_tuning.xml` `<Global cooldown_seconds="N" />` (must be in `(0, 3600]`)
2. Restart Bannerlord (provider caches via `Reuse.Singleton`; save-load is NOT enough)

### Add a new ability icon
See #101 — currently 41 of 50 careers have no PNG. Drop a 256x256 PNG into `Main/_Module/GUI/SpriteParts/ui_dots_career_system/CareerSystem/Abilities/<career_id>_ability.png` and add the corresponding `<Name>CareerSystem\Abilities\<career_id>_ability</Name>` registration in `Main/_Module/GUI/DOTSSpriteData.xml`.
...
Reference review of [TheOldRealms/TOR_Core](https://github.com/TheOldRealms/TOR_Core) (the Warhammer "Old Realms" total-conversion) career-screen UI, compared against DOTS's career screen after the 2026-05-30 revamp. Read-only review of a blobless shallow clone (since deleted); no TOR build/scripts were run.

## License / attribution

TOR_Core is **GPL v3**. DOTS's career system shares TOR's exact file structure (`CareerScreenVM.cs`, `CareerChoiceGroupObjectVM.cs`, `CareerChoiceObjectVM.cs`, `GUI/Prefabs/CareerSystem/CareerScreen.xml`) — it was originally **ported from TOR**. The TOR team granted DOTS explicit permission to reuse their code, and the repo is public. **Attribution:** DOTS's CareerSystem UI derives from TOR_Core (GPLv3), used with permission. The per-tier rank-title convention (`tor_career_rank{1,2,3}_name`) was adopted here; DOTS's names are authored fresh for Middle-earth.
...
## Where DOTS is ahead of TOR

1. **No blank-node bug.** TOR's pips vanish when `FreeCareerPoints = 0` (only looked fine in screenshots that had spare points). DOTS's `IsUnavailable` dim-pip keeps the strip always readable.
2. **Decoupled + tested.** DOTS's registry/service/adapter split is unit-tested; TOR's VMs hardwire `Hero.MainHero`.
...
- Renown/condition gating + chains overlay — DOTS uses level gating + "Requires Level N" text by design.
- Battle-Prayers button — Warhammer-specific.
- TOR's 2-state pip + `Hero.MainHero` coupling — DOTS's 3-state + service split is strictly better.
...
public class CareerScreenVM : ViewModel
{
        ...
        Action onClose)
    {
        ...
        _screenTitle = new TextObject("{=dots_career_screen_title}Career").ToString();
        _doneLbl = new TextObject("{=dots_career_done}Done").ToString();
        _abilityLabel = new TextObject("{=dots_career_ability_label}Career Ability").ToString();
        _tier1Label = new TextObject("{=dots_career_tier1}Tier 1").ToString();
        _tier2Label = new TextObject("{=dots_career_tier2}Tier 2").ToString();
        _tier3Label = new TextObject("{=dots_career_tier3}Tier 3").ToString();
    ...
    public override void RefreshValues()
    {
        ...
        FreeCareerPointsText = new TextObject("{=dots_career_free_points}Free Points: {COUNT}")
        ...
        Tier2RequirementText = new TextObject("{=dots_career_tier_requirement}Requires Level {LEVEL}")
            .SetTextVariable("LEVEL", _registry.GetTierUnlockLevel(2)).ToString();
        Tier3RequirementText = new TextObject("{=dots_career_tier_requirement}Requires Level {LEVEL}")
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "rg -n \"class SpriteData|class SpriteCategory|class SpritePart|SpriteSheetSize|Deserialize\\(\" E:\\Decompiled_Bannerlord E:\\Steam\\steamapps\\common\\\"Mount & Blade II Bannerlord\\\"\\bin\\Win64_Shipping_Client -g \"*.cs\"" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "rg -n \"Tier2GateBottomHalf|Tier3GateTopHalf|Tier3GateFull|Tier1Locked|@|DataSource=\\\"\\{|Command\\.\" 'Main/_Module/GUI/Prefabs/CareerSystem/CareerScreen.xml'" in C:\Users\mikew\source\repos\DOTS
exec
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/_Module/GUI/DOTSSpriteData.xml'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/Features/CareerSystem/UI/CareerAbilityEffectVM.cs'; Get-Content -Raw 'DOTS.Tests/Features/CareerSystem/CareerRegistryTests.cs'; Get-Content -Raw 'DOTS.Tests/Features/CareerSystem/CareerConfigProviderTests.cs'; Get-Content -Raw 'DOTS.Tests/Features/CareerSystem/CareerScreenVMTests.cs'" in C:\Users\mikew\source\repos\DOTS
...
public class CareerConfigProvider : ICareerConfigProvider
{
    ...
    {
        var path = Path.Combine(_pathService.ModuleDataPath, "career_system", "dots_careers.xml");
    ...
    {
        var path = Path.Combine(_pathService.ModuleDataPath, "career_system", "dots_career_choices.xml");
    ...
    {
        var path = Path.Combine(_pathService.ModuleDataPath, "career_system", "dots_ability_templates.xml");
        if (!File.Exists(path))
        {
    ...
    {
        var path = Path.Combine(_pathService.ModuleDataPath, "career_system", "dots_ability_tuning.xml");
        if (!File.Exists(path))
        {
...
<SpriteData>
  <SpriteCategories>
    ...
    <SpriteCategory>
      <Name>ui_dots_career_system</Name>
      <AlwaysLoad />
      <SpriteSheetCount>2</SpriteSheetCount>
      <SpriteSheetSize ID="1" Width="4096" Height="4096" />
  ...
  <SpriteParts>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>SpecialResources\dots_caster_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>2418</SheetX>
      <SheetY>2029</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>SpecialResources\dots_dunlending_ale_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>4</SheetX>
      <SheetY>2179</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>SpecialResources\dots_elven_wine_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>1036</SheetX>
      <SheetY>3061</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>SpecialResources\dots_gems_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>2068</SheetX>
      <SheetY>3061</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_lake_fish_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>4</SheetX>
      <SheetY>4</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_marks_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>1036</SheetX>
      <SheetY>4</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_plunder_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>2068</SheetX>
      <SheetY>4</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_tribal_relics_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>4</SheetX>
      <SheetY>1036</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_war_banners_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>1036</SheetX>
      <SheetY>1036</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_war_drums_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>2068</SheetX>
      <SheetY>1036</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>SpecialResources\dots_war_spoils_icon</Name>
      <Width>1024</Width>
      <Height>1024</Height>
      <SheetX>4</SheetX>
      <SheetY>2068</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>DOTS\CharacterScreen\menuoptions_border</Name>
      <Width>523</Width>
      <Height>52</Height>
      <SheetX>1540</SheetX>
      <SheetY>3001</SheetY>
    ...
    <SpritePart>
      <SheetID>5</SheetID>
      <Name>DOTS\CharacterScreen\menuoptions_mask</Name>
      <Width>523</Width>
      <Height>52</Height>
      <SheetX>3403</SheetX>
      <SheetY>2291</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>DOTS\Encyclopedia\banner</Name>
      <Width>596</Width>
      <Height>973</Height>
      <SheetX>1036</SheetX>
      <SheetY>3076</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>DOTS\Encyclopedia\clan_diamond_border</Name>
      <Width>1199</Width>
      <Height>1199</Height>
      <SheetX>1211</SheetX>
      <SheetY>972</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>DOTS\Encyclopedia\clan_diamond_mask</Name>
      <Width>1000</Width>
      <Height>1000</Height>
      <SheetX>2044</SheetX>
      <SheetY>2068</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>DOTS\Encyclopedia\fief_square_border</Name>
      <Width>1000</Width>
      <Height>1000</Height>
      <SheetX>1036</SheetX>
      <SheetY>2068</SheetY>
    ...
    <SpritePart>
      <SheetID>1</SheetID>
      <Name>DOTS\Encyclopedia\home_rectangle_border</Name>
      <Width>916</Width>
      <Height>2708</Height>
      <SheetX>2680</SheetX>
      <SheetY>4</SheetY>
    ...
    <SpritePart>
      <SheetID>2</SheetID>
      <Name>DOTS\Encyclopedia\lord_rectangle_border</Name>
      <Width>1000</Width>
      <Height>716</Height>
      <SheetX>1842</SheetX>
      <SheetY>3232</SheetY>
    ...
    <SpritePart>
      <SheetID>1</SheetID>
      <Name>DOTS\GameMenu\game_menu_frame_border</Name>
      <Width>623</Width>
      <Height>811</Height>
      <SheetX>1257</SheetX>
      <SheetY>3260</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>DOTS\GameMenu\game_menu_frame_canvas</Name>
      <Width>618</Width>
      <Height>798</Height>
      <SheetX>1036</SheetX>
      <SheetY>2179</SheetY>
    ...
    <SpritePart>
      <SheetID>1</SheetID>
      <Name>DOTS\GameMenu\game_menu_inner_panel</Name>
      <Width>13</Width>
      <Height>297</Height>
      <SheetX>4057</SheetX>
      <SheetY>214</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>DOTS\Nameplates\player_ring_icon</Name>
      <Width>1130</Width>
      <Height>1049</Height>
      <SheetX>2418</SheetX>
      <SheetY>972</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>DOTS\Nameplates\settlement_banner_large</Name>
      <Width>1000</Width>
      <Height>1000</Height>
      <SheetX>3052</SheetX>
      <SheetY>2068</SheetY>
    ...
    <SpritePart>
      <SheetID>3</SheetID>
      <Name>DOTS\Nameplates\settlement_banner_large_background</Name>
      <Width>1199</Width>
      <Height>1199</Height>
      <SheetX>4</SheetX>
      <SheetY>972</SheetY>
    ...
    <SpritePart>
      <SheetID>4</SheetID>
      <Name>DOTS\Nameplates\settlement_nameplate_background</Name>
      <Width>1136</Width>
      <Height>175</Height>
      <SheetX>2718</SheetX>
      <SheetY>3691</SheetY>
    ...
    <SpritePart>
      <SheetID>5</SheetID>
      <Name>DOTS\Nameplates\settlement_nameplate_large</Name>
      <Width>616</Width>
      <Height>76</Height>
      <SheetX>1783</SheetX>
      <SheetY>2129</SheetY>
    ...
    <SpritePart>
      <SheetID>1</SheetID>
      <Name>DOTS\SPScoreboard\background_overlay</Name>
      <Width>2668</Width>
      <Height>2160</Height>
      <SheetX>4</SheetX>
      <SheetY>4</SheetY>
    ...
    <SpritePart>
      <SheetID>1</SheetID>
      <Name>DOTS\SPScoreboard\divider_nameplate</Name>
      <Width>1192</Width>
      <Height>112</Height>
      <SheetX>4</SheetX>
      <SheetY>3920</SheetY>
    ...
    <SpritePart>
      ...
      <SheetX>4</SheetX>
      <SheetY>4</SheetY>
      <CategoryName>ui_dots_bannericons</CategoryName>
    </SpritePart>
    <SpritePart>
      <SheetID>16</SheetID>
      <Name>10001</Name>
    ...
    <SpritePart>
      ...
      <SheetX>2060</SheetX>
      <SheetY>4</SheetY>
      <CategoryName>ui_dots_career_system</CategoryName>
    </SpritePart>
    <SpritePart>
      <SheetID>1</SheetID>
  ...
  <Sprites>
    ...
    <GenericSprite>
      <Name>SpecialResources\dots_caster_icon</Name>
      <SpritePartName>SpecialResources\dots_caster_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_dunlending_ale_icon</Name>
      <SpritePartName>SpecialResources\dots_dunlending_ale_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_elven_wine_icon</Name>
      <SpritePartName>SpecialResources\dots_elven_wine_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_gems_icon</Name>
      <SpritePartName>SpecialResources\dots_gems_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_lake_fish_icon</Name>
      <SpritePartName>SpecialResources\dots_lake_fish_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_marks_icon</Name>
      <SpritePartName>SpecialResources\dots_marks_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_plunder_icon</Name>
      <SpritePartName>SpecialResources\dots_plunder_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_tribal_relics_icon</Name>
      <SpritePartName>SpecialResources\dots_tribal_relics_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_war_banners_icon</Name>
      <SpritePartName>SpecialResources\dots_war_banners_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_war_drums_icon</Name>
      <SpritePartName>SpecialResources\dots_war_drums_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SpecialResources\dots_war_spoils_icon</Name>
      <SpritePartName>SpecialResources\dots_war_spoils_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>SPGeneral\GameMenu\ambush_icon</Name>
    ...
    <GenericSprite>
      <Name>DOTS\CharacterScreen\menuoptions_border</Name>
      <SpritePartName>DOTS\CharacterScreen\menuoptions_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\CharacterScreen\menuoptions_mask</Name>
      <SpritePartName>DOTS\CharacterScreen\menuoptions_mask</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\banner</Name>
      <SpritePartName>DOTS\Encyclopedia\banner</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\clan_diamond_border</Name>
      <SpritePartName>DOTS\Encyclopedia\clan_diamond_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\clan_diamond_mask</Name>
      <SpritePartName>DOTS\Encyclopedia\clan_diamond_mask</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\fief_square_border</Name>
      <SpritePartName>DOTS\Encyclopedia\fief_square_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\home_rectangle_border</Name>
      <SpritePartName>DOTS\Encyclopedia\home_rectangle_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Encyclopedia\lord_rectangle_border</Name>
      <SpritePartName>DOTS\Encyclopedia\lord_rectangle_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\GameMenu\game_menu_frame_border</Name>
      <SpritePartName>DOTS\GameMenu\game_menu_frame_border</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\GameMenu\game_menu_frame_canvas</Name>
      <SpritePartName>DOTS\GameMenu\game_menu_frame_canvas</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\GameMenu\game_menu_inner_panel</Name>
      <SpritePartName>DOTS\GameMenu\game_menu_inner_panel</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Nameplates\player_ring_icon</Name>
      <SpritePartName>DOTS\Nameplates\player_ring_icon</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Nameplates\settlement_banner_large</Name>
      <SpritePartName>DOTS\Nameplates\settlement_banner_large</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Nameplates\settlement_banner_large_background</Name>
      <SpritePartName>DOTS\Nameplates\settlement_banner_large_background</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Nameplates\settlement_nameplate_background</Name>
      <SpritePartName>DOTS\Nameplates\settlement_nameplate_background</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\Nameplates\settlement_nameplate_large</Name>
      <SpritePartName>DOTS\Nameplates\settlement_nameplate_large</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\SPScoreboard\background_overlay</Name>
      <SpritePartName>DOTS\SPScoreboard\background_overlay</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>DOTS\SPScoreboard\divider_nameplate</Name>
      <SpritePartName>DOTS\SPScoreboard\divider_nameplate</SpritePartName>
    </GenericSprite>
    <GenericSprite>
      <Name>10000</Name>
...
public class CareerConfigProviderTests
{
    ...
    public void Setup()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "dots_test_" + Path.GetRandomFileName());
...
<CareerChoices>
  ...
  <Choice id=""c2"" type=""Passive"" description=""test2"" icon_sprite=""icon"">
    ...
    {
        File.WriteAllText(Path.Combine(_tempDir, "career_system", "dots_careers.xml"), content);
    }

    private void WriteChoicesXml(string content)
    {
        File.WriteAllText(Path.Combine(_tempDir, "career_system", "dots_career_choices.xml"), content);
    }

    private void WriteAbilityTuningXml(string content)
    {
        File.WriteAllText(Path.Combine(_tempDir, "career_system", "dots_ability_tuning.xml"), content);
    }
}
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Select-String -Path 'Main/_Module/GUI/Prefabs/CareerSystem/CareerScreen.xml' -Pattern 'Tier2GateBottomHalf','Tier3GateTopHalf','Tier3GateFull','Tier1Locked','@','DataSource=\"\\{','Command\\.' | Select-Object LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main/Features/CareerSystem' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerDefinition\\(','new CareerChoiceGroupDefinition\\(' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'DOTS.Tests' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerDefinition\\(','new CareerChoiceGroupDefinition\\(' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Select-String -Path 'Main/_Module/GUI/DOTSSpriteData.xml' -Pattern '<Name>ui_dots_career_system</Name>','<SpriteSheetCount>2</SpriteSheetCount>','<SpriteSheetSize ID=\"2\" Width=\"256\" Height=\"256\" />','CareerSystem\\career_point_pip' -Context 0,8 | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 567ms:
...
[32;1mFilename   : [0mDOTSSpriteData.xml
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\DOTSSpriteData.xml
...
[32;1mLine       : [0m      <Name>ui_dots_career_system</Name>
[32;1mFilename   : [0mDOTSSpriteData.xml
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\DOTSSpriteData.xml
[32;1mPattern    : [0m<Name>ui_dots_career_system</Name>
[32;1mContext    : [0mMicrosoft.PowerShell.Commands.MatchInfoContext
[32;1mMatches    : [0m{0}
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\CareerConfigProvider.cs
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerAbilityServiceTests.cs
[32;1mLineNumber : [0m17
[32;1mLine       : [0m    private static readonly CareerDefinition Career = new CareerDefinition(

[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerCampaignBehaviorTests.cs
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerCreationHandlerTests.cs
[32;1mLineNumber : [0m28
[32;1mLine       : [0m        _registry.GetCareer("warboss").Returns(new CareerDefinition(

[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerPassiveServiceTests.cs
[32;1mLineNumber : [0m18
[32;1mLine       : [0m    private static readonly CareerDefinition WarbossCareer = new CareerDefinition(

[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerRegistryTests.cs
[32;1mLineNumber : [0m18
[32;1mLine       : [0m    private static readonly CareerDefinition WarbossCareer = new CareerDefinition(
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerScreenVMTests.cs
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerSwitchServiceTests.cs
[32;1mLineNumber : [0m39
[32;1mLine       : [0m        _registry.GetCareer("warboss").Returns(new CareerDefinition(
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\MutationServiceTests.cs
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CharacterCreation\CareerMenuServiceTests.cs
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.TwoDimension\\TaleWorlds.TwoDimension\\SpriteData.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.TwoDimension\\TaleWorlds.TwoDimension\\SpritePart.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.TwoDimension\\TaleWorlds.TwoDimension\\SpriteCategory.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.TwoDimension\\TaleWorlds.TwoDimension\\SpriteSheet.cs'" in C:\Users\mikew\source\repos\DOTS
exec
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "ilspycmd 'E:\\Steam\\steamapps\\common\\Mount & Blade II Bannerlord\\bin\\Win64_Shipping_Client\\TaleWorlds.TwoDimension.dll' -t TaleWorlds.TwoDimension.SpriteData" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "ilspycmd 'E:\\Steam\\steamapps\\common\\Mount & Blade II Bannerlord\\bin\\Win64_Shipping_Client\\TaleWorlds.TwoDimension.dll' -t TaleWorlds.TwoDimension.SpriteCategory" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'E:\\Decompiled_Bannerlord\\Engine' -Recurse -Filter '*.cs' | Select-String -Pattern 'LoadTexture\\(','SpriteSheets\\\\' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "ilspycmd 'E:\\Steam\\steamapps\\common\\Mount & Blade II Bannerlord\\bin\\Win64_Shipping_Client\\TaleWorlds.TwoDimension.dll' -t TaleWorlds.TwoDimension.SpritePart" in C:\Users\mikew\source\repos\DOTS
 succeeded in 579ms:
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'E:\\Decompiled_Bannerlord' -Recurse -Filter '*.cs' | Select-String -Pattern 'SpriteParts','SpriteSheets','Generate.*Sprite','SpriteData\\(','SpriteCategory' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.Engine.GauntletUI\\TaleWorlds.Engine.GauntletUI\\TwoDimensionEngineResourceContext.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Engine\\TaleWorlds.TwoDimension\\TaleWorlds.TwoDimension\\TwoDimensionContext.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'docs/features/gui-sprite-system.md'" in C:\Users\mikew\source\repos\DOTS
 succeeded in 513ms:
using TaleWorlds.Library;
...
GUI/SpriteData/ui_taom/                      ← Compiled sprite sheets
        ↓ (referenced by)
GUI/DOTSSpriteData.xml                       ← Declares categories + sheet dimensions
...
A PNG at `SpriteParts/ui_taom/SpecialResources/dots_gems_icon.png` becomes sprite ID `dots_gems_icon`. Reference it in XML as `Sprite="dots_gems_icon"` or in C# as `Context.SpriteData.GetSprite("dots_gems_icon")`.
...
│   └── ui_taom/                      ← DOTS's sprite category
...
│       │   ├── dots_gems_icon.png    ← Erebor (gemstone wheel)
│       │   ├── dots_caster_icon.png  ← Gondor (White Tree coin)
│       │   └── dots_marks_icon.png   ← Rohan (horse coin)
│       └── ... (other subfolders)
└── DOTSSpriteData.xml                ← Master sprite category declaration
```
...
5. No `DOTSSpriteData.xml` changes needed unless you exceed sheet capacity
...
DOTS uses UIExtenderEx for two types of injection:
...
| `dots_{resource}_icon` | 45x45 | 8 remaining | Map bar resource icons |
...
| `Main/_Module/GUI/DOTSSpriteData.xml` | Sprite category declaration |
...
2. Place at `GUI/SpriteParts/ui_taom/SpecialResources/dots_{resource_id}_icon.png`
3. Set `icon_sprite="dots_{resource_id}_icon"` in `special_resources_config.xml`
...
3. Set `portrait_sprite="career_{career_id}_portrait"` in `dots_careers.xml`
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/_Module/GUI/SpriteParts/Config.xml'" in C:\Users\mikew\source\repos\DOTS
exec
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main/_Module/GUI/SpriteParts/ui_dots_career_system' -Recurse | Select-Object FullName,Length | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main/_Module/GUI/SpriteData' -Recurse -ErrorAction SilentlyContinue | Select-Object FullName,Length | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 514ms:
<Config>
  <SpriteCategory Name="ui_dots_bannericons">
    <AlwaysLoad />
  </SpriteCategory>
  ...
  <SpriteCategory Name="ui_dots_career_system">
    <AlwaysLoad />
  </SpriteCategory>
...
[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Portraits
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\career_butt
           on_placeholder.png
[32;1mLength   : [0m1091005

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\career_poin
           t_pip.png
[32;1mLength   : [0m46231

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\locked_gate
           _bottom.png
...
[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\minus_sign_
           icon.png
[32;1mLength   : [0m156205

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\plus_sign_i
           con.png
[32;1mLength   : [0m158008

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\c
...
[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\e
           otheod_windrider_ability.png
[32;1mLength   : [0m124761

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\i
           ronguard_ability.png
[32;1mLength   : [0m106619

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\k
           night_of_belfalas_ability.png
[32;1mLength   : [0m91380

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\m
           arksman_of_aldburg_ability.png
[32;1mLength   : [0m107853

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\r
...
[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Abilities\w
           atchman_of_stangard_ability.png
[32;1mLength   : [0m96239

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteParts\ui_dots_career_system\CareerSystem\Portraits\c
...
[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\ui_factionmap
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres
[32;1mLength   : [0m

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_anduin_vale.png
[32;1mLength   : [0m29209

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_aru_thani_of_bellakar.png
[32;1mLength   : [0m28857

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_clans_of_dunland.png
[32;1mLength   : [0m28970

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_clans_of_enedwaith.png
[32;1mLength   : [0m26074

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_clans_of_forochel.png
[32;1mLength   : [0m30131

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dark_lands_of_mordor.png
[32;1mLength   : [0m27762

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dominion_of_isengard.png
[32;1mLength   : [0m26838

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dominion_of_mordor.png
[32;1mLength   : [0m27762

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dwarves_of_erebor.png
[32;1mLength   : [0m28890

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dwarves_of_ered_luin.png
[32;1mLength   : [0m26793

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_dwarves_of_orocarni.png
[32;1mLength   : [0m26200

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_easterlings_of_rhun.png
[32;1mLength   : [0m28382

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_elves_of_lindon.png
[32;1mLength   : [0m28114

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_elves_of_lothlorien.png
[32;1mLength   : [0m28461

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_elves_of_mirkwood.png
[32;1mLength   : [0m30000

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_elves_of_neldoreth.png
[32;1mLength   : [0m30811

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_elves_of_rivendell.png
[32;1mLength   : [0m29897

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_faithful_of_bellakar.png
[32;1mLength   : [0m28857

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_fangorn_forest.png
[32;1mLength   : [0m28038

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_forodwaith.png
[32;1mLength   : [0m26544

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_goblins_of_goblin_town.png
[32;1mLength   : [0m30968

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_golden_realm_of_rhun.png
[32;1mLength   : [0m28382

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_havens_of_umbar.png
[32;1mLength   : [0m27719

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_high_kingdom_of_lindon.png
[32;1mLength   : [0m28114

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_hill_men_of_dunland.png
[32;1mLength   : [0m28970

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_hill_men_of_rhudaur.png
[32;1mLength   : [0m29631

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_iron_hills.png
[32;1mLength   : [0m29859

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_khudorom_of_khand.png
[32;1mLength   : [0m29388

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_arthedain.png
[32;1mLength   : [0m28425

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_cardolan.png
[32;1mLength   : [0m31096

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_dale.png
[32;1mLength   : [0m28171

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_dorwinion.png
[32;1mLength   : [0m29344

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_erebor.png
[32;1mLength   : [0m28890

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_ered_duin.png
[32;1mLength   : [0m26793

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_gondor.png
[32;1mLength   : [0m29034

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_imladris.png
[32;1mLength   : [0m29897

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_lasgalen.png
[32;1mLength   : [0m30000

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_lothlorien.png
[32;1mLength   : [0m28461

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_neldoreth.png
[32;1mLength   : [0m30811

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_rhudaur.png
[32;1mLength   : [0m29631

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_kingdom_of_rohan.png
[32;1mLength   : [0m27991

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_ered_luin.png
[32;1mLength   : [0m29587

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_forochel.png
[32;1mLength   : [0m30131

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_gundabad.png
[32;1mLength   : [0m27468

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_gwaer.png
[32;1mLength   : [0m30235

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_narager.png
[32;1mLength   : [0m29983

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_orcs_of_the_misty_mountains.p
           ng
[32;1mLength   : [0m30968

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_overlordship_of_dol_guldur.pn
           g
[32;1mLength   : [0m39411

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_overlordship_of_gundabad.png
[32;1mLength   : [0m27468

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_realm_of_angmar.png
[32;1mLength   : [0m28448

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_realm_of_cardolan.png
[32;1mLength   : [0m31096

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_realm_of_dorwinion.png
[32;1mLength   : [0m29344

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_remnants_of_angmar.png
[32;1mLength   : [0m28448

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_shadow_of_dol_guldur.png
[32;1mLength   : [0m39411

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_stewardship_of_gondor.png
[32;1mLength   : [0m29034

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_stronghold_of_ered_gwaer.png
[32;1mLength   : [0m30235

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_stronghold_of_narager.png
[32;1mLength   : [0m29983

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_taskralan_of_harwan.png
[32;1mLength   : [0m29141

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_tribes_of_harad.png
[32;1mLength   : [0m29141

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_vale_of_anduin.png
[32;1mLength   : [0m29209

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_variags_of_khand.png
[32;1mLength   : [0m29388

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\banner_wildmen_of_enedwaith.png
[32;1mLength   : [0m26074

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_anduin_vale.png
[32;1mLength   : [0m100710

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_aru_thani_of_bellakar.png
[32;1mLength   : [0m105045

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_clans_of_dunland.png
[32;1mLength   : [0m25010283

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_clans_of_enedwaith.png
[32;1mLength   : [0m107171

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_clans_of_forochel.png
[32;1mLength   : [0m113396

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dark_lands_of_mordor.png
[32;1mLength   : [0m24054147

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dominion_of_isengard.png
[32;1mLength   : [0m29853685

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dominion_of_mordor.png
[32;1mLength   : [0m24054147

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dwarves_of_erebor.png
[32;1mLength   : [0m28793270

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dwarves_of_ered_luin.png
[32;1mLength   : [0m107094

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_dwarves_of_orocarni.png
[32;1mLength   : [0m107753

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_easterlings_of_rhun.png
[32;1mLength   : [0m29132029

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_elves_of_lindon.png
[32;1mLength   : [0m101645

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_elves_of_lothlorien.png
[32;1mLength   : [0m30572551

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_elves_of_mirkwood.png
[32;1mLength   : [0m28656805

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_elves_of_neldoreth.png
[32;1mLength   : [0m117133

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_elves_of_rivendell.png
[32;1mLength   : [0m31548921

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_emblem.png
[32;1mLength   : [0m294

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_faithful_of_bellakar.png
[32;1mLength   : [0m105045

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_fangorn_forest.png
[32;1mLength   : [0m116618

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_forodwaith.png
[32;1mLength   : [0m100279

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_goblins_of_goblin_town.png
[32;1mLength   : [0m103329

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_golden_realm_of_rhun.png
[32;1mLength   : [0m29132029

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_havens_of_umbar.png
[32;1mLength   : [0m28440953

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_high_kingdom_of_lindon.png
[32;1mLength   : [0m101645

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_hill_men_of_dunland.png
[32;1mLength   : [0m25010283

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_hill_men_of_rhudaur.png
[32;1mLength   : [0m100621

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_iron_hills.png
[32;1mLength   : [0m128341

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_khudorom_of_khand.png
[32;1mLength   : [0m125343

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_arthedain.png
[32;1mLength   : [0m5445003

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_cardolan.png
[32;1mLength   : [0m92205

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_dale.png
[32;1mLength   : [0m119235

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_dolguldur.png
[32;1mLength   : [0m26931683

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_dorwinion.png
[32;1mLength   : [0m105110

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_dunland.png
[32;1mLength   : [0m25010283

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_erebor.png
[32;1mLength   : [0m28793270

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_ered_duin.png
[32;1mLength   : [0m107094

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_gondor.png
[32;1mLength   : [0m27765584

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_gundabad.png
[32;1mLength   : [0m30203946

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_harad.png
[32;1mLength   : [0m30995098

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_imladris.png
[32;1mLength   : [0m31548921

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_isengard.png
[32;1mLength   : [0m29853685

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_lasgalen.png
[32;1mLength   : [0m28656805

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_lothlorien.png
[32;1mLength   : [0m30572551

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_mordor.png
[32;1mLength   : [0m24054147

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_neldoreth.png
[32;1mLength   : [0m117133

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_rhudaur.png
[32;1mLength   : [0m100621

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_rhun.png
[32;1mLength   : [0m29132029

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_rivendell.png
[32;1mLength   : [0m31548921

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_rohan.png
[32;1mLength   : [0m30142191

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_kingdom_of_umbar.png
[32;1mLength   : [0m28440953

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_ered_luin.png
[32;1mLength   : [0m109169

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_forochel.png
[32;1mLength   : [0m113396

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_gundabad.png
[32;1mLength   : [0m30203946

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_gwaer.png
[32;1mLength   : [0m108720

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_narager.png
[32;1mLength   : [0m105135

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_orcs_of_the_misty_mountains.
           png
[32;1mLength   : [0m103329

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_overlordship_of_dol_guldur.p
           ng
[32;1mLength   : [0m26931683

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_overlordship_of_gundabad.png
[32;1mLength   : [0m30203946

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_realm_of_angmar.png
[32;1mLength   : [0m97080

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_realm_of_cardolan.png
[32;1mLength   : [0m92205

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_realm_of_dorwinion.png
[32;1mLength   : [0m105110

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_remnants_of_angmar.png
[32;1mLength   : [0m97080

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_shadow_of_dol_guldur.png
[32;1mLength   : [0m26931683

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_stewardship_of_gondor.png
[32;1mLength   : [0m27765584

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_stronghold_of_ered_gwaer.png
[32;1mLength   : [0m108720

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_stronghold_of_narager.png
[32;1mLength   : [0m105135

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_taskralan_of_harwan.png
[32;1mLength   : [0m30995098

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_tribes_of_harad.png
[32;1mLength   : [0m30995098

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_vale_of_anduin.png
[32;1mLength   : [0m100710

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_variags_of_khand.png
[32;1mLength   : [0m125343

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\faction_wildmen_of_enedwaith.png
[32;1mLength   : [0m107171

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_anduin_vale.png
[32;1mLength   : [0m75966

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_aru_thani_of_bellakar.png
[32;1mLength   : [0m230205

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_browlands.png
[32;1mLength   : [0m125790

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_chajaphan_of_abanissa.png
[32;1mLength   : [0m464287

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_clans_of_andrast.png
[32;1mLength   : [0m51562

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_clans_of_druwaith_iaur.png
[32;1mLength   : [0m46665

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_clans_of_dunland.png
[32;1mLength   : [0m50647

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_clans_of_forochel.png
[32;1mLength   : [0m66660

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_dominion_of_isengard.png
[32;1mLength   : [0m10529

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_dominion_of_mordor.png
[32;1mLength   : [0m422874

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_eregion.png
[32;1mLength   : [0m46859

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_fangorn_forest.png
[32;1mLength   : [0m56147

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_goblins_of_blue_craig.png
[32;1mLength   : [0m27540

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_goblins_of_goblin_town.png
[32;1mLength   : [0m32582

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_golden_realm_of_rhun.png
[32;1mLength   : [0m560262

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_havens_of_umbar.png
[32;1mLength   : [0m176003

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_high_kingdom_of_lindon.png
[32;1mLength   : [0m180338

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_khudorom_of_khand.png
[32;1mLength   : [0m404374

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_angaladh.png
[32;1mLength   : [0m43744

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_arthedain.png
[32;1mLength   : [0m279232

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_cardolan.png
[32;1mLength   : [0m128826

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_dale.png
[32;1mLength   : [0m104314

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_dorwinion.png
[32;1mLength   : [0m77070

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_erebor.png
[32;1mLength   : [0m192720

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_ered_duin.png
[32;1mLength   : [0m94486

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_ered_mithrin.png
[32;1mLength   : [0m78959

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_imladris.png
[32;1mLength   : [0m38906

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_lasgalen.png
[32;1mLength   : [0m61067

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_lothlorien.png
[32;1mLength   : [0m37312

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_moria.png
[32;1mLength   : [0m47847

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_neldoreth.png
[32;1mLength   : [0m101996

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_rhudaur.png
[32;1mLength   : [0m106103

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_rohan.png
[32;1mLength   : [0m98661

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_south_rhovanion.pn
           g
[32;1mLength   : [0m168702

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_kingdom_of_zigalnara.png
[32;1mLength   : [0m188006

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_map_boundary.png
[32;1mLength   : [0m2745861

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_nurunkhizdin.png
[32;1mLength   : [0m26683

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_overlordship_of_dol_guldur.pn
           g
[32;1mLength   : [0m41832

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_overlordship_of_gundabad.png
[32;1mLength   : [0m47071

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_remnants_of_angmar.png
[32;1mLength   : [0m91390

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_stewardship_of_gondor.png
[32;1mLength   : [0m354489

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_stronghold_of_ered_gwaer.png
[32;1mLength   : [0m165520

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_stronghold_of_narager.png
[32;1mLength   : [0m162210

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_taskralan_of_harwan.png
[32;1mLength   : [0m510644

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_taskralan_of_shaghana.png
[32;1mLength   : [0m614520

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\region_wildmen_of_enedwaith.png
[32;1mLength   : [0m101164

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_anduin_vale_full.png
[32;1mLength   : [0m95985

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_aru_thani_of_bellakar
           _full.png
[32;1mLength   : [0m256654

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_browlands_full.png
[32;1mLength   : [0m146807

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_chajaphan_of_abanissa
           _full.png
[32;1mLength   : [0m519988

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_clans_of_andrast_full
           .png
[32;1mLength   : [0m67629

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_clans_of_druwaith_iau
           r_full.png
[32;1mLength   : [0m63024

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_clans_of_dunland_full
           .png
[32;1mLength   : [0m68849

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_clans_of_forochel_ful
           l.png
[32;1mLength   : [0m83717

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_dominion_of_isengard_
           full.png
[32;1mLength   : [0m23440

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_dominion_of_mordor_fu
           ll.png
[32;1mLength   : [0m457408

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_eregion_full.png
[32;1mLength   : [0m63968

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_fangorn_forest_full.p
           ng
[32;1mLength   : [0m73566

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_goblins_of_blue_craig
           _full.png
[32;1mLength   : [0m41805

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_goblins_of_goblin_tow
           n_full.png
[32;1mLength   : [0m49429

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_golden_realm_of_rhun_
           full.png
[32;1mLength   : [0m600345

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_havens_of_umbar_full.
           png
[32;1mLength   : [0m200958

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_high_kingdom_of_lindo
           n_full.png
[32;1mLength   : [0m202865

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_khudorom_of_khand_ful
           l.png
[32;1mLength   : [0m436939

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_angaladh_f
           ull.png
[32;1mLength   : [0m61124

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_arthedain_
           full.png
[32;1mLength   : [0m305759

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_cardolan_f
           ull.png
[32;1mLength   : [0m148642

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_dale_full.
           png
[32;1mLength   : [0m124121

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_dorwinion_
           full.png
[32;1mLength   : [0m96642

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_erebor_ful
           l.png
[32;1mLength   : [0m214634

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_ered_duin_
           full.png
[32;1mLength   : [0m113592

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_ered_mithr
           in_full.png
[32;1mLength   : [0m94852

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_imladris_f
           ull.png
[32;1mLength   : [0m55541

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_lasgalen_f
           ull.png
[32;1mLength   : [0m79121

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_lothlorien
           _full.png
[32;1mLength   : [0m52825

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_moria_full
           .png
[32;1mLength   : [0m64877

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_neldoreth_
           full.png
[32;1mLength   : [0m120987

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_rhudaur_fu
           ll.png
[32;1mLength   : [0m126291

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_rohan_full
           .png
[32;1mLength   : [0m117519

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_south_rhov
           anion_full.png
[32;1mLength   : [0m189296

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_kingdom_of_zigalnara_
           full.png
[32;1mLength   : [0m211149

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_map_boundary_full.png
[32;1mLength   : [0m2888451

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_nurunkhizdin_full.png
[32;1mLength   : [0m42539

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_overlordship_of_dol_g
           uldur_full.png
[32;1mLength   : [0m59165

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_overlordship_of_gunda
           bad_full.png
[32;1mLength   : [0m63140

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_remnants_of_angmar_fu
           ll.png
[32;1mLength   : [0m109070

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_stewardship_of_gondor
           _full.png
[32;1mLength   : [0m380341

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_stronghold_of_ered_gw
           aer_full.png
[32;1mLength   : [0m185719

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_stronghold_of_narager
           _full.png
[32;1mLength   : [0m183968

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_taskralan_of_harwan_f
           ull.png
[32;1mLength   : [0m544949

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_taskralan_of_shaghana
           _full.png
[32;1mLength   : [0m669550

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\FactionMap\fullres\region_wildmen_of_enedwaith_
           full.png
[32;1mLength   : [0m120226

[32;1mFullName : [0mC:\Users\mikew\source\repos\DOTS\Main\_Module\GUI\SpriteData\ui_factionmap\ui_factionmap.xml
[32;1mLength   : [0m1116
...
E:\Steam\steamapps\common\Mount & Blade II Bannerlord\Modules\DOTS\GUI\DOTSSpriteData.xml
E:\Steam\steamapps\common\Mount & Blade II Bannerlord\Modules\DOTS.Dependencies\GUI\Bannerlord.MBOptionScreenSpriteData.xml
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'E:\\Steam\\steamapps\\common\\Mount & Blade II Bannerlord\\Modules\\DOTS\\GUI' -Directory -ErrorAction SilentlyContinue | Select-Object Name | Format-Table -HideTableHeaders; Get-ChildItem -Path 'E:\\Steam\\steamapps\\common\\Mount & Blade II Bannerlord\\Modules\\DOTS\\GUI\\SpriteSheets' -Recurse -ErrorAction SilentlyContinue | Select-Object FullName,Length | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
...
'@ | powershell -NoProfile -Command -" in C:\Users\mikew\source\repos\DOTS
 succeeded in 1149ms:
codex
...
"'$choicesPath = '"'Main/_Module/ModuleData/career_system/dots_career_choices.xml'
"'$careersPath = '"'Main/_Module/ModuleData/career_system/dots_careers.xml'
"'$stringsPath = '"'Main/_Module/ModuleData/dots_module_strings.xml'
...
"Duplicate string ids in dots_module_strings.xml: $($dups.Count)"
...
"String ids with dots_career_grp/rank/tier_requirement prefixes:"
$prefixCount = @($stringNodes | Where-Object { $_.id -match '"'"'^(dots_career_grp_|dots_career_rank[123]_|dots_career_tier_requirement$)'"' }).Count
...
Duplicate string ids in dots_module_strings.xml: 0
Missing referenced keys: 0
Sample group attrs:
ranger_of_ithilien_t1_a => {=dots_career_grp_ranger_of_ithilien_t1_a}Hunters of the Ithilien Road => present=True
ranger_of_ithilien_t1_b => {=dots_career_grp_ranger_of_ithilien_t1_b}Watchers of Henneth AnnÃ»n => present=True
ranger_of_ithilien_t2_a => {=dots_career_grp_ranger_of_ithilien_t2_a}Scouts of North Ithilien => present=True
ranger_of_ithilien_t2_b => {=dots_career_grp_ranger_of_ithilien_t2_b}Bowmen of Faramir => present=True
ranger_of_ithilien_t3_a => {=dots_career_grp_ranger_of_ithilien_t3_a}Arrows of the Forbidden Pool => present=True
ranger_of_ithilien_t3_b => {=dots_career_grp_ranger_of_ithilien_t3_b}Shadows of Emyn Arnen => present=True
captain_of_osgiliath_t1_a => {=dots_career_grp_captain_of_osgiliath_t1_a}Wardens of the East Bank => present=True
captain_of_osgiliath_t1_b => {=dots_career_grp_captain_of_osgiliath_t1_b}Keepers of the Anduin Crossing => present=True
Sample rank attrs:
ranger_of_ithilien => {=dots_career_rank1_ranger_of_ithilien}Ithilien Scout | {=dots_career_rank2_ranger_of_ithilien}Ranger of Ithilien | {=dots_career_rank3_ranger_of_ithilien}Captain of the White Company
captain_of_osgiliath => {=dots_career_rank1_captain_of_osgiliath}Ohtar of the Crossing | {=dots_career_rank2_captain_of_osgiliath}Warden of Osgiliath | {=dots_career_rank3_captain_of_osgiliath}Captain of Osgiliath
knight_of_belfalas => {=dots_career_rank1_knight_of_belfalas}Squire of Dol Amroth | {=dots_career_rank2_knight_of_belfalas}Swan-Knight | {=dots_career_rank3_knight_of_belfalas}Knight of the Silver Swan
black_uruk_captain => {=dots_career_rank1_black_uruk_captain}Uruk Soldier | {=dots_career_rank2_black_uruk_captain}Black Uruk | {=dots_career_rank3_black_uruk_captain}Captain of the Red Eye
mulkerhili_cultist => {=dots_career_rank1_mulkerhili_cultist}Whisper-Sworn | {=dots_career_rank2_mulkerhili_cultist}Voice of LugbÃºrz | {=dots_career_rank3_mulkerhili_cultist}Herald of the Lidless Eye
snaga_rider => {=dots_career_rank1_snaga_rider}Snaga | {=dots_career_rank2_snaga_rider}Smoke-Raider | {=dots_career_rank3_snaga_rider}Shadow of LugbÃºrz
olog_hai_warchief => {=dots_career_rank1_olog_hai_warchief}War-Troll | {=dots_career_rank2_olog_hai_warchief}Olog-hai | {=dots_career_rank3_olog_hai_warchief}Warchief of LugbÃºrz
marksman_of_aldburg => {=dots_career_rank1_marksman_of_aldburg}Bowman of the Eastfold | {=dots_career_rank2_marksman_of_aldburg}Marksman of Aldburg | {=dots_career_rank3_marksman_of_aldburg}Arrow-warden of the Mark
String ids with dots_career_grp/rank/tier_requirement prefixes:
prefixCount=442

exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "@'
"'$path = '"'Main/_Module/ModuleData/career_system/dots_career_choices.xml'
...
Select-String -Path $path -Pattern '"'cave_troll_master','DISABLED','display_name=\\\"\\{=dots_career_grp_cave' -Context 2,2 | Select-Object -First 20 | Format-List
'@ | powershell -NoProfile -Command -" in C:\Users\mikew\source\repos\DOTS
 succeeded in 791ms:
...
Line       :   <!-- DISABLED 2026-05-14: Far Harad Halftroll choice tree WIP; not ready for live game yet. Re-enable 
             by uncommenting.
Filename   : dots_career_choices.xml
Path       : C:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\career_system\dots_career_choices.xml
Pattern    : DISABLED
...
IgnoreCase : True
LineNumber : 6772
Line       :           description="{=dots_cave_troll_master_root_desc}Troll Frenzy grants +25 health and drives your 
             trolls berserk."
Filename   : dots_career_choices.xml
Path       : C:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\career_system\dots_career_choices.xml
Pattern    : cave_troll_master
...
Line       :   <ChoiceGroup id="cave_troll_master_t1_a" 
             display_name="{=dots_career_grp_cave_troll_master_t1_a}Pit-Whip Driver" career_id="cave_troll_master" 
             tier="1">
Filename   : dots_career_choices.xml
Path       : C:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\career_system\dots_career_choices.xml
Pattern    : cave_troll_master
...
IgnoreCase : True
LineNumber : 6778
Line       :     <Choice id="cave_troll_master_t1_a_key" type="Keystone" description="{=dots_ctm_t1_a_key}Troll Frenzy 
...
IgnoreCase : True
LineNumber : 6781
Line       :     <Choice id="cave_troll_master_t1_a_p1" type="Passive" description="{=dots_ctm_t1_a_p1}+8% melee 
...
IgnoreCase : True
LineNumber : 6784
Line       :     <Choice id="cave_troll_master_t1_a_p2" type="Passive" description="{=dots_ctm_t1_a_p2}+5% troop 
...
IgnoreCase : True
LineNumber : 6787
Line       :     <Choice id="cave_troll_master_t1_a_p3" type="Passive" description="{=dots_ctm_t1_a_p3}+30 max 
...
IgnoreCase : True
LineNumber : 6790
Line       :     <Choice id="cave_troll_master_t1_a_p4" type="Passive" description="{=dots_ctm_t1_a_p4}+7% swing 
...
Line       :   <ChoiceGroup id="cave_troll_master_t1_b" 
             display_name="{=dots_career_grp_cave_troll_master_t1_b}Cave-Mouth Warden" career_id="cave_troll_master" 
             tier="1">
Filename   : dots_career_choices.xml
Path       : C:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\career_system\dots_career_choices.xml
Pattern    : cave_troll_master
...
IgnoreCase : True
LineNumber : 6796
Line       :     <Choice id="cave_troll_master_t1_b_key" type="Keystone" description="{=dots_ctm_t1_b_key}Troll Frenzy 
...
IgnoreCase : True
LineNumber : 6799
Line       :     <Choice id="cave_troll_master_t1_b_p1" type="Passive" description="{=dots_ctm_t1_b_p1}+8% 
...
IgnoreCase : True
LineNumber : 6802
Line       :     <Choice id="cave_troll_master_t1_b_p2" type="Passive" description="{=dots_ctm_t1_b_p2}+5% troop 
...
IgnoreCase : True
LineNumber : 6805
Line       :     <Choice id="cave_troll_master_t1_b_p3" type="Passive" description="{=dots_ctm_t1_b_p3}+5% troop 
...
IgnoreCase : True
LineNumber : 6808
Line       :     <Choice id="cave_troll_master_t1_b_p4" type="Passive" description="{=dots_ctm_t1_b_p4}+5% troop 
...
Line       :   <ChoiceGroup id="cave_troll_master_t2_a" 
             display_name="{=dots_career_grp_cave_troll_master_t2_a}Goblin-town Tyrant" career_id="cave_troll_master" 
             tier="2">
Filename   : dots_career_choices.xml
Path       : C:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\career_system\dots_career_choices.xml
Pattern    : cave_troll_master
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main/Features/CareerSystem' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerScreenVM|HeroLevel|\\.Level|IsTierAvailable\\(' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Select-String -Path 'E:\\Decompiled_Bannerlord\\Campaign\\TaleWorlds.CampaignSystem\\TaleWorlds.CampaignSystem\\Hero.cs' -Pattern 'public int Level','Level =>','GetSkillValue','HeroDeveloper' -Context 3,5 | Select-Object -First 30 | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main','DOTS.Tests' -Recurse -Filter '*.cs' | Select-String -Pattern 'GetTierUnlockLevel\\(','IsTierAvailable\\(' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\CareerCampaignBehavior.cs
[32;1mLineNumber : [0m36
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\CareerRegistry.cs
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\CareerSwitchService.cs
[32;1mLineNumber : [0m54
[32;1mLine       : [0m            var maxChoices = _registry.GetMaxChoicesForHero(hero.Level);

[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\ICareerRegistry.cs
[32;1mLineNumber : [0m15
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\Mutations\BuiltInCalculators.cs
[32;1mLineNumber : [0m14
[32;1mLine       : [0m            baseValue + hero.Level * p.GetFloat("factor", 0.01f));

[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\UI\CareerScreenVM.cs
[32;1mLineNumber : [0m17
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\UI\GauntletCareerScreen.cs
[32;1mLineNumber : [0m27
[32;1mLine       : [0m    private readonly int _heroLevel;
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content 'E:\\Decompiled_Bannerlord\\Campaign\\TaleWorlds.CampaignSystem\\TaleWorlds.CampaignSystem\\Hero.cs' | Select-Object -Skip 2230 -First 25" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content 'E:\\Decompiled_Bannerlord\\Campaign\\TaleWorlds.CampaignSystem\\TaleWorlds.CampaignSystem\\CharacterObject.cs' | Select-Object -Skip 390 -First 35" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'Main/Features/CareerSystem/UI/GauntletCareerScreen.cs'" in C:\Users\mikew\source\repos\DOTS
 succeeded in 565ms:
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'E:\\Decompiled_Bannerlord\\Core\\TaleWorlds.Library\\TaleWorlds.Library\\ViewModel.cs'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main/Features/CareerSystem/UI','DOTS.Tests/Features/CareerSystem' -Recurse -Filter '*.cs' | Select-String -Pattern 'IsUnavailable|IsFreeToTake|IsTaken|OnPropertyChanged\\(' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 1241ms:
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\UI\CareerChoiceGroupObjectVM.cs
[32;1mLineNumber : [0m44
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\Main\Features\CareerSystem\UI\CareerChoiceObjectVM.cs
[32;1mLineNumber : [0m13
...
[32;1mPath       : [0mC:\Users\mikew\source\repos\DOTS\DOTS.Tests\Features\CareerSystem\CareerChoiceObjectVMTests.cs
[32;1mLineNumber : [0m15
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Select-String -Path 'Main/_Module/GUI/DOTSSpriteData.xml' -Pattern '<Name>CareerSystem\\career_point_pip</Name>' -Context 5,8 | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 468ms:
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Select-String -Path 'Main/_Module/GUI/DOTSSpriteData.xml' -Pattern 'career_point' -Context 5,8 | Select-Object -First 4 | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 481ms:
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content 'Main/_Module/GUI/DOTSSpriteData.xml' | Select-Object -Skip 7278 -First 20; Get-Content 'Main/_Module/GUI/DOTSSpriteData.xml' | Select-Object -Skip 10770 -First 10" in C:\Users\mikew\source\repos\DOTS
 succeeded in 546ms:
      <Name>CareerSystem\plus_sign_icon</Name>
      <Width>256</Width>
      <Height>256</Height>
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'tools/apply_career_group_names.py'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'tools/apply_career_rank_names.py'" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'tools/career_group_names.json' | Select-Object -First 1" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-Content -Raw 'tools/career_rank_names.json' | Select-Object -First 1" in C:\Users\mikew\source\repos\DOTS
 succeeded in 518ms:
#!/usr/bin/env python3
...
`display_name="{=dots_career_grp_<id>}<Name>"` attribute onto each matching
<ChoiceGroup> in dots_career_choices.xml. Also emits the companion
localization-string snippet for harvesting into dots_module_strings.xml.
...
DEFAULT_XML = os.path.join(
    REPO, "Main", "_Module", "ModuleData", "career_system", "dots_career_choices.xml"
)
...
def key_for(group_id):
    return "dots_career_grp_" + group_id
...
def main():
    ...
    with open(args.strings_snippet, "w", encoding="utf-8", newline="\r\n") as f:
        f.write("<!-- Career choice-group path names (harvest into dots_module_strings.xml) -->\n")
        f.write("\n".join(string_lines) + "\n")
...
rank1_name/rank2_name/rank3_name="{=dots_career_rank{N}_<id>}<Name>" onto each
matching <Career> in dots_careers.xml. Emits the companion localization snippet.
...
DEFAULT_XML = os.path.join(
    REPO, "Main", "_Module", "ModuleData", "career_system", "dots_careers.xml"
)
...
def key_for(rank, career_id):
    return "dots_career_%s_%s" % (rank, career_id)
...
def main():
    ...
    open(args.xml, "wb").write(out)
    with open(args.strings_snippet, "w", encoding="utf-8", newline="\r\n") as f:
        f.write("<!-- Career per-tier rank titles (harvest into dots_module_strings.xml) -->\n")
        f.write("\n".join(string_lines) + "\n")
    print("\n[APPLIED] wrote %s" % args.xml)
    print("[APPLIED] wrote loc snippet %s (%d strings)" % (args.strings_snippet, len(string_lines)))
...
{
  "_meta": {
    "purpose": "Reviewed web-researched Middle-earth lore names for career choice-group sub-paths (DOTS career screen revamp, Phase B).",
    "format": "<group_id>: { name, attested, note }. attested=true means the term/figure/place is canonical Tolkien lore; attested=false is an honest original coined in-keeping (or derived from a DOTS-original id).",
    "applied_by": "tools/apply_career_group_names.py (writes display_name=\"{=dots_career_grp_<id>}<name>\" onto each <ChoiceGroup>)",
  ...
  "mulkerhili_cultist_t1_a": { "name": "Venomed Litany", "attested": false, "note": "coined for DOTS-original cult; poisoned-archery theme" },
  ...
  "avanc_luth_raider_t3_a": { "name": "Avanc-lúth War-Chieftain", "attested": false, "note": "Avanc-lúth = DOTS-coined clan (avanc = Welsh water-monster)" },
  ...
  "wolfskin_hunter_t1_a": { "name": "Pack-Marked Stalker", "attested": false, "note": "from Pack Leader's Mark; wolf-pack motif is DOTS theme" },
  ...
  "wolfskin_hunter_t3_b": { "name": "Master of the Great Pack", "attested": false, "note": "from 'Great Pack' +party-size; wolf-pack motif DOTS-coined" },
  ...
  "blademaster_of_ren_t1_a": { "name": "Court Duelists", "attested": false, "note": "Rên = DOTS-original Khand place; warrior-court duelists" },
  ...
  "blademaster_of_ren_t2_b": { "name": "Wardens of Rên", "attested": false, "note": "Rên = DOTS-original place" },
  ...
  "tribesman_of_jelut_t1_a": { "name": "Dune Raiders", "attested": false, "note": "Haradrim desert dwellers; Jelut = DOTS-coined place" },
  ...
  "pezarsani_javelineer_t1_a": { "name": "Sun-Spear Skirmishers", "attested": false, "note": "sun/desert imagery; Pezarsan = DOTS-coined place" },
  ...
  "mahud_beast_rider_t1_a": { "name": "Tusk Vanguard", "attested": false, "note": "mûmakil tusks; 'Mahûd' is a GW term used by DOTS, not Tolkien" },
  ...
  "codyan_legionaire_t1_a": { "name": "Côdyan Vanguard", "attested": false, "note": "Côdya/Côdyan = DOTS-coined Easterling formation" },
  "codyan_legionaire_t1_b": { "name": "Steadfast Shieldmen", "attested": false, "note": "generic Easterling shieldwall; defensive duration" },
  "codyan_legionaire_t2_a": { "name": "Wall of Rhûn", "attested": true, "note": "Rhûn attested (East-lands); shieldwall imagery DOTS's" },
  "codyan_legionaire_t2_b": { "name": "Ironclad Phalanx", "attested": false, "note": "DOTS-flavored; not canon terminology" },
  ...
  "lokhas_drus_marksman_t2_a": { "name": "Stormbow Marksmen", "attested": false, "note": "DOTS-coined" },
  "lokhas_drus_marksman_t2_b": { "name": "Plate-Piercers", "attested": false, "note": "Lôkhas Drûs = DOTS-coined armour-piercing corps" },
  ...
  "balchoth_kan_t2_b": { "name": "Iron Horsemen", "attested": false, "note": "DOTS-flavored heavy cavalry" },
...
{
  "_meta": {
    "purpose": "Reviewed web-researched per-tier RANK titles for careers (tier headers on the career screen). Adopted from TOR_Core's tor_career_rank{1,2,3}_name convention, authored clean for DOTS/LOTR.",
    "format": "<careerId>: { rank1:{name,attested}, rank2:{...}, rank3:{...} }. attested=true = canonical Tolkien term/title.",
    "applied_by": "tools/apply_career_rank_names.py (writes rank{1,2,3}_name=\"{=dots_career_rank{N}_<id>}<name>\" onto each <Career>)",
    "count": 147
  },
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main','DOTS.Tests' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerDefinition\\(','new CareerChoiceGroupDefinition\\(' -Context 0,12 | Select-Object Path,LineNumber,Line,Context | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 644ms:
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main','DOTS.Tests' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerDefinition\\(\\s*"'$'"','new CareerChoiceGroupDefinition\\(\\s*"'$'"' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "Get-ChildItem -Path 'Main','DOTS.Tests' -Recurse -Filter '*.cs' | Select-String -Pattern 'new CareerDefinition\\(\\s*\"','new CareerChoiceGroupDefinition\\(\\s*\"' | Select-Object Path,LineNumber,Line | Format-List" in C:\Users\mikew\source\repos\DOTS
 succeeded in 720ms:
...
exec
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "@'
"'$files = Get-ChildItem -Path '"'Main','DOTS.Tests' -Recurse -Filter '*.cs'
foreach ("'$file in $files) {
...
[xml]"'$strings = Get-Content -Raw $stringsPath
$mismatch=@()
$nodes = @($strings.SelectNodes('"'//*[@id]') | Where-Object { "'$_.id -match '"'"'^(dots_career_grp_|dots_career_rank[123]_|dots_career_tier_requirement$)'"' })
foreach ("'$n in $nodes) {
  $text = $n.text
  if ($text -match '"'"'^'"\\{=(["'^}]+)'"\\}') {
    if ("'$matches[1] -ne $n.id) { $mismatch += "$($n.id) textToken=$($matches[1])" }
  } elseif ($n.id -ne '"'dots_career_tier_requirement') {
...
"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "@'
"'$ErrorActionPreference='"'Stop'
[xml]"'$strings = Get-Content -Raw '"'Main/_Module/ModuleData/dots_module_strings.xml'
"'$all = @($strings.SelectNodes('"'//*[@id]'))
\"all ids="'$($all.Count)"
$nodes = @($all | Where-Object { $_.id -match '"'"'^(dots_career_grp_|dots_career_rank[123]_|dots_career_tier_requirement$)'"' })
"'$mismatch=@()
foreach ($n in $nodes) {
...
p='Main/_Module/ModuleData/dots_module_strings.xml'
root=ET.parse(p).getroot()
nodes=[]
for e in root.iter():
    idv=e.attrib.get('id')
    if idv and re.match(r'"'^(dots_career_grp_|dots_career_rank[123]_|dots_career_tier_requirement$)'"', idv):
        nodes.append(e)
mis=[]
for e in nodes:
    ...
    elif idv!='"'dots_career_tier_requirement':
...
This means `SheetID` is scoped through the sprite part's `CategoryName`, and each sheet has its own `Vec2i` size. I found no v1.4.5 C# runtime requirement that every sheet in a category must share the same dimensions. DOTS's manifest has `ui_dots_career_system` sheet 1 as `4096x4096`, sheet 2 as `256x256`, and `CareerSystem\career_point_pip` on sheet 2 at `0,0` with `256x256`, which fits exactly.
...
Checks performed:
- `dots_career_choices.xml`, `dots_careers.xml`, and `dots_module_strings.xml` all parse as well-formed XML.
...
- Missing referenced keys: `0`.
- Duplicate `<string id>` entries in `dots_module_strings.xml`: `0`.
- Career-prefixed string nodes including disabled cave-troll group keys plus `dots_career_tier_requirement`: `442`.
- ID/text-token mismatches: `0`.
