...
Feature #3 of the 7-feature port: SmartCavalryAI was ported from external developer drop into DOTS. Original module was written against Bannerlord v1.4 decompile; DOTS targets v1.3.15 (installed game version). Five Claude /deep-review agents already produced findings; all HIGH and MEDIUM were fixed in the same session. This prompt asks Codex for an INDEPENDENT adversarial pass to catch what those agents missed.
...
- `DOTS.Tests/Features/SmartCavalryAI/CavalryChargeServiceTests.cs` (50 tests)
- `DOTS.Tests/Features/SmartCavalryAI/CavalryPathPlannerTests.cs` (12 tests)
