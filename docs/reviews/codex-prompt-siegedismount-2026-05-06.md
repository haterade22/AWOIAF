...
- Main/Features/DotsSettings.cs -- the appended Battle Tactics/Siege Dismount group at the bottom (3 settings)
...
- DOTS.Tests/Features/SiegeDismount/SiegeDismountServiceTests.cs -- 33 unit tests
...
- Main/Features/DotsSettings.cs (Battle Tactics/Siege Dismount group at the bottom)
...
- Main/_Module/ModuleData/custom_settlements.xml -- DOTS custom castles, includes `castle_orthanc_gate`, `castle_gundabad_wall`
...
Critical: confirm `MissionBehavior.OnEndMission` is `protected virtual void` (not public). The DOTS port uses `protected override void OnEndMission()` -- if the access modifier is wrong, the override is not actually wired.
...
- Are MCM settings folded into DotsSettings.cs (not a per-feature settings class)?
