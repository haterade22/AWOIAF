...
DOTS Wave 1 cultural-feats expansion (commits bf9226f + ce07ebe, issue #273) adds 24 new cultural feats across 11 cultures, raising the feat total 105 -> 129. Every feat is "Q-class": it plugs into an EXISTING `CulturalFeatsService.Apply*` method via an added `culture.HasFeat(...)` check. No new GameModels, no new service methods, no Harmony patches, no conditional logic. The faction-map CC page (`factions.json`) gained 26 bonus lines surfacing the feats (Goblin's 2 feats appear on both Goblin Town and Blue Craig factions since both play the `goblin` culture).
...
1. dots_mordor_smithing | mordor | ApplySmithingFeats | -0.15 | AddFactor | true
2. dots_erebor_tariff_income | erebor | ApplyTariffIncomeFeats | 0.05 | AddFactor | true
3. dots_umbar_raid_damage | umbar | ApplyRaidDamageFeats | 0.20 | AddFactor | true
4. dots_umbar_food_consumption | umbar | ApplyFoodConsumptionFeats | -0.10 | AddFactor | true
5. dots_lothlorien_volunteer_rate | lothlorien | ApplyVolunteerRespawnFeats | -0.15 | AddFactor | false
6. dots_mirkwood_army_influence_cost | mirkwood | ApplyArmyInfluenceCost | 0.15 | AddFactor | false
7. dots_goblin_smithing | goblin | ApplySmithingFeats | -0.10 | AddFactor | true
8. dots_goblin_raid_damage | goblin | ApplyRaidDamageFeats | 0.10 | AddFactor | true
9. dots_mistymountainorcs_smithing | mistymountainorcs | ApplySmithingFeats | -0.15 | AddFactor | true
10. dots_mistymountainorcs_raid_damage | mistymountainorcs | ApplyRaidDamageFeats | 0.15 | AddFactor | true
11. dots_mistymountainorcs_construction_speed | mistymountainorcs | ApplyConstructionSpeedFeats | -0.10 | AddFactor | false
12. dots_dale_tariff_income | sturgia | ApplyTariffIncomeFeats | 0.10 | AddFactor | true
13. dots_dale_renown | sturgia | ApplyRenownFeats | 0.10 | AddFactor | true
14. dots_dale_loyalty | sturgia | ApplyLoyaltyFeats | -0.5 | Add | false
15. dots_khand_renown | battania | ApplyRenownFeats | 0.08 | AddFactor | true
16. dots_khand_tariff_income | battania | ApplyTariffIncomeFeats | -0.10 | AddFactor | false
17. dots_khand_food_consumption | battania | ApplyFoodConsumptionFeats | -0.10 | AddFactor | true
18. dots_khand_party_size | battania | ApplyPartySizeFeats | 0.05 | AddFactor | true
19. dots_harad_morale | aserai | ApplyMoraleFeats | 5 | Add | true
20. dots_harad_food_consumption | aserai | ApplyFoodConsumptionFeats | -0.15 | AddFactor | true
21. dots_harad_raid_damage | aserai | ApplyRaidDamageFeats | 0.15 | AddFactor | true
22. dots_harad_army_influence_cost | aserai | ApplyArmyInfluenceCost | 0.15 | AddFactor | false
23. dots_rhun_loyalty | khuzait | ApplyLoyaltyFeats | -0.5 | Add | false
24. dots_rhun_raid_damage | khuzait | ApplyRaidDamageFeats | 0.15 | AddFactor | true
...
4. **XSLT culture feat attachment.** The 13 XSLT-culture feats (dale/khand/harad/rhun) live in `Culture[@id='sturgia'|'battania'|'aserai'|'khuzait']/cultural_feats` templates in spcultures.xslt. Verify each template APPENDS via `<xsl:apply-templates select="@*|node()"/>` BEFORE the new `<feat/>` lines (preserving vanilla + prior DOTS terrain feats), and that the parent `Culture[@id='X']` template does NOT exclude `cultural_feats` from its passthrough (which would prevent the sub-template firing). A dropped passthrough = silently deleted vanilla feats. CONFIRM or DISPUTE.

5. **Register-id ↔ XML-id exact match.** Every `Register("dots_X")` string must EXACTLY equal a `<feat id="dots_X"/>`. A typo = dead feat. Cross-check all 24. CONFIRM or DISPUTE.
...
- The vanilla `DefaultSettlementLoyaltyModel.CalculateLoyaltyChange` (or whichever method DotsSettlementLoyaltyModel overrides) to confirm a negative culture loyalty add is summed into the daily loyalty change and won't underflow/assert.

### 2. Service routing audit
For each of the 24 feats, confirm the HasFeat check is in the CORRECT Apply* method (matching its axis), and that each Apply* method used is actually invoked by a live `Dots*Model` GameModel (ApplySmithingFeats->DotsSmithingModel, ApplyTariffIncomeFeats->DotsClanFinanceModel, ApplyRaidDamageFeats->DotsRaidModel, ApplyFoodConsumptionFeats->DotsFoodConsumptionModel, ApplyVolunteerRespawnFeats->DotsVolunteerModel, ApplyArmyInfluenceCost->DotsArmyManagementModel, ApplyConstructionSpeedFeats->DotsBuildingConstructionModel, ApplyRenownFeats->DotsBattleRewardModel, ApplyLoyaltyFeats->DotsSettlementLoyaltyModel, ApplyMoraleFeats->DotsPartyMoraleModel, ApplyPartySizeFeats->DotsPartySizeModel). If ANY method is never called by a model, every feat routed to it is DEAD — flag HIGH.
...
- Every feat string-id in DotsCulturalFeats.cs Register(...) appears in exactly one culture's <cultural_feats> block (correct file + correct culture).
- Every factions.json bonus line is keyed {=dots_faction_...} and the key exists in dots_module_strings.xml.
