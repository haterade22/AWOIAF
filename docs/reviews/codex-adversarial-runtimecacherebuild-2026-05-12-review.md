...
- Reviewed required DOTS files and new tests: DONE
- `dotnet test DOTS.Tests --no-restore`: BLOCKED by sandbox. Even with `DOTNET_CLI_HOME` redirected into the workspace, MSBuild failed on `C:\Users\mikew\AppData\Local\Microsoft SDKs` access.
...
- `DOTS.Tests/Features/EditorCacheRebuild/RuntimeCacheRebuildServiceTests.cs`
...
5. **Neighbor count assumption: CONFIRMED.** Vanilla `AddNeighbor` writes both `settlement1 -> settlement2` and `settlement2 -> settlement1`, and `Serialize` writes the sum of every neighbor list. DOTS's `expectedNeighborPairs * 2` check matches v1.3.15.
...
7. **MCM lambda static-ness: DISPUTED.**
   - `IoC.Resolve` before `IoC.Configure()` would throw through the lambda and be surfaced by the catch at `DotsSettings.cs:452-461`.
...
File: `DOTS.Tests/Features/EditorCacheRebuild/RuntimeCacheRebuildServiceTests.cs:195`
