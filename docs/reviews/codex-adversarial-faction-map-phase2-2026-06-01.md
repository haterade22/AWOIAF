...
Scope: `Main/_Module/ModuleData/factionmap/factions.json`, `dots_module_strings.xml`, faction-map localization/display path, `DotsCulturalFeats.cs`, XSLT-wrapped vanilla cultural feats, and troop-name cross-checks.
...
Automated check: 599 unique `{=...}` tokens in `factions.json`; all 599 have matching `<string id="...">` entries in `dots_module_strings.xml`.

| key | result |
|---|---|
| `dots_faction_stewardship_of_gondor_name` | MATCHED |
| `dots_faction_stewardship_of_gondor_perk_2_desc` | MATCHED |
| `dots_faction_dominion_of_mordor_bonus_4` | MATCHED |
| `dots_faction_dominion_of_isengard_bonus_7` | MATCHED |
| `dots_faction_overlordship_of_dol_guldur_perk_2_name` | MATCHED |
| `dots_faction_kingdom_of_rohan_bonus_6` | MATCHED |
| `dots_faction_difficulty_5` | MATCHED in XML; not a JSON token because it is produced by `FormatDifficultyText` |
| `dots_faction_havens_of_umbar_special_unit_0_name` | MISSING / not a live JSON key; the live key is `dots_faction_havens_of_umbar_su_0_name` |
| `dots_faction_kingdom_of_imladris_weakness_2` | MATCHED |
| `dots_faction_clans_of_dunland_strength_3` | MATCHED |

## Known Suspect Verdicts

1. **Content accuracy vs. shipped cultural feats: CONFIRMED.** Most DOTS custom feats are represented, including the explicit Isengard, Dol Guldur, Mordor, and Rohan checks in the prompt. However, four village notable-count feats are not mentioned, Erebor's description overstates the smithing feat, and inherited vanilla-feat content for Dale/Dunland/Khand/Harad/Rhun contains false or partial claims.

2. **XSLT-wrapped culture feat inheritance: CONFIRMED.** Rohan is correct as DOTS-only because `spcultures.xslt:858-865` replaces vlandia feats. Dunland and Khand inherit Battanian feats, but the page misstates Battanian forest/construction effects. Dale inherits Sturgian grain/army/decision-penalty feats, but the page claims Sturgian forest/winter effects that do not exist. Harad and Rhun omit inherited negatives.
...
4. **JSON ↔ XML key alignment: DISPUTED for live keys.** Every live JSON token has an XML entry. One requested spot-check key, `dots_faction_havens_of_umbar_special_unit_0_name`, is not a live key because the data uses `_su_` abbreviations; that is a naming-convention issue, not a missing live XML entry.
...
6. **String token escaping safety: DISPUTED.** `factions.json` and `dots_module_strings.xml` decode as UTF-8, contain 37 U+2212 minus signs each, and `dots_module_strings.xml` parses as XML. `Select-String '[&<>]' factions.json` returned no matches. The harvester escapes `&`, `<`, `>`, and `"`.
...
[HIGH] Main/Features/FactionMap/FactionDisplayHelper.cs:92 — Localization bypass — Hover tooltips can display raw `{=dots_faction_...}Default` names because `PolygonWidget.cs:185` stores `faction.Name` directly, `FactionHoverService.cs:32` returns it as `FactionName`, and `ShowHoverTooltip` passes `change.FactionName` to `TooltipProperty` without `Localize`. Fix by localizing the hover title at the same boundary as selected faction content, while preserving raw-key comparison if needed for color lookup.
...
[MEDIUM] Main/_Module/ModuleData/factionmap/factions.json:454 — Content accuracy — Erebor's description says "Master Smiths halve forge costs", but `DotsCulturalFeats.cs:432-435` initializes Master Smiths as "Smithing energy cost reduced by 30%" with `EffectBonus = -0.3f`. The perk and bonus rows are correct; the description is not. Fix the description to "reduces smithing energy by 30%" or equivalent.
