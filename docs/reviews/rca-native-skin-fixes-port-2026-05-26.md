...
**Feature:** NativeSkinFixes adopt + port into DOTS (v1.4.5, in-repo, pattern-scanning)
...
**Generalisation: when porting native (C++) code with hot-path execution into DOTS, treat the port as a fresh implementation for audit purposes, not a "lift and shift" where the upstream is trusted.** Specifically:
...
**Rule:** when porting native (C++) code from an upstream mod into DOTS, do NOT rely on "the upstream worked." Audit hot-path logging, SEH filter breadth, atomic counters, and lock usage as if writing from scratch.
...
**How to apply:** every `__try`/`__except` block in DOTS-vendored C++ code must specify the expected exception class. Default-broad is rejected at review time.
...
**How to apply:** every DOTS feature with a degraded/partial-success state must have TWO localization keys, one per state, asserted distinct in tests.
...
- `dotnet build Main/DOTS.csproj` — 0 errors, 1 unrelated warning
- `dotnet test DOTS.Tests --filter "FullyQualifiedName~NativeSkinFixes"` — 10/10 pass (8 original + 2 new for the distinct banner contract)
...
- LOW UX fix: new `DegradedMessageKey` + `DegradedMessageDefault` in `NativeSkinFixesInstaller`; new `dots_nativeskinfixes_degraded` entry in `dots_module_strings.xml`; new test `DegradedMessageDefault_IsDistinctFromLoadedDefault` asserts the contract.
