...
This wasn't documented anywhere in DOTS rules or in any feedback memory until this commit, because EquipPresets is the first feature in DOTS that uses this pattern. (`FiefManagement` / `CareerSystem` are full-screen replacements; the CompanionTactics overlays attach to `MissionScreen` which has different layer semantics.)
