...
- Main/Features/MixedFormations/MixedFormationsSettingsProvider.cs -- wraps DotsSettings.Instance for testability
...
- Main/Features/DotsSettings.cs -- the appended `Battle Tactics/Mixed Formations` group at the bottom (4 settings)
...
- DOTS.Tests/Features/MixedFormations/{LayoutPositionerTests, FormationLayoutServiceTests}.cs -- 36 unit tests
...
9. NEW SUSPECT -- DotsSettings PROPERTIES INSERTED BY MIXEDFORMATIONS SESSION FOR PARALLEL WIP. `DotsSettings.cs` was extended with `Battle Tactics/Smart Cavalry` group (6 properties) and `Fief Management` group (3 properties) to unblock parallel SmartCavalryAI and FiefManagement sessions. CONFIRM:
...
- Main/Features/DotsSettings.cs -- the appended `Battle Tactics/Mixed Formations` group, the `Battle Tactics/Smart Cavalry` group, the `Fief Management` group
- Main/IoC.cs -- the line `MixedFormationsIoC.RegisterMixedFormationsFeature(container)` and the `using DOTS.Features.MixedFormations;` import
- Main/SubModule.cs -- the line `_harmony.PatchCategory("Patch30_MixedFormations")` in `OnSubModuleLoad`, the line `mission.AddMissionBehavior(new MixedFormationsMissionBehavior())` in `OnMissionBehaviorInitialize`, and the `using DOTS.Features.MixedFormations.Hooks;` import

### Test Files

- DOTS.Tests/Features/MixedFormations/LayoutPositionerTests.cs (11 tests)
- DOTS.Tests/Features/MixedFormations/FormationLayoutServiceTests.cs (25 tests)
...
### DotsSettings CROSS-REFERENCE

- Verify `EnableMixedFormations` (bool, default true), `MixedFormationsDefaultLayout` (int 0-3, default 0), `MixedFormationsCycleHotkey` (string, default "L"), `MixedFormationsDebug` (bool, default false) are present in DotsSettings.cs with correct attributes (SettingPropertyBool, SettingPropertyInteger, SettingPropertyText, SettingPropertyGroup with GroupOrder=21).
- Verify the dropped settings (`InfantryRowDepth`, `RangedRowDepth`) are NOT in DotsSettings -- the original developer's module had them as dead config; per `feedback_user_facing_promise_must_match_code.md` they were removed on port. Confirm the audit was thorough.
...
- Did you paste code blocks from both DOTS source and vanilla decompiled source?
