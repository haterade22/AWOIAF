...
**Evidence:** `Popup.GreenButton` and `Popup.GreenButton.Text` are not defined in any `Native/`, `SandBox/`, or `SandBoxCore/` brush XML in 1.4.5. They are also not in DOTS's own `Main/_Module/GUI/Brushes/*.xml`. The nearest valid vanilla green-button brush is `ButtonGreenBrush` in `Native/GUI/Brushes/Brush.xml`. `Popup.Frame` (line 370) exists, confirming the developer was using the `Popup.*` family — `GreenButton` was simply missing.
...
**File:** `Main/_Module/ModuleData/dots_module_strings.xml:836-844` (new keys)
**Evidence:** `dots_career_switch`, `dots_career_switched_open_screen`, `dots_career_switch_title`, `dots_career_switch_choose`, `dots_career_switch_subtitle`, `dots_career_choice_while_active`, `dots_career_choice_passive_tooltip`, `dots_career_choice_keystone_tooltip` are in the English source but absent from all 12 `Languages/<lang>/std_dots_module_strings.xml` files.
**Runtime impact:** Non-English players see raw `{=dots_career_switch}I wish to discuss my career path.` text (engine fallback to the post-`}` default). Functional but cosmetically broken.
...
4. Re-verify: `dotnet test DOTS.Tests` + `./build.ps1 -RunTests`.
