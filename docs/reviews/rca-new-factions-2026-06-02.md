...
diplomacy, ~50 settlements in the external `DOTS_Map` module, recruitment, faction map, ~130 lords/heroes,
...
|---|-----|-----|----------|------------|-------------------|
| 1 | HIGH | `village_type="VillageType.cattle_range"` on 7 new villages — `cattle_range` is not a registered v1.4.5 VillageType (it's `cattle_farm`). Null VillageType → NRE in `Village.UpdateTotalProduction`. | Invalid engine-entity id from plausible naming | I picked `cattle_range` by plausible name; never verified against `DefaultVillageTypes`. `validate_moduledata.py` does NOT validate `village_type` ids AND does not read the external `DOTS_Map` settlements at all. | **Added `VALID_VILLAGE_TYPES` guard** in `tools/assign_orc_village_types.py` (`fief_types` raises on an unregistered id). Memory: verify VillageType stringIds against the engine, never plausible naming. |
| 2 | HIGH | `DOTS_Map/SubModule.xml` missing `<DependedModule Id="DOTS"/>` — settlements reference `Culture.goblin/mistymountainorcs/rivendell` + `Faction.clan_*` defined in DOTS Main; load order unsafe. | Cross-module data dependency not declared | Authored external-module data referencing DOTS-Main entities but didn't update DOTS_Map's module manifest. **REPEAT** of the rule established in `rca-bandit-management-2026-05-27` / `feedback_cross_module_data_dependency_declaration`. | **Added `<DependedModule Id="DOTS"/>` + `<DependedModuleMetadata id="DOTS" order="LoadBeforeThis"/>`.** Repeat-offender: run the cross-module dependency check during AUTHORING (not just review) whenever editing `DOTS_Map/ModuleData/*` that references Main entities. |
...
| 4 | MED | Goblin kingdom lord region code is `GB` in the generator but `lord_GT*` in `dots_new_factions_layout.json`; layout also documented only 2 goblin clans vs 5 generated. | Documentation/source drift | Changed the lord region code without syncing the layout JSON (which is documentation-only; no script reads its `ruler_hero`/`clans`). | Synced layout: goblin `ruler_hero`/clan `leader_hero` → `lord_GB*`, added `clan_goblin_3..5`. |
...
Findings 1 and 2 share one root: **data authored for the external `DOTS_Map` module was not cross-checked
...
*repo* ModuleData, not the external `DOTS_Map`, and (b) has no VillageType-validity or DependedModule
...
is to apply the cross-module-dependency check while editing `DOTS_Map/ModuleData/*`, not only in review.
...
2. **Cross-module data dependency (REPEAT).** Already codified as
   ...
   `DOTS_Map/ModuleData/*` that references DOTS-Main `Culture.`/`Faction.`/`PartyTemplate.` ids — add the
   `<DependedModule Id="DOTS"/>` in the same edit.
...
| **W1** | **MED** | **Completeness workflow** | **`execution/alignment.json` had no entry for the 4 new kingdoms** → `AlignmentService.GetKingdomSide` returns `Neutral` → `AreEnemyAlignments`=always-true + `AreSameAlignment`=always-false → `DotsExecutionRelationModel` execution-relation penalties mis-scored AND the `DiplomacyService.IsWarAllowed` same-alignment war-block backstop silently disabled for goblin/mistymountainorcs/bluecraig/lindon. | Kingdom-enumerating config not updated for new factions | Authored kingdoms/cultures/diplomacy/faction-map/recruitment but missed `alignment.json` — a SEPARATE kingdom-enumerating config that neither the deep-review nor Codex enumerated. | Added orcs=`evil`, lindon=`free`. **Generalize the existing `feedback_faction_map_update_with_cultural_feats` rule: when adding a faction, update EVERY kingdom/culture-enumerating config — alignment.json included.** |
...
| C4 | LOW | Codex | `dots_new_factions_layout.json` listed only `clan_bluecraig_1/_2` vs live `_1..5`. | Doc/source drift | Deep-review synced the goblin clans but the same fix wasn't applied to bluecraig. | Added `clan_bluecraig_3..5`. |
...
- **"Add a faction" means update EVERY kingdom/culture-enumerating config.** W1 (`alignment.json`) is the
  ...
  `dots_spkingdoms.xml`, `dots_spcultures.xml`, `clans.xml`, `lords.xml`, `heroes.xml`, `diplomacy.json`,
...
education system) search: `dots_child_equipment_templates.xml` (IsChildEquipmentTemplate),
`dots_lord_template_equipment.xml` (IsLordTemplate + IsTeenagerEquipmentTemplate), and
`dots_education_equipment_templates.xml` (childhood-education events). The clone pipeline produced
...
had **no entity in the worldmap scene** (`DOTS_Map/SceneObj/Main_map/scene.xscene` — 0 refs; every other new
...
removed the orphan from the live `settlements.xml` (backup) + `dots_new_factions_layout.json`; 0 settlements
