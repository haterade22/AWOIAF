...
- `CLAUDE.md` — DOTS project rules, especially "Critical Rules", "Adapter Pattern (ADR-007)", "Service-Locator anti-pattern", "Config Providers MUST Validate"
...
- `Main/Features/DotsSettings.cs` (lines around `RebuildDistanceCacheAction`) — MCM button entry point
...
- `DOTS.Tests/Features/EditorCacheRebuild/RuntimeCacheRebuildServiceTests.cs` — new tests
...
Walk these specific hypotheses. For each, decompile vanilla code, read DOTS source, decide CONFIRMED / DISPUTED / NEEDS_INVESTIGATION. Include line numbers when CONFIRMED.
...
**Modified:**
- `Main/Features/DotsSettings.cs` (MCM button addition)
...
**Tests:**
- `DOTS.Tests/Features/EditorCacheRebuild/RuntimeCacheRebuildServiceTests.cs` (11 new tests)
...
Evidence: <quote vanilla source if relevant; quote DOTS source>
