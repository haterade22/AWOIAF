...
DOTS ID CHEATSHEET (prevents false positives from ID confusion):
...
Using the code from Section 1, answer:
  ...
  b) {specific question about how DOTS interacts with vanilla}
...
-- Culture IDs: check against dots_spcultures.xml + spcultures.xslt
-- Kingdom IDs: check against DOTS_spkingdoms.xml
...
- Garrison wage gate found by comparing DOTS vs vanilla IsGarrison (TroopProgression)
FAILURES to avoid:
...
**What happened:** Codex called `characterObject.IsMounted` in DotsPartyTroopUpgradeModel a bug, claiming it should be `upgradeTarget.IsMounted`. Vanilla Bannerlord's `KhuzaitRecruitUpgradeFeat` uses the exact same `characterObject.IsMounted` check.
...
**Review:** BannerColorPersistence (2026-04-05)
**What happened:** Codex flagged global clan color scope as a "regression." In DOTS's LOTR setting, global scope is likely intentional — each faction keeps its iconic colors regardless of kingdom. Codex didn't consider this.
...
**What worked:** DotsPregnancyModel fully reimplements vanilla. Codex compared the math and found the human config values (200/195) produce ~56% higher fertility than vanilla's curve. This led to discovering that the config was intentional (Dunedain) but docs/tests were stale.
...
**What happened:** Codex assumed `empire`=Rohan and `vlandia`=Arthedain, then flagged phase-1 war pairs as wrong. Actual mapping: `empire`=Dunland, `vlandia`=Rohan. The config was correct all along. False positive from not having the DOTS kingdom→LOTR mapping.
**Prevention:** Include "DOTS KINGDOM MAPPING" reference block in every prompt. Prevents ID-to-faction confusion.
...
- [ ] Cross-config coverage: when feature X reads from config A and config B, verify every entry in A has a corresponding entry in B (e.g., every CC culture in cultures.json has at least one career in dots_careers.xml). Review #24 found a crash from this gap.
