...
3. After every Edit that left the build in a temporarily-failing state (even for legitimate reasons — e.g., FiefManagementGameState ctor signature drift in a parallel session was intermixed with my changes), an external process scanned the build output and:
   ...
   - Auto-commented `using DOTS.Features.CompanionTactics.*` directives in `SubModule.cs` and `IoC.cs`
...
- `Main/Features/DotsSettings.cs` settings are present at GroupOrder 27/28/29.
...
- `Main/SubModule.cs:67-70` — `using DOTS.Features.CompanionTactics.*` directives COMMENTED OUT.
...
- `Main/DOTS.csproj:70` — `<Compile Remove="Features\CompanionTactics\**\*.cs" />` ACTIVE (excludes the feature from the compile).
- `DOTS.Tests/DOTS.Tests.csproj:36` — same exclusion ACTIVE.
...
1. **NEW RULE in `.claude/rules/harness-facts.md`** — document the parallel-port build hook behavior:
   ...
   > - `using DOTS.Features.<Feature>.*` directives in `SubModule.cs` / `IoC.cs` get re-commented
   ...
   > - Run the build from `Main/DOTS.csproj` (not the test project) with `-p:DisableModuleCopy=true`
