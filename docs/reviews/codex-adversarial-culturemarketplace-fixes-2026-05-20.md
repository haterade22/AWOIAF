...
Review basis: DOTS source in this workspace, the prior CultureMarketplace review, the fix prompt, installed `Alliance.Wargs` item XML, and installed Bannerlord v1.3.15 source material generated through `tools/dots-src.ps1`. I did not use `E:\Decompiled_Bannerlord`.

Tests: attempted `dotnet test DOTS.Tests --filter "FullyQualifiedName~CultureMarketplace"`. The run was blocked by sandbox/SDK filesystem access while MSBuild tried to read `C:\Users\mikew\AppData\Local\Microsoft SDKs`. No test results are claimed.
...
Installed v1.3.15 source, generated through `pwsh tools/dots-src.ps1 path TaleWorlds.ObjectSystem.MBObjectManager`:

`C:\Users\mikew\.dots-src\v1.3.15\TaleWorlds.ObjectSystem.MBObjectManager.cs:75`
...
| Culture ID | Validity |
| --- | --- |
| `isengard` | Valid DOTS custom culture; appears in DOTS XML, e.g. `cc_body_properties.xml:126` and Isengard clan rows. |
| `mordor` | Valid DOTS custom culture; appears in DOTS XML, e.g. `dots_careers.xml:83`. |
| `gundabad` | Valid DOTS custom culture; appears in DOTS XML, e.g. `cc_body_properties.xml:131`. |
| `dolguldur` | Valid DOTS custom culture; appears in DOTS XML, e.g. `cc_body_properties.xml:136`. |
