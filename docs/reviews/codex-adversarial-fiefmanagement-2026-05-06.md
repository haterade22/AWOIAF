...
Answer: the constructor is covered by the swap only in the clean map case, but restoring immediately after construction is not enough. Vanilla `TownManagementReserveControlVM.ExecuteConfirm()` later reads `Settlement.CurrentSettlement.Town`, after DOTS has restored the field.
...
Answer: the target exists, but the Manage path never reaches it because DOTS directly calls `PushState(new FiefManagementGameState(...))` instead of creating/registering the state first.
...
- `FiefManagementDebug`: mismatch. Hint promises HUD messages, but consumers only call `IModLogger`; `FileLogger` writes to `Logs/dots_debug_*.log`.
...
| 7 | P3 | `Main/Features/DotsSettings.cs:285-286` | The debug hint promises HUD diagnostics, but the only debug consumers write to `IModLogger`/file. | Change the hint or display debug diagnostics via `InformationManager.DisplayMessage`. |
