...
Review scope: `Main/Features/QuickActions/**/*.cs`, QuickActions adapters, `DotsSettings.cs`, `IoC.cs`, `SubModule.cs`, and `DOTS.Tests/Features/QuickActions/*.cs`.
...
The tests do not catch this because they mock `IInventoryVMAdapter.TrySellItem` as a boolean and do not expose stack amount (`DOTS.Tests/Features/QuickActions/QuickActionsServiceTests.cs:49`).
...
Walkthrough: vanilla `ExecuteSellAllItems()` calls `TransferAll(false)`. If the other party is a settlement, vanilla enters `TransferAllForSettlement`, tracks `LeftInventoryOwnerGold`, and transfers units while accumulated sale value remains under available settlement gold. The DOTS lambda does not enter `TransferAllForSettlement`; it invokes `ProcessSellItem(item, true)` once per matching row.
...
### Scenario D: Old save missing `DOTS_IsInventorySearchAvailable`, MCM disabled
...
|---|---------|------------------------------|----------|
| 1 | `MultiSelectionInquiryData` callback signatures | DISPUTED | v1.3.15 constructor takes both `Action<List<InquiryElement>> affirmativeAction` and `Action<List<InquiryElement>> negativeAction`; DOTS passes `chosen => { ... }` and `_ => { }` at `QuickActionsService.cs:83-89`. |
...
| 4 | Modifier preservation in unequip | DISPUTED | DOTS calls `roster.AddToCounts(element, 1)` at `PlayerEquipmentAdapter.cs:42`, where `element` is `EquipmentElement`. v1.3.15 `ItemRoster.AddToCounts(EquipmentElement,int)` stores an `ItemRosterElement(rosterElement, 0)` and preserves `ItemModifier`; the `ItemObject` overload is the one that creates `new EquipmentElement(item)` and would discard modifiers. |
