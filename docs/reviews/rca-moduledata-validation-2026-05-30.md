...
**Feature:** `tools/validate_moduledata.py` + `tools/dots_schema.py` + `tools/schemas/*.json` — a schema-driven cross-reference + validation engine for DOTS ModuleData XML, adopted (idea-only, MIT) from `TheOldRealms/TOR_Tools`.
...
| 2 | HIGH | Equipment schema glob `dots_equipment_sets_*.xml` — `dots_lord_template_equipment.xml`, `dots_child_equipment_templates.xml`, `dots_education_equipment_templates.xml` were outside ALL schemas → no dup-id / civilian-type check on them | Partial-enumeration | Built the glob from the filenames visible in a truncated `Glob` listing, not from the full `equipmentsets/` directory contents. | Broadened to `equipmentsets/*.xml`. Schema `applies_to` should glob the directory broadly unless a file must be excluded for cause. |
| 3 | MED | Dead fields: `Schema.id_space`, `Registries.dots_npc_def_files` (cost a full rglob), unused `Schema.description` | Aspirational plumbing | Ported TOR's schema-model richness without wiring every field to a consumer in the same change. | Removed `id_space` + `dots_npc_def_files`; gave `description` a consumer (CLI prints it per schema). Repeat of `feedback_no_aspirational_enum_values`. |
...
Findings #1 and #2 are the same mistake twice: **generalising a rule from a partial sample instead of enumerating from the full source of truth.** #1 derived ref shapes from one troop file; #2 derived a glob from a truncated file listing. This is a *repeat-offender* pattern in DOTS — it is exactly what these existing memories already warn about:
...
|---|-----|-----|-------------------------------|-----|
| C1 | HIGH | Culture registry polluted: `_CULTURE_DEF_RE` rglob'd the whole tree, so `<Culture id="empire_w">` in `dots_careers.xml` (career-eligibility groups) + `dale` in `cc_body_properties.xml` + comment placeholders became "valid cultures" → `UNKNOWN_CULTURE` false-negative (a `Culture.dale` typo would pass) | Both the tooling and data-flow agents inspected the culture registry but ASSUMED `<Culture id=>` only appears in culture-definition files. Neither grepped the element across all ModuleData. | Build cultures from authoritative files only (`dots_spcultures.xml` + vanilla `spcultures.xml`) via new `_scan_files`; registry 41→36, all 26 real refs still resolve. |
| C2 | HIGH | NPCCharacter dup-id/enum checks missed 248 defs in `dots_wanderers.xml`, `dots_education_character_templates.xml`, `named_companions/` | I noticed the gap MYSELF during deep-review and *deferred* it (deleted `dots_npc_def_files` instead of using it), under-weighting the risk. The completeness agent flagged test gaps, not file-coverage gaps. | Broadened npccharacter schema `applies_to` to all NPC-def files + a description note mandating future files be added. |
...
| C4 | MED | Civilian rule under-matched: only ids with `_civ`, only the first `<EquipmentSet>`. Real `child_template_*` civilian rosters (114/114 tagged) were uncovered. | The tooling agent reasoned about the rule abstractly ("low risk in practice"); Codex READ `dots_child_equipment_templates.xml` + `dots_lord_template_equipment.xml` and found real civilian rosters without `_civ`. | Detect `child_template`/`_civ` (demonstrably civilian), check ALL EquipmentSets; **exclude** `child_education_*` (0/784 tagged — unknown convention, would be 784 false positives). Documented scope gap. |
...
| D4 | LOW | No automated test of the MCP server (manual/in-process only). | Added `tools/tests/test_dots_mcp_server.py` (skips if `mcp` SDK absent; install-independent assertions). |
