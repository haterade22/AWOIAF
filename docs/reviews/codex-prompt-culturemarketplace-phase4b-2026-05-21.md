...
- Does the behavior delegation pattern match DOTS precedents (SpecialResources, SettlementGuards)? Or is there a closer existing pattern that would have been cleaner (e.g., having the adapter expose a `WithSettlement(...)` opaque token)?
