...
LOTRAOM 1.2.12 ships a `StartingEquipmentGold` feature that grants the player **starting funds** at character creation finalize and **persists the youth option's equipment roster** onto the player hero. DOTS (1.3.15) had only half of this:
...
Result: every DOTS campaign started the player with vanilla default 1000 denars and vanilla default starting equipment regardless of culture or youth option. This issue ports the LOTRAOM feature to DOTS 1.3.15.
...
| `Main/_Module/ModuleData/charactercreation/youth_menu.json` | `dots_youth_sturgia_1` `title_type` retainer→guard (no vanilla `sturgia_retainer` roster) |
...
This work passed the full DOTS completion workflow Phases 1–3:

- **Phase 1** — `/verify` build + tests green
- **Phase 1** — `/deep-review` (5 parallel Claude agents). Surfaced 4 fixes, all applied this session: `empire`/Dunland missing from XML; `dots_youth_sturgia_1` title_type retainer→guard; `CareerMenuService:227` route through helper; defensive `DeadBattleEquipment` guard
