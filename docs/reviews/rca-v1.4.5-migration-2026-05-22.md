...
The DOTS repo's git log shows `ChildCreatorAdapter.cs` has been in the codebase since at least late 2025. It went through prior reviews. None caught the IsFemale issue because:
...
3. The reflection helper itself (`DOTS.Core.Infrastructure.ReflectionHelper.SetFieldValue`) doesn't validate that the field exists on the runtime instance type — it relies on the type passed in (`BasicCharacterObject`) being correct.

## Preventive actions

### Code fix (THIS COMMIT)
- `Main/Adapters/ChildCreatorAdapter.cs:30` — replaced reflection with `hero.IsFemale = isFemale`. Removed dead imports (`DOTS.Core.Infrastructure`, `TaleWorlds.CampaignSystem.Extensions`, `TaleWorlds.Library`). Build green, 2,323/2,325 tests pass.
...
### Open question
- Are there OTHER reflection sites in DOTS that target `<PropertyName>k__BackingField` on a base class whose subclass overrides the property? Out of scope for this RCA; suggested follow-up: grep for `ReflectionHelper.SetFieldValue.*BackingField` and verify each against its runtime instance type's override chain.
...
|---|---|---|---|---|---|
| 5 | 🔥 P1 | `Main/_Module/SubModule.xml:10-15` — DependedModules list omits `DOTS.Dependencies`. After restoring the project from git `0b16cca`, the module-load-order dependency edge between DOTS and DOTS.Dependencies was lost. In a clean launcher profile, enabling DOTS does NOT auto-enable DOTS.Dependencies before Native, so the pre-Native Harmony/UIExtender setup is skipped — the entire purpose of the Dependencies module is defeated. | XML dependency graph gap | All 5 Claude agents reviewed the changeset, but none cross-checked the SubModule.xml dependency list against the actual restored Dependencies project. The Standards agent looked at DOTS's own conventions; the Completeness agent flagged IoC + tests but didn't trace module-load order; no agent specifically traced *which projects must list which others as DependedModule*. | Fixed in this commit: added `<DependedModule Id="DOTS.Dependencies" />` + `<DependedModuleMetadata id="DOTS.Dependencies" order="LoadBeforeThis" />`. Process change: **add to `/deep-review` Agent 4 (Completeness) prompt: when reviewing a CampaignBehavior/SubModule that depends on another DOTS-owned module's runtime presence, verify the dependency edge exists in SubModule.xml.** Not a generic Bannerlord lesson — specific to DOTS's modular architecture. |
...
| Agent 4 — Completeness | ❌ | Looked at tests + docs + IoC, not module-load dependencies. Did NOT check that DOTS lists DOTS.Dependencies as a DependedModule. |
...
4. (Codex) SubModule.xml missing DOTS.Dependencies dep edge — P1
