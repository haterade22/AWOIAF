...
This is a focused review of the SECOND change to CustomBattles in this session. The original feature (custom battle DOTS factions/commanders/troops) was reviewed at length in `docs/archive/codex-reviews-2026-04/codex-adversarial-custombattles-2026-04-05.md`; the filter+cap was reviewed at `docs/reviews/codex-adversarial-custombattles-filter-cap-2026-05-04.md`. Do NOT re-review unrelated CustomBattles surface.
...
NOTE: "rohan" is NOT a valid ID — Rohan uses "vlandia". "dol_guldur" is NOT valid — use "dolguldur". "empire" means Dunland in DOTS.
...
1. **Prefix `bool` return semantics.** The new Prefix returns `bool` to skip the vanilla body when `SelectedItem` is null. Harmony interprets `false` as "skip original AND skip postfix-modifies-result". Verify: are there ANY postfixes on `CustomBattleSideVM.OnCharacterSelection` (vanilla mods, BUTR, MCM, or other DOTS patches) that we'd skip silently? Grep DOTS source. If any exist, the Prefix should return `true` after manually doing the null-skip work, not `false`.

2. **`Refresh` overload resolution ambiguity.** DOTS's call is `selector.Refresh(items, 0, existingOnChange)` where `items` is `IEnumerable<CharacterItemVM>`. There are three Refresh overloads on `SelectorVM<T>`: `IEnumerable<string>`, `IEnumerable<TextObject>`, and `IEnumerable<T>`. C# overload resolution should pick the third (T = `CharacterItemVM`), but verify the compiled IL targets the correct overload. (Specifically: would a future TaleWorlds DLL update change the overload set in a way that ambiguates this call?)
...
If any signature differs from what DOTS assumes, flag it as INCOMPATIBLE.
...
**Scenario C — User clicks a faction that has zero DOTS lords (data mismatch):**
