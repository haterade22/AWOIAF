...
- `Main/_Module/ModuleData/career_system/dots_ability_tuning.xml` -- the new `<Global cooldown_seconds="30" />` element.
- `Main/_Module/ModuleData/career_system/dots_careers.xml` -- 50 careers; `charge_type` and `max_charge` attrs were stripped.
- `Main/_Module/ModuleData/career_system/dots_career_choices.xml` -- still contains 98 `property="MaxCharge"` mutation entries (potential dead config -- see Known Suspect 1).
...
- `Main/Features/CareerSystem/Models/DotsClanTierModel.cs` -- ctor injection of ICareerPassiveService
- `Main/Features/CareerSystem/Models/DotsAgentStatCalculateModel.cs` -- ctor injection of ICareerPassiveService
- `Main/Features/CareerSystem/Models/DotsAgentApplyDamageModel.cs` -- ctor injection of ICareerPassiveService
- `Main/SubModule.cs` lines 300-318 -- GameModel construction with resolved careerPassiveService

### XML / config
- `Main/_Module/ModuleData/career_system/dots_ability_tuning.xml` -- new `<Global cooldown_seconds="30" />`
- `Main/_Module/ModuleData/career_system/dots_careers.xml` -- 50 entries with `charge_type` / `max_charge` removed
- `Main/_Module/ModuleData/career_system/dots_career_choices.xml` -- NOT changed but consumed by mutation system (98 `property="MaxCharge"` entries)
- `Main/_Module/ModuleData/career_system/dots_ability_templates.xml` -- NOT changed (still has `cooldown="0"` per-template -- now ignored)
- `Main/_Module/GUI/Prefabs/CareerSystem/AbilityHUD.xml` -- NOT changed (consumes HudVM)

### Tests
- `DOTS.Tests/Features/CareerSystem/CareerAbilityServiceTests.cs` -- NEW, 11 methods
- `DOTS.Tests/Features/CareerSystem/CareerAbilityTests.cs` -- +6 ReadyProgress01 tests (20 total)
- `DOTS.Tests/Features/CareerSystem/CareerConfigProviderTests.cs` -- +5 Global parsing tests
- `DOTS.Tests/Features/CareerSystem/Abilities/{Cavalry,Infantry,Ranged}AbilityExecutorTests.cs` -- ctor signature update
- `DOTS.Tests/Features/CareerSystem/CareerCampaignBehaviorTests.cs`, `CareerCreationHandlerTests.cs`, `CareerPassiveServiceTests.cs`, `CareerRegistryTests.cs`, `CareerScreenVMTests.cs`, `CareerSwitchServiceTests.cs`, `MutationServiceTests.cs`, `CharacterCreation/CareerMenuServiceTests.cs` -- updated for trimmed `CareerDefinition` ctor
...
For each of these scenarios, read the relevant DOTS source and answer with file:line citations:
...
1. **Cross-reference `dots_careers.xml` against the cleaned-up parser.** The parser now reads only: `id`, `display_name`, `description`, `portrait_sprite`, `ability_template_id`, `min_clan_tier`, `root_choice_id`, `EligibleCultures/Culture id=`, `ChoiceGroups/Group id=`. Verify that for every `<Career>` element in `dots_careers.xml`, ALL those required attributes are present and non-empty (except `description` and `min_clan_tier` which can default).

2. **Cross-reference `EligibleCultures/Culture id=` values across all 50 careers against the CHEATSHEET.** Flag any culture id that is not a valid DOTS/vanilla culture.

3. **Cross-reference `ChoiceGroups/Group id=` against `<ChoiceGroup id=>` declarations in `dots_career_choices.xml`.** Flag any group id referenced by a career but not defined.

4. **Cross-reference `ability_template_id` against `<AbilityTemplate id=>` in `dots_ability_templates.xml`.** Flag any orphan reference.

5. **Sprite registry cross-reference -- already known: 41 of 50 ability icons missing.** The runtime path is `CareerSystem\Abilities\<career_id>_ability` per `CareerPerkMissionBehavior.RefreshHudCache`. The 9 registered icons in `Main/_Module/GUI/DOTSSpriteData.xml` are: captain_of_osgiliath, crossbow_master, eotheod_windrider, ironguard, knight_of_belfalas, marksman_of_aldburg, ram_rider, ranger_of_ithilien, watchman_of_stangard. Issue #101 already filed. Do NOT re-flag this -- but DO confirm the runtime path matches the registered names (no path-construction bug).
