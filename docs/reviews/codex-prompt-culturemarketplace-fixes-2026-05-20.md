...
## DOTS ID CHEATSHEET (unchanged from prior review)
...
- DOTS.Tests/Features/CultureMarketplace/ItemPoolAdapterPrefixTests.cs
...
For C5's seeded warg entries: confirm `isengard,mordor,gundabad,dolguldur` are all valid DOTS culture IDs (per cheatsheet). No invalid `dol_guldur` underscore variant or `dolguldor` typo.
