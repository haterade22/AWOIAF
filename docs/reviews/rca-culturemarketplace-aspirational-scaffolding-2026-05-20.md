...
| C2 | LOTRLOME `culture=` attributes map to valid DOTS IDs | At least 5 horse harnesses use invalid `Culture.rohan` |
...
**Fix:** added boot-time per-culture pool diagnostic logging in [`CultureItemPoolService.BuildPools`](../../Main/Features/CultureMarketplace/CultureItemPoolService.cs). The next session, the user can check the DOTS log for lines like:
