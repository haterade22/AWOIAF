...
/ `CalculateWeaponLength` faithfully. Tool lives **outside the DOTS repo**; this RCA is filed here because
the central lesson (fidelity over self-consistency when porting engine code) is DOTS-relevant for all
future engine-porting work. The DOTS-specific core-5 agents (ADR-007/Harmony/IoC/MCM/ModuleData/sprites)
...
**Lesson (generalizable to DOTS):** when porting engine code, *self-consistency is not the spec — the
