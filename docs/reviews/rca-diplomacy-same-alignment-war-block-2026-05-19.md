...
| 3 | LOW | No integration test for `DotsKingdomDecisionPermissionModel.IsWarDecisionAllowedBetweenKingdoms` delegation | Test coverage | `Kingdom` is sealed — the thin-model pattern intentionally leaves boundary delegation untested. ADR-008 explicitly permits this for `Entry Points` ("Not required — Harmony/GameModel — test via game"). | None — accepted by design. Risk: if someone later adds non-trivial logic to the override body, the test gap widens. The csharp-architecture rule already says "no inline branching in GameModel overrides", which is the real guard. |
