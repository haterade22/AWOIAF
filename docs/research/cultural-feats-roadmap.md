...
The #260 faction-map rewrite exposed that several cultures have thin cultural-feat coverage (Dale/Khand had 1 DOTS feat; Harad/Rhûn 2; the new Goblin / Misty Mountain Orcs cultures 4 baseline). This roadmap enumerates lore-fitting additional feats per culture (positive + negative), classified by implementation cost, so future sessions can pick the next batch without re-researching.
...
| **E** | Extension — adds a new override method to an EXISTING `Dots*Model` (e.g. post-victory morale on `DotsPartyMoraleModel`) | ~20-40 LOC + new service `Apply*` method + tests |
| **N** | New model — a brand-new `Dots*Model` overriding a vanilla `Default*Model` not yet wrapped (prisoner take rate, sight range, persuasion, education XP) | ~80-120 LOC + IoC reg + ADR-007 adapter + tests |
...
- **Mirkwood — Thranduil's Vaults** — −10% garrison wage (garrison wage lives in `DotsPartyWageModel` in TroopProgression, not `CulturalFeatsService` — different file, but still a HasFeat add) [+]
...
Existing `Dots*Model`s with untapped methods on the same vanilla base (extend these, no new model needed):

| Existing model | Untapped methods → feat ideas |
|---|---|
| `DotsPartyMoraleModel` | `GetVictoryMoraleChange` / `GetDefeatMoraleChange` (glory/fanaticism), `GetDailyStarvationMoralePenalty`, `GetDailyNoWageMoralePenalty` (iron will) |
| `DotsSettlementMilitiaModel` | `CalculateMilitiaChange` (rate), `CalculateMilitiaSpawnRate` (melee/ranged split — Mirkwood/Lothlorien archery) |
| `DotsSmithingModel` | `GetSkillXpForSmithingInFreeBuildMode` (Noldor lore), `ResearchPointsNeedForNewPart`, `GetCraftedWeaponModifier` (master-smith quality) |
| `DotsBattleRewardModel` | `CalculateInfluenceGain` (martial nobility), `GetLootItemChancesForWinnerParties`, `CalculateGoldLossAfterDefeat` (stoic defeat) |
| `DotsClanFinanceModel` | `CalculateOwnerIncomeFromWorkshop` / `CalculateOwnerIncomeFromCaravan` / `CalculateVillageIncome` (Umbar mercantile) |
| `DotsArmyManagementModel` | `CalculateDailyCohesionChange` (disciplined host / fractious horde) |
| `DotsRaidModel` | `GetRaidLootMultiplier` (plunderers) |
| `DotsCaravanModel` | `GetInitialTradeGold`, `GetEliteCaravanSpawnChance` (maritime tradition) |
