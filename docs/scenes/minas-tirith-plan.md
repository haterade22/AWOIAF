...
## Context
Building a full Minas Tirith scene for DOTS in the Bannerlord editor. MT has 7 levels (circles) with the Citadel at top. The scene needs to support **siege** (levels 1 and 7 only, with scene transition between them), **civilian** mode (all 7 levels walkable), and look visually accurate to Tolkien's vision. KEYForce has provided MT prefabs/blocks. Sceners will execute — this plan provides the A-to-Z roadmap.
