# ADOD_Beasts — full architecture, lifecycle & DOTS port comparison

> **Purpose.** Understand the ENTIRE `ADOD_Beasts` mod end-to-end — every class, the start-to-finish runtime
> lifecycle, and *why* it was built the way it was — and place DOTS's port inside that full picture (what we
...
> (both builds), line-by-line audited against DOTS and verified against installed v1.4.5 DLLs (`dots-src`).
...
**DOTS's relationship to each is different:**
- The **elephant trample + mount-lock** is a **faithful 1-for-1 port** — but re-homed onto a **non-rideable
  creature-troop** (DOTS's `dots_war_elephant` Monster) instead of ADOD's ridden mount, per
  `feedback_nonhumanoid_creature_troop_not_mount.md`. The **howdah is not ported** (a separate future feature).
- The **wolf is not ported at all.** DOTS's **spider** solves the same "riderless non-humanoid combatant" problem
...
This is the end-to-end process. Each step notes ADOD's mechanism and DOTS's equivalent.
...
LOAD TIME
  SubModule.OnSubModuleLoad
    └─ new Harmony("mod.adodbeasts.bannerlord").PatchAll()   → applies ElephantCameraPatches + CharacterObjectPatch
        DOTS: single shared _harmony, explicit per-feature Patch calls; elephant needs ZERO patches.
...
GAME START (campaign)
  SubModule.OnGameStart
    ...
    └─ wrap existing AgentStatCalculateModel in ADODAgentStatCalculateModel  (the mount-lock + pilot buffs)
        DOTS: resolves IElephantAttackService; constructs DotsAgentStatCalculateModel (subclass, not a wrapper)
...
CAMPAIGN MAP (per day)
  ADODBeastsCampaignBehavior.WolfDailyTick
    └─ if near winter coord (Vec2(615,2000), <300f) && rand<wolfAcquireChance → "Take the Wolf" inquiry → _acquiredWolfId
        ...
        DOTS: NONE. The elephant has no campaign state; the spider is a rostered troop. No SaveDefiner needed.
...
MISSION INIT
  SubModule.OnBeforeMissionBehaviorInitialize
    └─ if (Campaign && not Arena && not AtmosphereIndoor) → mission.AddMissionBehavior(new ADODBeastsMissionLogic())
        DOTS: SubModule.OnMissionBehaviorInitialize adds ElephantMissionBehavior UNCONDITIONALLY (no arena/indoor
...
PER AGENT SPAWN  — ADODBeastsMissionLogic.OnAgentBuild(agent, banner)
    ...
    └─ DOTS: OnAgentBuild only adds the agent to a `_elephants` shadow list if its Monster.StringId==dots_war_elephant.
...
PER FRAME  — ADODBeastsMissionLogic.OnMissionTick(dt)
    ...
    └─ DOTS: ElephantMissionBehavior.OnMissionTick iterates `_elephants`, per elephant rolls TrampleChancePerTick
...
ON HIT  — ADODBeastsMissionLogic.OnAgentHit → 30% elephantHit SFX; wolf retaliation targeting
    DOTS: none (hit-SFX deferred as cosmetic; no wolf).

MISSION END / TEARDOWN
  ADOD: OnBattleEnded → clear _animals/_targets/_agentControllers, remove wolf proxy entity.
  DOTS: ElephantMissionBehavior.OnRemoveBehavior → clear `_elephants`. (OnRemoveBehavior is broader than
...
ADOD only writes the trample + howdah. DOTS makes the elephant a *Monster-swap troop* (the project's standing rule
that non-humanoid creatures are troops, never rideable mounts — mounts crash on the campaign-map party-icon
`ForceUpdateBoneFrames` path). That single re-homing is why DOTS drops the howdah, the rider camera, the
GetPower patch, and the mount-AI — they're all properties of the ridden-mount idiom DOTS doesn't use.

## 3. Subsystem-by-subsystem — the WHY + the DOTS comparison
...
- **DOTS port (verdict: HIGH faithfulness, no bugs):** every constant is 1-for-1 (`ElephantConfig.cs`); the gate +
  damage formula are extracted to a unit-tested `IElephantAttackService` (ADR-002/007); the mount-lock folds into
  the shared `DotsAgentStatCalculateModel`. **Safer than ADOD** in two places: `?.` null-guards on
  `targetMount.Monster` (ADOD had a latent NRE), and exact `Monster.StringId == "dots_war_elephant"` identity vs
  ADOD's brittle `Name.ToLower().Contains("elephant")` substring. **Crucially, DOTS moved the tick off the dead
  `OnTickAsAI` onto `MissionLogic.OnMissionTick`** (see §5) — so DOTS's trample actually fires on 1.4.5 where
  ADOD's would not. The one equivalent-but-different choice: DOTS rolls the probability *before* the distance/facing
...
- **DOTS:** zero howdah. The full line-level port spec + the 4 v1.4.5 drifts it must fix live in
...
- **DOTS spider (independent, better-fit):** a **behavior tree** (Selector: no-enemy→sleep / engage→move→bite→sleep)
...
The lifecycle skeleton (§2) plus the support classes. DOTS's relevant ports are faithful; the rest is correctly
scoped out:
- **`ADODBeastsSaveDefiner`** persists the **wolf** (`_acquiredWolfId`), NOT the elephant → **the elephant feature
  needs no SaveDefiner**, and DOTS correctly ships none. (If the wolf is ever ported, it needs its own
...
- **`ADODBeastsSettings`** (MCM): `elephantCameraDistance` (rider camera, moot for a troop) + `wolfAcquireChance`
  (wolf). DOTS uses compile-time `ElephantConfig.cs` consts; no MCM knob for the elephant.
- **`ADODBeastsMissionView`/VM**: the **wolf HP bar**, not elephant. Not ported.
- **Patches:** `ElephantCameraPatches` (rider camera pull-back — no rider in DOTS) + **`CharacterObjectPatch`**
  (adds howdah-crew power to the elephant's `GetPower` — tied to the howdah DOTS doesn't have). Both correctly
  absent.
- **`NativeHook` (EasyHook) — DEAD IMPORT.** `using NativeHook;` appears but is **never used** anywhere in the
  ...
  engine-driven Monster-swap troop (DOTS). No NativeHook port required. *(This corrects an earlier impression that
  the native hooks were load-bearing for movement; they are not — they're an unused reference.)*

## 4. Port verdict

DOTS's elephant port is **faithful where it matters and safer in several places**: the trample (constants + gates
...
DOTS's elephant is a non-rideable creature-troop, not ADOD's ridden mount + howdah. **No bugs found in the port.**

## 5. The 1.2.12 → 1.4.5 API drift catalogue (apply to any future ADOD port)

| ADOD API (1.2.12) | v1.4.5 reality | Fix |
|---|---|---|
| `AgentComponent.OnTickAsAI(float)` | **does not exist** (only `OnTick`/`OnTickParallel`) | use `OnTick(dt)` (auto-called by `Agent.Tick`). DOTS's elephant already does (`MissionLogic.OnMissionTick`); a verbatim component port would be a dead override / compile error. |
...
| `ActionIndexCache.Name` | `GetName()` | DOTS already migrated (elephant) |
...
2. **Elephant SubModule gate (low-risk refinement):** DOTS adds `ElephantMissionBehavior` to *every* mission; ADOD
