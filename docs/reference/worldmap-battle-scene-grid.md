...
map**, what the `worldmap_battle_scene_grid` texture actually is, and how to **re-author it for DOTS's
...
DOTS_Map's `SceneObj/Main_map/references.txt` references `worldmap_colorgrade_*` textures — those are the
...
## Current DOTS state (verified on disk 2026-05-31)

| Fact | Evidence |
|---|---|
| DOTS_Map ships a full custom `Main_map` (11 MB `scene.xscene` + 56 MB `terrain.bin`), **baked 2026-05-28** | `Modules/DOTS_Map/SceneObj/Main_map/` |
| 3 active modules own `Main_map`: SandBox, NavalDLC, **DOTS_Map** → DOTS_Map's map is used only if it loads **last** | filesystem scan |
...
| `Patch0_BattleScenes` is **ENABLED** (re-enabled 2026-06-01, `Main/SubModule.cs:159`) → loads DOTS's `sp_battle_scenes.xml` | grep |
...
- **Patch0 enabled (current, since 2026-06-01):** the **active** file is **DOTS's `Modules/DOTS/ModuleData/sp_battle_scenes.xml`** (covers all 0–255, 0 crash suspects). Indices 158–255 resolve to `battle_terrain_r`.
...
> (as of 2026-05-31 the *deployed DOTS* file covers all 256 indices with 0 crash suspects — but that file is
...
| Code change | **none** (Patch0 stays disabled) | re-enable `Patch0_BattleScenes` so DOTS's extended XML loads |
...
*(Scene ids above were read from DOTS's `sp_battle_scenes.xml`; the exact `terrain`/`TerrainType` of each in the
...
1. Author the grid texture **externally** at **1024×1024** ("native's size; not sure if other sizes work").
   **R channel = scene index** 0–255 — the value that must appear in a `<Scene map_indices="…">` of
   ...
   `battle_terrain_020` example list is verbatim DOTS's xml). **G channel = party entry orientation** (which side
   parties enter from).
...
4. Ensure `sp_battle_scenes.xml` covers every R-byte value the grid uses, and confirm `DOTS_Map` loads **after**
...
DOTS's `sp_battle_scenes.xml` so extended indices 158–255 resolve). The historical disabled-state notes below are
kept for context.
...
- [dots-map-settlement-naming.md](dots-map-settlement-naming.md) — DOTS_Map is a live external module, not a repo shadow
