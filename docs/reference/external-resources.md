# External Resources for DOTS

Curated, verified external references for improving DOTS (Bannerlord 1.4.5 LOTR total conversion). Compiled 2026-05-29 via web research with per-resource verification (fetched URLs; the docs hierarchy was personally re-checked to correct an official-vs-community mislabel). This is the high-value subset — not an exhaustive dump. Cite this node instead of re-searching.
...
| **[apidoc.bannerlord.com](https://apidoc.bannerlord.com/)** — *OFFICIAL API ref* | 🔎 | TaleWorlds.Core/CampaignSystem/Engine API surface. **⚠️ lags at v1.3.14** — for 1.4.5 keep using `pwsh tools/dots-src.ps1` + `/verify-bindings` (decompilation is our source of truth). |
...
- **[Game-mods semantic versioning](https://github.com/pragasette/game-mods-semver)** ✅ — *same major = save-safe; major bump = new game.* Worth formalizing as DOTS's save-compat contract (complements the `Save-compat:` commit trailer).
...
- **No published v1.4.5 API docs** — decompilation (`dots-src` / `/verify-bindings`) is the source of truth. Already handled.
...
- **No LOTR-mod-authoring resource library** — DOTS's own domain docs (`docs/ai-includes/new-culture-authoring.md`, etc.) are the asset; nothing external matches.
