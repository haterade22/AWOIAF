...
**Authoritative signatures** still come from `pwsh tools/dots-src.ps1 path <Type>` (runs `ilspycmd` on the
*installed* shipping DLLs, auto-detects version). Use the decompiled folders for *browsing*; use `dots-src` for
...
(NAudio.* — managed audio lib, editor-build only, used by the Kit's audio tooling. MinHook/`DOTS.NativeSkinFixes`
are DOTS-vendored native hooks, not shipped by TaleWorlds — see CLAUDE.md.)
...
| `SaveSystem` (2) | `SaveableTypeDefiner`, the save serializer (DOTS's `*SaveDefiner` plug in here). |
...
| `SandBox.*`, `StoryMode.*` (separate top-level, not `TaleWorlds.`) | The campaign content layer (cultures, settlements, the main story) — DOTS's XSLT targets `SandBoxCore`. |
