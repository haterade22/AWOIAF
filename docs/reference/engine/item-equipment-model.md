...
> `ItemComponent`) and equipped (`Equipment` + `EquipmentElement` + `EquipmentIndex`). DOTS authors **thousands** of
...
- **DOTS authors thousands of items** (LOTRLOME_Armory: `body_armors.xml`/`head_armors.xml`/`leg_armors.xml`/… +
  `LOTRAOM_weapons/shields/horses`). The **mount items** (spider_mount_a, dots_war_elephant) are `Type="Horse"` +
...
- **`FillFrom` slot-merge** — DOTS's career starting-equipment override `FillFrom`s a roster onto the culture default;
...
- The mount slot is `Horse`=`ArmorItemEndSlot`=10; `is_mountable="false"` on the `HorseComponent` keeps a creature mount
  from being rideable (DOTS creature-troops).
...
- DOTS gotchas: `feedback_adapter_modifier_preserving_overload`, `feedback_inventory_mutations_via_vanilla_inventorylogic`, `feedback_lotrlome_armor_cover_attributes`; `.claude/rules/moduledata-validation.md`.
