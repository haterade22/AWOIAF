...
> [adod-beasts-architecture-and-dots-port.md](../adod-beasts-architecture-and-dots-port.md) (the wolf uses this
...
`CreateHorseAgentFromRosterElements` (Mission.cs:4275). **DOTS:** this is the chokepoint `Patch45`/the spider
...
**This is the path the working ADOD wolf uses (public `SpawnMonster`), and the path DOTS's spider *should* use.**
DOTS's spider instead *reflects* `CreateAgent`+`BuildAgent` directly (`SpiderDetachedAgentSpawner`) — a hand-rolled
...
- `SpawnMonster` (Entry B) — riderless creatures (ADOD wolf; DOTS's intended spider + elephant-as-creature path).
