...
`SpawnTroop`'s `specialActionSetSuffix`, Mission.cs:4480). This is the mechanism behind DOTS's
...
DOTS's spider/elephant gate on `agent.ActionSet.IsValid` before driving actions (an unbound/invalid action set =
no animations).
