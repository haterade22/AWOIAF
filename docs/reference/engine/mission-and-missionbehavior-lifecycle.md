...
> registered, dispatched (per-frame + per-event), and torn down. **Every in-battle DOTS feature plugs in here**
...
(it has crashed DOTS twice — 3 ports in 2026-05 + the inlined `BehaviorTreeWrapper.dll` in 2026-05-24). Phase-4
confirmation: the null-cast is `Mission.cs:4610`.

## DOTS relevance
- **All in-battle DOTS behaviors** are `MissionLogic` subclasses added in `Main/SubModule.cs`
...
- `OnAgentBuild` is the per-spawn hook (DOTS elephant adds spawned elephants to its shadow list here; ADOD spawns
  the howdah here).
- `OnRemoveBehavior` (per-mission teardown) is where DOTS clears its per-mission lists (broader than `OnBattleEnded`).
