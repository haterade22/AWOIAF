...
> future DOTS usable machine. Part of the phased engine study; builds on Phase 8 (these ARE `ScriptComponentBehavior`s).
...
- **The howdah** (`ADODHowdah : UsableMachine`, `ADODHowdahStandingPoint : StandingPoint`) is the worked example —
  ...
  [howdah-crew-mechanism.md](../features/elephant/howdah-crew-mechanism.md). Any future DOTS usable machine (deployable,
...
- **Vanilla templates to copy:** `SiegeWeapon` subclasses (ballista/mangonel/ram/tower) — decompile the closest one
  for the operate/crew pattern when authoring a DOTS machine.
