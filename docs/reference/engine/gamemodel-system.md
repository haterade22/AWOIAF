...
> override mechanism behind DOTS's ~40 `DotsXxxModel : DefaultXxxModel` overrides (the CLAUDE.md GameModel table —
...
wins.** Because `DotsFooModel : DefaultFooModel : FooModel`, a Dots model added *after* the vanilla `DefaultFooModel`
...
- **Inheritance (DOTS's way):** `AddModel(new DotsFooModel(...))` where `DotsFooModel : DefaultFooModel` overrides
...
- **Decorator/wrapper (`AddModel<T>` + `MBGameModel<T>`):** `gameModel.Initialize(GetModel<T>())` hands the *previous*
  ...
  model); DOTS prefers inheritance + `base` (simpler — see the [ADOD comparison](../adod-beasts-architecture-and-dots-port.md)).
...
gameStarter)` runs after the base game registers its models, so DOTS adds its `DotsXxxModel`s there.
...
- **DOTS overrides ~40 models** (CLAUDE.md "GameModel Overrides" table): `DotsPartyWageModel`,
  `DotsPartySpeedModel`, `DotsSettlementLoyaltyModel`, `DotsBattleRewardModel`, `DotsAgeModel`, …, each
  `: DefaultXxxModel`, overriding specific methods + calling `base`. Registered in `Main/SubModule.cs OnGameStart`
  via `campaignStarter.AddModel(new DotsXxxModel(...))`.
- **One override per model type** — last-wins. If two DOTS models subclass the same `DefaultXxxModel`, only the
  last-added wins. This is why DOTS **consolidates** several concerns into **one** override of a shared slot: e.g.
  `DotsAgentStatCalculateModel` folds the elephant mount-lock **and** the career agent-stat passives into a single
  `AgentStatCalculateModel` override (Phase via the elephant work) — there's only one slot.
- **The available surface:** any `DefaultXxxModel` DOTS hasn't overridden is fair game to override (subclass +
...
- DOTS: `Main/SubModule.cs OnGameStart` `AddModel` calls; `.claude/rules/gamemodels.md` + `csharp-patterns.md` (the Override Pattern); CLAUDE.md GameModel table (~40 overrides).
