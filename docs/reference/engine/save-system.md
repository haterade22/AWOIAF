...
> `[SaveableField]` for custom classes) and the **base+localId collision gotcha** that has crashed DOTS. Many DOTS
...
types + `MBObjectBase`). **This is the default DOTS persistence path** — most features just `SyncData` a few fields
...
→ a hard crash at `Module.Initialize`** (duplicate save-id). DOTS's convention: **bases step by 100**, so each
...
- **Default path = `SyncData` primitives/strings.** DOTS features routinely avoid a `SaveableTypeDefiner` by storing
...
- **The elephant needs no SaveDefiner** (it's battle-only state) — confirmed in the [ADOD comparison](../adod-beasts-architecture-and-dots-port.md); ADOD's SaveDefiner persists the *wolf* (`_acquiredWolfId`). A creature-*troop* with no campaign state needs nothing here.
...
- New saveable validation: DOTS has no engine-level guard against base-id collision — discipline is on the author +
...
- DOTS precedent + the collision rule: memory `feedback_saveable_typedefiner_localid_offset`; ADOD `ADODBeastsSaveDefiner` (the wolf) in the [ADOD comparison](../adod-beasts-architecture-and-dots-port.md).
- `CampaignBehaviorBase.SyncData(IDataStore)` — the per-behavior persistence override (DOTS features override it widely).
