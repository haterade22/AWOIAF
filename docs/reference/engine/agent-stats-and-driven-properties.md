...
> is the model DOTS's `DotsAgentStatCalculateModel` overrides for **career passives + the war-elephant mount-lock**.
...
- **`UpdateAgentStats(agent, agentDrivenProperties)`** (:28, abstract) — **recompute** the ADP whenever state changes.
  ...
  perk application. **This is the hot method DOTS overrides.**
...
- **`DotsAgentStatCalculateModel`** (`Main/Features/CareerSystem/Models/DotsAgentStatCalculateModel.cs`, registered
  ...
  `AgentStatCalculateModel` and the SandBox concrete is `SandboxAgentStatCalculateModel`, so DOTS inherits the
...
- DOTS: `Main/Features/CareerSystem/Models/DotsAgentStatCalculateModel.cs` (`: SandboxAgentStatCalculateModel`; `UpdateAgentStats`/`CanAgentRideMount`/`GetEffectiveMaxHealth`), `Main/SubModule.cs`:447 (`AddModel<AgentStatCalculateModel>`). Gotcha memories: `feedback_engine_scale_research`, `feedback_clamp_nan_infinity_propagates`; `.claude/rules/gamemodels.md` (thin-boundary rule 4). Linked: gamemodel-system.md (Phase 7), mount-and-rider-runtime.md (Phase 14, the `UpdateAgentStats`-on-mount call).
