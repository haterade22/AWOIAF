...
> *every* DOTS feature use, and it's what the moduledata validator guards. Part of the phased engine study.
...
turns it into `Monster` objects. (DOTS's `dots_npccharacter`/`Items`/`SPCultures`/`partyTemplates` registrations
...
- **Every DOTS feature resolves objects here:** `GetObject<Monster>("spider"|"dots_war_elephant")`, items,
...
- **Cross-module resolution is order-tolerant** at runtime (everything's merged before `LoadXML`), so DOTS resolving
...
- New object types a mod adds need a `RegisterType<T>` (in a `BeforeRegisterTypes`/`OnRegisterTypes` override) + the
  XML registered under the list name; DOTS mostly reuses vanilla types (Monster/Item/Character/Culture), so it
