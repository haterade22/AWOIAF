...
> parallel to phase 4's mission backbone, used by **every** DOTS `CampaignBehavior` (CultureConversion,
...
- **Every DOTS `CampaignBehavior`** subscribes here. Common events: `DailyTickEvent`/`HourlyTickEvent`/`TickEvent`,
...
- **`SyncData` is the persistence half** (Phase 6) — most DOTS features `SyncData` primitives/composite strings; no
...
- DOTS precedent + gotchas: memories `feedback_imbevent_remove_one_unavailable`, `feedback_campaignbehavior_no_ongameend`; behaviors registered in `Main/SubModule.cs OnGameStart` via `AddBehavior`.
