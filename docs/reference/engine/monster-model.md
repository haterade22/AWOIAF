...
> data source for a creature's entire rig — it's what phases 1–2 read from, and **exactly what DOTS authors per
...
- `<Flags Mountable= CanRear= RunsAwayWhenHit= CanCharge= CanWander= IsHumanoid= …/>` → behavior flags. For a
  DOTS creature-troop: `Mountable="false"` (never a rideable mount — the map-icon `ForceUpdateBoneFrames` crash),
  `IsHumanoid="false"`.
...
- The flags drive mount/AI; `Mountable` gates the map-party-icon path DOTS must avoid for creatures.

## DOTS relevance + gotchas
- This file is **exactly** what DOTS authors per creature: `lotr_monster_spider.xml`, `lotr_monster_elephant.xml`.
