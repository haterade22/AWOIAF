...
> this covers the *objects* those behaviors read + mutate. **Every** DOTS campaign feature (CultureConversion,
...
## DOTS relevance + gotchas (the recurring campaign-object rules)
...
- **Lookup by id** (Phase 5): `MBObjectManager.Instance.GetObject<Settlement>(stringId)` / `Settlement.Find(id)` /
  `Hero.FindFirst(...)`; missing id → clean `null`. DOTS keys ~81 recruitment pools on hard-coded settlement ids.
...
layer is where DOTS features live precisely because it's pure managed simulation.
