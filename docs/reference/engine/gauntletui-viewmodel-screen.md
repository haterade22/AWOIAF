...
> DOTS extends it (subclassing + UIExtenderEx mixins/prefab-extensions). Covers DOTS's career screen, messengers,
...
prefab-extensions). DOTS does both.
...
DOTS VMs (career, messengers, quick actions) subclass `ViewModel`.
...
- **Mixins** (`[ViewModelMixin]`) — attach new `[DataSourceProperty]` + behavior to an *existing* vanilla VM (e.g. add
  a property/button to `SPInventoryVM`). DOTS: QuickActions, career.
...
## DOTS relevance + gotchas (the UI memory cluster)
