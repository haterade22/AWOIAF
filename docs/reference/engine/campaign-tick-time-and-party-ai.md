...
> *heartbeat* that drives every DOTS `DailyTick`/`HourlyTick` behavior. It completes the campaign layer (Phases 9,16,17)
...
out to every event receiver (:1256) — this is the dispatch DOTS's CastleRecruitment Patch42 postfixes.
...
- **Every DOTS periodic behavior is driven here** (Phase 9): CultureConversion daily completion, CastleRecruitment
...
- **`CampaignTime` is the unit for all DOTS deadlines**: Messengers travel days, Siege-defense deadline,
