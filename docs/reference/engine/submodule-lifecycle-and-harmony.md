...
> **One process, traced from DOTS's own entry point + the engine base** (v1.4.5): the meta-layer — *how* DOTS's code
...
> "how does any of our code run" layer that every DOTS feature sits on. Capstone of the phased engine study.

## WHAT it is

A Bannerlord module ships a **`MBSubModuleBase`** subclass — the engine instantiates it and calls lifecycle methods at
fixed points (load, pre-menu, game start, mission init, tick, unload). Inside those, DOTS does three things:
**(1) Harmony-patch** engine methods (Prefix/Postfix/Transpiler/Finalizer), **(2) register GameModel overrides**
(`AddModel`, Phase 7), **(3) register CampaignBehaviors** (`AddBehavior`, Phase 9). DOTS's is `SubModule : MBSubModuleBase`
(Main/SubModule.cs:82).

## HOW it works — the lifecycle (DOTS's SubModule.cs)

| Override | When | What DOTS does |
|---|---|---|
| **`OnSubModuleLoad()`** (:91) | Earliest — module DLL loaded, before menu | `_harmony = new Harmony("com.dots.mod")` (:104); apply most patch categories via **`_harmony.PatchCategory("PatchNN_X")`** (:133-242); wire static patch fields via `.Initialize(service, …)` (:207-235); IoC bootstrap. |
...
| **`OnGameStart(Game, IGameStarter)`** (:294) | A game (campaign) is starting | `if (gameStarter is CampaignGameStarter cs)` → **`cs.AddBehavior(new XxxBehavior(...))`** (every CampaignBehavior, Phase 9) + **`cs.AddModel(new DotsXxxModel(...))`** (every GameModel, Phase 7/15/16). |
...
- **`new Harmony(id)`** — the patch *owner* (`"com.dots.mod"`); all DOTS patches belong to this owner (used by PatchShield's allowlist — `feedback_harmony_owner_allowlist_from_vendored_dll_enumeration`).
- **Categories:** a patch class carries `[HarmonyPatch(typeof(Target), "Method")]` + `[HarmonyPatchCategory("PatchNN_X")]`; **`_harmony.PatchCategory("PatchNN_X")`** applies all patches in that group. DOTS applies categories *selectively* (some conditionally, e.g. `Patch37_CrashReport` :109), so a category can be skipped without disabling everything.
- **Patch kinds:**
...
`MBSubModuleBase` is the only entry the engine calls — everything DOTS does hangs off its lifecycle. The split matters:
patches that touch *type metadata only* go early (`OnSubModuleLoad`); registrations that need a `CampaignGameStarter` go
in `OnGameStart`; patches that need a live `Mission` go in `OnMissionBehaviorInitialize`. Harmony lets DOTS modify
