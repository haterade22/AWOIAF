...
3. **Unregistered GameModel** — a `Dots*Model` that compiles but is never `AddModel`'d; the engine silently uses the vanilla `Default`.
...
| [`gamemodel-bases.md`](./gamemodel-bases.md) | Auto-generated: each `Dots*Model`, its `Default*Model` base, and the exact v1.4.5 signature of every overridden method. |
...
| [`../../../tools/snapshot_api_surface.ps1`](../../../tools/snapshot_api_surface.ps1) | Regenerates the two auto-generated files from the installed DLLs. Auto-derives the type list from `DOTS.dll`. |

## The gate (`DOTS.Tests/Migration/`)

Runs under `dotnet test` — no game launch. DOTS.Tests references the v1.4.5 `TaleWorlds.*.dll`; `GameAssemblies.cs` pre-loads the SandBox/CustomBattle/StoryMode module DLLs from the install so every engine type resolves.
...
```bash
dotnet test DOTS.Tests/DOTS.Tests.csproj --filter "TestCategory=BindingVerification"
```
