...
**Purpose.** DOTS reaches into private/internal TaleWorlds members by *string name* in many places. The C# compiler cannot verify those — a rename/move/removal in a Bannerlord update makes the lookup return `null`, and the reflecting code logs-and-survives, so the feature silently degrades with **no crash to investigate**. This file is the authoritative inventory of every reflection touchpoint, and the data source for the offline binding gate `DOTS.Tests/Migration/ReflectionSiteBindingTests.cs`.

It also exists so that an agent working on DOTS does not need the external decompile dump (`E:\Decompiled_Bannerlord\`) just to answer "what private member does feature X reach into, and is it still there in v1.4.5?"
...
```
dotnet test DOTS.Tests/DOTS.Tests.csproj --filter "FullyQualifiedName~ReflectionSiteBindingTests"
```
...
Every `[HarmonyPatch(...)]` target — including patches whose target is resolved by a `TargetMethod()` / `TargetMethods()` body (e.g. SettlementGuards' manual patches, the `MapConversationTableau` / `CultureStageView` `TypeByName` lookups) — is **auto-discovered and resolved** by `DOTS.Tests/Migration/HarmonyPatchBindingTests.cs`. That suite enumerates all 110 `[HarmonyPatch]` / `TargetMethod`-bearing types in `DOTS.dll` and resolves each target exactly as Harmony does at `PatchAll` time. No manual catalogue is needed for them; do not duplicate them below.
...
## Category D — DOTS-internal reflection (not engine drift)

Reflection whose target is a DOTS-owned type or a dynamic member name. Not affected by Bannerlord updates; intentionally not gated.
...
| `CareerSystem/Mutations/MutationService.cs:105` | `typeof(AbilityTemplateData).GetProperty(propertyName)` | `AbilityTemplateData` is a DOTS type; `propertyName` is data-driven |
| `CrashReport/Hooks/Native2ManagedPatcher.cs:41,52` | `typeof(CrashReportPatchHelper)`, `typeof(Native2ManagedBridge)` | DOTS finalizer/bridge types |
...
- `DOTS.Tests/Migration/ReflectionSiteBindingTests.cs` (Category B is its data source)
