...
DOTS accumulated ~290 markdown files across `docs/` plus ~96 auto-memory feedback files plus a 705KB CHANGELOG. The structure grew organically — `docs/features/`, `docs/reviews/`, `docs/adrs/`, `docs/ai-includes/`, `docs/migration/`, `.claude/rules/`, and memory each developed strong local conventions but no unifying access pattern.
...
**Link syntax**: continue using markdown `[text](path)` links throughout. Do **not** migrate to Obsidian `[[wikilinks]]`. GitHub renders markdown natively; Claude tools (Read, Edit, Grep) handle markdown links uniformly; conversion across 290+ files has high cost and no clear benefit at DOTS's scale. Auto-memory files retain their existing `[[name]]` convention (the auto-memory system specifically supports it).

**Out of scope for this ADR**:
- No vector store or RAG layer. Karpathy notes the wiki scale (~100 articles, ~400K words) works without one; DOTS is comparable.
...
- **Why rejected**: cost of conversion outweighs graph-view benefit at DOTS's current scale. Markdown links work in every renderer we use.
...
- **Cons**: `.claude/` is *workflow* knowledge (how to use Claude on DOTS). The mod-domain knowledge (what DOTS does, why it does it, what RCAs taught us) lives in `docs/` + `memory/`. They're complementary, not interchangeable.
...
Managed wrapper for `DOTS.NativeSkinFixes.dll`. See [character-creation](./character-creation.md) for the parent CC flow.
