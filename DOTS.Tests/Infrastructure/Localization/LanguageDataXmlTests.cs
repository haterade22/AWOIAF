...
namespace DOTS.Tests.Infrastructure.Localization;
    ...
    public void AllLanguageDirs_HaveCcStringsFile()
    {
        foreach (var lang in SupportedLanguageDirs)
        {
            ...
            var paths = doc.Descendants("LanguageFile")
                ...
                .ToList();
            Assert.IsTrue(
                paths.Any(p => p.Contains("dots_cc_strings")),
                $"Languages/{lang}/language_data.xml missing dots_cc_strings reference");
        }
    }
    ...
    public void AllLanguageDirs_HaveCareerStringsFile()
    {
        foreach (var lang in SupportedLanguageDirs)
        {
            ...
            var paths = doc.Descendants("LanguageFile")
                ...
                .ToList();
            Assert.IsTrue(
                paths.Any(p => p.Contains("dots_career_strings")),
                $"Languages/{lang}/language_data.xml missing dots_career_strings reference");
        }
    }
    ...
    public void AllLanguageDirs_HaveMessengerStringsFile()
    {
        foreach (var lang in SupportedLanguageDirs)
        {
            ...
            var paths = doc.Descendants("LanguageFile")
                ...
                .ToList();
            Assert.IsTrue(
                paths.Any(p => p.Contains("dots_messenger_strings")),
                $"Languages/{lang}/language_data.xml missing dots_messenger_strings reference");
        }
    }
