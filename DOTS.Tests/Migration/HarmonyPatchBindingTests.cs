...
/// <summary>
/// Binding-verification gate for every Harmony patch in DOTS. Enumerates all <c>[HarmonyPatch]</c>
/// types in the DOTS assembly and resolves each one's target method against the *installed*
...
/// under `dotnet test` with no game launch because DOTS.Tests references the v1.4.5 TaleWorlds DLLs
...
public class HarmonyPatchBindingTests
{
    ...
    {
        Type[] all;
        try { all = DotsAssembly.GetTypes(); typesFailedToLoad = 0; }
