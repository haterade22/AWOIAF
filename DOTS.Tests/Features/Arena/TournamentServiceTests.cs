...
using DOTS.Features.Arena;

namespace DOTS.Tests.Features.Arena;

/// <summary>
/// TournamentService unit tests. BuildPrizePool requires Items.All (sealed engine cache)
/// and is tested in-game only.
/// </summary>
[TestClass]
public class TournamentServiceTests
{
    ...
    // --- ShouldDismountInTournament ---
    // GoT is human-only — no custom-skeleton races, so always returns false.

    [TestMethod]
    public void ShouldDismountInTournament_AnyRaceId_ReturnsFalse()
    {
        Assert.IsFalse(_sut.ShouldDismountInTournament(0));
        Assert.IsFalse(_sut.ShouldDismountInTournament(1));
        Assert.IsFalse(_sut.ShouldDismountInTournament(999));
    }
}
