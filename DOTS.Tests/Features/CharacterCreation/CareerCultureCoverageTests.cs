...
public class CareerCultureCoverageTests
{
    ...
    public void EveryCCCulture_HasAtLeastOneEligibleCareer_OrIsDocumentedException()
    {
        ...
        // Arrange — load career eligible cultures from dots_careers.xml
        var careersPath = Path.Combine(ModuleDataPath, "career_system", "dots_careers.xml");
        Assert.IsTrue(File.Exists(careersPath), $"dots_careers.xml not found at {careersPath}");
        ...
        Assert.AreEqual(0, uncoveredCultures.Count,
            ...
            "Either add careers for these cultures in dots_careers.xml, or add them to documentedExceptions in this test.");
    }
    ...
    public void EveryCareerInXml_HasMatchingJsonEntry()
    {
        ...
        Assert.AreEqual(0, missingFromJson.Count,
            $"Careers in dots_careers.xml with no matching career_menu.json entry: {string.Join(", ", missingFromJson)}. " +
    ...
    public void EveryJsonEntry_HasMatchingCareerInXml()
    {
        ...
        Assert.AreEqual(0, orphanedJson.Count,
            $"career_menu.json entries with no matching Career in dots_careers.xml: {string.Join(", ", orphanedJson)}. " +
            "Remove orphaned entries or add the missing Career to dots_careers.xml.");
    }
