...
namespace DOTS.Tests.Features.EquipPresets;
...
public class PresetSaveableTypeDefinerTests
{
    ...
    public void BaseId_MatchesPlanned726900501()
    {
        ...
        Assert.AreEqual(726900501, PresetSaveableTypeDefiner.SaveBaseId,
            "BaseId 726900501 — verified unique across DOTS at port time. Document range usage if changed.");
    }

    [TestMethod]
    public void BaseId_UniqueAcrossDiscoverableDefinersInDotsAssembly()
    {
        // Filter using GetExportedTypes which doesn't load every dependency. ReflectionTypeLoadException
        // on full GetTypes() is a known DOTS.Tests env failure (MCMv5 not in test output) — we ignore it
        ...
        Assert.IsTrue(allDefiners.Contains(typeof(PresetSaveableTypeDefiner)),
            "PresetSaveableTypeDefiner should be discoverable in the DOTS assembly");
