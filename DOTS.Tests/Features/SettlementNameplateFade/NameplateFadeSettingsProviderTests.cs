...
namespace DOTS.Tests.Features.SettlementNameplateFade;

[TestClass]
public class NameplateFadeSettingsProviderTests
{
    // DotsSettings.Instance is null in test environments (MCM v5 isn't loaded), so the
    // provider falls back to its compiled defaults. These tests verify the fallbacks
    // match the MCM slider defaults declared in DotsSettings.cs — drift between the two
