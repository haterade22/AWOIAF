...
1. **Agree on a run tag**: propose one based on today's date (e.g. `may27a`). The branch `dots-lint/<tag>` must not already exist.
2. **Create the branch**: `git checkout -b dots-lint/<tag>` from current HEAD.
...
The loop runs on `dots-lint/<tag>` — isolated branch, no risk to `bannerlord-1.4.5`. When the loop stops (metric=0 or stuck):
