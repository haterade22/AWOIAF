---
name: dots-src
description: Decompile and cache TaleWorlds v1.3.15 types via `dots-src path <Type>`. Returns absolute path for grep/cat composition. Use before patching, overriding, or writing adapters.
argument-hint: [path|list|remove|clean] <FullyQualifiedType>
---

# dots-src — one-command TaleWorlds source lookup

Wraps `ilspycmd` against the **installed v1.3.15 DLLs** and caches at `~/.dots-src/v1.3.15/`. Inspired by [vercel-labs/opensrc](https://github.com/vercel-labs/opensrc). Use this **before** every `[HarmonyPatch]`, `GameModel` override, or adapter that touches a TaleWorlds type — never guess signatures from the v1.4 dump at `E:\Decompiled_Bannerlord\`.
...
cat $(pwsh tools/dots-src.ps1 path TaleWorlds.MountAndBlade.Formation)
```
...
1. **DLL index cache** — `~/.dots-src/dll-index.json` remembers `type → dll` from prior lookups. Instant hit.
...
| Situation | Action |
|---|---|
| About to write `[HarmonyPatch(typeof(X), nameof(X.Y))]` | `dots-src path X` first — verify `Y` exists with the expected signature |
| About to override `GetCharacterWage` on `DefaultPartyWageModel` | `dots-src path DefaultPartyWageModel` — copy the exact base signature |
| About to call a TaleWorlds API from an adapter | `dots-src path <Type>` — confirm method exists in v1.3.15 (not just v1.4) |
| Bug investigation: "vanilla does X, what does its code actually look like?" | `dots-src path <ClassName>` then `rg` for the symptom |
| Want to know what's already cached | `dots-src list` |

## When NOT to use

| Situation | Use instead |
|---|---|
| Looking for a class but don't know the FQN | `grep -r "ClassName" E:/Decompiled_Bannerlord/` to find namespace, **then** `dots-src path` to verify v1.3.15 |
...
- `.claude/skills/research/SKILL.md` — broader "decompile and analyze before implementing" workflow. Use that for full analysis sessions; use `dots-src` for one-shot signature lookups.
