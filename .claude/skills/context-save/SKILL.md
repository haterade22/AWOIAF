...
Adapted from [garrytan/gstack/context-save](https://github.com/garrytan/gstack/tree/main/context-save) — DOTS version drops the gstack-specific telemetry/sync scaffolding and writes plain markdown.
...
- Switching between unrelated work streams (e.g., DOTS → another repo → back)
...
- The write goes into `.claude/state/context/`. Freeze interaction:
  - If you've frozen to `Main/Features/<X>/`, `DOTS.Tests/`, or any other directory that does NOT contain `.claude/state/context/`, the snapshot write WILL BE BLOCKED by the `/freeze` hook. Run `/unfreeze` first or widen the boundary.
