#!/usr/bin/env bash
# context-budget scanner — DOTS-adapted
...
scan_memory() {
    ...
    # stripped, then prefixed by `<drive>--` (e.g., `c--Users-mikew-source-repos-DOTS`).
    # Substring matching ("anything containing DOTS") is unreliable when the
    # machine has multiple projects whose paths share a substring (DOTS,
    # DOTS-Online, taommod). Build the canonical slug instead.
    local repo_native
...
ISSUES=()
echo
echo "DOTS Context Budget Report"
echo "=========================="
echo
