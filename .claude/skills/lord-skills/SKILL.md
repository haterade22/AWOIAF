---
name: lord-skills
description: Give DOTS lords lore-driven skill values and traits via the DOTS SkillSet system. Use when a canonical lord has wrong stats or a culture roster needs a balance pass.
...
Hero stats come from `skill_template="SkillSet.dots_..."`, NOT from the NPCCharacter's `<skills>` block — **explicit `<skills>` on heroes are IGNORED by the engine** (kept as documentation only). To change a hero's skills, edit the SkillSet (or repoint `skill_template`). See memory `feedback_skill_template_overrides_explicit_skills.md` — Boromir showed OneHanded=145 instead of 295 because someone hand-edited `<skills>`.

## Workflow
1. **Identify the NPC** — grep `characters/lords.xml`, `lords.xslt`, `heroes.xslt` for the name → NPCCharacter id. Classify the layer: `lords.xml` (DOTS-new, wins at runtime) > `lords.xslt` (vanilla override, loses to lords.xml on same id) > `heroes.xslt` (bio text, stat-irrelevant).
...
3. **Generate** — `python tools/apply_culture_skills_traits.py --all-cultures --apply` emits 3 outputs: `dots_lord_skill_sets.xml`, updated `lords.xml` + `lords.xslt` (`skill_template` swaps + populated blocks).
