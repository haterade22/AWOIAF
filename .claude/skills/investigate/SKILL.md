...
5. **Check the simpler-fix-first list.** Per `feedback_simpler_fix_first.md`: before investigating engine internals, check whether the bug is in:
   ...
   - `dots_*.xml` data files (XML schema or value problems)
...
Check whether the bug matches a known DOTS/Bannerlord failure pattern before writing any fix:
...
Root cause:        [what was actually wrong, in DOTS terms]
...
Regression test:   [DOTS.Tests/.../FooTests.cs:line]
Files touched:     [N]
