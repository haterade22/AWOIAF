...
This is the **harness/agent failure** companion to `/investigate` (DOTS-specific code bugs). When the failing thing is the agent itself — looping, drifting, burning tokens without progress — use this skill to debug the agent before debugging the code.
...
- For DOTS code bugs (Harmony failures, GameModel misbehavior, etc.) → use `/investigate`
...
- `feedback_*.md` memory entry (if it's a DOTS-specific lesson)
