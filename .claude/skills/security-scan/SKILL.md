---
name: security-scan
description: Use when auditing DOTS's Claude Code config for security issues — committed secrets, over-broad permissions, hook exfiltration, MCP risk, hidden-unicode injection. Runs tools/audit_claude_config.py.
allowed-tools: Bash, Read
---

# Security Scan — audit DOTS's own Claude config

A deterministic, offline security scan of DOTS's Claude Code configuration surface (`.claude/`, `.mcp.json`, `settings*.json`, `CLAUDE.md`, `AGENTS.md`). Ported as a DOTS-calibrated subset of obra-style "scan your own agent config" tooling (affaan-m/ECC AgentShield), 2026-05-29. See `docs/reviews/` for the review that motivated it.
...
## DOTS calibration (important)

This is **not** a blind port of the upstream ruleset. DOTS *mandates* fail-open hooks (`|| true`, `2>/dev/null`, always `exit 0`) per `.claude/rules/harness-facts.md`, so that pattern is deliberately NOT flagged here (it is upstream). When porting any future rule, calibrate it to DOTS's conventions first.
