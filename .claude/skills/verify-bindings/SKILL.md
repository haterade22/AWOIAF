...
description: Verify DOTS's Harmony patch, GameModel, and reflection bindings against the installed Bannerlord engine and refresh the committed API snapshot. Use after an engine update or patch change.
argument-hint: [check|refresh|full]
---

# Verify TaleWorlds API Bindings

Re-verify that DOTS's engine touchpoints still bind against the **installed** Bannerlord version, and keep the committed v1.4.5 signature snapshot in sync. This is the standing, offline form of the migration S6 smoke-test gate.
...
- After adding/changing a Harmony patch, a `Dots*Model` GameModel, or a string-name reflection site against an engine member.
...
```bash
dotnet test DOTS.Tests/DOTS.Tests.csproj -p:DisableModuleCopy=true --filter "TestCategory=BindingVerification"
```

Three test classes under `DOTS.Tests/Migration/`:
- `HarmonyPatchBindingTests` — every `[HarmonyPatch]` / `TargetMethod` target resolves as Harmony resolves it.
- `GameModelOverrideBindingTests` — every `Dots*Model` is registered in `SubModule.cs`, overrides ≥1 base virtual, and doesn't shadow a base virtual without `override`.
...
| GameModel `never AddModel'd` | a `Dots*Model` compiles but isn't registered | add `campaignStarter.AddModel(new DotsXModel(...))` in `Main/SubModule.cs` |
...
Regenerates `docs/reference/taleworlds-api-snapshot/{gamemodel-bases,patch-targets}.md` from the installed DLLs (auto-derives the type list from `DOTS.dll`). Then prove reproducibility:
