...
| TaleWorlds API signatures | installed DLLs | `pwsh tools/dots-src.ps1 path <Type>` (NOT the decompiled dump) |
...
DOTS ships full `<Prefab>` **clones** of many vanilla GUI prefabs (party screen, custom-battle, encyclopedia, nameplates, game-menu — ~32 of DOTS's 48 prefabs are clones). A clone overrides the vanilla prefab of the same filename by load order. Vanilla **renames widget attributes and re-schemas prefabs between versions**; a clone frozen at an older version keeps the obsolete attribute, the engine silently ignores it, and the widget mis-renders or **never renders** — with no log, no crash. This is the same stale-vs-vanilla failure as the data XML above, applied to UI.
...
1. `diff -w --strip-trailing-cr <vanilla-1.4.5> <dots-clone>` and classify each delta: **rename-casualty / stale-attribute** vs **intentional DOTS customization**.
2. If the clone has **no** intentional content (pure stale copy), **re-sync**: replace it with the vanilla 1.4.5 file verbatim, re-applying only the genuine DOTS tweaks on top.
3. If it's a DOTS-original or a deliberate redesign (e.g. the Encyclopedia `EncyclopediaClanListElement` banner swaps, the `SettlementNameplateItem` diamond layout, the `CharacterCreation*Stage` theming), **leave the structure alone** and only fix the obsolete attribute in place.
...
- v1.4.5 renamed `<culture>_house_a_interior_house` interior scenes; 61 stale `scene_name` refs across DOTS towns.
- DOTS's `sp_battle_scenes.xml` mapped map indices 158–255 to `battle_terrain_extended`, a scene that doesn't exist on disk.
