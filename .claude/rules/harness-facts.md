...
| **DOTS hooks MUST fail open.** A hook's own bug must never block the user: swallow internal errors (`2>/dev/null`, `\|\| true`), and non-`deny` hooks always `exit 0`. A `deny` is only ever an *intentional* gate decision, never an accidental crash. | this repo's design (every Stop/PostToolUse hook exits 0; PreToolUse gates deny deliberately) | This is why `tools/audit_claude_config.py` (`/security-scan`) deliberately does NOT flag `\|\| true` / `2>/dev/null` / `exit 0` as exfil — they're mandated here, though upstream AgentShield flags them. Calibrate ported security rules to this. |
...
## Authoring a new hook — mirror the sibling's FULL convention set (EMPIRICAL: DOTS 2026-05-29)
...
## Parallel-port build watcher (EMPIRICAL: DOTS 2026-05-06, CompanionTactics port session)
...
When multiple feature ports run simultaneously in the DOTS working tree, an external watcher monitors build output and **auto-edits source files in response to build failures**. Symptoms confirmed during the CompanionTactics port:

| What gets re-added | Where | When |
|---|---|---|
| `<Compile Remove="Features\<Feature>\**\*.cs" />` | `Main/DOTS.csproj` and `DOTS.Tests/DOTS.Tests.csproj` | After ANY build failure that mentions the feature |
| Comments around `using DOTS.Features.<Feature>.*;` directives | `Main/SubModule.cs`, `Main/IoC.cs` | Same trigger |
...
4. Run the build IMMEDIATELY in the same response (`dotnet build Main/DOTS.csproj -p:DisableModuleCopy=true --verbosity quiet`).
...
**Rule:** when spawning multiple `Agent` calls in one message that may edit overlapping single-owner files (`Main/DOTS.csproj`, `DOTS.Tests/DOTS.Tests.csproj`, `Main/IoC.cs`, `Main/SubModule.cs`, `Directory.Build.props`), pass `isolation: "worktree"` on each call. Each agent then operates in its own git worktree on a temporary branch — the shared DOTS working tree is never touched in parallel, and the build watcher cascade above cannot fire.
...
| `feedback_parallel_port_build_watcher.md` (EMPIRICAL: DOTS 2026-05-06) | The cascade exists; the workaround is heroic Edit-cycles that burn ~30s each. The RCA documents ~2 hours lost. |
