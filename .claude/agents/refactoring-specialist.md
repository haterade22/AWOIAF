...
| `feature-builder` | Build new features from scratch following DOTS conventions | New code |
...
## Method (Martin Fowler-style discipline, DOTS-flavored)

1. **Confirm tests green.** `dotnet test DOTS.Tests` must pass. If not, fix first.
...
4. **Test after each refactoring.** `dotnet test DOTS.Tests` must still pass. If a test fails, the refactoring changed behavior — revert and re-think.

5. **Per DOTS conventions:**
   - No `#region` (ADR-003)
...
Tests:          dotnet test DOTS.Tests — N passed, 0 failed
...
- The "cleanest" refactor would conflict with DOTS conventions → keep the convention; if the convention is wrong, that's an ADR change, not a refactoring decision

Source: VoltAgent/awesome-claude-code-subagents (adapted with DOTS ADR rules + boundary vs `/deslop` and other skills).
