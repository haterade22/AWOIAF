...
description: Decompile and analyze TaleWorlds game classes for DOTS mod development. Use when implementing adapters, Harmony patches, GameModels, or investigating bugs.
tools:
  - Bash
  - Read
  - Grep
...
- Decompiler (in order): **`dots-src`** (`pwsh tools/dots-src.ps1 path <Type>`, primary) → `E:\Decompiled_Bannerlord\` (v1.4.5 dump, browse-only) → `ilspycmd` / `ilspy` MCP (fallback)
...
```
`dots-src` runs `ilspycmd` against the installed v1.4.5 DLLs and caches under `~/.dots-src/v1.4.5/`. Use it first.

### Fallback: ILSpy MCP Server
The `ilspy` MCP server is configured in `.mcp.json`. Use it when `dots-src` can't resolve a type:
```
