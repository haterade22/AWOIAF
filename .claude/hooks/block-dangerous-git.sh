...
#
# Guarded (work-destroying ops DOTS did not previously cover):
...
#
# Calibrated to DOTS: confirm-not-block; fail-open (any parse error → allow).
# Concept (recalibrated) from mattpocock/skills git-guardrails, whose version
# hard-blocked ALL pushes with no override — wrong for DOTS.
#
...
MSG="CONFIRM destructive git op: ${REASON}. Command: \\\"${CMD_ESC}\\\". This can permanently lose uncommitted/unpushed work and DOTS has no undo for it. Approve ONLY if you intend to discard those changes — otherwise commit or stash first. (Pushes are guarded separately by validate-push.sh.)"
