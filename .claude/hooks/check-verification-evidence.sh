...
if [[ $NEEDS_REMINDER -eq 1 ]]; then
  if [[ ! -f "$REMINDED" ]]; then
    echo "REMINDER: C# source changed but no build/test has run since your last edit. Per .claude/rules/evidence-over-claims.md (verification before \"done\"), run ./build.ps1 -RunTests (or dotnet build Main/DOTS.csproj + dotnet test DOTS.Tests) and read the output before claiming the work complete. A subagent's self-report does not count as verification." >&2
