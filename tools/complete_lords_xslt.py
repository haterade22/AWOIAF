#!/usr/bin/env python3
"""
Lords XSLT Completion Script for DOTS
...
def parse_current_xslt(filepath):
    """Parse current lords.xslt with regex to extract DOTS overrides."""
...
def merge_lords(vanilla, xslt_data):
    """Merge vanilla attributes with DOTS overrides."""
    merged = {}
    ...
    for lord_id, v_data in vanilla.items():
        ...
        for attr_name in ATTR_ORDER:
            ...
            # DOTS overrides for specific attributes
        ...
        # For new lords, generate placeholder DOTS name
        ...
        # Face: DOTS face wins if exists, else vanilla
...
def generate_lord_template(lord_id, data):
    ...
    if data['is_new'] and data['is_dead']:
        lines.append(f'    <!-- {lord_id} — DEAD LORD: needs DOTS customization -->')
    elif data['is_new']:
        lines.append(f'    <!-- {lord_id} — NEW: needs DOTS customization -->')
    else:
