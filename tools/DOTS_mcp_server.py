#!/usr/bin/env python3
"""DOTS ModuleData MCP server (stdio).

Exposes the schema-driven validation engine + query API (tools/dots_schema.py,
tools/dots_query.py) as MCP tools, so Claude agents can query mod-data integrity
...
Requires the `mcp` Python SDK. Run standalone to smoke-test:
    python tools/dots_mcp_server.py
Register as a stdio MCP server in .mcp.json (command: python, args:
[tools/dots_mcp_server.py]) + enable in .claude/settings.local.json. See
...
import dots_schema as ts
import dots_query as tq
...
def _build_query() -> tq.ModuleDataQuery:
    ...
    try:
        ...
        print(f"[dots-moduledata] failed to initialize: {exc!r}\n"
...
mcp = FastMCP("dots-moduledata")


@mcp.tool()
def validate_moduledata(codes: list[str] | None = None) -> dict:
    """Validate all DOTS ModuleData XML against the schemas. Returns
