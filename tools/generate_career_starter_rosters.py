...
Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml.

For each DOTS culture with a dedicated LOTRLOME_Armory folder, parse the
...
ROSTERS_XML = ROOT / "Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml"
...
def generate_rosters_for_culture(dots_culture: str, cfg: dict) -> list[str]:
    """Generate all 6 rosters (3 archetypes x 2 genders) for one culture."""
    culture_id = cfg.get("culture_id", dots_culture)
    ...
    if not body_id or not leg_id:
        print(f"  WARN  {dots_culture}: missing body or leg armor (body={body_id}, leg={leg_id})", file=sys.stderr)
        return []

    print(f"  {dots_culture}: body={body_id} ({body_armor}) / leg={leg_id} ({leg_armor})")
    ...
    for archetype in ("ranged", "cavalry", "infantry"):
        ...
        if archetype == "ranged":
            ...
            slots = [("Item0", weapons[0]), ("Item1", weapons[1]), ("Item2", weapons[2]),
                     ("Body", body_id), ("Leg", leg_id)]
        for gender in ("m", "f"):
            roster_id = f"player_career_{dots_culture}_{archetype}_{gender}"
...
<!--
  DOTS career-archetype starting equipment rosters.
...
def main():
    ...
    parts = [GONDOR_BLOCK]
    for dots_culture, cfg in CULTURES.items():
        rosters = generate_rosters_for_culture(dots_culture, cfg)
        if rosters:
            parts.append(f"\n    <!-- ═══ {dots_culture.upper()} ═══ -->\n")
            parts.append("\n\n".join(rosters))
