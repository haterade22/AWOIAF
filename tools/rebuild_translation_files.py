...
def rebuild_language(lang):
    ...
    # ─── DOTS module: 6 source XMLs ─────────────────────────────────────────────
    dots_sources = [
        ("dots_module_strings.xml",                       f"std_dots_module_strings_{locale}.xml"),
        ("dots_wanderer_strings.xml",                     f"std_dots_wanderer_strings_{locale}.xml"),
        ("named_companions/named_companion_strings.xml",  f"std_dots_named_companion_strings_{locale}.xml"),
        ("dots_cc_strings.xml",                           f"std_dots_cc_strings_{locale}.xml"),
        ("dots_career_strings.xml",                       f"std_dots_career_strings_{locale}.xml"),
        ("dots_messenger_strings.xml",                    f"std_dots_messenger_strings_{locale}.xml"),
        ("dots_xslt_strings.xml",                         f"std_dots_xslt_strings_{locale}.xml"),
    ]
    dots_target_dir = REPO_ROOT / "Main" / "_Module" / "ModuleData" / "Languages" / lang
    for src_rel, tgt_name in dots_sources:
        ...
        tgt_path = dots_target_dir / tgt_name
        ordered = []
        ...
        print(f"  DOTS {tgt_name}: {len(ordered)} entries (override={stats['override']} cache={stats['cache']} en_fallback={stats['english_fallback']})")

    # ─── DOTS_Map: settlements.xml ──────────────────────────────────────────────
    settlements_xml = GAME_ROOT / "DOTS_Map" / "ModuleData" / "settlements.xml"
    map_target = GAME_ROOT / "DOTS_Map" / "ModuleData" / "Languages" / lang / "loc_settlements.xml"
    if settlements_xml.exists():
        ...
        print(f"  DOTS_Map loc_settlements.xml: {len(ordered)} entries (override={stats['override']} cache={stats['cache']} en_fallback={stats['english_fallback']})")
