#!/usr/bin/env python3
"""
Character Creation Bonus Audit for DOTS
...
DOTS's CC grants bonuses at menu-selection time via the vanilla engine
...
(1 / 10 / 1); the imbalance is STRUCTURAL:
  - 6 ungated bonus stages (vanilla has 5): parents, childhood, youth, education,
    adulthood, + a DOTS-added career stage.
...
CAREERS_XML = os.path.join(
    os.path.dirname(__file__), "..", "Main", "_Module", "ModuleData", "career_system", "dots_careers.xml"
)

# The five vanilla-replaced narrative stages (culture-gated except childhood) plus
# the DOTS-added career stage. Order = chronological CC flow.
MENU_FILES = {
...
# culture-base add. Per-pick value identical to DOTS (1 / 10 / 1).
...
def load_career_eligibility(valid_culture_ids=None):
    """career_id -> set(culture_id) from dots_careers.xml <EligibleCultures>.
    ...
    if unknown:
        print(f"WARNING: dots_careers.xml <EligibleCultures> references non-culture ids "
...
def main():
    ap = argparse.ArgumentParser(description="Audit DOTS character-creation bonuses per culture.")
    ...
    w("Per-pick value is `(focus, skill-level, attr-level)`. Vanilla defaults are "
      "`(1, 10, 1)` — DOTS should not exceed that per pick.\n")
    ...
    for c in cultures:
        ...
        for stage, pool in stage_pools:
            # Within a DOTS stage the payload is uniform; read it from the pool.
    ...
    w(f"**Vanilla baseline (decompiled v1.4.5):** {VANILLA_BONUS_STAGES} bonus stages, "
      ...
      "single skill/attribute cannot soak every pick the way DOTS's repeated, ungated themes allow.\n")

    # ---- Section 2.5: rebalance applied + remaining lever ------------------
    w("## 2b. Rebalance applied (target: match vanilla) + remaining lever\n")
    w("**Applied (`--apply`, 2026-05-30):** the two DOTS-added bonus sources were zeroed to land "
