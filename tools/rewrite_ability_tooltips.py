...
matching dots_ability_tuning.xml. Preserves the lore flavor lead-in for each
ability; replaces the generic "for a short duration" suffix with specifics.

Targets:
  Main/_Module/ModuleData/career_system/dots_ability_templates.xml   (engine-side fallback)
  Main/_Module/ModuleData/dots_career_strings.xml                    (localization registry)

Archetype tuning (from dots_ability_tuning.xml):
...
TEMPLATES = ROOT / "Main/_Module/ModuleData/career_system/dots_ability_templates.xml"
STRINGS   = ROOT / "Main/_Module/ModuleData/dots_career_strings.xml"
...
def main():
    ...
    for path in (TEMPLATES, STRINGS):
        ...
        for career_id, archetype, lore in ABILITIES:
            key = f"dots_ability_{career_id}_tt"
