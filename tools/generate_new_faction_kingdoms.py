...
into dots_spkingdoms.xml / characters/clans.xml / characters/lords.xml / characters/heroes.xml.

Templates are read by id from the live files (gundabad for the two orc kingdoms, rivendell
for Lindon) and parametrized — no hand-copied XML. Idempotent via DOTS-NEWFACTIONS markers.
...
KINGDOMS = {
    "goblin": dict(
        ...
        lord_skill=("dots_orc_chieftain_skills", "dots_orc_warrior_skills", "dots_orc_female_skills"),
    ...
    "lindon": dict(
        ...
        lord_skill=("dots_elf_king_skills", "dots_elf_warrior_skills", "dots_elf_lady_skills"),
...
def main():
    spk = open(os.path.join(MD, "dots_spkingdoms.xml"), encoding="utf-8").read()
    ...
    for kid, k in KINGDOMS.items():
        ...
        blk = re.sub(r'name="\{=[^}]*\}[^"]*"', f'name="{{=dots_{kid}_name}}{k["name"]}"', blk, count=1)
        blk = re.sub(r'short_name="\{=[^}]*\}[^"]*"', f'short_name="{{=dots_{kid}_short_name}}{k["short"]}"', blk, count=1)
        blk = re.sub(r'title="\{=[^}]*\}[^"]*"', f'title="{{=dots_{kid}_title}}{k["title"]}"', blk, count=1)
        blk = re.sub(r'ruler_title="\{=[^}]*\}[^"]*"', f'ruler_title="{{=dots_{kid}_ruler_title}}{k["ruler"]}"', blk, count=1)
        blk = re.sub(r'text="\{=[^}]*\}[^"]*"', f'text="{{=dots_{kid}_desc}}{k["desc"]}"', blk, count=1)
    ...
    for kid in KINGDOMS:
        spk = upsert(spk, "</Kingdoms>", new_kingdoms[kid], f"DOTS-NEWFACTIONS:{kid}")
        clans = upsert(clans, "</Factions>", new_clans[kid], f"DOTS-NEWFACTIONS:{kid}")
        lords = upsert(lords, "</NPCCharacters>", new_lords[kid], f"DOTS-NEWFACTIONS:{kid}")
        heroes = upsert(heroes, "</Heroes>", new_heroes[kid], f"DOTS-NEWFACTIONS:{kid}")

    for path, content, label in [(os.path.join(MD, "dots_spkingdoms.xml"), spk, "dots_spkingdoms.xml"),
