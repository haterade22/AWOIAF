#!/usr/bin/env python3
"""Generate the missing v1.4.3 mandatory equipment rosters across DOTS's 12 cultures.
...
dots_child_equipment_templates.xml (post-S5a migration of IsNobleTemplate
...
(dots_equipment_sets_<culture>.xml), extracts the items from the FIRST
...
Output: Main/_Module/ModuleData/equipmentsets/dots_lord_template_equipment.xml
...
OUTPUT_FILE = EQUIP_DIR / "dots_lord_template_equipment.xml"

# 12 DOTS custom cultures (is_main_culture="true") + 6 XSLT-renamed vanilla cultures.
...
# To prevent that for DOTS's XSLT-renamed cultures (vlandia=Rohan, empire=Dunland, etc.),
# generate rosters for them too — sourcing items from each DOTS per-culture equipment file.
CULTURES: List[Tuple[str, Optional[str], Optional[str]]] = [
    # (culture_id, equipment_file_stem, fallback_for_missing_battle)
    # === 12 DOTS custom cultures (dots_spcultures.xml) ===
    ("gondor", "gondor", None),
    ("mordor", "mordor", None),
    ("erebor", "erebor", None),
...
def extract_first_roster(file_stem: str, id_pattern: str) -> Optional[str]:
    ...
    """
    file_path = EQUIP_DIR / f"dots_equipment_sets_{file_stem}.xml"
...
def generate() -> str:
    ...
    sections = [
        ...
        '     DOTS custom culture. Items are sourced from existing per-culture equipment files;',
    ...
    for culture, file_stem, fallback in CULTURES:
        ...
        # Teen rosters: civilian items are appropriate (DOTS doesn't have separate teen items).
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_battle_male",
            culture=culture,
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_battle_female",
            culture=culture,
            equipment_set_xml=battle_set_clean,
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_civilian_male",
            culture=culture,
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_civilian_female",
            culture=culture,
            equipment_set_xml=civilian_set_clean,
            flags_attrs='IsLordTemplate="true" IsFemaleTemplate="true"',
        ))
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_teen_male",
            culture=culture,
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_lord_teen_female",
            culture=culture,
            equipment_set_xml=teen_set,
        ...
        # cultures get via dots_child_equipment_templates.xml.
        if not file_stem:
            sections.append(build_roster(
                roster_id=f"dots_{culture}_lord_child_male",
                culture=culture,
                equipment_set_xml=civilian_set_clean,
                flags_attrs='IsLordTemplate="true" IsChildEquipmentTemplate="true"',
            ))
            sections.append(build_roster(
                roster_id=f"dots_{culture}_lord_child_female",
                culture=culture,
                equipment_set_xml=civilian_set_clean,
        ...
        sections.append(build_roster(
            roster_id=f"dots_{culture}_ruler_battle_male",
            culture=culture,
            equipment_set_xml=battle_set_clean,
            flags_attrs='IsKingdomRulerTemplate="true"',
        ))
        sections.append(build_roster(
            roster_id=f"dots_{culture}_ruler_battle_female",
            culture=culture,
            equipment_set_xml=battle_set_clean,
            flags_attrs='IsKingdomRulerTemplate="true" IsFemaleTemplate="true"',
        ))
        sections.append(build_roster(
            roster_id=f"dots_{culture}_ruler_civilian_male",
            culture=culture,
            equipment_set_xml=civilian_set_clean,
            flags_attrs='IsKingdomRulerTemplate="true"',
        ))
        sections.append(build_roster(
            roster_id=f"dots_{culture}_ruler_civilian_female",
            culture=culture,
            equipment_set_xml=civilian_set_clean,
            flags_attrs='IsKingdomRulerTemplate="true" IsFemaleTemplate="true"',
        ))
...
def main(argv: List[str] | None = None) -> int:
    ...
    if args.apply:
        ...
        print('    <XmlNode>')
        print('      <XmlName id="EquipmentRosters" path="equipmentsets/dots_lord_template_equipment"/>')
