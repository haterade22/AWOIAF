...
kingdoms = {
    'vlandia': {
        'dots_name': 'Rohirrim', 'kingdom': 'rohan',
    ...
    'empire': {
        'dots_name': 'Dunlending', 'kingdom': 'dunland',
    ...
    'aserai': {
        'dots_name': 'Haradrim', 'kingdom': 'harad',
    ...
    'khuzait': {
        'dots_name': 'Easterling', 'kingdom': 'rhun',
...
def generate_xslt_template(culture_id, config, culture_elem):
    ...
    L = []
    L.append(f'\t<!-- Rename {culture_id} to {config["dots_name"]} -->')
...
\t\t\t<xsl:attribute name="name">{{=DOTS_sturgia_culture}}Barding</xsl:attribute>
\t\t\t<xsl:attribute name="text">{{=DOTS_sturgia_desc}}{sturgia_text}</xsl:attribute>
\t\t\t<xsl:apply-templates select="node()"/>
\t\t</xsl:copy>
\t</xsl:template>
...
\t\t<xsl:copy>
\t\t\t<xsl:apply-templates select="@*[local-name() != 'name' and local-name() != 'text']"/>
\t\t\t<xsl:attribute name="name">{{=DOTS_battania_culture}}Variag</xsl:attribute>
\t\t\t<xsl:attribute name="text">{{=DOTS_battania_desc}}{battania_text}</xsl:attribute>
\t\t\t<xsl:apply-templates select="node()"/>
\t\t</xsl:copy>
\t</xsl:template>""")
...
# Write the file
with open('c:/Users/mikew/source/repos/DOTS/Main/_Module/ModuleData/spcultures.xslt', 'w', encoding='utf-8') as f:
