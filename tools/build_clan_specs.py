...
TROOPLESS_TABLE = {
    ...
    "khand":      ("battania",   None,        "FF8A5A1E", "FF5A1E18"),  # Variags: no DOTS pool -> colors only
}
...
def build_spec(lotr, clan_culture, pool_culture, base1, base2, registry, troop_index):
    ...
    n = len(clans)
    note = ("Colors-only (no DOTS troop pool); per-clan color = %s/%s base + per-clan variation." % (base1, base2)
            if colors_only else
