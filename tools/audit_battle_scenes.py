#!/usr/bin/env python3
"""
Compare DOTS's sp_battle_scenes.xml against vanilla (SandBox + NavalDLC) and against
on-disk SceneObj folders. sp_battle_scenes.xml maps campaign-map terrain cells
(map_indices) to a battle-terrain Scene id; if DOTS lists a Scene id that no longer
...
FILES = {
    "DOTS": MODULES / "DOTS" / "ModuleData" / "sp_battle_scenes.xml",
...
def main() -> None:
    ...
    print("=" * 70)
    print("CRASH SUSPECTS — DOTS battle Scene id with NO SceneObj folder on disk")
    print("=" * 70)
    ...
    if missing:
        ...
        print("  (none — every DOTS battle scene id exists on disk)")

    print("\n" + "=" * 70)
    print("DIFF — DOTS Scene ids NOT present in vanilla's current set")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("DIFF — vanilla Scene ids DOTS does NOT list (dropped coverage)")
    print("=" * 70)
    for s in sorted(vanilla - taom):
        print(f"  {s}")

    # map_indices coverage
    ti = map_indices(FILES["DOTS"])
    print("\n" + "=" * 70)
    print("map_indices coverage (DOTS)")
    print("=" * 70)
    full = set(range(256))
    uncovered = sorted(full - ti)
    print(f"  DOTS covers {len(ti)} distinct indices; range {min(ti) if ti else '-'}..{max(ti) if ti else '-'}")
    if uncovered:
