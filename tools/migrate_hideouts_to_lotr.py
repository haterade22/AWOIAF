#!/usr/bin/env python3
"""
Migrate the 99 vanilla-bandit hideouts in DOTS_Map/settlements.xml to LOTR-themed
...
Default base: $BANNERLORD_GAME_DIR/Modules/DOTS_Map -- override via --tm-path.
"""
...
def find_dots_map_dir(arg_path: str | None) -> Path:
    if arg_path:
        p = Path(arg_path)
        if not p.is_dir():
    ...
    if not game_dir:
        # Fall back to canonical DOTS dev install path
        game_dir = r"E:\Steam\steamapps\common\Mount & Blade II Bannerlord"
    p = Path(game_dir) / "Modules" / "DOTS_Map"
    if not p.is_dir():
        sys.exit(
            f"ERROR: DOTS_Map module not found at {p}.\n"
...
def main() -> int:
    ...
    ap.add_argument("--tm-path", default=None, help="Override DOTS_Map module path")
    args = ap.parse_args()

    if not (args.dry_run or args.apply):
        ap.error("Pass --dry-run or --apply")

    tm_dir = find_dots_map_dir(args.tm_path)
