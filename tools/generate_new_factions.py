#!/usr/bin/env python3
"""Generate all data files for the three new DOTS factions:
...
This script (PART 1) writes the standalone NEW files:
  ...
  Main/_Module/ModuleData/equipmentsets/dots_equipment_sets_{c}.xml
...
def main():
    ...
    src_equip = open(os.path.join(MD, "equipmentsets", "dots_equipment_sets_gundabad.xml"), encoding="utf-8").read()
    ...
    for culture, cfg in ORC_CULTURES.items():
        ...
        p = os.path.join(MD, "equipmentsets", f"dots_equipment_sets_{culture}.xml")
        ...
        print(f"[{culture}] race={cfg['race']}: troops_{culture}.xml (~{ntroops} troops), "
              f"npcs_{culture}.xml (~{nnpcs} npcs), dots_equipment_sets_{culture}.xml (~{nrost} rosters) — all well-formed")
    ...
    for culture in ORC_CULTURES:
        for f in [f"troops/troops_{culture}.xml", f"characters/npcs_{culture}.xml", f"equipmentsets/dots_equipment_sets_{culture}.xml"]:
