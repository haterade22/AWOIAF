...
DOTS debug log, the latest native rgl_log (ProgramData), and the latest crash dump,
...
def main():
    ...
    debug_log = newest(os.path.join(logs_dir, "dots_debug_*.log"))
    rgl_log = newest(os.path.join(args.programdata, "rgl_log_*.txt"), exclude="errors")
    crash_dirs = sorted(glob.glob(os.path.join(logs_dir, "dots_crash_*")),
    ...
    if debug_log:
        ...
        intercepts = count_substr(dl, "intercept char=dots_spider")
