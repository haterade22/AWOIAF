#!/usr/bin/env python3
"""
DOTS AI Translation Tool — produces first-draft translations via Claude API.

Walks DOTS, DOTS_Map, and LOTRLOME_Armory language XML files; finds entries
...
DOTS_LANG_DIR = REPO_ROOT / "Main" / "_Module" / "ModuleData" / "Languages"
DOTS_MAP_LANG_DIR = GAME_ROOT / "DOTS_Map" / "ModuleData" / "Languages"
...
def english_source_files(module: str) -> list[tuple[Path, str]]:
    ...
    For DOTS, English source is the per-module XML at ModuleData root.
    For DOTS_Map and Armory, the English source is the inline {=KEY}default in the data XML;
    we read these from the target-language file (it was populated as an English template earlier).
    """
    pairs = []
    if module in ("DOTS", "all"):
        # DOTS source XMLs at ModuleData root
        for src_name, tgt_template in [
            ("dots_module_strings.xml",                       "std_dots_module_strings_{locale}.xml"),
            ("dots_wanderer_strings.xml",                     "std_dots_wanderer_strings_{locale}.xml"),
            ("named_companions/named_companion_strings.xml",  "std_dots_named_companion_strings_{locale}.xml"),
            ("dots_cc_strings.xml",                           "std_dots_cc_strings_{locale}.xml"),
            ("dots_career_strings.xml",                       "std_dots_career_strings_{locale}.xml"),
            ("dots_messenger_strings.xml",                    "std_dots_messenger_strings_{locale}.xml"),
            ("dots_xslt_strings.xml",                         "std_dots_xslt_strings_{locale}.xml"),
        ]:
            src = REPO_ROOT / "Main" / "_Module" / "ModuleData" / src_name
            if src.exists():
                pairs.append(("DOTS", src, tgt_template))
    return pairs
...
def discover_entries(lang: str, module_filter: str) -> list[Entry]:
    ...
    # DOTS module
    if module_filter in ("DOTS", "all"):
        dots_lang_dir = DOTS_LANG_DIR / lang
        for _, source_xml, tgt_template in english_source_files("DOTS"):
            target_file = dots_lang_dir / tgt_template.format(locale=locale)
            entries.extend(_diff_files(source_xml, target_file))

    # DOTS_Map module — settlements.xml has inline {=KEY}default; we use the populated target as English source
    if module_filter in ("DOTS_Map", "all"):
        target_file = DOTS_MAP_LANG_DIR / lang / "loc_settlements.xml"
        # English source: extract inline keys from the settlements.xml itself
        source_xml = GAME_ROOT / "DOTS_Map" / "ModuleData" / "settlements.xml"
...
def _diff_files(source: Path, target: Path) -> list[Entry]:
    ...
    src_map = _parse_string_xml(source, strip_keys=True)  # for DOTS source XMLs with {=KEY}prefix
...
def _diff_against_settlement_source(settlements_xml: Path, target: Path) -> list[Entry]:
    """Special handler for DOTS_Map settlements: source XML has inline {=KEY}default."""
...
SYSTEM_PROMPT = """You are an expert translator for the Lord of the Rings video game mod "DOTS" (Dawn of the Stag), a Bannerlord total conversion. Translate English game text into {target_language} for in-game display.
...
def main():
    p = argparse.ArgumentParser(description="DOTS AI Translation Tool")
    p.add_argument("--lang", required=True, choices=list(LANGUAGES.keys()),
                   help="Target language code")
    p.add_argument("--module", default="all", choices=["DOTS", "DOTS_Map", "Armory", "all"])
    ...
    print(f"\n[DOTS Translator] Target: {lang_name} ({lang} / {locale})")
