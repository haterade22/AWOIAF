#!/usr/bin/env python3
"""Generate one GitHub issue draft per DOTS culture (skills+traits work).
...
CULTURES = [
    ...
    ("dale",       "sturgia",    "Dale (Bardings)",
     82, "man",
     "DOTS Dale roster is mostly DOTS-invented Slavic-named wives + nobles (Apolanea, "
     ...
     "No explicit canonical overrides — DOTS names don't map to Tolkien-canon Bard/Brand."),
    ...
    ("mirkwood",   "mirkwood",   "Mirkwood (Woodland Realm)",
     ...
     "Lothuial (DOTS queen), Feren (captain), Galion (butler/steward). Other Mirkwood "
    ...
    ("lothlorien", "lothlorien", "Lothlórien",
     ...
     "Polearm 295, Leadership 290), 1 placeholder lord (DOTS-invented)",
     ...
     "Rúmil, Orophin) are in Mirkwood not Lothlórien per DOTS's data."),
    ...
    ("dolguldur",  "dolguldur",  "Dol Guldur",
     59, "orc",
     "All DOTS-invented orc names (no canonical Khamûl entry in lords.xml — he'd be a "
    ...
    ("gundabad",   "gundabad",   "Mount Gundabad",
     50, "orc",
     "All DOTS-invented pale_uruk names. 5 compound chieftains (G1_1..G5_1); Bolgath "
     ...
     "DOTS uses the same race for both."),
    ...
    ("isengard",   "isengard",   "Isengard (Saruman)",
     ...
     "(DOTS bin lookup uses `uruk_hai` race). Berserkers + Warg-Riders covered by "
     "`orc_berserker` / `orc_warg` variants."),

    ("dunland",    "empire",     "Dunland (Hillmen / Saruman's auxiliaries)",
     68, "man",
     "DOTS Dunland is all-female shieldmaiden + raider warband leaders (Eldith Grey-Claw, "
     ...
     "via bio keyword scan. No canonical Tolkien named characters in DOTS's roster.",
    ...
    ("harad",      "aserai",     "Harad (Haradrim Southrons)",
     73, "man",
     "DOTS Harad is mostly Mumakil-lord wives + serpent-banner heirs + trade-caravan "
     ...
     "canonical override (Khadurak is named in DOTS as Taskral; his explicit override "
     "is in a separate enemy-lord roster TBD)."),

    ("khand",      "battania",   "Khand (Variags)",
     56, "man",
     "DOTS Khand has Welsh/Gaelic-named warband leaders + wives. Auto-assigned to "
    ...
    ("easterling", "khuzait",    "Easterlings of Rhûn",
     71, "man",
     "DOTS Easterlings: Mongol-named wives + heirs + chieftains. Auto-assigned to "
     ...
     "if DOTS exposes him as a player-meetable character (currently not in lords.xml under "
     "khuzait — likely in Dolguldur roster)."),

    ("umbar",      "umbar",      "Umbar (Corsairs)",
     10, "man",
     "DOTS Umbar has 10 Black Númenórean corsair lords with authentic Adunaic names "
    ...
    ("shaghana",   "shaghana",   "Shaghana",
     9, "man",
     "DOTS-invented southern-desert sub-culture (9 male warlords ages 27-55). No canonical "
     ...
     "if/when DOTS defines distinct combat/diplomatic style."),

    ("abanissa",   "abanissa",   "Abanissa",
     8, "man",
     "DOTS-invented coastal sub-culture (8 male warlords ages 28-52). Same treatment as "
...
(2026-05-27), {lore} adult NPCs in DOTS had empty `<skills/>` blocks or vanilla-default
...
Tolkien lore (where applicable) or sensible archetypes (for DOTS-invented characters).
...
- All adult {lore} NPCs receive one of the shared DOTS archetype templates (10 base
...
never get "young_lord" archetype from low age (immortal — DOTS age fields are placeholders).
...
SOURCES_PER_CULTURE = {
    ...
    "shaghana": [
        "_No canonical Tolkien source — Shaghana is DOTS-invented._",
    ],
    "abanissa": [
        "_No canonical Tolkien source — Abanissa is DOTS-invented._",
    ],
}
...
def main():
    ...
    lines = ["# Lords Skills+Traits — GitHub Issue Drafts", "",
             "One draft per DOTS culture. To create each issue:", "",
             "```bash",
