...
Idempotent: each inserted region is wrapped in <!-- DOTS-NEWFACTIONS:<id>:BEGIN/END -->
markers and stripped before re-insert, so re-running is safe.

Targets:
  dots_wanderers.xml                       <- 10 wanderers per culture
  equipmentsets/dots_wanderer_equipment.xml<- wanderer companion roster per culture
  dots_partyTemplates.xml                  <- 12 kingdom party templates per culture
  dots_spcultures.xml                      <- <Culture> block per culture (NO cultural_feats)
  dots_module_strings.xml                  <- str_faction_*/str_culture_* per culture
...
LAYOUT = json.load(open(os.path.join(ROOT, "tools", "dots_new_factions_layout.json"), encoding="utf-8"))
...
ORC = {
    "goblin":            {"race": "goblin", "tag": "Goblin", "raceword": "Goblin", "short": "gob",
                          ...
                          "feats": ["dots_goblin_party_size", "dots_goblin_volunteer_rate",
                                    "dots_goblin_snow_speed", "dots_goblin_food_consumption"]},
    "mistymountainorcs": {"race": "orc", "tag": "Misty Mountains", "raceword": "Orc", "short": "mmo",
                          ...
                          "feats": ["dots_mistymountainorcs_army_influence_cost", "dots_mistymountainorcs_party_size",
                                    "dots_mistymountainorcs_snow_speed", "dots_mistymountainorcs_food_consumption"]},
}
...
def main():
    src_cult = open(os.path.join(MD, "dots_spcultures.xml"), encoding="utf-8").read()
    src_wand = open(os.path.join(MD, "dots_wanderers.xml"), encoding="utf-8").read()
    src_wequip = open(os.path.join(MD, "equipmentsets", "dots_wanderer_equipment.xml"), encoding="utf-8").read()
    src_party = open(os.path.join(MD, "dots_partyTemplates.xml"), encoding="utf-8").read()
    src_strings = open(os.path.join(MD, "dots_module_strings.xml"), encoding="utf-8").read()
    src_child = open(os.path.join(MD, "equipmentsets", "dots_child_equipment_templates.xml"), encoding="utf-8").read()
    src_lord = open(os.path.join(MD, "equipmentsets", "dots_lord_template_equipment.xml"), encoding="utf-8").read()
    src_edu = open(os.path.join(MD, "equipmentsets", "dots_education_equipment_templates.xml"), encoding="utf-8").read()
    ...
    gund_str_lines = [ln for ln in src_strings.splitlines()
                      if "gundabad" in ln and "<string" in ln and "dots_faction_" not in ln]
    ...
    for culture, cfg in ORC.items():
        ...
        out_cult = upsert_before(out_cult, "</SPCultures>", cblock, f"DOTS-NEWFACTIONS:{culture}")
        # --- wanderers ---
        out_wand = upsert_before(out_wand, "</NPCCharacters>", transform(gund_wanderers, culture, cfg),
                                 f"DOTS-NEWFACTIONS:{culture}")
        # --- wanderer roster ---
        ...
        out_strings = upsert_before(out_strings, "</strings>" if "</strings>" in out_strings else "</base>",
                                    transform(gund_strings, culture, cfg), f"DOTS-NEWFACTIONS:{culture}")
    ...
    for path, content in [(os.path.join(MD, "dots_spcultures.xml"), out_cult),
                          (os.path.join(MD, "dots_wanderers.xml"), out_wand),
                          (os.path.join(MD, "equipmentsets", "dots_wanderer_equipment.xml"), out_wequip),
                          (os.path.join(MD, "dots_partyTemplates.xml"), out_party),
                          (os.path.join(MD, "dots_module_strings.xml"), out_strings),
                          (os.path.join(MD, "equipmentsets", "dots_child_equipment_templates.xml"), out_child),
                          (os.path.join(MD, "equipmentsets", "dots_lord_template_equipment.xml"), out_lord),
                          (os.path.join(MD, "equipmentsets", "dots_education_equipment_templates.xml"), out_edu)]:
    ...
    sm = re.sub(r'[ \t]*<!-- DOTS-NEWFACTIONS-REG:BEGIN -->.*?<!-- DOTS-NEWFACTIONS-REG:END -->\n', '',
                sm, flags=re.DOTALL)
    ...
    def xmlnode(idv, path):
        return (f'    <XmlNode>\n'
                ...
                f'      </IncludedGameTypes>\n'
                f'    </XmlNode>\n')
    reg = "    <!-- DOTS-NEWFACTIONS-REG:BEGIN -->\n"
    for culture in ORC:
        reg += xmlnode("NPCCharacters", f"troops/troops_{culture}")
        reg += xmlnode("NPCCharacters", f"characters/npcs_{culture}")
        reg += xmlnode("EquipmentRosters", f"equipmentsets/dots_equipment_sets_{culture}")
    reg += "    <!-- DOTS-NEWFACTIONS-REG:END -->\n"
    anchor = re.search(r'(<XmlNode>\s*<XmlName id="EquipmentRosters" path="equipmentsets/dots_equipment_sets_gundabad"/>.*?</XmlNode>\n)',
                       sm, re.DOTALL)
    if not anchor:
