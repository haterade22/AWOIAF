...
Scope (and explicit non-goals — keep this tight):
  ...
  - Not a game-data graph — tools/dots_schema.py + the dots-moduledata MCP own
...
def bridges(nodes, adj) -> list[tuple[Path, Path]]:
    ...
    O(E·(V+E)) — ~1s at DOTS's current ~315 nodes / ~500 edges, growing roughly
