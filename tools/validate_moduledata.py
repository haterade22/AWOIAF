#!/usr/bin/env python3
"""Schema-driven validator for DOTS ModuleData — one reusable pass that
...
prints a severity-classified report. See tools/dots_schema.py for the engine
and docs/features/moduledata-validation.md for the full design.

Checks (each maps to a recurring DOTS bug class):
  ...
  DUPLICATE_CULTURE_ID       same Culture id defined twice in dots_spcultures.xml
...
def main() -> int:
    ...
    ap.add_argument("--moduledata", default=str(MODULEDATA),
                    help="Path to DOTS Main/_Module/ModuleData")
    ...
    if not game_modules.exists():
        ...
        # registry will be DOTS-only, so external refs will look broken.
