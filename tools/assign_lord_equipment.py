"""
Assign LOTR equipment templates to DOTS lords.

Replaces vanilla SandBoxCore equipment template references (ase_, bat_, emp_,
khu_, stu_, vla_) with DOTS culture-specific equipment templates from the
dots_equipment_sets_*.xml files.
...
LORDS_XML = r'c:\Users\mikew\source\repos\DOTS\Main\_Module\ModuleData\characters\lords.xml'
...
def build_template_id(family, has_default, is_battle, suffix):
    """Build the full DOTS template ID."""
    if is_battle:
...
def main():
    ...
    for lord in lords:
        ...
        for line_idx in lord['eq_lines']:
            ...
            # Skip if already a DOTS template
    ...
    print(f"\nSkipped: {skipped_inline} inline, {skipped_already_taom} already DOTS, {skipped_unknown_culture} unknown culture")
