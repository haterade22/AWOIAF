...
<#
.SYNOPSIS
    For each culture, list Armory items (and their meshes) that no DOTS
    troop or equipment-set of that culture equips.

.DESCRIPTION
    Three-way join:
      1. Armory items     — itemId → (mesh, culture, sourceFile)
      2. DOTS troop XML   — itemIds referenced under troops_<culture>.xml
                            and dots_equipment_sets_<culture>.xml
      3. DOTS generic XML — itemIds referenced under lords.xml,
                            dots_char_creation_equipment.xml,
                            dots_wanderer_equipment.xml, npcs_*.xml,
    ...
    Per culture, classify each declared item as one of:
      ...
      - UNUSED        : referenced nowhere in DOTS
...
.PARAMETER DotsModuleData
    DOTS's ModuleData root. Default: <repo>/Main/_Module/ModuleData
...
# Default DOTS ModuleData
$repoRoot = Split-Path $PSScriptRoot -Parent
if (-not $DotsModuleData) {
    $DotsModuleData = Join-Path $repoRoot 'Main\_Module\ModuleData'
}
if (-not $OutputDir) {
...
Write-Progress2 "taom data:     $DotsModuleData"
Write-Progress2 "output:        $OutputDir"

# -----------------------------------------------------------------------------
# Phase 1 — Item inventory from Armory (and DOTS-defined items, if any)
# -----------------------------------------------------------------------------
...
# Also pick up any items defined under DOTS (rare but possible)
$taomItemXmls = @(Get-ChildItem -Path $DotsModuleData -Filter '*.xml' -Recurse -File |
    Where-Object { $_.FullName -match '\\(items|equipment)\\' -or $_.Name -like 'dots_*items*.xml' })
foreach ($f in $taomItemXmls) {
    Read-ItemsFile -Path $f.FullName
}
Write-Progress2 "phase 1: total $($items.Count) items after DOTS scan"

# -----------------------------------------------------------------------------
# Phase 2 — Item-id references across DOTS XML
# -----------------------------------------------------------------------------

# All DOTS XML files
$taomXmls = Get-ChildItem -Path $DotsModuleData -Filter '*.xml' -Recurse -File
...
Write-Progress2 "phase 2: $($fileRefs.Count) DOTS XML files contain Item.* references"

# -----------------------------------------------------------------------------
# Phase 3 — Classify each DOTS XML file by associated culture (filename-based)
# -----------------------------------------------------------------------------

# Map culture suffix -> canonical culture id (matching Armory item culture values)
# Most DOTS files use file-suffix == culture id; XSLT cultures (rohan etc.) use lore names in filenames but engine ids in Armory.
$cultureAliases = @{
    'gondor'      = 'gondor'
    'mordor'      = 'mordor'
    'erebor'      = 'erebor'
    ...
    'dolguldur'   = 'dolguldur'
    'umbar'       = 'umbar'
    # XSLT cultures — DOTS filenames use LOTR names; Armory items use engine ids
    'rohan'       = 'vlandia'
    'dunland'     = 'empire'
    'harad'       = 'aserai'
    'rhun'        = 'khuzait'
...
function Get-FileClassification {
    ...
    # troops_<culture>.xml or dots_equipment_sets_<culture>.xml -> culture-specific
    if ($name -match '^troops_([a-z0-9_]+)\.xml$') {
        $tag = $Matches[1]
        if ($cultureAliases.ContainsKey($tag)) { return @{ Type='culture'; Culture=$cultureAliases[$tag] } }
    }
    if ($name -match '^dots_equipment_sets_([a-z0-9_]+)\.xml$') {
        $tag = $Matches[1]
        if ($cultureAliases.ContainsKey($tag)) { return @{ Type='culture'; Culture=$cultureAliases[$tag] } }
...
$L.Add('')
$L.Add('Joins Armory items (declared with `culture="Culture.X"`) against DOTS item-id refs grouped by source-file culture (`troops_<X>.xml`, `dots_equipment_sets_<X>.xml`, `npcs_<X>.xml`).')
$L.Add('')
...
$L.Add('')
$L.Add('**Properly used:** referenced by `troops_<culture>.xml` or `dots_equipment_sets_<culture>.xml`.  ')
$L.Add('**Hero-only:** referenced only by generic files (lords.xml, dots_char_creation_equipment.xml, dots_wanderer_equipment.xml, etc.).  ')
...
$L.Add('**Unused:** declared with this culture but nothing in DOTS references the item ID anywhere.')
$L.Add('')
...
foreach ($row in ($summary | Sort-Object Culture)) {
    ...
    if ($row.Unused -gt 0) {
        ...
        $L.Add('### Unused items (grouped by slot, no DOTS ref anywhere)')
        $L.Add('')
