#!/usr/bin/env python3
"""Build the authoritative DOTS clan registry: every clan that exists in-game
...
Sources:
  - DOTS  characters/clans.xml          (DOTS-authored noble + bandit clans)
  - vanilla SandBox/ModuleData/spclans.xml (the base spclans.xslt transforms)
  - DOTS  spclans.xslt                  (which vanilla clans are renamed/removed + culture overrides)
...
def main():
    ...
    # 1) DOTS-authored clans (clans.xml) — these win, source=xml
    dots_ids = set()
    ...
    for row in faction_rows(VANILLA_SPCLANS):
        ...
        if cid in dots_ids:
            continue  # already DOTS-authored
    ...
    print("DOTS CLAN REGISTRY  (clans existing in-game after spclans.xslt)\n")
