...
Outputs:
  ...
  2. All scene_names referenced by DOTS_Map settlements.
  3. CRASH SUSPECTS: scene_names referenced anywhere that have NO scene folder on disk.
  4. Vanilla-vs-DOTS diff (scenes DOTS uses that vanilla doesn't, and vice versa).
"""
...
SETTLEMENT_FILES = {
    ...
    "DOTS_Map": MODULES / "DOTS_Map" / "ModuleData" / "settlements.xml",
    "DOTS_shadow": MODULES / "DOTS" / "ModuleData" / "settlements.xml",
}
...
def main() -> None:
    ...
    print("=" * 70)
    any_missing = False
    for label in ("vanilla_SandBox", "vanilla_NavalDLC", "DOTS_Map", "DOTS_shadow"):
        d = refs[label]
    ...
    # Diff
    print("\n" + "=" * 70)
    print("DIFF — DOTS_Map scenes NOT used by vanilla (custom or possibly stale)")
    print("=" * 70)
