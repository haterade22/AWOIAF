...
def render_briefing(topic: str, topic_dir: Path, files: list[Path], candidates: dict[Path, list[str]]) -> str:
    ...
    out.append("")
    if not candidates:
        out.append("_(no existing DOTS doc matched these keywords — research node will have an empty cross-references section unless the skill finds additional connections.)_")
    else:
    ...
    out.append("")
    out.append("<list DOTS features / RCAs / ADRs / memory entries that connect to this topic>")
    out.append("")
