...
<#
.SYNOPSIS
    Generates a committed snapshot of the TaleWorlds engine signatures that DOTS's GameModel
    overrides and Harmony patches depend on, so research does not require the external decompile
    dump (E:\Decompiled_Bannerlord\).

.DESCRIPTION
    Reflects over the *installed* Bannerlord v1.4.5 assemblies + DOTS.dll and writes two markdown
    files under docs/reference/taleworlds-api-snapshot/:
      - gamemodel-bases.md : each Dots*Model, its Default*Model base, and the exact base signature
    ...
    The type list is auto-derived from DOTS.dll, so the snapshot tracks the code without a curated
...
$testBin = Join-Path $repoRoot "DOTS.Tests\bin\Debug\net472"
if (-not (Test-Path (Join-Path $testBin 'DOTS.dll'))) {
    $testBin = Join-Path $repoRoot "DOTS.Tests\bin\Release\net472"
}
if (-not (Test-Path (Join-Path $testBin 'DOTS.dll'))) {
    throw "DOTS.dll not found in test bin — build DOTS.Tests first (dotnet build DOTS.Tests/DOTS.Tests.csproj)."
}
...
$taom = [Reflection.Assembly]::LoadFrom((Join-Path $testBin 'DOTS.dll'))
...
[void]$gm.AppendLine('')
[void]$gm.AppendLine('Auto-generated by tools/snapshot_api_surface.ps1. For each Dots*Model, the base Default*Model')
...
[void]$pt.AppendLine('class in DOTS.dll and the v1.4.5 engine method it patches, resolved the way Harmony resolves it.')
[void]$pt.AppendLine('Live-verified every build by DOTS.Tests/Migration/HarmonyPatchBindingTests.')
[void]$pt.AppendLine('')
