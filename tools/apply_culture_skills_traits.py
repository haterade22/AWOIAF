...
LORD_SKILL_SETS = REPO / "Main" / "_Module" / "ModuleData" / "dots_lord_skill_sets.xml"
...
CULTURES = {
    ...
    'dale': {
        ...
        'canonical': {
            # The DOTS Dale roster is largely DOTS-invented wives + nobles; few canonical heroes here.
        },
    },
    ...
    'mirkwood': {
        ...
        'canonical': {
            ...
            'lord_M1_2': dict(archetype='elf_queen'),   # Lothuial — DOTS-invented queen
    ...
    'dolguldur': {
        ...
        'canonical': {
            # DOTS lords.xml has no canonical Khamûl entry by name; D_* IDs are generic orc captains.
    ...
    # ====================================================================
    # SHAGHANA — DOTS-invented southern desert culture
    # ====================================================================
    'shaghana': {
        'culture_id': 'shaghana',
        'lore_name': 'Shaghana',
    ...
    # ====================================================================
    # ABANISSA — DOTS-invented coastal/southern culture
    # ====================================================================
    'abanissa': {
        'culture_id': 'abanissa',
        'lore_name': 'Abanissa',
...
def default_archetype(age: int, female: bool, culture_data: dict = None) -> str:
    ...
    if race == 'elf':
        # Elves don't really age in DOTS age numbers (placeholder ages); treat all as mature
...
def get_skill_template_name(npc_id: str, age: int, female: bool, bio: str, culture_data: dict) -> str:
    """Return the DOTS SkillSet id this NPC should reference via skill_template.

    - Canonical NPC with explicit `skills=` override: dots_canonical_<id>_skills
    - Otherwise: dots_<archetype>_skills
    """
    canonical = culture_data.get('canonical', {})
    if npc_id in canonical and 'skills' in canonical[npc_id]:
        return f'dots_canonical_{npc_id}_skills'
    if npc_id in canonical and 'archetype' in canonical[npc_id]:
        return f"dots_{canonical[npc_id]['archetype']}_skills"
    return f'dots_{archetype_from_bio(bio, age, female, culture_data)}_skills'


def build_skill_sets_xml() -> str:
    """Generate the full dots_lord_skill_sets.xml content.

    Includes:
    - One <SkillSet> per archetype in BASE_ARCHETYPES (dots_<arch>_skills)
    - One <SkillSet> per canonical NPC with explicit `skills=` (dots_canonical_<id>_skills)
    ...
    for arch_name, arch_data in BASE_ARCHETYPES.items():
        sets[f'dots_{arch_name}_skills'] = arch_data['skills']
        ...
        for npc_id, c in cdata.get('canonical', {}).items():
            if 'skills' in c:
                ...
                if 'archetype' in c:
                    ...
                    sets[f'dots_canonical_{npc_id}_skills'] = base
                else:
                    # Full explicit skills dict (no archetype base)
                    sets[f'dots_canonical_{npc_id}_skills'] = dict(c['skills'])
...
def main():
    ...
    ap.add_argument('--skillsets-only', action='store_true',
                    help='Only regenerate dots_lord_skill_sets.xml; skip lords.xml/xslt edits')
    ...
    print(f"Generated dots_lord_skill_sets.xml ({n_sets} SkillSets)")
    if args.apply:
