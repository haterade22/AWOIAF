#!/usr/bin/env python3
"""Schema-driven cross-reference + validation engine for DOTS ModuleData XML.

This is DOTS's port of the IDEAS behind TheOldRealms/TOR_Tools (MIT) —
its `SchemaDefinition`/`CrossReferenceService`/`ValidationService` — into DOTS's
...
Design goals:
  ...
  * Prefix-based ref resolution (Item. / NPCCharacter. / Culture. / PartyTemplate.)
    ...
    existing DOTS validators already match.
...
class Validator:
    ...
    def _entry_by_line(self, text: str, entry_el: str, id_attr: str) -> dict:
        ...
        Handles both single-line and multi-line element opens (DOTS troop files
    ...
    def _is_civilian_roster(roster_id: str) -> bool:
        ...
        `child_template_` catches the child rosters (dots_child_equipment_templates.xml
...
# Party templates are defined as <MBPartyTemplate id="..."> (DOTS + vanilla
...
def _scan_files(file_list, pattern):
    ...
    files (dots_spcultures.xml + vanilla spcultures.xml), never from feature
...
def build_registries(moduledata, game_modules, armory_root=None) -> Registries:
    """Build cross-reference registries from the real game install + DOTS repo.
    ...
    # a directory rglob — feature config files (dots_careers.xml career groups,
    ...
    if game_modules is None:
        ...
        # are DOTS-only and therefore INCOMPLETE — every reference to a vanilla
        ...
        # and enum checks remain reliable from DOTS data + the vanilla-culture
