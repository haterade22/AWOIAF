#!/usr/bin/env python3
"""
Audit every `id="Item.X"` reference in DOTS ModuleData XML against the actual
v1.4.5 item registry (vanilla + LOTRLOME_Armory + Alliance.Wargs + DOTS-owned).
...
DOTS_MODULEDATA = ROOT / "Main/_Module/ModuleData"

GAME_MODULES = Path("E:/Steam/steamapps/common/Mount & Blade II Bannerlord/Modules")

# Modules whose Item XMLs build the runtime registry. DOTS's own XMLs are
# included so DOTS-authored items count as valid.
ITEM_REGISTRY_ROOTS = [
...
def collect_references(dots_root: Path) -> dict[str, list[tuple[Path, int]]]:
    """
    Walk every XML under DOTS ModuleData and collect `id="Item.X"` refs.
    Returns dict: item_id -> [(file_path, line_number), ...]
    """
    refs = defaultdict(list)
    for xml in dots_root.rglob("*.xml"):
        try:
            text = xml.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            continue
...
def main():
    ...
    print("Collecting DOTS ModuleData Item.X references...", file=sys.stderr)
    refs = collect_references(DOTS_MODULEDATA)
    print(f"  {len(refs)} distinct items referenced from DOTS", file=sys.stderr)
