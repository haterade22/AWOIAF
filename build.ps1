# DOTS Bannerlord Mod Build Script (.NET Framework 4.7.2)
...
Write-Host "--- DOTS Bannerlord Mod Build Script (.NET Framework 4.7.2) ---" -ForegroundColor Cyan
...
& dotnet build "DOTS.sln" -c $Configuration --no-restore
if ($LASTEXITCODE -ne 0) {
...
if ($RunTests) {
    ...
    & dotnet test "DOTS.Tests" -c $Configuration --no-build
    if ($LASTEXITCODE -ne 0) {
