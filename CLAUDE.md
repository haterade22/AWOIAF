...
Bannerlord 1.4 total conversion mod (DOTS - Dawn of the Stag)
...
| Run tests | `dotnet test DOTS.Tests` |
...
| **Verify Before Reference** | Before writing `Sprite="X"` read `DOTSSpriteData.xml`. Before `PrefabExtension` injection, decompile vanilla target to check child assumptions. Before `IoC.Resolve` in hot path, use lazy cache. |
...
**Convert vague asks into testable objectives BEFORE the first Edit.** "Fix the bug" is not a task — "write a failing test that reproduces the NRE, make it pass, verify no regressions in `DOTS.Tests`" is. State the pass/fail criterion up front so the work has a defined end. This is what `/investigate` Phase 1 and `/verify` enforce locally; the same discipline applies to any non-trivial change, not just debugging. (Source: karpathy-skills `goal-driven-execution`.)
...
**When porting C++ code from an upstream mod into DOTS (`Dependencies/*.NativeHooks/`, scene scripts, or any other vendored native), audit the port from scratch. "Upstream worked" only means "produced correct output" — it does NOT mean the port is fit to ship in DOTS.**
...
| `/issue [bug\|feature\|crash] [desc]` | Create a GitHub issue with all required DOTS sections |
...
| `/lord-skills [name\|culture]` | Assign lore-driven lord skills + traits via the DOTS SkillSet system (`docs/ai-includes/lord-skills-authoring.md`). |
...
| `/finish-branch [branch] [base]` | Integrate a merge-ready branch into trunk: FF-check → merge → regenerate backlinks → CHANGELOG → delete branch (local+remote) → push (confirm). Post-`/ship`; DOTS trunk-based, not Git Flow. |
| `/adopt-external [url]` | Review an external repo/article and fold the useful parts into DOTS: security-vet first → map novel-vs-duplicative → tiered recommendation → port (never install) → adversarial review → commit. Follows `docs/ai-includes/external-repo-adoption.md`. |
| `/security-scan` | Audit DOTS's own Claude config (`.claude/`, `.mcp.json`, `settings*.json`, `CLAUDE.md`) for committed secrets, over-broad permissions, hook exfiltration, MCP risk, hidden-unicode injection. Runs `tools/audit_claude_config.py`. |
...
When the user's message matches one of these patterns, **proactively invoke** the listed skill via the Skill tool. The skill has structured workflows, gates, and DOTS-specific patterns that produce better results than ad-hoc help. (Note: invoking the Skill tool still respects the user's tool-permission settings — the user may see a confirmation prompt unless allowlisted.)

### Strong proactive-invoke triggers

| User intent / phrase | Invoke | Confidence gate |
|----------------------|--------|-----------------|
| "this is broken", "why isn't this working", "it was working yesterday", crash logs, stack traces, exceptions from a DOTS patch or service | **`/investigate`** — never debug ad-hoc; the Iron Law is non-negotiable | None — always |
...
| Bash script bug, build infra issue, MCP server error, hook script crash — anything outside DOTS C# | **`debugger` agent** (Task tool) | None — but route DOTS C# bugs to `/investigate` instead |
...
| User shares an external repo/article "to see what we can adopt", "review this repo", "next repo", or pastes a GitHub URL to evaluate for adoption into DOTS | **`/adopt-external`** | None — always. It runs the security-vet-first → map → tier → port-never-install → review → commit cycle. |
...
| `tests.md` | `DOTS.Tests/**` | TDD, naming, AAA pattern, coverage |
| `xml-data.md` | `ModuleData/**/*.xml` | NPC naming, region codes, formatting |
| `troops.md` | `troops/**`, `dots_partyTemplates.xml`, `TroopProgression/**` | Troop checklist, races, party templates, save compat |
...
| `moduledata-validation.md` | `troops/troops_*.xml`, `characters/*.xml`, `equipmentsets/*.xml`, `dots_spcultures.xml`, `dots_partyTemplates.xml`, `named_companions/*.xml`, `dots_wanderers.xml`, `dots_education_character_templates.xml`, `tools/schemas/*.json` | Run `python tools/validate_moduledata.py` before committing ModuleData edits; schemas are source-of-truth; add new NPCCharacter-def files to `dots_npccharacter.json` applies_to. |
| `vanilla-data-comparison.md` | `**/settlements.xml`, `**/sp_battle_scenes.xml`, `**/spcultures.xml`, `**/dots_spcultures.xml`, `**/spclans.xml`, `**/spkingdoms.xml`, `**/*.xslt` | Compare against current installed vanilla before modifying mirrored data. Vanilla renames/removes scenes & re-schemas XML between versions → stale DOTS refs crash. Scene-ref audit tools + post-bump checklist. |
...
| `feature-builder` | Build features following DOTS architecture |
| `debugger` | Generic debugging for non-DOTS-specific issues (tooling, scripts, CI). Use `/investigate` for DOTS C# bugs. |
...
3. **The relevant tool reminder** — e.g. "for TaleWorlds signatures use `pwsh tools/dots-src.ps1 path <Type>`"; "build/test with `dotnet … -p:DisableModuleCopy=true`, not `./build.ps1`".
...
| Compare DOTS data against current vanilla before editing mirrored XML | [.claude/rules/vanilla-data-comparison.md](./.claude/rules/vanilla-data-comparison.md) — settlements/sp_battle_scenes/spcultures/xslt; auto-loads when those files are edited |
...
| Add or fix lord skills + traits (any culture, any canonical character) | [lord-skills-authoring.md](./docs/ai-includes/lord-skills-authoring.md) — DOTS SkillSet system, archetype catalog, per-NPC override recipes, gotchas |
...
12 supported languages (BR, CNs, CNt, DE, FR, IT, JP, KO, PL, RU, SP, TR) × 1 module (DOTS) = GoT string set. Translation infrastructure is fully ported from TAOM — run `tools/translate_with_claude.py` once GoT strings are authored.

| Component | Location | Notes |
|-----------|----------|-------|
| **Translator-facing guide** | [docs/localization/TRANSLATOR_GUIDE.md](./docs/localization/TRANSLATOR_GUIDE.md) | Full workflow, AI pipeline, manual fallback |
| **Source loc XMLs** (English defaults + translator's discoverable key list) | `Main/_Module/ModuleData/dots_*_strings.xml` (×6) + `dots_xslt_strings.xml` | 7 source files, ~6,238 entries. Each entry uses `text="{=KEY}default"` format. |
| **Per-language translation files** | `Main/_Module/ModuleData/Languages/<LANG>/std_dots_*.xml` | 7 files per language. Engine auto-discovers via `language_data.xml`. |
| **External module translations** | `<game>/Modules/DOTS_Map/ModuleData/Languages/<LANG>/loc_settlements.xml` | Not in repo (deployed straight to game install). |
...
| **Validation tests** | [DOTS.Tests/Infrastructure/Localization/LanguageDataXmlTests.cs](./DOTS.Tests/Infrastructure/Localization/LanguageDataXmlTests.cs) | Enforces 7 LanguageFile refs per language, well-formed XML, no missing files. |

**When adding new DOTS C# code that displays text to the player:** wrap with `{=KEY}default` format (e.g., `new TextObject("{=dots_my_feature_label}My Feature")`) and add the key to `dots_module_strings.xml`. Then re-run the translation tool to propagate to all 11 AI-translated languages.
...
**When XSLT files inject new text with `{=KEY}default`:** the keys need to be harvested into `dots_xslt_strings.xml` (see commit `20713a1` for the precedent). Run `tools/translate_with_claude.py` after the harvest to propagate.
...
| Mod tests | `DOTS.Tests/` (MSTest + NSubstitute) |
...
| CulturalFeats | `Main/Features/CulturalFeats/` (DotsCulturalFeats, 16 GameModel overrides) |
| CustomBattles | `Main/Features/CustomBattles/` (Custom battle factions, commanders, troops) |
| Arena | `Main/Features/Arena/` (DotsTournamentModel — per-participant culture armor) |
| MainMenuCustomizer | `Main/Features/MainMenuCustomizer/` (hide Campaign, rename Sandbox → "Enter The Age Of Rebellion") |
...
| Vendored Main-module DLLs | `Main/_Module/bin/Win64_Shipping_Client/` — `MinHook.x64.dll`. Allowlisted in `.gitignore`. `DOTS.dll` + `DOTS.pdb` stay ignored (build outputs, regenerated each build). MCMv5 is provided by DOTS.Dependencies (`Bannerlord.MBOptionScreen*.dll` + `MCM.UI.Adapter.MCMv5.dll`) + the `Bannerlord.MCM` NuGet — do NOT vendor `MCMv5.dll` here. **As of 2026-05-24, the `BehaviorTrees.dll` and `BehaviorTreeWrapper.dll` vendored binaries are gone — their source was decompiled and inlined to `Main/BehaviorTrees/` + `Main/BehaviorTreeWrapper/` and compiles into `DOTS.dll`. Do NOT re-vendor; edit the inlined source instead.** |
| DOTS.Dependencies stub modules | `Stubs/Bannerlord.{Harmony,UIExtenderEx,ButterLib,MBOptionScreen}/_Module/SubModule.xml` — four alias stubs that declare the standard BUTR module IDs so third-party mods depending on those IDs are toggleable in the vanilla launcher. Each now (DR3 Phase 4 — 2026-05-27) ships a real `<SubModule>` entry referencing `DOTS.Dependencies.AliasStubSubModule` (try/catch-wrapped MBSubModuleBase). Versions use `vX.Y.99.0` strategy to satisfy any reasonable lower-bound version pin. Deployed via `DeployDOTSDependenciesStubs` MSBuild target. When a BUTR `PackageReference` BUMPS its minor (e.g., Harmony 2.4.x → 2.5.x), bump the matching stub to the new minor's `.99.0`. See `docs/migration/dr3-maintenance.md` "Stub modules" + "Maintenance rule (v99 strategy)" sections. |
| DOTS.Dependencies defensive infrastructure | `Dependencies/Foundation/` (DR3 Phase 4 — 2026-05-27). 11 classes that port BetaDeps v0.7.5.1's runtime error-tolerance framework under MIT clean-room rewrite: `RuntimeLog` (path resolver), `DiagLog` (threadsafe append-only logger to `<game>/Modules/DOTS.Dependencies/diag.log`), `ReflectionUtils` (small helpers), `VersionProbe` (Bannerlord version + branch detect), `IncompatibleModDetector` (crash-loop detection via `session-launching.marker` + `last-good-modlist.txt` diff — read-only, no XML mutation), `PatchShield` (Finalizer on every Harmony patch, catches MissingMethod/MissingField/TypeLoad trinity, auto-unpatches offending owner), `SaveShield` + `FailureRecord` + `FailedModsCatalog` (Finalizer on 10 TaleWorlds save/load/mission methods, attributes culprit via stack walk, writes to `failed-mods-catalog.txt`), `SubModuleConstructionGuard` (Harmony Finalizer on MBSubModuleBase ctors), `CollectAssemblyTypesShim` (Finalizer on `Assembly.GetTypes` to handle `ReflectionTypeLoadException`). Wired into `AliasStubSubModule.ctor` (early phase) + `Dependencies/SubModule.OnSubModuleLoad` (late phase) + `OnGameInitializationFinished` (success marker). Opt-out flags: `patchshield-disabled.flag`, `saveshield-swallow-disabled.flag` in the module dir. See `docs/migration/dr3-maintenance.md` "Defensive infrastructure" section. |
| BehaviorTrees library (inlined) | `Main/BehaviorTrees/` — generic BT engine (Selector, Sequence, RandomSelector, Decorators, Tasks, blackboard). No TaleWorlds dependencies. Compiles into `DOTS.dll`. |
| BehaviorTreeWrapper library (inlined) | `Main/BehaviorTreeWrapper/` — Bannerlord/Agent bindings layered over `BehaviorTrees` (`BehaviorTreeMissionLogic`, `BehaviorTreeAgentComponent`, event-subscription listeners). `BehaviorTreeMissionLogic : MissionLogic` (NOT just `MissionBehavior` — see the regression rule in `feedback_missionbehaviortype_logic_requires_missionlogic_inheritance.md`). Compiles into `DOTS.dll`. |
...
| **DOTS_Map settlements (LIVE)** | `<game>/Modules/DOTS_Map/ModuleData/settlements.xml` + `Languages/<LANG>/loc_settlements.xml` × 12. **External module — NOT in repo.** Engine-registered via `DOTS_Map/SubModule.xml`. The repo's `Main/_Module/ModuleData/settlements.xml` is a **STALE SHADOW** (last touched 2026-04-06, NOT registered in Main `SubModule.xml`, position data has diverged); existing PS scripts target it but they don't affect in-game behavior. For live display-name renames use `tools/Apply-MapVillageNames.py`. See [`docs/reference/dots-map-settlement-naming.md`](docs/reference/dots-map-settlement-naming.md). Memory: `feedback_dots_map_live_vs_stale_shadow.md`. |
...
| Messengers config | `Main/_Module/ModuleData/messengers/messenger_config.json` (advanced tuning — `accidentChancePerHour` + `travelSpeedMultiplier`; rejects NaN/Infinity/out-of-range; player-facing knobs in MCM `DotsSettings.Messengers`) |
| CareerSystem CC config | `Main/_Module/ModuleData/charactercreation/career_menu.json` (50 career CC skill/attribute bonuses) |
| CareerSystem starter equipment | `Main/_Module/ModuleData/equipmentsets/dots_career_starting_equipment.xml` (per-(culture, archetype, gender) rosters; Gondor only as of 2026-05-19) + LOTRLOME_Armory `LOTRLOME_items/<culture>/starter_armors.xml` (15 starter armor items per culture; remember `covers_legs="true"` on leg items + `covers_hands="true"` on glove items — see `feedback_lotrlome_armor_cover_attributes.md`) |
| CareerSystem sprites | `Main/_Module/GUI/SpriteParts/ui_dots_career_system/CareerSystem/` (portraits 800x400, ability icons 256x256, dedicated atlas) |
...
|-----------|-----------|---------|
| `DotsCharacterStatsModel` | `DefaultCharacterStatsModel` | `MaxCharacterTier => 10` (vanilla 6) |
| `DotsPartyWageModel` | `DefaultPartyWageModel` | Extended tier wages (T0-T10) + culture wage/garrison/Rohan mounted feats + career TroopWages passive |
| `DotsVolunteerModel` | `DefaultVolunteerModel` | `MaxVolunteerTier => 6` (vanilla 4) |
| `DotsArmyManagementModel` | `DefaultArmyManagementCalculationModel` | Culture army influence award/cost feats |
| `DotsPartySpeedModel` | `DefaultPartySpeedCalculatingModel` | Culture forest speed + Rohan infantry speed feats + career PartyMovementSpeed passive |
| `DotsSettlementProsperityModel` | `DefaultSettlementProsperityModel` | Culture hearth growth feats |
| `DotsSettlementMilitiaModel` | `DefaultSettlementMilitiaModel` | Culture veteran militia feats |
| `DotsBuildingConstructionModel` | `DefaultBuildingConstructionModel` | Culture construction speed feats |
| `DotsVillageProductionModel` | `DefaultVillageProductionCalculatorModel` | Culture production feats |
| `DotsCaravanModel` | `DefaultCaravanModel` | Umbar caravan cost feat |
| `DotsBattleRewardModel` | `DefaultBattleRewardModel` | Umbar renown feat + career BattleRenownGain passive |
| `DotsPartyTroopUpgradeModel` | `DefaultPartyTroopUpgradeModel` | Mounted recruit cost feats (Isengard, Rohan) + career TroopUpgradeCost passive |
| `DotsPartySizeModel` | `DefaultPartySizeLimitModel` | Party size feats (Mordor, Gundabad, DG, Isengard, Gondor) + career PartySize passive |
| `DotsFoodConsumptionModel` | `DefaultMobilePartyFoodConsumptionModel` | Food consumption feats (elves, Dol Guldur) |
| `DotsSettlementLoyaltyModel` | `DefaultSettlementLoyaltyModel` | Settlement loyalty feats (Gondor, Erebor, elves, Rohan) + JSON-tunable revolt thresholds + dampened different-culture penalties (RevoltTuning feature) |
| `DotsPartyMoraleModel` | `DefaultPartyMoraleModel` | Party morale feats (Gondor, Rohan, Erebor, elves) + career TroopMorale passive |
| `DotsSmithingModel` | `DefaultSmithingModel` | Smithing energy cost feats (Erebor, Isengard) + career EnchantmentCostReduction passive |
| `DotsClanFinanceModel` | `DefaultClanFinanceModel` | Tariff income feat (Umbar) |
| `DotsRaidModel` | `DefaultRaidModel` | Raid damage feats (Mordor, Gundabad, Isengard) + career TroopDamage passive |
| `DotsMilitaryPowerModel` | `DefaultMilitaryPowerModel` | Configurable T7-T10 troop power (MCM + JSON) |
| `DotsCombatSimulationModel` | `DefaultCombatSimulationModel` | Configurable blunt/cut damage ratio per battle type (MCM) |
| `DotsPartyHealingModel` | `DefaultPartyHealingModel` | Cultural survival bonuses (JSON per-faction death chance multiplier) |
| `DotsTournamentModel` | `DefaultTournamentModel` | Per-participant culture armor + culture-specific prize pools (Tierf-based) for regular and elite rewards |
| `DotsDiplomacyModel` | `DefaultDiplomacyModel` | Custom diplomacy logic for GoT faction relationships |
| `DotsExecutionRelationModel` | `DefaultExecutionRelationModel` | Culture-specific relation penalties for executions |
| `DotsInformationRestrictionModel` | `DefaultInformationRestrictionModel` | Encyclopedia visibility restrictions per settings |
| `DotsSiegeEventModel` | `DefaultSiegeEventModel` | Adds Trebuchet to defender siege engine options (for Minas Tirith et al.); preserves vanilla Fire-variant perk gating |
| `DotsTargetScoreModel` | `DefaultTargetScoreCalculatingModel` | Besieger army: commitment stickiness (4×), faction priority lists, strength gate bypass per faction, distance compensation; `Patch22_ArmyTargeting` border proximity floor |
...
| `Patch19_CustomBattles` | Custom battle DOTS factions/commanders/troops | `CustomBattleData`, `CustomBattleHelper`, `BannerlordMissions` |
...
Phase 2: CODEX ADVERSARIAL REVIEW (Claude dispatches directly, no user terminal step)
  ...
  6. Verify each Codex finding by reading DOTS source + decompiling vanilla targets — implement confirmed fixes
...
| **filesystem** | Project | File operations across DOTS, Bannerlord Modules, LOTRAOM assets | `.mcp.json` |
...
| **dots-moduledata** | Project | Query DOTS ModuleData integrity (validate, item/troop/culture exists, find-references, list cultures/schemas) — wraps `tools/dots_query.py`. Needs the `mcp` SDK; restart Claude to load. See `docs/features/moduledata-validation.md`. | `.mcp.json` |
...
| Validate/query DOTS ModuleData (does item/troop/culture id exist? what references id X? are there broken refs?) | **dots-moduledata** MCP (`validate_moduledata`, `item_exists`, `troop_exists`, `culture_exists`, `find_references`, `list_cultures`) — or `python tools/validate_moduledata.py` if the MCP isn't loaded | Grep ModuleData / hand-rolling a one-off ref validator |
| Research before implementing | **`dots-src path <Type>`** for signatures, **Read/Grep** decompiled source for browsing patterns, **Serena** for symbol nav, **ilspy** MCP as fallback | Manual decompilation workflow |

### TaleWorlds Research — Lookup Order

**Always use `dots-src` first.** It runs `ilspycmd` against the installed v1.4.5 DLLs and caches under `~/.dots-src/v1.4.5/` (the script auto-detects the version from `Version.xml`, so old `~/.dots-src/v1.3.15/` caches remain on disk but are unused). The v1.4.5 dump at `E:\Decompiled_Bannerlord\` is fine for browsing namespaces/patterns; for authoritative method signatures still prefer `dots-src` against the installed DLLs.

| Step | Action | When |
|------|--------|------|
| 0. **[Engine process docs](./docs/reference/engine/)** | Pre-filtered, DOTS-relevant, file:line-cited docs for 19 engine subsystems | **First** for "how does X work" questions (lifecycle, formation, mount/rider, campaign-mission seam, heartbeat, spawn pipeline). Saves raw decompile time when the process is already documented. |
| 1. **`pwsh tools/dots-src.ps1 path <Type>`** | One command — decompiles v1.4.5 DLL on cache miss, returns absolute path | **For signature verification** (Harmony patch, GameModel override, adapter, API call) — authoritative; run after you understand the process conceptually |
...
| 3. **ILSpy MCP** | `mcp__ilspy__decompile_type` / `mcp__ilspy__list_types` | Fallback if `dots-src` fails (e.g., need a full DLL type listing) |

See `.claude/skills/dots-src/SKILL.md` for full usage. Composes with standard tools:
```bash
...
Project-level MCP servers (Serena, GitHub, filesystem, git, ilspy, dots-moduledata) are configured in `.mcp.json` at the project root and must be listed in `.claude/settings.local.json → enabledMcpjsonServers` to be trusted. (`dots-moduledata` is DOTS-authored — `tools/dots_mcp_server.py` — and requires the `mcp` Python SDK; a Claude restart is needed to pick up a newly-added server.) User-level servers (sequential-thinking, context7) are configured in `~/.claude/.mcp/user.json` and enabled globally.
...
| `tools/validate_moduledata.py` | **Unified schema-driven cross-reference + validation engine** (read-only; preferred over the two per-task validators above). Driven by `tools/schemas/*.json` (source of truth) + `tools/dots_schema.py`. Catches broken Item/NPCCharacter/Culture/PartyTemplate refs, duplicate NPC/culture/roster/Armory-item ids, missing civilian `equipmentType`, invalid `default_group`. Adopted from `TheOldRealms/TOR_Tools` (MIT). Unit-tested (`tools/tests/test_validate_moduledata.py`). See `docs/features/moduledata-validation.md` + `.claude/rules/moduledata-validation.md`. | `--game-modules`, `--moduledata`, `--json`, `--code`, `--warnings-as-errors` |
| `tools/dots_query.py` | Query API over the validation engine (`item_exists` / `troop_exists` / `culture_exists` / `find_references` / `validate` / `list_cultures`). Pure stdlib; backs the MCP server. Unit-tested (`tools/tests/test_dots_query.py`). | (library) |
| `tools/dots_mcp_server.py` | **MCP stdio server** (`dots-moduledata` in `.mcp.json`) exposing the query API as 9 tools for interactive agent use. FastMCP; needs the `mcp` SDK; restart Claude to load. | `python tools/dots_mcp_server.py` (smoke-test) |
...
| `tools/cleanup_deleted_troops_212.py` | Sweep deleted-troop refs from `dots_partyTemplates.xml`, `troop_weights.xml`, `troop_resource_costs.xml` (issue #212) | `--dry-run`, `--apply` |
